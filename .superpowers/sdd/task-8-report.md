...
npx vitest run --config packages/tests/shared-server/vitest.deno.config.mjs \
...
- Expiry probe `5d98a086-6df1-4b55-b78c-e6dbb30dbcbf` is deleted by maintenance.
  ...
  `session-disconnected` event with `reason: expired` for the scenario.
