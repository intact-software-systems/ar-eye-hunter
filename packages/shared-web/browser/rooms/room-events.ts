...
class RoomEvents implements RallarRoomEventsPort {
          ...
          async (signal) =>
            await replayStateEventPages({
              ...
              replayEvent: async (event) =>
                await this.replayEvent(event, listener ?? options.listener),
            }),
          operationOptions,
        ),
    );
  ...
  async dispatch(message: UnvalidatedRallarMessage): Promise<void> {
    ...
    await Promise.all(
      subscriptions.map(
        async (subscription) =>
          await notifyStateEventListener(subscription.listener, event, groupMessage),
      ),
    );
  }
  ...
  private async replayEvent(
    ...
    await Promise.all(
      subscriptions.map(
        async (subscription) =>
          await notifyStateEventListener(subscription.listener, event, message),
      ),
    );
    return 'replayed';
  }
