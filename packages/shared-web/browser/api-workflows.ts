...
import { refreshCompleteStateSnapshotCollections } from '@shared-web/browser/state-read/collection-refresh.ts';
    ...
    const results = await flow
        ...
        .parallel(
            ...joinedGroups.map((snapshot) => {
                ...
                const groupSessionGenerationId = snapshot.activeSessions.find((session) =>
