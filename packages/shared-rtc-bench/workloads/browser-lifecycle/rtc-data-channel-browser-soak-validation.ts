...
export const RTC_DATA_CHANNEL_BROWSER_SOAK_CONTRACT = {
    ...
    iterations: 25,
    scriptPath: 'packages/shared-rtc-bench/workloads/browser-lifecycle/rtc-data-channel-browser-soak.mjs',
...
function validateSoak(rawEvidence: Record<string, RtcBaselineJson>, iterationIdPrefix: string) {
    ...
    return [
        ...issues,
        ...countIssues,
        ...soak.results.flatMap((iteration, index) => validateIteration(iteration, index, iterationIdPrefix))
    ];
}
...
export function computeRtcDataChannelBrowserSoakAttempt(
    ...
    const samples = attempt.samples.map((sample) => computeRtcDataChannelBrowserSoakSample(sample, baselineId));
    return {
        ...attempt,
