...
function parseDiagnosticArguments(
    ...
    const confinedOutput = out.startsWith('tmp/perf/results/') &&
        !out.includes('\\') &&
        out
            .split('/')
...
function parseFrozenDepth(
    ...
    if (depth !== undefined) {
        return { ok: true as const, value: depth };
    }
    return {
        ok: false as const,
        issues: [
...
function createExpectedSampleIds(
    ...
    const prefix = `rtc-b02-data-channel-drain-depth-${depth}-${phase}-` + String(outerOrdinal).padStart(3, '0');
    return Array.from(
        { length: 5 },
