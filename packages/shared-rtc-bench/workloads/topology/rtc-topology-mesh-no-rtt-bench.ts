...
function createExpectedSampleIds(
  ...
  const prefix =
    `rtc-b03-topology-mesh-sessions-${sessions}-${phase}-` + String(outerOrdinal).padStart(3, '0');
  return Array.from(
    { length: 5 },
...
function createIdentity(
  worker: RtcTopologyMeshAcceptedArguments,
  index: number,
...
function validateResult(input: RtcTopologyMeshInput, result: RtcTopologyMeshResult) {
  ...
  const validSessions =
    JSON.stringify(result.sessionIds) === JSON.stringify(createSessionIds(input.sessions));
