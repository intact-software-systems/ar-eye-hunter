...
import { runRtcTopologyDeliveryLogWorkloads } from './run-rtc-topology-delivery-log-workloads.ts';
...
function readOptions(args: readonly string[]): BenchmarkOptions {
    ...
    for (const argument of args) {
        if (argument.startsWith('--out=')) {
            ...
            throw new TypeError(`Unsupported RTC topology delivery benchmark option: ${argument}`);
        }
    }
