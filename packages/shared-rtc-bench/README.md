...
## Synthetic RTC workload boundaries

The signaling and data-channel workers use the production RTC owners over
complete package-owned native ports in `workloads/native-rtc/`. Native peer
construction is setup for the data-channel and multicast cases; the existing
queue fill/replacement/drain, channel lifecycle, and multicast planning intervals
remain explicit in each worker. Heartbeat callback churn now registers and
removes callbacks on actual `QRtcDataChannel` instances prepared before timing.
The multicast planning interval includes the actual connection service's
readiness query. These are synthetic native effects, not browser throughput.

RTC-B01's ICE queue case still prepares 25,000 candidates before timing and
measures only `flushRtcIceCandidateQueue`: detach the queue, add each candidate
sequentially, and update both success counters. The same operation serves the
production offer/answer paths. It now calls one synchronous owner callback per
successful addition so a diagnostic reset during an awaited addition updates
the current counters. Failed native additions are normalized and reported, and
candidates enqueued during the drain remain for the next drain.

The diagnostics case retains its accepted one-retry/one-exhaustion input. Its
explicit public `reconnectAttempts = 5` setup selects the exhausted starting
state; it does not claim that five reconnects ran. Timer dispatch remains
deterministic, but obtains and immediately cancels a complete native timer
handle instead of pretending a numeric handle satisfies every host API. Listener
cleanup uses a complete native EventTarget port. Native error events carry the
complete RTC error contract, and cleanup follows measurement.

The acceptance identities, input sizes, sample counts, payload sizes, loops, and
counter requirements are unchanged. Complete native ports, actual readiness and
callback owners, and the ICE success callback have different setup and call
costs from the previous partial fakes. Performance comparisons must use the
same current harness on both source revisions. Existing artifacts retain their
original source/harness identity; they are not current validation or evidence of
an improvement from this fixture closure.
...
| Accepted workload          | ICE queue                    | `workloads/signaling/rtc-ice-candidate-queue-bench.ts`                               | `npm run perf:rtc-baseline -- capture ...`                | Candidate count and accepted worker identity                                                                                                                                                                                                                 | `flushRtcIceCandidateQueue`                                                            | Package-local native peer stand-in                             | Queue flush only                                                             | `validateResult` and accepted-sample envelope checks                         | Create-new diagnostic JSON or accepted B01 samples              | `tests/workloads/signaling/rtc-signaling-benchmark-lifecycle.test.ts`           | B01 accepted tooling                            |
...
| Accepted workload          | Group peer ownership         | `workloads/group-coordination/webrtc-group-manager-peer-owners-bench.ts`             | Direct Deno entry; package `check:deno`; B04 capture held | `--groups`, `--peers-per-group`, `--lookups`, `--runs`, and `--out`                                                                                                                                                                                          | `WebRtcGroupManager.ownerGroupsOfPeer` and `isPeerDialAllowedByAnyGroup`               | Package-local repositories, snapshots, and queue-box harness   | Peer-owner lookup loop only                                                  | Executable records owner counts; owning test performs Deno check             | Caller-selected, overwrite-capable diagnostic JSON              | `tests/workloads/group-coordination/webrtc-group-coordination-benches.test.ts`  | B04 catalogued tooling; capture held            |
