...
function parseInitialize(options: RtcBaselineCliOptions, issues: RtcBaselineCliIssue[]) {
            ...
            values.forEach((workloadId, index) => {
                ...
                if (first !== index) {
                    ...
                    if (unsupported) {
                        ...
                        workloadIds.push(readRtcBaselineLiteral(workloadId, workloads));
                    }
                }
            });
        }
    ...
    return {
        ...
        conditionalEnvironmentDecision: supplied.length === 3
            ? {
...
function parseAttempt(
    ...
    if (options['outer-ordinal'] !== undefined && !outer.ok) {
        issues.push(...outer.issues);
    }
    if (options['producer-exit-status'] !== undefined && !status.ok) {
...
function buildCommand(
    ...
    if (name === 'initialize') {
        return parseInitialize(options, issues);
    }
    if (name === 'capture') {
        return {
            kind: name,
