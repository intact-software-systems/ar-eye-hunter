...
import type { RtcBaselineExclusiveFileLock, RtcBaselineFilePort } from './rtc-baseline-file-port.ts';
import {
...
async function recoveryFailure(input: RecoveryFailureInput): Promise<RtcBaselineResult<void>> {
    ...
    if (ageMs < config.staleAfterMs) {
        const message = `Writer process ${metadata.processId} is dead, but its lock has not reached the ` +
