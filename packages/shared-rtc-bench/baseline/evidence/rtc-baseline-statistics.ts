...
export type RtcBaselineRelativeMedianChange =
    | RtcBaselineDefinedRelativeMedianChange
    | RtcBaselineUndefinedRelativeMedianChange;
...
export interface PersistedEnvironment {
    ...
    observation: Pick<RtcBaselineRuntimeObservationDto, 'git' | 'runtime' | 'host' | 'resolvedConfiguration'>;
}
...
export function evaluateRtcBaselineWorkloadRepeatOutcome(
    ...
    const triggering = primary.filter((metric) => requiresRtcBaselineRepeat(metric, input.executionContext));
    ...
    const missing = triggering.flatMap((metric) => {
        if (repeats.has(persistedMetricKey(metric))) {
            return [];
        }
        ...
        for (const field of provenanceFields) {
            if (observation.grouping[field] !== partition.grouping[field]) {
                issues.push({
                    ...
                    message: `Metric cohort mixes ${field}: ` +
...
export function compareRtcBaselinePairedCohorts(
    ...
    return {
        ...
        value: {
            ...
            relativeMedianChange: baselineSummary.median === 0
