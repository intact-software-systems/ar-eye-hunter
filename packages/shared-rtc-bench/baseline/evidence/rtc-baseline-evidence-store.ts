...
import { createRtcBaselineArtifactFiles, type RtcBaselineStoredFile } from './rtc-baseline-artifact-files.ts';
import { createRtcBaselineConfinedPath, rtcBaselineUnconfinedPathFailure } from './rtc-baseline-confined-path.ts';
...
export function createRtcBaselineFileStore(input: {
    ...
    return {
        ...
        writeJsonCreateNew: (baselineId, relativePath, value) => {
            ...
            return withLock(baselineId, (writer) => writer.writeJsonCreateNew(baselineId, relativePath, value));
        },
