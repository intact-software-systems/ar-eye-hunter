...
type MetadataJsonValue = null | boolean | number | string | readonly MetadataJsonValue[] | MetadataJsonObject;
...
export function decodeRtcBaselineWriterLockMetadata(
    ...
    if (value.state === 'released') {
        ...
        if (!hasOnlyKeys(value, keys) || !isUtcTimestamp(value.releasedAtUtc)) {
            return null;
        }
        return {
            schema,
