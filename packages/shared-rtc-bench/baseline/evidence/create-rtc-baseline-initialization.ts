...
import { rtcBaselineUnconfinedPathFailure, type RtcBaselineConfinedPath } from './rtc-baseline-confined-path.ts';
...
export function createRtcBaselineInitialization(input: {
    ...
    async function initialize(
        ...
        if (
            ...
            !initializationPaths.every((relativePath) => confinedPath.isConfined(baselineId, relativePath))
        ) {
        ...
        const cleanupIssues = written.issue === null ? [] : await removeWrittenArtifacts(written.written);
        const release = await lock.value.release();
        if (written.issue === null) {
            return release;
        }
        return {
            ok: false,
