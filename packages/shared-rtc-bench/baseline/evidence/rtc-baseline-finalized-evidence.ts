...
function issue(path: string, code: string, message: string): RtcBaselineIssueDto {
...
export function createRtcBaselineFinalizedEvidence(
    ...
    function finalize(input: { baselineId: string; }) {
        return dependencies.withFinalizationLock(input.baselineId, (writer) => finalizeLocked(writer, input));
    }
