...
import type { PersistedMetric, RtcBaselinePersistedMetricComparison } from './rtc-baseline-statistics.ts';
...
export interface RtcBaselineSummaryArtifactRecord
    extends Omit<RtcBaselineSummaryDto, 'sampleOutcomes' | 'cohortOutcomes' | 'metricSummaries'> {
...
export type RtcBaselineStoredArtifact = RtcBaselineStoredArtifactByKind[keyof RtcBaselineStoredArtifactByKind];
...
export type RtcBaselinePairedComparison =
    & RtcBaselinePairedComparisonRecord
    & (
        | { outcome: 'conclusive'; }
        | {
...
export function classifyRtcBaselineArtifactPath(relativePath: string) {
    if (relativePath === RTC_BASELINE_ENVIRONMENT_FILE) {
        return 'environment' as const;
    }
    if (relativePath === RTC_BASELINE_MANIFEST_FILE) {
        return 'manifest' as const;
    }
    if (relativePath === RTC_BASELINE_SUMMARY_FILE) {
        return 'summary' as const;
    }
    if (/^results\/samples\/[^/]+\.json$/.test(relativePath)) {
    ...
    if (/^results\/failures\/[^/]+\.json$/.test(relativePath)) {
        return 'failure-outcome' as const;
    }
...
export function canonicalizeRtcBaselineRawReferences(
    ...
    return {
        ok: true,
        value: [...byPath.values()].sort((left, right) => left.relativePath.localeCompare(right.relativePath))
    };
}
...
export function inspectRtcBaselineStoredArtifactBytes(input: {
    ...
    if (pathKind === 'raw') {
        return { ok: true, value: { kind: null, json: null } };
    }
    if (pathKind === null) {
        return rejected(
            `$.${input.relativePath}`,
    ...
    if (pathKind === 'failure-outcome') {
        return { ok: true, value: { kind: null, json } };
    }
...
export function encodeRtcBaselineChecksumEntries(
    ...
    const orderedEntries = [...entries].sort((left, right) => left.relativePath.localeCompare(right.relativePath));
