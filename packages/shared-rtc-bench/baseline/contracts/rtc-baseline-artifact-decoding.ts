...
import { validateRtcBaselineNumberRule, validateRtcBaselineStringRule } from './rtc-baseline-artifact-validation.ts';
...
function validate(input: ValidationInput) {
    ...
    if (rule.kind === 'nullable') {
        if (value !== null) {
            validate({ value, rule: rule.value, path, issues });
        }
        return;
    }
    if (rule.kind === 'json') {
    ...
    if (rule.kind === 'scalar') {
        const valid = (rule.nullable && value === null) ||
        ...
        for (const [field, child] of Object.entries(rule.fields)) {
            ...
            if (!Object.hasOwn(record, field)) {
                if (child.kind !== 'optional') {
                    issues.push(issue(childPath, 'missing-field', 'Required.'));
                }
            }
            else {
                validate({
    ...
    if (rule.kind === 'array') {
        ...
        for (let index = 0; index < value.length; index += 1) {
            ...
            if (!(index in value)) {
                ...
                validate({ value: value[index]!, rule: rule.item, path: childPath, issues });
            }
        }
        return;
    }
    ...
    if (rule.kind === 'string') {
        issues.push(...validateRtcBaselineStringRule(value, rule, path));
    }
}
...
export const decodeRtcBaselineFinalizationFailure = decoder<Artifact.RtcBaselineFinalizationFailureDto>(
    shapes['finalization-failure']
);
export const decodeRtcBaselineSummary = decoder<RtcBaselineSummaryArtifactRecord>(shapes.summary);
export const decodeRtcBaselineRuntimeObservation = decoder<Artifact.RtcBaselineRuntimeObservationDto>(shapes.runtime);
