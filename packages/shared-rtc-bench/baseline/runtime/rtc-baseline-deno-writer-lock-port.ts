...
function requireRegularFile(value: { isFile: boolean; isSymlink: boolean }) {
  if (value.isSymlink) throw new Error('Writer lock path may not be a symbolic link.');
  if (!value.isFile) throw new Error('Writer lock path must be a regular file.');
}
...
async function readMetadata(file: RtcBaselineDenoFile) {
  ...
  while (offset < bytes.length) {
    ...
    if (read === null) break;
    if (read === 0) throw new Error('Writer lock metadata read made no progress.');
    offset += read;
  }
...
async function writeMetadata(file: RtcBaselineDenoFile, bytes: Uint8Array) {
  ...
  while (offset < bytes.length) {
    ...
    if (written === 0) throw new Error('Writer lock metadata write made no progress.');
    offset += written;
  }
...
async function openWriterLock(runtime: RtcBaselineDenoPort, path: string) {
  try {
    ...
    if (!AlreadyExists || !(error instanceof AlreadyExists)) throw error;
  }
