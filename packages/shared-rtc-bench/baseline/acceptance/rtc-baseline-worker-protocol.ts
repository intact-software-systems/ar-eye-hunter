...
interface RunRtcBaselineAcceptedWorkerCliInput<CapabilityInput, Diagnostic extends { readonly mode: 'diagnostic'; }> {
    ...
    return runRtcBaselineAcceptedWorkerSamples({
        ...
        createSample: ({ identity, result, issues }) => {
            ...
            return {
                ...
                metrics: input.metrics(result).filter((metric) => Number.isFinite(metric.value) && metric.value >= 0),
