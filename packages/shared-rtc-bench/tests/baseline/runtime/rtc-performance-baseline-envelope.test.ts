...
import { createRtcBaselineEnvelope } from '../../../baseline/runtime/rtc-baseline-envelope.ts';

function createTestRtcBaselineEnvelope(dependencies: unknown) {
    return createRtcBaselineEnvelope(dependencies as Parameters<typeof createRtcBaselineEnvelope>[0]);
}
...
describe('RTC baseline public envelope', () => {
    it('delegates every accepted mutation and propagates typed left values', async () => {
        ...
        const envelope = createTestRtcBaselineEnvelope({
            acceptance,
    ...
    it('delegates finalization and all five finalized-reader operations exactly once', async () => {
        ...
        const pairedInput = {
            ...
            candidateBaselineId: '20260807-ffffffffffff-e1-local',
            primaryComparisonCohortId: value.baselineId,
            candidateComparisonCohortId: '20260807-ffffffffffff-e1-local',
