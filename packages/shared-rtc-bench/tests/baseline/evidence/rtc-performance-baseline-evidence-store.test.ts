...
function createNodePort() {
    ...
    return {
        ...
        createDirectory: (path: string, options: { recursive: boolean; }) => mkdir(path, options).then(() => undefined),
...
function createMemoryPort() {
    ...
    const port: RtcBaselineFilePort = {
        ...
        readFile: async (path: string) => {
            ...
            if (entry?.kind !== 'file' || !entry.bytes) {
...
describe('RTC baseline evidence store', () => {
    ...
    it.each([
        ...
        },
        {
            ...
            message: 'Writer process 122 is dead, but its lock has not reached the 300000ms stale threshold.'
        }
    ...
    it('keeps a complete finalization pair immutable and recovers a checksum-only orphan', async () => {
        ...
        const publish = (summaryBytes = bytes) => store.withFinalizationLock(baselineId, (writer) => writer.publishSummary(baselineId, summaryBytes, checksum));
    ...
    it('atomically reserves initialization so a concurrent loser never deletes the winner', async () => {
        ...
        memory.port.createDirectory = async (path, options) => {
            ...
            if (!options.recursive && memory.entries.has(path)) {
                throw new Error('exists');
            }
    ...
    it('keeps failed initialization cleanup inside its lock and preserves a replacement writer', async () => {
        ...
        memory.port.tryAcquireExclusiveFileLock = async (path) => {
            ...
            return {
                ...
                async release() {
                    await lock.release();
                    replacementResult = await replacementStore.withFinalizationLock(
                        baselineId,
                        (writer) => writer.writeJsonCreateNew(baselineId, 'environment.json', {})
                    );
                }
            };
        };
    ...
    it('confines a clean real root and never follows traversal or final symlinks', async () => {
        ...
        try {
            ...
            expect(
                await store.withFinalizationLock(baselineId, (writer) => writer.publishSummary(baselineId, new Uint8Array(), new Uint8Array()))
            ).toEqual(symlinkFailure);
