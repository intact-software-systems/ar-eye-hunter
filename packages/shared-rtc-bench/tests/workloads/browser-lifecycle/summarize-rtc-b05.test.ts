...
import { computeRtcBaselineMetricSummary, requiresRtcBaselineRepeat } from '../../../baseline/evidence/rtc-baseline-statistics.ts';
    ...
    it('keeps isolated steady-iteration spikes out of the repeat decision', () => {
        const steadyOpenMedians = Array.from({ length: 5 }, (_, attemptIndex) =>
            computeSteadyOpenMedianDuration(
                Array.from({ length: 25 }, (_, iterationIndex) => iterationIndex === 0 ? 13 : iterationIndex === attemptIndex + 1 ? 20 : 7)
            ));
