...
export type GroupRef = GroupScope &
  Readonly<{
    groupId: GroupId;
  }>;

type GroupBase = GroupRef &
  Readonly<{
    slug: string | null;
    displayName: string;
    ...
    /**
     * The active member principal ids pinned at the last formation epoch
     * boundary (plan decision 4.1). Manager election ranks this recorded set,
     * never live membership, so churn between transitions cannot flap the
...
type GroupPresenceSessionBase = GroupRef &
  Readonly<{
    sessionId: SessionId;
    principalId: PrincipalId;
...
export type GroupPresenceSummary = GroupRef &
  Readonly<{
...
export type GroupPresenceAdmission = GroupRef &
  Readonly<{
    principalId: PrincipalId;
...
export type GroupEvent = GroupRef &
  Readonly<{
    eventId: string;
