...
import type { CreateRallarOntologyCatalogInput, RallarOntologyCatalog } from './rallar-ontology-registry-contracts.ts';
import type {
...
function copyBinding(binding: RallarOntologyBinding): RallarOntologyBinding {
    if (binding.target.kind === 'wire-constant') {
        return {
            ...
            target: {
                ...binding.target,
                propertyPath: binding.target.propertyPath === undefined ? undefined : [...binding.target.propertyPath]
            }
        };
    }
