...
import { createRallarOntologyIssue, sortRallarOntologyIssues } from './rallar-ontology-identity-validation.ts';
import type { CreateRallarOntologyCatalogInput, RallarOntologyIssue } from './rallar-ontology-registry-contracts.ts';
...
function validateDeclaredDependencies(input: ValidateDeclaredDependenciesInput): void {
    input.source.requiredVocabularyVersionIris.forEach((requiredVersionIri, requirementIndex) => {
        ...
        if (selected === undefined || !vocabularySatisfies(selected, requiredVersionIri)) {
            const requirementPath = `catalog.vocabularies[${input.sourceIndex}]` +
