...
A validated initial AL control enqueue runs the normal read, computation, and
validation. When it needs new ownership, it atomically retains the canonical payload,
its immutable identity, and compact `admit-message` work through
[`retainALOutboundPendingAdmission`](./al-outbound-pending-admission.ts). That first
ownership write does not enter the sender queue or browser Web Lock. A
`pending-admission` result means durable ownership exists; it does not mean transport
ran. A spoofed control type whose envelope does not decode as a canonical control stays
on the ordinary serialized path.
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Initial validated control needing ownership | Dispatch admission reads and computes normally, then atomically retains the canonical payload, identity, and `admit-message` row without entering sender/browser serialization.                                                                         | `pending-admission` establishes durable ownership and wakes the existing worker. A retention conflict returns no ownership, so the calling work owner can retry.                                                  |
| Other `enqueueIfAbsent`                     | Dispatch admission reads validated state and computes a bundle. Its commit compares sender versions and original supersedence observations, then writes admission state and QueueBox work atomically under the existing sender queue and browser lock.  | The runtime wakes the existing worker after commit, outside admission's sender/browser lock.                                                                                                                      |
...
| `admit-message` work                        | `ALOutboundMessageEffects` reads pending admission authority, rechecks the retained deadline, and commits the retained policy through dispatch admission's unchanged sender queue and browser lock.                                                     | The claim completes. Rejected or expired authority completes without admitting; a not-ready authority reschedules with its own delay.                                                                             |
