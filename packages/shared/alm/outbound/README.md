...
| `enqueueAllIfAbsent`         | One sender's messages under one sender-queue slot and one browser lock: each is read, computed and validated as `enqueueIfAbsent` does, then `commitBundles` fences the sender version once, runs every bundle's own fences, writes them all and bumps the version once. An invalid member, a version that moved between the members' reads, two bundles writing one row, or any store answer other than `committed` sends every member back through `enqueueIfAbsent`'s own commit, one after another. | The runtime wakes the existing worker once for the group. Each message still reports its own `commit-phases` event.                                                                                               |
