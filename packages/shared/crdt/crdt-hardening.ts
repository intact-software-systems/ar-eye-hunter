...
export type RallarCrdtRolloutLabel =
  'disabled' | 'experimental-local' | 'experimental-live' | 'durable-beta' | 'production';

export type RallarCrdtConsistencyPhase =
  'local-only' | 'topic-bridge' | 'durable-log' | 'production';
...
export function evaluateRallarCrdtFeaturePolicy(
  ...
  const disabledReason =
    flags?.killSwitchReason ?? 'CRDT document type is disabled by rollout policy.';
...
>(
  ...
  if (hasEncryptedRecords && !input.allowEncryptedWithSuppliedState) {
    const reason =
      'Encrypted CRDT logs require an explicit client/key-authorized compact snapshot ' +
