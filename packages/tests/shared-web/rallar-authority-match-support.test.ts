...
describe('Rallar authority browser match support', () => {
    it('delegates authority-client configuration, lifecycle, and commands', async () => {
        ...
        let receivedConfig:
            | RallarGameAuthorityClientConfig<Command, Snapshot, Event, Presence>
            | undefined;
        const match = createRallarAuthorityBrowserMatch<Command, Snapshot, Event, Presence>(config, {
    ...
    it('rejects browser-director authority before creating the client', () => {
        ...
        expect(() =>
            ...
            } as never, { createAuthorityClient })
        ).toThrow(
...
function authorityMatchConfig(): RallarAuthorityBrowserMatchConfig<Command, Snapshot, Event, Presence> {
    return {
        rallar: {} as never,
...
function fakeAuthorityClient(): RallarGameAuthorityClientHandle<Command, Snapshot, Event, Presence> {
    return {
        start: vi.fn(async () => ({
