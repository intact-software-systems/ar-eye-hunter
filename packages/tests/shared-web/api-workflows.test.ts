...
describe('state API workflows', () => {
  ...
  it('orchestrates client and group heartbeats and tolerates missing group presence ' +
    'without retrying 404s', async () => {
  ...
  it('repairs missing client presence once after a scoped heartbeat 404', async () => {
    ...
    vi.spyOn(Date, 'now').mockReturnValue(1000);
    const clientSessionPath =
      '/api/state/apps/ar-eye-hunter/workspaces/default/clients/principal-1' +
      '/instances/principal-1/sessions/session-1';
    const groupSessionPath =
      '/api/state/apps/ar-eye-hunter/workspaces/default/groups/group-1' +
      '/sessions/session-1';
    stubFetch(({ url, method }) => {
      if (
        method === 'POST' &&
  ...
  it('surfaces member forbidden from create-and-join presence ' +
    'without raw membership repair', async () => {
  ...
  it('reuses a distinct presence repair request ID across heartbeat retries', async () => {
    ...
    const repairRequestIds = fetchCalls
      .filter(
        (call) =>
          call.method === 'PUT' && call.physicalUrl.includes('/sessions/session-1/requests/'),
      )
      .map((call) => readMutationRequestId(call.physicalUrl));
