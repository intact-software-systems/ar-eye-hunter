...
export function createApiMiddlewareTestDouble(
  ...
  return {
    ...
    middleware: {
      ...
      webRtcOverlayMulticastManager: toServiceTestDouble<
        Middleware['webRtcOverlayMulticastManager']
...
function createWebSocketQueueBoxDouble(
  ...
  const queueBox: Middleware['webSocketQueueBox'] = toServiceTestDouble<
    Middleware['webSocketQueueBox']
  >({
...
function createWebSocketClientDouble(): Middleware['webSocketQueueBox']['socket'] {
  const socket: Middleware['webSocketQueueBox']['socket'] = toServiceTestDouble<
    Middleware['webSocketQueueBox']['socket']
  >({
    close: vi.fn(),
...
function createWebRtcConnectionServiceDouble(
  ...
  const connectionService: Middleware['webRtcConnectionService'] = toServiceTestDouble<
    Middleware['webRtcConnectionService']
  >({
...
function createRtcRxStreamerDouble(
  ...
  const rtcRxStreamer: Middleware['rtcRxStreamer'] = toServiceTestDouble<
    Middleware['rtcRxStreamer']
  >({
    enqueueOutboxIfAbsent: vi.fn(async (message: ALMessage) => ({
      status: 'enqueued' as const,
