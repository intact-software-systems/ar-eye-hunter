...
class RtcControlHandoffFixture {
    ...
    async seedFullInboundControlPage(): Promise<readonly InboundClaimTrace[]> {
        const bootstrap = newALAckControlMessage(
            { v: 2, senderId: 'bootstrap-peer', msgId: 'unknown-control', ts: BATCH_STARTED_AT_MS },
            {
                ackedMsgId: 'unknown',
                originPeerId: 'self',
                logicalRecipientPeerId: 'bootstrap-peer',
                carrier: 'ws',
    ...
    async expectRtcDeliveryBeforeControlLocks(
        ...
        expect(this.dependencies.observations.controlStatuses).toEqual(
            Array.from({ length: BACKGROUND_CONTROL_COUNT }, () => 'admitted')
        );
...
async function observeRtcControlTraffic(
    ...
    const sendControl = service.enqueueOutboxAllIfAbsent.bind(service);
    vi.spyOn(service, 'enqueueOutboxAllIfAbsent').mockImplementation(async (messages) => {
        observations.controlSendCount += messages.length;
        const result = await sendControl(messages);
        observations.controlStatuses.push(...result.map((entry) => entry.verdict.kind));
        return result;
    });
...
function traceInboundClaims(input: TraceInboundClaimsInput): void {
    ...
    vi.spyOn(input.queue, 'reserveEntries').mockImplementation(async (request: ResourceInboxReservationRequest) => {
        ...
        const inbound = [...reserved.values()].filter((entry) => entry.typeId === toALInboundWorkType(input.namespace, 'ws'));
        if (inbound.length > 0) {
            reservationBatch += 1;
        }
    ...
    vi.spyOn(input.queue, 'releaseEntries').mockImplementation(async (
        releases: Parameters<QueueBoxResourceEntryRepository['releaseEntries']>[0]
    ) => {
        const released = await releaseEntries(releases);
        for (const { entry, disposition } of releases.filter((candidate) => candidate.entry.typeId === toALInboundWorkType(input.namespace, 'ws'))) {
            const effect = decodeALInboundWorkEntry(entry, input.namespace);
            input.trace.push({
                phase: 'released',
                batch: reservationBatch,
