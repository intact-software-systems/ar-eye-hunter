...
describe('ALWorkQueuePort', () => {
    ...
    it('peeks nothing while no work is retained, then reports retained work as ready now', async () => {
        const now = 10_000;
        const queue = new InMemoryQueueBox(
            undefined,
            () => Temporal.Instant.fromEpochMilliseconds(now)
        ...
        expect(await port.peekNextReadyAt()).toBeUndefined();

        // The second type alone proves the peek scans past the first, and a new row carries no schedule.
        await port.retainIfAbsent(newWorkEntry('AL_TEST_B', 'b-1'));
        expect(await port.peekNextReadyAt()).toBe(now);
    });

    it('peeks retained work ahead of a later retry across every configured type', async () => {
        const now = 10_000;
        const queue = new InMemoryQueueBox(
            undefined,
            () => Temporal.Instant.fromEpochMilliseconds(now)
        ...
        await port.retainIfAbsent(newWorkEntry('AL_TEST_A', 'a-1'));
        const [claimA] = await port.claim({ maxCount: 4, observedEntries: undefined });
        await port.release(claimA, { status: 'not-ready', readyAtMs: now + 5_000 });
        expect(await port.peekNextReadyAt()).toBe(now + 5_000);
    ...
    it('peeks the earliest ready time across every configured type', async () => {
        const now = 10_000;
        const queue = new InMemoryQueueBox(
            undefined,
            () => Temporal.Instant.fromEpochMilliseconds(now)
        ...
        await port.retainIfAbsent(newWorkEntry('AL_TEST_B', 'b-1'));
        const [claimB] = await port.claim({ maxCount: 4, observedEntries: undefined });
        await port.release(claimB, { status: 'not-ready', readyAtMs: now + 2_000 });

        expect(await port.peekNextReadyAt()).toBe(now + 2_000);
    });
