...
export const HANDLER_FINALIZED_SUMMARY_SCENARIOS: readonly HandlerFinalizedSummaryScenario[] = [
    ...
    scenario('event is missing a mandatory actor', false, (reserved) => updateEvent(reserved, (event) => removeKey(event, 'actor'))),
...
function mandatoryWorkKeyScenarios(): readonly HandlerFinalizedSummaryScenario[] {
    return MANDATORY_WORK_KEYS.map((key) =>
        scenario(`missing mandatory work key: ${key}`, false, (reserved) => updateWork(reserved, (work) => removeKey(work, key)))
    );
}
