...
describe('authoritative state contracts', () => {
    it('requires complete authoritative scopes, values, events, results, and receipts', () => {
        ...
        expectTypeOf<OptionalKeys<GroupTopologyConfigView>>()
            .toEqualTypeOf<never>();
        expectTypeOf<OptionalKeys<GroupTopologyManagementView>>()
            .toEqualTypeOf<never>();
    });

    it('keeps only the named authoritative semantic-absence allowlist', () => {
        expectTypeOf<OptionalKeys<SerializedGraphInfoSnapshot>>()
            .toEqualTypeOf<
    ...
    it('keeps sparse inputs separate and omission-sensitive', () => {
        ...
        expectTypeOf<OptionalKeys<GroupTopologyConfigPatch>>().toEqualTypeOf<
            ...
            | 'meshParamK'
        >();
        expectTypeOf<OptionalKeys<PutGroupTopologyConfigRequest>>()
    ...
    it('compares group and presence revisions as one componentwise causal tuple', () => {
        ...
        expect(compareGroupCausalRevision(
            { groupRevision: 3, presenceRevision: 4 },
            base,
        )).toBe('dominates');
        expect(compareGroupCausalRevision(
        ...
        expect(compareGroupCausalRevision(
            { groupRevision: 3, presenceRevision: 2 },
            base,
        )).toBe('incomparable');
    });

    it('makes non-terminal member lifecycle variants terminal-stamp free', () => {
        expectTypeOf<Extract<GroupMember, { status: 'invited' }>['joined']>()
            .toEqualTypeOf<null>();
        expectTypeOf<Extract<GroupMember, { status: 'active' }>['joined']>()
            .toEqualTypeOf<GroupAuditStamp>();
        expectTypeOf<Extract<GroupMember, { status: 'banned' }>['joined']>()
            .toEqualTypeOf<GroupAuditStamp | null>();
        expectTypeOf<Extract<GroupMember, { status: 'invited' }>['left']>()
            .toEqualTypeOf<null>();
        expectTypeOf<Extract<GroupMember, { status: 'invited' }>['removed']>()
            .toEqualTypeOf<null>();
        expectTypeOf<Extract<GroupMember, { status: 'invited' }>['banned']>()
            .toEqualTypeOf<null>();
        expectTypeOf<Extract<GroupMember, { status: 'active' }>['left']>()
            .toEqualTypeOf<null>();
        expectTypeOf<Extract<GroupMember, { status: 'active' }>['removed']>()
            .toEqualTypeOf<null>();
        expectTypeOf<Extract<GroupMember, { status: 'active' }>['banned']>()
            .toEqualTypeOf<null>();
    });
    ...
    it('keeps mutation receipts compact and references effects by identity', () => {
        ...
        expectTypeOf<ClientMutationReceipt>().toHaveProperty('attemptCount')
            .toEqualTypeOf<number>();
        expectTypeOf<ClientMutationReceipt>()
            .toHaveProperty('acceptedStorageRevision')
            .toEqualTypeOf<number | null>();
        expectTypeOf<ClientMutationReceipt>().toHaveProperty('eventId')
            .toEqualTypeOf<string | null>();
        expectTypeOf<ClientMutationReceipt>().toHaveProperty('outboxIds')
        ...
        expectTypeOf<GroupMutationReceipt>().toHaveProperty('attemptCount')
            .toEqualTypeOf<number>();
        expectTypeOf<GroupMutationReceipt>()
            .toHaveProperty('acceptedStorageRevision')
            .toEqualTypeOf<number | null>();
        expectTypeOf<GroupMutationReceipt>().toHaveProperty('eventId')
            .toEqualTypeOf<string | null>();
        expectTypeOf<GroupMutationReceipt>().toHaveProperty('outboxIds')
            .toEqualTypeOf<readonly string[]>();
        ...
        expectTypeOf<RtcTopologyPublicationWorkClaim>()
            ...
            .toEqualTypeOf<GroupStateCausalRevision>();
        expectTypeOf<RtcTopologyPublicationWorkClaim>()
            .toHaveProperty('eventId').toEqualTypeOf<null>();
        expectTypeOf<RtcTopologyPublicationWorkClaim>()
