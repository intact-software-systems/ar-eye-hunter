...
type _FixtureOptionalKeysAreDistributive = Assert<Equal<OptionalKeys<VariantOptionalFixture>, 'variantOnly'>>;
...
describe('authoritative state contracts', () => {
    ...
    it('keeps only the named authoritative semantic-absence allowlist', () => {
        expectTypeOf<OptionalKeys<SerializedGraphInfoSnapshot>>().toEqualTypeOf<AuthoritativeOptionalKeyAllowlist['SerializedGraphInfoSnapshot']>();
    });

    it('keeps sparse inputs separate and omission-sensitive', () => {
        expectTypeOf<OptionalKeys<MutationActorInput>>().toEqualTypeOf<'actorPrincipalId' | 'actorSessionId' | 'reason' | 'traceId' | 'requestId'>();
        expectTypeOf<OptionalKeys<GroupTopologyConfigPatch>>().toEqualTypeOf<'topologyKind' | 'degreeLimit' | 'treeMinSize' | 'meshMinSize' | 'meshParamK'>();
    ...
    it('makes non-terminal member lifecycle variants terminal-stamp free', () => {
        ...
        expectTypeOf<Extract<GroupMember, { status: 'active'; }>['joined']>().toEqualTypeOf<GroupAuditStamp>();
        expectTypeOf<Extract<GroupMember, { status: 'banned'; }>['joined']>().toEqualTypeOf<GroupAuditStamp | null>();
