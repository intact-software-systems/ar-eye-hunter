...
describe('Recipe Console Monitor explicit windows', () => {
    ...
    it('retains same-context movement, clamps mutation shrink, and resets a changed context', async () => {
        await renderHarness({ contextKey: 'context-a', total: 137 });
    ...
    it('resets diagnostics for severity or transport but ignores those filters elsewhere', async () => {
        await renderHarness({
            contextKey: 'context-a',
            total: 130,
            section: 'diagnostics'
    ...
    it('recovers focused Next when the first failure window shrinks from 61 to 60', async () => {
        const failures = (count: number) =>
            Array.from(
                { length: count },
                (_, index) => ({
    ...
    it('mounts exact agent, recipe, readiness, and diagnostic budgets with ordinal keys', async () => {
        const exactId = 'agent/late\u202e/\u754c/' + 'x'.repeat(120);
        const agents = Array.from({ length: 85 }, (_, index) =>
            agentRow(
                index,
                index === 0 ? exactId : `agent-${index % 2}`
            ));
        const recipes = Array.from({ length: 65 }, (_, index) => recipeRow(index, index === 0 ? exactId : `recipe-${index % 2}`));
        const readiness = Array.from({ length: 65 }, (_, index) => readinessRow(index, index === 0 ? exactId : `agent-${index % 2}`));
        const diagnostics = Array.from({ length: 55 }, (_, index) => diagnosticRow(index, index === 0 ? exactId : `agent-${index % 2}`));
        const onInspect = vi.fn();
    ...
    it('resets only the diagnostics cursor when its active filters change', async () => {
        ...
        async function renderDiagnostics(severity?: 'warning' | 'error') {
            ...
            await act(async () =>
                root?.render(createElement(MonitorDiagnostics, {
                    model: monitorModel({ runtimeDiagnostics: diagnostics }),
                    onFilter,
                    onInspect,
                    severity
    ...
    it('keeps disclosure cursors alive while closed and mounts no closed rows', async () => {
        ...
        root = createRoot(container);
        await act(async () =>
            root?.render(createElement(MonitorEvidenceDisclosure, {
                model,
                onInspect,
