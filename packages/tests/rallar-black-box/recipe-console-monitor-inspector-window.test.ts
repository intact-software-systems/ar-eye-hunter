...
describe('Recipe Console Monitor inspector windows', () => {
    ...
    it('browses command evidence failure-first in 16-row windows and activates the exact late item', async () => {
        const commandId = 'command::\u2067late\u2069';
        const failures = Array.from({ length: 2 }, (_, index) => failure(`failure-${index}`, commandId));
        const diagnostics = Array.from({ length: 2 }, (_, index) => diagnostic(`diagnostic-${index}`, commandId));
        const timeline = Array.from({ length: 2 }, (_, index) => timelineItem(`timeline-${index}`, commandId));
        const events = Array.from({ length: 14 }, (_, index) => event(index === 13 ? 'event::last\u2067|\u2069' : `event-${index}`, commandId));
        ...
        await render(
            model({
                events: Array.from({ length: 20 }, (_, index) => event(`next-command-event-${index}`, nextCommandId))
            }),
            { kind: 'command', id: nextCommandId },
            onSelect
        );
        expect(document.activeElement).toBe(container.querySelector(
            '[data-monitor-window-focus-anchor="Command evidence"]'
    ...
    it('windows 46 correlated failure destinations at 40 and preserves destination patches', async () => {
        ...
        expect(
            container.querySelector('[data-evidence-destination="command"]')
        ...
        expect(
            commandDestination?.querySelector(
    ...
    it('resets diagnostic-link windows by exact selection, clamps polls, and drops stale evidence', async () => {
        const first = diagnostic('diagnostic::first', 'command-a', Array.from({ length: 45 }, (_, index) => `failure-first-${index}`));
        ...
        expect(container.querySelector('[role="group"][aria-label="Diagnostic failure links window"]'))
            .toBeNull();
        expect(
            windowButtons('monitor-inspector-diagnostic-failure-links')
                .map((button) => button.textContent)
        ).toEqual([
            'Failurefailure-now-0',
            'Failurefailure-now-1'
        ]);

        const second = diagnostic('diagnostic::second\u2067|\u2069', 'command-b', Array.from({ length: 45 }, (_, index) => `failure-second-${index}`));
    ...
    it('browses 65 role-scoped recipe choices at 60 and resets for another recipe', async () => {
    ...
    it('does not render window controls at or below an inspector budget', async () => {
        ...
        await render(
            model({
                events: Array.from({ length: 16 }, (_, index) => event(`budget-${index}`, commandId))
            }),
    ...
    it.each(['later content row', 'window control'] as const)(
        ...
        async (focusedTarget) => {
            ...
            expect(
                windowButtons('monitor-inspector-command-evidence').map(
    ...
    it('recovers focused Next when first command evidence shrinks from 17 to 16', async () => {
    ...
    function controlButton(label: string, text: 'Previous' | 'Next'): HTMLButtonElement {
        ...
        if (!button) {
            throw new Error(`${text} ${label} button is unavailable.`);
        }
        return button;
    }
...
function diagnostic(
    ...
    return {
        ...
        severity: 'error',
        topic: 'rtc',
        diagnosticTypeId: 'test-diagnostic',
        message: eventId,
        summary: eventId,
        payloadSummary: '{}'
    };
