...
describe('Recipe Console retention pressure windows', () => {
    ...
    it('bounds 205 preview candidates and every global consequence list at 100',
        async () => {
            ...
            for (const label of [
                ...
                const consequence = [...container.querySelectorAll('details')]
                    .find(row => row.querySelector('summary')?.textContent
                        ?.startsWith(label));
    ...
    it('resets a new preview revision and recovers focus from the old window',
        async () => {
            const input = {
                pairCount: 205,
                candidateCount: 205,
    ...
    it('unmounts closed linked consequences and traverses exact 201-row lists',
        async () => {
            const preview = scalePreview({
                pairCount: 4,
                candidateCount: 1,
    ...
    it('bounds the confirmation dialog and successful deletion result independently',
        async () => {
            const preview = scalePreview({
                pairCount: 205,
                candidateCount: 205,
    ...
    it('coordinates every nested and global disclosure under one surface budget',
        async () => {
            const preview = scalePreview({
                pairCount: 4,
                candidateCount: 4,
            ...
            const globalSummaries = [...container.querySelectorAll(
                'details > summary',
            )].filter(summary => globalLabels.some(label =>
                summary.textContent?.startsWith(label)
            ));
...
function scalePreview(input: Readonly<{
    pairCount: number;
    ...
    return {
        ...
        wouldDeleteDistributedRunIds:
            fixture.retention.wouldDeleteDistributedRunIds,
