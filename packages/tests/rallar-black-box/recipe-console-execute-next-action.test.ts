...
import { deriveExecuteActionPolicy, type ExecuteActionPolicyInput } from '../../../apps/rallar-black-box/src/recipe-console/execute/execute-action-policy.ts';
import { deriveExecuteNextAction, type ExecuteNextActionInput } from '../../../apps/rallar-black-box/src/recipe-console/execute/execute-next-action.ts';
...
describe('Recipe Console Execute next action', () => {
    it.each(
        [
            ...
            [
                'registering',
                input({ targetCount: 0, targetableCount: 0, launchedExpectedCount: 3, launchedReadyCount: 2 }),
                'registering',
                '2 of 3 browser agents ready'
            ],
            [
                'registering beside existing agents',
                input({
                    targetableCount: 1,
            ...
            [
                'selecting a fully registered cohort',
                input({
                    targetableCount: 4,
                    launchedExpectedCount: 3,
                ...
                'registering',
                '3 of 3 browser agents ready'
            ],
            [
                'manual target adjustment after cohort selection',
                input({
                    targetCount: 2,
                    targetableCount: 3,
                    launchedExpectedCount: 3,
            ...
            [
                ...
                'waiting-for-ack',
                '2 of 3 agents acknowledged staging'
            ],
            [
                'review start',
                input({
            ...
            [
                'monitor terminal after agents disconnect',
                input({
                    targetableCount: 0,
                    runState: 'passed',
                    policy: policy({
    ...
    )('derives %s as one current step', (_label, facts, step, actionLabel) => {
