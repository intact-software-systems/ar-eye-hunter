...
import {
    deriveFleetMapModel,
    FLEET_MAP_RENDER_BUDGETS,
} from '../../../apps/rallar-black-box/src/recipe-console/fleet/fleet-map-model.ts';
import type {
...
function route(index: number, failedCount = 0): FleetGeographyRoute {
    return {
        routeId: `agent-${String(index).padStart(3, '0')}->agent-${
...
function geography(input: Readonly<{
    ...
    return {
        ...
        summary: {
            ...
            failedHistoricalAgents: agents.filter(item =>
                (item.historical?.failedOutcomes ?? 0) > 0
            ).length,
...
describe('Recipe Console Fleet map model', () => {
    ...
    it('enforces every DOM budget and pins valid selected evidence beyond the cut', () => {
        ...
        const regions = Array.from({ length: 30 }, (_, index) => region(index, {
            failed: 1,
    ...
    it('orders each layer by severity, recency, and collision-safe identity', () => {
        ...
        expect(forward.agentMarkers.items.map(item => item.agent.agentId))
            .toEqual(['a / b', 'a', 'offline-old', 'stale-new',
                'connected-new', 'current-failure', 'recovered-failure']);
    ...
    it('projects only evidence-backed candidates from the shared geography model', () => {
        ...
        expect(withRoute.agentMarkers.items.find(
