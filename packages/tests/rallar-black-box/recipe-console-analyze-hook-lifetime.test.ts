...
type RecipeConsoleControlConnection = Parameters<typeof useAnalyzeWorkspace>[0]['connection'];
...
describe('Recipe Console rendered Analyze worker lifetime', () => {
    ...
    it('binds cursor pending authority, recovers from failure, and suppresses stale window replies', async () => {
        const props = harnessProps({
            connection: createConnection(createExecution()),
            navigate: vi.fn(),
            urlState: urlState('analyze', 'dist-a'),
    ...
    async function beginControlLoad(
        ...
        if (!completion) {
            throw new Error('Analyze controller did not start a control load.');
        }
        return { completion };
    }
    ...
    async function setInput(value: string): Promise<void> {
        ...
        if (!input || !setter) {
            throw new Error('Expected responsive Analyze input.');
        }
        await act(async () => {
...
class ControllableWorker {
    ...
    request<Type extends AnalyzeWorkerRequest['type']>(
        ...
        if (!match) {
            throw new Error(`Expected Analyze worker request ${type}.`);
        }
