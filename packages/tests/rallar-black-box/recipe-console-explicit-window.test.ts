...
import { useExplicitWindow, useExplicitWindowFocusRecovery } from '../../../apps/rallar-black-box/src/recipe-console/ui/use-explicit-window.ts';
...
describe('Recipe Console explicit window model', () => {
    ...
    it('reveals a high ordinal on its exact containing page and clamps shrink', () => {
        ...
        expect(revealExplicitWindowIndex(
            deriveExplicitWindowModel({
                fingerprint: 'selector:empty',
                revision,
                total: 0,
                windowSize: 100
            }, createExplicitWindowState('selector:empty', revision)),
            217
        ))
    ...
    it('resets the range when an exact source revision changes', () => {
        const firstRevision = {};
        const nextRevision = {};
        const previous = revealExplicitWindowIndex(
            deriveExplicitWindowModel({
                fingerprint: 'selector:runs',
                revision: firstRevision,
                total: 250,
                windowSize: 100
            }, createExplicitWindowState('selector:runs', firstRevision)),
            217
        );
...
describe('useExplicitWindow', () => {
    ...
    it('owns local movement while deriving a new fingerprint and shrink immediately', async () => {
...
describe('ExplicitWindowControls', () => {
    ...
    it('keeps focused controls mounted while pending and blocks repeat requests', async () => {
        let previousCalls = 0;
        let nextCalls = 0;
        await renderControls({
            pending: true,
    ...
    it('announces an explicit unknown empty state without inventing zero matches', async () => {
        const model = deriveExplicitWindowModel({
...
describe('explicit window focus recovery', () => {
    ...
    async function render(fingerprint: string, total: number) {
        ...
        await act(async () =>
            root?.render(createElement(FocusHarness, {
                fingerprint,
                total
            }))
        );
    ...
    it('preserves a retained row and focuses the range when that row unmounts', async () => {
    ...
    it('does not recover a pending boundary action that navigation rejects', async () => {
        function PendingBoundary() {
            const model = deriveExplicitWindowModel({
                fingerprint: 'pending-boundary',
                total: 128,
                windowSize: 64
    ...
    it('does not recover an unavailable row after focus legitimately moved outside', async () => {
        await render('artifact:a', 150);
        ...
        row?.focus();
        if (!row) {
            throw new Error('Expected focused row.');
        }
        row.disabled = true;
        row.dispatchEvent(
