...
describe('Recipe Console Execute pressure windows', () => {
    ...
    it('recovers focused window content when an update drops below the control budget', async () => {
        function list(items: readonly string[]) {
            return createElement(ExecuteWindowedList<string>, {
                ...
                label: 'Focus rows',
                ordered: true,
                renderItem: (item) => createElement('li', {}, createElement('button', { type: 'button' }, item)),
    ...
    it('windows late preflight rows and warnings while retaining full preflight counts', async () => {
        ...
        const warnings = Array.from({ length: 240 }, (_, index) => index === 239 ? 'late preflight warning 239' : `warning ${index}`);
        const entry = {
            ...catalogEntry,
    ...
    it('windows 240 inspector commands and prerequisites and reaches their exact late IDs', async () => {
        ...
        const prerequisites = Array.from({ length: 240 }, (_, index) => index === 239 ? 'late prerequisite 239' : `Prerequisite ${index}`);
        const entry = {
            ...catalogEntry,
            item: {
                ...catalogEntry.item,
