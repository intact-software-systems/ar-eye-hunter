...
import { createDistributedRunTuningCandidate } from
    '../../../packages/shared-test/rallar-bb-test/distributed-run-tuning-candidate.ts';
...
describe('distributed tuning candidate hardening', () => {
    ...
    it('returns RFC 6901 schema paths and command-specific agent/preflight paths', () => {
        ...
        dynamicRole.targetPolicy = {
            mode: 'role-map', roles: { 'bad/key~x': 'agent-a' },
        };
        ...
        expect(agentResult).toMatchObject({
            ok: false,
            errors: expect.arrayContaining([expect.objectContaining({
                code: 'agent-validation', path: `${STREAM}/roomId`,
            })]),
        });
        ...
        embedded.recipes[0].recipe.commands = [{
            ...
            recipe: {
                recipeId: 'embedded',
                commands: [{
                    kind: 'rtc.stream', commandId: 'embedded-stream',
                    roomId: '', send: {}, count: 2,
                }],
            },
        }];
