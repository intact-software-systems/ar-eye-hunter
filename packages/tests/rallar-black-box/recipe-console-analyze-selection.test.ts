...
describe('Recipe Console Analyze selection', () => {
    ...
    it('returns one stable empty singleton with zero option work while Analyze is inactive at 5,000 run pairs', () => {
        const controls = Array.from({ length: 5_000 }, (_, index) => controlRun(`control-${index}`, index));
        const distributed = Array.from({ length: 5_000 }, (_, index) => distributedRun(`distributed-${index}`, `control-${index}`, index));
