...
import { SEARCHABLE_LISTBOX_WINDOW_SIZE, type SearchableListboxOption } from '../../../apps/rallar-black-box/src/recipe-console/ui/searchable-listbox-model.ts';
import { SearchableWindowedListbox } from '../../../apps/rallar-black-box/src/recipe-console/ui/SearchableWindowedListbox.tsx';
...
describe('SearchableWindowedListbox', () => {
    ...
    it('mounts no closed options, reveals a high selected key, and traverses exact pages', async () => {
    ...
    it('resets every search to its first exact result window without committing', async () => {
        const onSelect = vi.fn();
        const options = fixtureOptions(203).map((option, index) => ({
            ...option,
            searchText: index < 101
                ? `common ${
                    index === 0 ? 'first-needle' : index === 50
                        ? 'middle-needle'
                        : index === 100
                        ? 'last-needle'
                        : ''
    ...
    it('keeps one stable polite atomic range status across filtered count bands', async () => {
        ...
        expect(
            container.querySelector('[data-searchable-listbox-empty]')
                ?.getAttribute('aria-hidden')
        ).toBe('true');
    });

    it('blocks a stale search commit, then commits only Enter and touch click once', async () => {
        const onSelect = vi.fn();
        ...
        if (!setter || !stale) {
            throw new Error('Expected stale search fixture.');
        }
        await act(async () => {
        ...
        if (!option) {
            throw new Error('Expected filtered option.');
        }
    ...
    it('moves the active descendant across pages and restores trigger focus on Escape', async () => {
        await renderSelector({
    ...
    it('closes on Tab without preventing navigation or implicitly committing', async () => {
        const onSelect = vi.fn();
        await renderSelector({ onSelect, options: fixtureOptions(101) });
        await clickTrigger(container);
    ...
    it.each(['Previous', 'Next', 'range', 'option'] as const)(
        ...
        async (targetKind) => {
            ...
            if (!target) {
                throw new Error(`Expected popup target ${targetKind}.`);
            }
            target.focus();
    ...
    it('dismisses on Tab from a window control and outside pointer or focus', async () => {
        const onSelect = vi.fn();
        await renderSelector({ onSelect, options: fixtureOptions(201) });
        await clickTrigger(container);
        ...
        if (!outside) {
            throw new Error('Expected outside target.');
        }
        expect(
            container.querySelector('[data-searchable-windowed-listbox]')
                ?.contains(outside)
        ).toBe(false);
        await clickTrigger(container);
        await act(async () =>
    ...
    it('keyboard-toggles and escapes an expanded duplicate-key error on the trigger', async () => {
        ...
        if (!trigger) {
            throw new Error('Expected duplicate-key trigger.');
        }
        trigger.focus();
    ...
    it('does not navigate, commit, or dismiss while an IME composition is active', async () => {
        const onSelect = vi.fn();
        await renderSelector({ onSelect, options: fixtureOptions(101) });
        await clickTrigger(container);
        const search = searchInput(container);
    ...
    it('re-reveals a high controlled selection after a batched filtered reopen', async () => {
        const options = fixtureOptions(250);
        await renderSelector({ options, selectedKey: 'key-217' });
        const trigger = await clickTrigger(container);
    ...
    it('re-reveals a high controlled selection when an open context changes', async () => {
        const options = fixtureOptions(250);
        const revision = {};
        await renderSelector({
            contextKey: 'runs:a',
    ...
    it('keeps duplicate values distinct and renders long bidi identifiers exactly', async () => {
    ...
    it('reports disabled, unavailable, and empty states without inventing a selection', async () => {
        ...
        expect(
            container.querySelector('[data-searchable-listbox-unavailable]')
                ?.textContent
        ).toContain('missing\u202e/id');
        expect(
            container.querySelector('[data-searchable-listbox-unavailable] bdi')
                ?.getAttribute('dir')
        ).toBe('ltr');
        await clickTrigger(container);
        expect(rangeText(container)).toBe('Showing 1–1 of 1 options.');
    ...
    it('hands focus to a persistent enabled fallback when an open picker is disabled', async () => {
        const options = fixtureOptions(101);
        await renderSelector({ options });
        await clickTrigger(container);
        expect(document.activeElement).toBe(searchInput(container));
    ...
    it('does not steal external focus when an open picker becomes disabled', async () => {
        const options = fixtureOptions(101);
        await renderSelector({ options });
        await clickTrigger(container);
        const outside = container.querySelector<HTMLButtonElement>(
    ...
    it('recovers focused window controls when an option revision crosses the budget', async () => {
        const firstRevision = {};
        await renderSelector({
...
async function clickTrigger(container: HTMLElement): Promise<HTMLButtonElement> {
    ...
    if (!trigger) {
        throw new Error('Expected listbox trigger.');
    }
    await act(async () => trigger.click());
    return trigger;
}
...
function searchInput(container: HTMLElement): HTMLInputElement {
    ...
    if (!input) {
        throw new Error('Expected listbox search input.');
    }
    return input;
}
...
async function clickWindow(
    ...
    if (!button) {
        throw new Error(`Expected ${label} window button.`);
    }
