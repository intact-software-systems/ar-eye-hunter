...
async function clickSummary(
    ...
    const summary = [...owner?.querySelectorAll('summary') ?? []].find((row) => row.textContent?.startsWith(label));
...
describe('RetentionPanel', () => {
    ...
    it('always offers one visible preview action and issues one preview request', async () => {
        ...
        const button = [...container.querySelectorAll('button')].find((item) => item.textContent === 'Preview cleanup');
    ...
    it('shows every exact candidate and consequence before requesting review', async () => {
    ...
    it('renders exact whitespace and bidi-bearing IDs in an isolated LTR owner', async () => {
    ...
    it('keeps drifted consequences visible as stale and never offers review', async () => {
        await render(cleanupController({
            state: {
                status: 'drift',
    ...
    it('announces idle, busy, success, error, and unavailable state text', async () => {
        const expected = [
    ...
    it('shows the exact successful deletion result without exposing secret state', async () => {
        await render(cleanupController({
            state: {
                status: 'succeeded',
                confirmation: {
