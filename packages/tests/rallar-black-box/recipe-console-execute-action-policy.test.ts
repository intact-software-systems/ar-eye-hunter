...
describe('Recipe Console Execute action policy', () => {
    ...
    )('enables the current lifecycle action directly for state %s', (
        runState,
        action
    ) => {
        const policy = deriveExecuteActionPolicy(input({
    ...
    )('keeps terminal %s runs refreshable/exportable but not cancellable', (
        runState
    ) => {
        const policy = deriveExecuteActionPolicy(input({
            runState,
    ...
    )('blocks guided mutations when %s is unsafe', (
        field,
        value,
        code
    ) => {
    ...
    it('does not require arming for Create, Stage, Start, or Cancel', () => {
        ...
        expect(
            deriveExecuteActionPolicy(input({
                runState: 'draft',
                hasKnownRun: true
            })).stage.enabled
        ).toBe(true);
