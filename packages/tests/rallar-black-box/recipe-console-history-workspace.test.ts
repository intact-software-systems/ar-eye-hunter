...
describe('HistoryWorkspace', () => {
    ...
    it('shows committed filters, exact provenance, copy, legacy handoff, and empty truth', async () => {
        ...
        const copy = [...container.querySelectorAll('button')].find((button) => button.textContent === 'Copy filtered link');
        await act(async () => copy?.click());
        expect(onCopyLink).toHaveBeenCalledTimes(1);
        const legacy = [...container.querySelectorAll('a')].find((anchor) => anchor.textContent === 'Open legacy Runs');
    ...
    it('bounds 5,000 runs behind controls outside the table scroll', async () => {
    ...
    it('preserves an equal poll page, resets filters and source, and recovers focus', async () => {
    ...
    it('preserves the current window across detail-only document revisions', async () => {
        const fixture = createRecipeConsoleControlScaleFixture({ pairCount: 161 });
