...
import { deriveRecipeConsoleControlSelection } from '../../../apps/rallar-black-box/src/recipe-console/control/control-selection.ts';
import {
...
import { createRecipeConsoleShareHref, parseRecipeConsoleUrl } from '../../../apps/rallar-black-box/src/recipe-console/routing/url-state-codec.ts';
import type { RecipeConsoleUrlState } from '../../../apps/rallar-black-box/src/recipe-console/routing/url-state-contract.ts';
import type { ControlRetentionCandidate } from '../../../packages/shared-test/rallar-bb-test/control-retention.ts';
import type {
...
describe('Recipe Console History model', () => {
    ...
    it.each(
        [
            ...
            [
                'partial unavailable collection',
                query({
                    status: 'partial',
                    source: 'unavailable',
    ...
    )('projects %s provenance without overstating evidence', (_label, controlQuery, expected) => {
        expect(
            deriveRecipeConsoleHistoryModel({
                urlState: urlState(),
                query: controlQuery
    ...
    it('renders but quarantines duplicate, unsafe, malformed, missing, and ambiguous identities', () => {
        ...
        expect(model.rows.filter((row) => row.distributedRunId === 'duplicate'))
            .toHaveLength(2);
        expect(
            model.rows.filter((row) => row.distributedRunId === 'duplicate')
                .every((row) => row.quarantined && !row.actions.identity)
        ).toBe(true);
...
describe('Recipe Console retention selection reconciliation', () => {
    it('clears only exact deleted associations and preserves filters, timing, and unrelated state', () => {
        ...
        const capture = captureRetentionSelectionBeforeCleanup({
            ...
            candidates: [retentionCandidate('control-delete', [
                'distributed-focus-delete',
                'distributed-right-delete'
            ])]
        });
    ...
    it('proves filter to Candidate to cleanup to copied URL and back-forward restoration', () => {
        ...
        const candidateRow = model.rows.find((row) => row.distributedRunId === 'candidate-delete')!;
