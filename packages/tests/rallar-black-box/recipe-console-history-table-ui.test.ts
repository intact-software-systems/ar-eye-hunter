...
import type { RecipeConsoleHistoryModel, RecipeConsoleHistoryRow } from '../../../apps/rallar-black-box/src/recipe-console/history/history-model.ts';
import { HistoryTable } from '../../../apps/rallar-black-box/src/recipe-console/history/HistoryTable.tsx';
...
const baselinePatch = { compareLeft: 'distributed-safe' } satisfies Partial<RecipeConsoleUrlState>;
...
describe('HistoryTable', () => {
    ...
    async function render(model: RecipeConsoleHistoryModel, onBaseline = vi.fn(), onCandidate = vi.fn()) {
        root = createRoot(container);
        await act(async () =>
            root?.render(createElement(HistoryTable, {
                model,
                onBaseline,
                onCandidate
            }))
    ...
    it('keeps the named scroll region keyboard-focusable and exposes exact evidence', async () => {
    ...
    it('shows quarantined, malformed, duplicate, missing, and ambiguous reasons without actions', async () => {
        const blocked = [
            historyRow({
                key: 'history-row:1',
