...
import { createAnalyzeWorkerRuntime } from
    '../../../apps/rallar-black-box/src/recipe-console/analyze/analyze-worker-runtime.ts';
...
describe('Recipe Console Analyze worker runtime', () => {
    it('accepts transferred bytes without parsing and derives one bounded model only after start', async () => {
        ...
        try {
            ...
            expect(messages(posted)).toEqual([{
                type: 'accepted', operationGeneration: 7,
            }]);
            ...
            const complete = messages(posted).find(
                (message): message is Extract<AnalyzeWorkerResponse, { type: 'complete' }> =>
    ...
    it('parses a raw Control artifact envelope only after start and enforces file-count bounds in-worker', async () => {
        ...
        await harness.runtime.handle({
            type: 'offer', operationGeneration: 4,
            artifact: {
                source: 'control', label: 'Raw Control artifact',
                files: [], controlEnvelope: envelopeBytes,
                expectedControlIdentity,
            },
        });
        expect(harness.messages).toEqual([{
            type: 'accepted', operationGeneration: 4,
        }]);
        ...
        await excessive.runtime.handle({
            type: 'offer', operationGeneration: 5,
            artifact: {
                source: 'control', label: 'Too many files', files: [],
        ...
        expect(mismatched.messages.at(-1)).toMatchObject({
            type: 'failed', operationGeneration: 6,
    ...
    it('enforces the existing v2 Control artifact base-file contract in-worker', async () => {
        ...
        const invalidArtifacts = [
            ...
            {
                ...fixture,
                files: Object.fromEntries(Object.entries(fixture.files).filter(
        ...
        for (const [index, artifact] of invalidArtifacts.entries()) {
            ...
            await harness.runtime.handle({
                type: 'start', operationGeneration: index + 1,
            });
    ...
    it('validates exact long Control identities independently of bounded display fields', async () => {
        ...
        await harness.runtime.handle({
            type: 'offer', operationGeneration: 1,
            artifact: {
                source: 'control', label: 'Long identity Control artifact', files: [],
                expectedControlIdentity,
        ...
        await mismatch.runtime.handle({
            type: 'offer', operationGeneration: 1,
            artifact: {
                source: 'control', label: 'Wrong expected identity', files: [],
                expectedControlIdentity: await createAnalyzeControlIdentityDigest({
    ...
    it('classifies an unusable distributed envelope without crossing artifact payload text', async () => {
        ...
        await harness.runtime.handle({
            type: 'offer', operationGeneration: 8,
            artifact: {
                source: 'local-files', label: 'Malformed envelope',
                files: transferFiles({
        ...
        expect(harness.messages.at(-1)).toMatchObject({
            type: 'failed', operationGeneration: 8,
    ...
    it('finds first, middle, and last scale evidence and returns only bounded worker projections', async () => {
        ...
        for (const [index, needle] of [
            ...
            const response = harness.messages.findLast(message =>
        ...
        expect(recursiveKeys(complete?.projection)).not.toEqual(expect.arrayContaining([
            'files', 'snapshots', 'catalog', 'events', 'results',
        ]));
        expect(recursiveKeys(complete?.projection).some(key =>
        ...
        const allEvidence = harness.messages.findLast(
            (message): message is Extract<AnalyzeWorkerResponse, { type: 'search-complete' }> =>
                message.type === 'search-complete',
        );
        ...
        const nextWindow = harness.messages.find(
            (message): message is Extract<AnalyzeWorkerResponse, { type: 'window-complete' }> =>
        ...
        await harness.runtime.handle({
            type: 'select', modelGeneration: 9, selectionGeneration: 1,
            requestId: 301, evidenceId: selectedId,
        });
        expect(harness.messages).toContainEqual(expect.objectContaining({
        ...
        await harness.runtime.handle({
            type: 'tune', modelGeneration: 9, tuneGeneration: 1,
            requestId: 302, focusRunId: complete?.projection.distributedRunId,
        });
        const tune = harness.messages.find(
            (message): message is Extract<AnalyzeWorkerResponse, { type: 'tune-complete' }> =>
                message.type === 'tune-complete',
        );
        ...
        await harness.runtime.handle({
            type: 'tune', modelGeneration: 9, tuneGeneration: 2,
            requestId: 303,
    ...
    it('round-trips oversized evidence authority through a bounded opaque handle', async () => {
        ...
        const handle = complete?.initialWindow.entries.find(entry =>
            entry.id.startsWith('opaque-id:')
        )?.id;
    ...
    it('returns typed payload-free failures and releases all authority on disposal', async () => {
        ...
        await harness.runtime.handle({
            type: 'offer', operationGeneration: 1,
            artifact: {
                source: 'local-files', label: secret,
        ...
        await harness.runtime.handle(offer(2, createRecipeConsoleScaleFixture({
            eventCount: 6, resultCount: 3,
        })));
        expect(harness.messages.at(-1)).toMatchObject({
            type: 'failed', error: { code: 'worker-disposed', recoverable: false },
        });
    });
    ...
    it('rejects malformed wire requests with a typed secret-free boundary error', async () => {
        ...
        await harness.runtime.handle({
            type: 'not-a-worker-operation', payload: secret,
        } as never);

        expect(harness.messages).toEqual([{
            type: 'failed',
            error: {
                code: 'invalid-request', stage: 'offer', recoverable: true,
            },
        }]);
        expect(JSON.stringify(harness.messages)).not.toContain(secret);
        harness.runtime.dispose();
    ...
    it('rejects malformed nested query and Tune fields without throwing or leaking payloads', async () => {
        ...
        await expect(harness.runtime.handle({
            type: 'search', modelGeneration: 1, queryGeneration: 1,
            requestId: 81, query: { query: { secret } }, windowSize: 64,
        } as never)).resolves.toBeUndefined();
        await expect(harness.runtime.handle({
            type: 'tune', modelGeneration: 1, tuneGeneration: 1,
            requestId: 82, timingMetric: { secret },
        } as never)).resolves.toBeUndefined();
    ...
    it('rejects oversized offer metadata and RPC text before derivation', async () => {
        ...
        await offerHarness.runtime.handle({
            type: 'offer', operationGeneration: 1,
            artifact: {
                source: 'control', label: 'Digest with smuggled metadata', files: [],
        ...
        const requests = [
            {
                type: 'search', modelGeneration: 1, queryGeneration: 1,
                requestId: 91, query: { query: oversized }, windowSize: 64,
            },
            {
                type: 'window', modelGeneration: 1, queryGeneration: 1,
                windowGeneration: 1, requestId: 92, query: {}, cursor: oversized,
                windowSize: 64,
            },
            {
                type: 'select', modelGeneration: 1, selectionGeneration: 1,
                requestId: 93, evidenceId: oversized,
            },
            {
                type: 'tune', modelGeneration: 1, tuneGeneration: 1,
                requestId: 94, focusRunId: oversized,
            },
        ] as const;
        ...
        expect(harness.messages.slice(-requests.length)).toEqual([
            expect.objectContaining({
                type: 'failed', error: expect.objectContaining({ stage: 'search' }),
            }),
            expect.objectContaining({
                type: 'failed', error: expect.objectContaining({ stage: 'window' }),
            }),
            expect.objectContaining({
                type: 'failed', error: expect.objectContaining({ stage: 'selection' }),
            }),
            expect.objectContaining({
                type: 'failed', error: expect.objectContaining({ stage: 'tune' }),
            }),
        ]);
    ...
    it('suppresses late search and window replies by independent generation', async () => {
        ...
        expect(harness.messages.slice(searchStart).filter(
            message => message.type === 'search-complete'
        )).toEqual([expect.objectContaining({
            type: 'search-complete', queryGeneration: 2, requestId: 402,
        })]);
        ...
        if (cursor) {
            ...
            await Promise.all([
                harness.runtime.handle({
                    type: 'window', modelGeneration: 1, queryGeneration: 3,
                    windowGeneration: 1, requestId: 404, query: {}, cursor,
                    windowSize: 64,
                }),
                harness.runtime.handle({
                    type: 'window', modelGeneration: 1, queryGeneration: 3,
                    windowGeneration: 2, requestId: 405, query: {}, cursor,
                    windowSize: 64,
                }),
            ]);
            expect(harness.messages.slice(windowStart).filter(
                message => message.type === 'window-complete'
            )).toEqual([expect.objectContaining({
                type: 'window-complete', windowGeneration: 2, requestId: 405,
            })]);

            await harness.runtime.handle(search(
                1, 4, 406, fixture.needles.events.middle,
            ));
            const crossQueryStart = harness.messages.length;
            await harness.runtime.handle({
                type: 'window', modelGeneration: 1, queryGeneration: 3,
                windowGeneration: 3, requestId: 407, query: {}, cursor,
                windowSize: 64,
            });
            expect(harness.messages.slice(crossQueryStart).filter(
                message => message.type === 'window-complete'
            )).toEqual([]);
        }
        harness.runtime.dispose();
    ...
    it('reimports the transferred portable envelope with equivalent identity and counts', async () => {
        ...
        if (exported) {
            await reimported.runtime.handle({
                ...
                artifact: {
                    source: 'local-files', label: 'Reimported envelope',
...
function offer(
    ...
    return {
        type: 'offer', operationGeneration,
        artifact: {
            source: 'local-files', label: `Scale ${operationGeneration}`,
            generatedAtEpochMs: fixture.generatedAtEpochMs,
            artifactSchemaVersion: fixture.artifactSchemaVersion,
            files: transferFiles(fixture.files),
        },
...
function search(
    ...
    return {
        type: 'search', modelGeneration, queryGeneration, requestId,
        query: { query }, windowSize: 64,
    };
}
...
function transferFiles(
    ...
    return Object.entries(files).flatMap(([name, value]) => {
        ...
        return [{ name, bytes: bytes.buffer.slice(
            bytes.byteOffset,
