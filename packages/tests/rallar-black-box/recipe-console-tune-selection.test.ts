...
function run(
    ...
    return {
        ...
        state: 'failed',
        createdAtEpochMs: input.start - 100,
        updatedAtEpochMs: input.updated,
        startedAtEpochMs: input.start,
        completedAtEpochMs: input.end,
        targetAgentIds: input.agents,
        commandLinks: [],
        manifest: {
            schemaVersion: 1,
            distributedRunId: input.id,
...
const left = run({
    ...
    agents: ['agent-left', 'agent-shared'],
...
const right = run({
    ...
    agents: ['agent-shared', 'agent-right'],
