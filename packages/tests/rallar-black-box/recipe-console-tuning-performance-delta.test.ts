...
describe('Recipe Console narrow tuning performance comparison', () => {
    it.each([
    ...
    ] as const)('compares %s through its deterministic statistic', (
        timingMetric,
        statistic,
        unit,
        left,
        ...
        const comparison = compareDistributedRunTuningPerformance({
            ...
            left: tuningPerformance({
                ...
                stream: {
                    duration: {
                        count: 100, minMs: 20, p50Ms: 50, p95Ms: 100,
                        p99Ms: 120, maxMs: 140, averageMs: 55,
                        spreadRatio: 2, outlierCount: 1,
                    },
                    maxStartDriftMs: 20,
            ...
            right: tuningPerformance({
                ...
                stream: {
                    duration: {
                        count: 100, minMs: 30, p50Ms: 100, p95Ms: 250,
                        p99Ms: 300, maxMs: 320, averageMs: 120,
                        spreadRatio: 2.5, outlierCount: 4,
                    },
                    maxStartDriftMs: 120,
    ...
    it('compares RTC frame cadence drift drop and backpressure evidence together', () => {
        const comparison = compareDistributedRunTuningPerformance({
            ...
            right: tuningPerformance({ stream: {
                plannedFrames: 200,
                completedFrames: 160,
                failedFrames: 40,
                droppedFrames: 40,
