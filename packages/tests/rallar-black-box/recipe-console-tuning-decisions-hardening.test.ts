...
import { deriveDistributedRunTuningDecisions } from
    '../../../packages/shared-test/rallar-bb-test/distributed-run-tuning-decisions.ts';
import {
    failure,
    streamCommand,
    targetResolution,
...
describe('Recipe Console tuning decision safety', () => {
    ...
    it('suppresses exact knobs when reference or traversal limitations hide recipe truth', () => {
        ...
        const truncated = tuningManifest({ commands: [
            streamCommand(),
            ...Array.from({ length: 2_000 }, (_, index) => ({
                kind: 'health' as const, commandId: `health-${index}`,
            })),
        ] });
            ...
            analysis: tuningAnalysis({
                ...
                performance: tuningPerformance({
                    ...
                    stream: {
                        slowestAgents: [
                            {
                                agentId: 'stream-b', streamCount: 1,
                                plannedFrames: 100, completedFrames: 100, maxMs: 5_000,
                            },
                            {
                                agentId: 'stream-a', streamCount: 1,
                                plannedFrames: 100, completedFrames: 100, maxMs: 100,
                            },
                        ],
                    },
                }),
