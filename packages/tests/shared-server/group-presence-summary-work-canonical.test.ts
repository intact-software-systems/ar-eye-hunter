...
describe('GroupPresenceSummaryWork canonical persisted command', () => {
    it.each(INVALID_SUMMARY_SCENARIOS)(
        'rejects $name before read or effects',
        async ({ mutate }) => {
            const begin = vi.fn();
            ...
            const read = vi.spyOn(worker, 'read').mockRejectedValue(
                new Error('summary read must not run for malformed work'),
            );
