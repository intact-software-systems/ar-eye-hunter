...
import { RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE } from '@shared-server/rallar-system/repositories/RtcTopologySnapshotRepository.ts';
...
describe('scalar RTC topology authority cutover', () => {
    it('conditionally invalidates the complete affected dependency family', async () => {
        ...
        await seed(runtime, RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE, groupStateGroupStorageKey(affected), {
            groupRef: affected,
        ...
        await seed(runtime, RTC_TOPOLOGY_PUBLICATION_WORK_INDEX_NAMESPACE, 'old-work', {
            groupRef: affected,
            publicationId: 'old-work:4:1',
            workId: 'old-work'
        });
    ...
    it('rolls back the guard request and family deletes after bounded conflicts', async () => {
        ...
        await seed(runtime, RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE, groupStateGroupStorageKey(groupRef), {
            groupRef,
    ...
    it('replays the exact deterministic enqueue after an acknowledgement conflict', async () => {
        ...
        runtime.beforeConditionalWrite = async (operation, namespace, key) => {
            if (
                ...
                if (!current) {
                    throw new Error('recompute request disappeared');
                }
    ...
    it.each([
        ['extra field', (request: Record<string, unknown>) => ({ ...request, extra: true }), NEVER_EXPIRE_AT_TIMESTAMP],
    ...
    it('autonomously retries transient failures and stops without rescheduling', async () => {
        ...
        if (!retry) {
            throw new Error('worker did not schedule an autonomous retry');
        }
    ...
    it('requires an explicit terminal policy for an absent group', async () => {
        ...
        expect(
            await drainRtcTopologyScalarRecomputeRequests(
                runtime,
