...
import {
  ...
  withRoutingLexicalOverrides,
} from './mutation-routing-execution-paths.ts';
import {
  evaluateStaticTruth,
  readExactStaticString,
  resolveStaticValues,
} from './mutation-static-semantics.ts';
...
function appendInvocation(
  ...
  const invocations = references.map((reference) =>
    invocationLexical(callable, [...reference.boundSlots, ...slots], path.lexical)
  );
...
function resolveCallableReferences(
  ...
  if (node?.type === 'CallExpression' || node?.type === 'OptionalCallExpression') {
    if (readCallFamily(node, lexical) !== 'bind') return [];
  ...
  if (resolved.unknown || resolved.values.length === 0) return [];
  return resolved.values.flatMap((candidate) =>
    resolveCallableReferences(candidate, expected, lexical, new Set(resolving).add(key))
  );
}
...
function resolveCallSlots(
  ...
  if (family === 'apply') return resolveArgumentSequence(arguments_[1], lexical, new Set());
...
function resolveArgumentSequence(
  ...
  const node = unwrap(value);
  if (!node) return [unknownArgumentSlot()];
  if (node.type === 'ArrayExpression' || node.type === 'TupleExpression') {
    return resolveDirectSlots(rawNodes(node.elements), lexical);
  }
  if (node.type === 'Identifier') {
    const key = lexical.bindings.identifierKey(node);
    if (!key || resolving.has(key)) return [unknownArgumentSlot()];
    const resolved = lexical.resolveIdentifier(node);
    if (resolved.values.length === 0) return [unknownArgumentSlot()];
    const alternatives = resolved.values.map((candidate) =>
      resolveArgumentSequence(candidate, lexical, new Set(resolving).add(key))
    );
...
function findCallableOwner(program: AstNode, callable: AstNode): AstNode {
  let owner = program;
  visit(program, (node) => {
    if (node !== callable && isFunction(node) && containsNode(node, callable)) owner = node;
  });
  return owner;
}
