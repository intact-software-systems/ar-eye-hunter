...
describe('Mutation route owner boundary traversal contracts', { timeout: 30_000 }, () => {
  ...
  it('always rejects incomplete and duplicate inventories', () => {
    expect(validateMutationRouteInventory([])).toContain('Expected 54 entrypoints, found 0');
    expect(validateMutationRouteInventory(MUTATION_ROUTE_INVENTORY.slice(0, 53))).toContain(
      'Expected 54 entrypoints, found 53',
    );
    expect(
      validateMutationRouteInventory([
        ...MUTATION_ROUTE_INVENTORY.slice(0, 53),
        MUTATION_ROUTE_INVENTORY[0]!,
      ]),
    ).toEqual(
      expect.arrayContaining([
        expect.stringContaining('Duplicate mutation route'),
        'Inventory must cover all 50 AppInbox command types',
      ]),
    );
  });
