...
describe('SpaStatisticsService', () => {
  ...
  it('uses bounded group snapshot pages for workspace and realtime statistics', async () => {
    ...
    const service = new SpaStatisticsService({
      ...
      groupStateService: {
        ...
        readCurrentSnapshot: (ref: { groupId: string }) =>
  ...
  it('uses the newest events when the recent-events service method is unavailable', async () => {
    ...
    const service = new SpaStatisticsService({
      ...
      groupStateService: {
        listSnapshots: () =>
          Promise.resolve([createGroupSnapshot('room-1', [['alice', 'member']])]),
        listSnapshotsPage: () =>
          Promise.resolve({
            snapshots: [],
            scannedGroupCount: 0,
...
function createService(input: TestServiceInput): SpaStatisticsService {
  ...
  return new SpaStatisticsService({
    ...
    groupStateService: {
      ...
      listEvents: (ref) =>
        Promise.resolve(
          Array.from({ length: groupEventCounts[ref.groupId] ?? 0 }, (_, index) =>
            createGroupEvent(ref.groupId, index),
          ),
        ),
