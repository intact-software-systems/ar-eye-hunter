...
function createTransactionalSql(
    ...
    function rootSql(_values: readonly PSqlParameter[]): object;
    function rootSql<Result>(
        _strings: TemplateStringsArray,
        ..._values: readonly PSqlParameter[]
    ): Promise<Result>;
    function rootSql(): never {
        throw new Error('Root SQL should not be called.');
    }
    return Object.assign(rootSql, {
        begin: async <T>(fn: (sql: PSqlSql) => Promise<T>): Promise<T> => await fn(createGuardedBatchTransactionSql(captured, resultRows))
    });
}

interface GuardedBatchTransactionSql extends PSqlSql {
    savepoint<T>(fn: (sql: PSqlSql) => Promise<T>): Promise<T>;
}

function createGuardedBatchTransactionSql(
    captured: CapturedQuery[],
    resultRows: PSqlRows
): GuardedBatchTransactionSql {
    function transactionSql(_values: readonly PSqlParameter[]): object;
    ...
    function transactionSql<Result>(
        ...
        captured.push({ source: stringsOrValues.join('?'), values });
        return Promise.resolve(resultRows) as Promise<Result>;
    }
    const runNested = async <T>(fn: (sql: PSqlSql) => Promise<T>): Promise<T> => await fn(createGuardedBatchTransactionSql(captured, resultRows));
    return Object.assign(transactionSql, { begin: runNested, savepoint: runNested });
}
