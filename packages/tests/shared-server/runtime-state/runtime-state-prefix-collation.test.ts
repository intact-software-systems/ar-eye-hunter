...
function captureQueries(): Readonly<{ sql: PSqlSql; queries: string[]; }> {
    ...
    return { sql: createCapturedQuerySql(queries), queries };
}

function createCapturedQuerySql(queries: string[]): PSqlSql {
    function sql<Result>(
    ...
    return Object.assign(sql, {
        begin: async <T>(fn: (transaction: PSqlSql) => Promise<T>): Promise<T> => await fn(createCapturedQuerySql(queries))
    });
}
