...
import type {
  ClientStateInboxAfterCommitResult,
  ClientStateInboxHandlerDependencies,
...
describe('shared-server client-state public API contract', () => {
  it('keeps every predecessor runtime export as an identical canonical binding', () => {
  ...
  it('keeps public, canonical, and compatibility runtime exports identical', () => {
    expect(sharedServer.createClientStateService).toBe(
      compatibilityService.createClientStateService,
    );
  ...
  it('does not leak internal client-state construction or handler bindings', () => {
  ...
  it('keeps predecessor public types equal to their canonical and compatibility contracts', () => {
    ...
    expectTypeOf<PublicSharedServer.ClientAuthorisedWsSessionConnectAppInboxPayload>().toEqualTypeOf<ClientAuthorisedWsSessionConnectAppInboxPayload>();
    expectTypeOf<PublicSharedServer.ClientAuthorisedWsSessionDisconnectAppInboxPayload>().toEqualTypeOf<ClientAuthorisedWsSessionDisconnectAppInboxPayload>();
...
void (undefined as
  | PublicClientStateMutationService
  | PublicClientStateServiceFactory
  | PublicClientStateServiceTimingFactory
  | PublicClientStateInboxHandlerDependencies
  | PublicClientStateInboxAfterCommitResult
  | ClientStateInboxHandlerDependencies
  | ClientStateInboxAfterCommitResult);
