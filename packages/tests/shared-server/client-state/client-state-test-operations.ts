  ...
  return {
    upsertPrincipal: async (scope, principalId, request) =>
      await context.execute(() =>
        toUpsertPrincipalCommandInput(scope, principalId, request, context.nextId()),
      ),
...
function createAuthorisedWsOperations(
  ...
  ClientStatePhaseTestDriver,
  'registerAuthorisedWsClientSession' | 'disconnectAuthorisedWsClientSession'
> {
  return {
    registerAuthorisedWsClientSession: async (auth, generationId, input = {}) =>
      await executeAuthorisedWsConnect(context, auth, generationId, input),
