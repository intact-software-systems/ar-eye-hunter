...
export function readAfterWrite(
  ...
  return {
    ...
    instance:
      computed.instance.operation === 'none' ? null : entryValue(computed.instance.value, 1),
...
export function requireWrite(
  computed: ReturnType<
    typeof import('@shared-server/rallar-system/client-state/mutation/compute/compute-client-mutation.ts').computeClientMutation
  >,
): ClientMutationComputedAppliedWrite {
...
export function instanceFrom(computed: ClientMutationComputedAppliedWrite): ClientInstance {
  if (computed.instance.operation === 'none') throw new Error('Expected instance candidate');
  return computed.instance.value;
}

export function sessionFrom(computed: ClientMutationComputedAppliedWrite): ClientSession {
  if (computed.session.operation === 'none') throw new Error('Expected session candidate');
