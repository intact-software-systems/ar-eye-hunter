import { createMutationBoundaryLexicalValues, type MutationBoundaryLexicalValues } from './mutation-boundary-lexical-values.ts';
...
import { asNode, asNodes, containsNode, isFunction, readBoundNames, readMemberName, readName, unwrap } from './mutation-routing-live-ast.ts';
import { evaluateMapEntriesProjection, evaluateMapProjection, readStaticCollectionMethod } from './mutation-routing-map-collection.ts';
import { evaluateObjectEntriesMap, evaluateStaticObjectCollection } from './mutation-routing-object-collection.ts';
import {
...
export function hasLiveAppInboxRegistration(
    ...
    if (typeArgument.type !== 'Identifier') {
        return false;
    }
...
function evaluateTypes(
    ...
    if (direct) {
        return knownRegistrationTypes([direct]);
    }
    ...
    if (method === 'map' || method === 'flatMap') {
        ...
        if (entriesProjection) {
            return entriesProjection;
        }
    }
