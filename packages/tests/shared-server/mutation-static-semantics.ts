...
export function resolveStaticValues(
    ...
    if (literal.found) {
        return exactResolution(literal.value);
    }
    if (node.type === 'Identifier') {
        ...
        const values = resolved.values.map((candidate) => resolveStaticValues(candidate, lexical, new Set(resolving).add(key)));
...
export function evaluateStaticTruth(
    ...
    if (truth.truePossible === truth.falsePossible) {
        return undefined;
    }
...
export function resolveStaticPropertyKeys(
    ...
    const resolved = resolveStaticValues(value, lexical);
    const names = new Set(
        [...resolved.values].flatMap((candidate) => typeof candidate === 'string' || typeof candidate === 'number' ? [String(candidate)] : [])
    );
    return {
        names,
