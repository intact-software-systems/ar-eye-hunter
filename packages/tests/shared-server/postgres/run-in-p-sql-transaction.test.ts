...
function createEntry(input: CreateEntryInput): ResourceEntry {
    const { key, typeId, status, attempts = 0 } = input;
    return {
        key,
...
const incomingEntry = createEntry({
    key: { topicId: 'app-command', resourceId: 'command-1', contextId: 'app-1' },
    typeId: 'APP_INBOX',
...
const outboxEntry = createEntry({
    key: { topicId: 'state-changed', resourceId: 'effect-1', contextId: 'app-1' },
    typeId: 'APP_OUTBOX',
