...
export function evaluateLexicalIdentifier(
  ...
  try {
    ...
    return importedPath && importedProgram
      ? evaluateExport(imported.imported, importedProgram, importedPath, context, resolving)
      : UNKNOWN_REGISTRATION_TYPES;
  } finally {
...
export function evaluateLexicalMember(
  ...
  if (resolved.unknown || resolved.values.length !== 1) return UNKNOWN_REGISTRATION_TYPES;
...
export function isStaticObjectEntries(value: AstNode | undefined): boolean {
  const node = unwrap(value);
...
function evaluateExport(
  ...
  for (const statement of asNodes(program.body)) {
    ...
    const found = evaluateExport(name, nextProgram, nextPath, parent, resolving);
    if (found.kind === 'known') return found;
  }
  return UNKNOWN_REGISTRATION_TYPES;
}

function findExportIdentifier(program: AstNode, name: string): AstNode | undefined {
  const declaration = findAstNode(
    program,
    (candidate) =>
      (candidate.type === 'VariableDeclarator' || candidate.type === 'FunctionDeclaration') &&
