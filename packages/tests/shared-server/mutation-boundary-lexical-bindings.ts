...
export function createMutationBoundaryLexicalBindings(
  ...
  const declarePattern = (value: unknown, scope: Scope): void => {
    ...
    if (node.type === 'ArrayPattern') {
      for (const element of asNodes(node.elements)) declarePattern(element, scope);
    }
  };
  ...
  const scanFunction = (node: AstNode, parent: Scope): void => {
    if (node.type === 'FunctionDeclaration') {
      ...
      if (id) declareName(parent, readName(id), id);
    }
    ...
    nodeScopes.set(node, scope);
    const id = asNode(node.id);
    if (id && node.type === 'FunctionExpression') declareName(scope, readName(id), id);
    for (const parameter of asNodes(node.params)) declarePattern(parameter, scope);
    scanChildren(node, scope, new Set(['id']));
  };

  const scanClass = (node: AstNode, parent: Scope): void => {
    const id = asNode(node.id);
    if (id && node.type === 'ClassDeclaration') declareName(parent, readName(id), id);
    const scope = createScope('class', parent, nextClassId++);
    nodeScopes.set(node, scope);
    if (id && node.type === 'ClassExpression') declareName(scope, readName(id), id);
    scanChildren(node, scope, new Set(['id']));
  };
  ...
  const scan = (value: unknown, scope: Scope): void => {
    ...
    if (Array.isArray(value)) {
      for (const child of value) scan(child, scope);
      return;
    }
    ...
    if (node.type === 'VariableDeclaration') {
      ...
      for (const specifier of asNodes(node.specifiers)) {
        ...
        if (local) declareName(scope, readName(local), local);
      }
    }
  ...
  return {
    ...
    thisKey: (value) => {
      ...
      while (scope) {
        if (scope.classId !== undefined) return `class:${scope.classId}:this`;
