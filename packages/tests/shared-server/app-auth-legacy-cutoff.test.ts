...
describe('AppAuthInboxService legacy compatibility cutoff', () => {
        ...
        try {
            ...
            for (const testCase of cases) {
                ...
                const result = await runAuthInboxCommand(
                    testCase.run(fixture.service, fixture.session),
                    fixture.queue,
                    fixture.reader,
                );
    ...
    it('returns not found when canonical websocket and agent tickets target a legacy row', async () => {
        ...
        try {
            for (const kind of ['websocket', 'agent'] as const) {
                ...
                await expect(runAuthInboxCommand(
                    pending,
                    fixture.queue,
                    fixture.reader,
                )).resolves.toMatchObject({ left: { status: 404 } });
                expect(await fixture.runtime.findEntry(ticketKey.namespace, ticketKey.key))
                    .toBeDefined();
                await expectLegacyRowsRemainUnavailable(fixture);
            }
        } finally {
    ...
    it('still fails closed for corrupt canonical session-index rows after cutoff', async () => {
        ...
        try {
            ...
            await expect(new AuthSessionRepository(runtime)
                .findBySessionId('corrupt-canonical-session'))
...
async function expectLegacyRowsRemainUnavailable(
    ...
    await expect(new AuthSessionRepository(fixture.runtime)
        .findBySessionId(LEGACY_SESSION_ID)).resolves.toBeUndefined();
    await expect(new AuthSessionRepository(fixture.runtime)
        .findByAccessToken(LEGACY_ACCESS_TOKEN)).resolves.toBeUndefined();
    expect(fixture.page).not.toHaveBeenCalled();
    expect(fixture.findAll).not.toHaveBeenCalled();
    expect(await fixture.runtime.findEntry(
        'auth-sessions:by-session',
...
async function seedCanonicalTicket(
    ...
    const namespace = kind === 'websocket'
