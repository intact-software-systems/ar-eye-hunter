...
describe('group topology config service', () => {
  it('keeps synchronous reconfigure options off config mutation requests', () => {
    type ConfigHasReconfigure = 'reconfigure' extends keyof PutGroupTopologyConfigRequest
      ? true
      : false;
  ...
  it('rejects an elapsed stable override expiry with explicit pure facts', () => {
    ...
    expect(() => computeTopologyConfigMutation(input)).toThrow(GroupTopologyConfigValidationError);
  });
...
function createConfigMutationInput(
  ...
  const override =
    input.overrideDegreeLimit === null
      ? null
      : {
