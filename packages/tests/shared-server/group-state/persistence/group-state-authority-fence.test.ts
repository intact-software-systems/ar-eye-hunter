...
describe('GroupStateRepository authority fence', () => {
    ...
    it('conflicts for a stale observed storage revision', async () => {
        ...
        expect(await repository.advanceAuthorityFence(observation!.authorityGuard)).toMatchObject({
            status: 'applied'
        });
        await expect(repository.advanceAuthorityFence(observation!.authorityGuard)).resolves.toEqual({
            status: 'conflict'
    ...
    it('keeps domain causal revision stable across physical authority fences', async () => {
        ...
        expect(after!.snapshot.causalRevision.groupRevision).toBe(
            before!.snapshot.causalRevision.groupRevision
        );
        expect(after!.authorityGuard.causalGroupRevision).toBe(
            before!.authorityGuard.causalGroupRevision
        );
    ...
    it('uses one dense batch read as the authority snapshot on capable repositories', async () => {
        ...
        const expiredEntry = await runtime.findEntry('group-state:members', expiredMemberKey);
        await runtime.upsert(
            'group-state:members',
            expiredMemberKey,
            expiredEntry!.value,
        ...
        expect([...runtime.data.values()].some((entry) => entry.key === expiredMemberKey)).toBe(true);
    });
});
        ...
        return await Promise.all(
            selectors.map(async (selector) => ({
                ...
                entries: selector.kind === 'key'
                    ? [await snapshot.findEntry(selector.namespace, selector.key)].filter(
                        (entry): entry is RuntimeStateEntry => entry !== undefined
                    )
                    : await snapshot.findEntriesByPrefix(selector.namespace, selector.keyPrefix)
            }))
        );
    }
}
