...
interface ExpectedTransactionWriter {
...
interface ExpectedHandlerDependencies {
...
describe('convergent group and presence state', () => {
    ...
    it('requires distinct durable and after-commit transaction results', () => {
        expectTypeOf<AppInboxMutationTransactionResult<DurableResult, AfterCommitResult>>()
            .toEqualTypeOf<Readonly<{ durableResult: DurableResult; afterCommitResult: AfterCommitResult; }>>();
    ...
    it('routes authenticated protocol enqueues through the discriminated descriptor contract', () => {
        expectTypeOf<InstanceType<typeof GroupStateInboxHandler>['processGroupStateMutation']>()
            .parameter(0)
            .toEqualTypeOf<AppInboxExecutionMetadata>();
