...
function createTopologyConfigMutationRead(settings: {
    ...
    const durable = settings.durableDegreeLimit === undefined
        ? null
        : storedTopologyConfig(settings.durableDegreeLimit, 'durable');
    const override = settings.overrideDegreeLimit === undefined || settings.overrideDegreeLimit === null
        ? null
        : {
...
function runtimeEntry<T>(key: string, value: T) {
    return {
        ...
        entry: {
            ...
            expireAtTimestamp: typeof value === 'object' && value !== null && 'expiresAtEpochMs' in value
