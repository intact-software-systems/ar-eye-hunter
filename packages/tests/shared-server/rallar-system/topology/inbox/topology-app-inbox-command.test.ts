...
describe('topology AppInbox durable command contract', () => {
    it('binds every operation discriminant to exactly its request and durable payload', () => {
        ...
        type CommandRelationships = {
            [Current in Operation]: Extract<TopologyAppInboxCommand, { operation: Current; }>['payload'] extends TopologyAppInboxPayload<Current> ?
                TopologyAppInboxPayload<Current> extends Extract<TopologyAppInboxCommand, { operation: Current; }>['payload'] ? true :
                false :
                false;
        }[Operation];
        type RequestRelationships = {
            ...
            ]: CreateTopologyAppInboxCommandInput<Current>['payload'] extends TopologyAppInboxRequestPayload<Current> ? true :
                false;
        }[Operation];
        type MismatchedPayloadIsRejected = TopologyAppInboxPayload<'deleteConfig'> extends
            Extract<TopologyAppInboxCommand, { operation: 'putConfig'; }>['payload'] ? false :
            true;
