...
describe('Postgres topology mutation worker concurrency', () => {
    postgresIt(
        ...
        async () => {
            ...
            const groupState = createTestGroupStateRepository(
                runtime,
                new PSqlGroupStateEventRepository(setupSql)
            );
    ...
    postgresIt(
        ...
        async () => {
            ...
            try {
                ...
                const latest = outputs.reduce((left, right) => (left.acceptedVersion ?? 0) > (right.acceptedVersion ?? 0) ? left : right);
