...
describe('RTC topology snapshot repository', () => {
  ...
  it('exposes value-verified offline topology key migration only with old writers stopped', async () => {
    ...
    expect(
      await runtimeRepository.findEntry(RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE, legacyAliasedKey),
    ).toBeUndefined();
  });
  ...
  it('normalizes an expiring legacy topology snapshot to a never-expiring canonical row', async () => {
    ...
    await expect(
      runtimeRepository.findEntry(RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE, legacyKey),
  ...
  it.each([
    ...
    async ({ expireAtTimestamp }) => {
      ...
      await expect(
        runtimeRepository.findEntry(RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE, repository.snapshotKey(ref)),
      ).resolves.toBeUndefined();
      await expect(
        runtimeRepository.findEntry(RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE, legacyKey),
  ...
  it('fails a snapshot migration conflict closed after one attempt without partial effects or backoff', async () => {
    ...
    const entriesBefore = await runtimeRepository.findAllEntries(RTC_TOPOLOGY_SNAPSHOTS_NAMESPACE);
    runtimeRepository.beforeConditionalWrite = async (operation, namespace, key) => {
      if (
        ...
        await runtimeRepository.upsert(namespace, key, JSON.stringify(snapshot), 20_000);
      }
    };
