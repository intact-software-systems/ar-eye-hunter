...
describe('RTC topology publication repository', () => {
    it('persists immutable topology publications and rejects a divergent loaded retry', async () => {
        ...
        expect(
            await putOrLoadTopologyPublication(
                repository,
                publication,
                snapshot,
            ),
    ...
    it('lets exactly one immutable publication claim a work id without locks', async () => {
        ...
        const seeded = await new RtcTopologySnapshotRepository(
            runtimeRepository,
        ).commitSnapshotGuard(
            createTopologySnapshot(createGroupRef(), 1),
            null,
    ...
    it('requests recomputation when the topology predecessor moves before commit', async () => {
        ...
        await expect(
            snapshots.commitSnapshotGuard(current, null),
        ...
        expect(
            await new RtcTopologyPublicationRepository(
                runtimeRepository,
    ...
    it('validates publication and work direct, list, and page rows before expiry', async () => {
        ...
        try {
            ...
            const workKey = repository.workIndexKey(
                groupRef,
                publication.workId,
            );
            ...
            await expect(
                repository.listWorkClaimEntries(),
            ).rejects.toMatchObject({
                code: 'rtc-topology-repository-invariant-corruption',
            });
            await expect(
                repository.listWorkClaimEntriesPage({ limit: 10 }),
            ).rejects.toMatchObject({
                code: 'rtc-topology-repository-invariant-corruption',
            });
            expect(
    ...
    it.each([
        ...
        async (defect) => {
            ...
            if (guard.status !== 'accepted') {
                throw new Error(
                    'Expected topology snapshot seed to be accepted',
                );
            }
        ...
        try {
            ...
            for (const defect of defects) {
                for (const surface of surfaces) {
                    ...
                    const message = publication.message as unknown as Record<
                        string,
                        unknown
                    >;
                    if (defect === 'typeId') {
                        delete (message.payload as Record<string, unknown>)
                            .typeId;
                    } else {
    ...
    it('offline-migrates value-verified legacy publication and work keys together', async () => {
        ...
        const upgradedPublication =
            toUpgradedLegacyPublication(legacyPublication);
        ...
        await expect(
            repository.findPublication(publication.publicationId),
        ).rejects.toMatchObject({
            code: 'rtc-topology-repository-invariant-corruption',
        });
        ...
        expect(
            await repository.findPublicationForWork(
                publication.groupRef,
                publication.workId,
            ),
    ...
    it('lets concurrent publication migrators converge on one exact canonical winner', async () => {
        ...
        await expect(
            repository.findPublicationForWork(
                upgraded.groupRef,
                upgraded.workId,
            ),
    ...
    it('rejects a concurrent publication migration winner with divergent physical expiry', async () => {
        ...
        vi.spyOn(runtimeRepository, 'begin').mockImplementationOnce(
            async (fn) => {
