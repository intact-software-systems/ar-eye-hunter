...
describe('RTC RTT persistence migration', () => {
  it('offline-migrates value-verified legacy RTT pair keys', async () => {
    ...
    expect(await repository.findMeasurement('session-a', 'session-b')).toEqual(measurement);
    expect(await runtimeRepository.findEntry(RTC_RTT_LATEST_NAMESPACE, legacyKey)).toBeUndefined();
  });
  ...
  it('offline-upgrades retained recompute intents without restoring runtime ownership', async () => {
    ...
    const { senderId: _senderId, delivery: _delivery, ...legacyIntent } = intent;
    const expireAtEpochMs =
      candidate.receipt.acceptedAtEpochMs + DEFAULT_RTC_RTT_MUTATION_RETENTION_MS;
    ...
    const upgraded = await runtime.findEntry(RTC_RTT_RECOMPUTE_OUTBOX_NAMESPACE, intent.outboxId);
