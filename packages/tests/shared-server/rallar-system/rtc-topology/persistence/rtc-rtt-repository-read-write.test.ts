...
describe('RTC RTT repository reads and writes', () => {
    it('keeps latest RTT measurements by sorted pair and expires stale entries', async () => {
        ...
        try {
            ...
            expect(
                await repository.listMeasurementsForSessionIds([
                    'session-a',
                    'session-b',
                ]),
            ).toEqual([second]);
    ...
    it('fails closed when the compatibility RTT helper sees equal-version divergence', async () => {
        ...
        await expect(
            repository.putMeasurementIfNewer(measurement),
        ).resolves.toBe(true);
        await expect(
            repository.putMeasurementIfNewer({ ...measurement }),
        ).resolves.toBe(false);
    ...
    it('uses stable code-unit ordering for Unicode RTT pairs and endpoint peers', async () => {
        ...
        await expect(
            repository.commitMeasurement(measurement, null, 100),
        ).resolves.toMatchObject({ status: 'accepted' });
        await expect(
            repository.findMeasurement(decomposed, composed),
        ).resolves.toEqual(measurement);
        await expect(repository.listMeasurements()).resolves.toEqual([
            measurement,
        ]);
        await expect(
        ...
        await expect(
            repository.commitEndpointAdmission(admission, null, 101),
        ).resolves.toMatchObject({ status: 'accepted' });
        await expect(
            repository.findEndpointAdmissionEntry('endpoint'),
    ...
    it.each([
        ...
        async ({ expectedRevision, version }) => {
            ...
            await expect(
                repository.commitEndpointAdmission(
                    admission,
                    expectedRevision,
                    100,
                ),
    ...
    it('keeps expired RTT reads observational without retries or cleanup writes', async () => {
        ...
        const before = await runtimeRepository.findEntry(
            RTC_RTT_LATEST_NAMESPACE,
            key,
        );
        ...
        await expect(
            runtimeRepository.findEntry(RTC_RTT_LATEST_NAMESPACE, key),
