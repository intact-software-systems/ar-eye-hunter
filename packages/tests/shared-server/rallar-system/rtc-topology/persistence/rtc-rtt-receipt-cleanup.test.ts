...
describe('RTC RTT receipt cleanup ownership', () => {
    it('deletes an expired receipt without obsolete recompute-intent siblings', async () => {
        ...
        await expect(
            cleanupExpiredRtcRttReceipts(harness.repository),
        ).resolves.toBe(1);
        await expect(
            harness.repository.probeMutationReceiptEntry(
                harness.receipt.receiptId,
            ),
    ...
    it('preserves a receipt when its optimistic revision changes before deletion', async () => {
        ...
        harness.runtime.beforeConditionalWrite = async (
            operation,
            namespace,
            key,
        ) => {
    ...
    it('leaves the retired recompute-intent namespace to generic runtime expiry', () => {
        expect(RTC_RTT_PROTECTED_RUNTIME_STATE_NAMESPACES).toContain(
            RTC_RTT_RECEIPTS_NAMESPACE,
        );
    ...
    it('reads and validates the receipt before entering its write transaction', () => {
        ...
        expect(
            cleanupSection.indexOf('probeMutationReceiptEntry('),
...
async function createReceiptHarness(input: {
    readonly nowOffsetFromExpiry: number;
}): Promise<
    Readonly<{
...
> {
    ...
    const expireAtEpochMs =
        acceptedAtEpochMs + DEFAULT_RTC_RTT_MUTATION_RETENTION_MS;
