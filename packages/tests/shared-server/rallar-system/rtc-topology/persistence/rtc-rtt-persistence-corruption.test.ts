...
describe('RTC RTT persistence corruption', () => {
    it('rejects wrong-pair RTT rows before expiry across direct, list, and page reads', async () => {
        ...
        try {
            ...
            await expect(repository.findMeasurement('session-a', 'session-b')).rejects.toMatchObject({
                code: 'rtc-topology-repository-invariant-corruption'
            });
            await expect(repository.listMeasurements()).rejects.toMatchObject({
                code: 'rtc-topology-repository-invariant-corruption'
            });
            await expect(repository.listMeasurementEntriesPage({ limit: 10 })).rejects.toMatchObject({
                code: 'rtc-topology-repository-invariant-corruption'
            });
            expect(
                await runtimeRepository.findEntry(RTC_RTT_LATEST_NAMESPACE, requestedKey)
            ).toBeDefined();
        }
        finally {
            vi.useRealTimers();
    ...
    it('uses injective RTT keys and validates endpoint admission direct, list, and page rows', async () => {
        ...
        try {
            ...
            expect(repository.measurementKey('a', 'b:c')).not.toBe(repository.measurementKey('a:b', 'c'));
            expect(repository.measurementKey('a%', '＿')).not.toBe(repository.measurementKey('a', '%＿'));
            ...
            await expect(repository.findEndpointAdmissionEntry('session-a')).rejects.toMatchObject({
                code: 'rtc-topology-repository-invariant-corruption'
            });
            await expect(repository.listEndpointAdmissionEntries()).rejects.toMatchObject({
                code: 'rtc-topology-repository-invariant-corruption'
            });
            await expect(
            ...
            expect(
                await runtimeRepository.findEntry(RTC_RTT_ENDPOINT_ADMISSION_NAMESPACE, key)
            ).toBeDefined();
        }
        finally {
            vi.useRealTimers();
