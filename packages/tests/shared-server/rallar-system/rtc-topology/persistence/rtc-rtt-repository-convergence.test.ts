...
describe('RTC RTT repository convergence', () => {
    it('optimistically admits only one of two endpoint-cap races', async () => {
        ...
        vi.spyOn(repository, 'listMeasurementEntries').mockImplementation(
            async () => {
        ...
        const groupAB = createRttGroupSnapshot('room-ab', [
            'session-a',
            'session-b',
        ]);
        const groupAC = createRttGroupSnapshot('room-ac', [
            'session-a',
            'session-c',
        ]);
    ...
    it.each(rttWriteCandidateCorruptions)(
        ...
        async ({ corrupt }) => {
            ...
            const malformed = corrupt(
                structuredClone(
                    createValidRttWriteCandidate(),
                ) as unknown as MutableRttWriteCandidate,
            );
        ...
        async (expiredAuthority) => {
            ...
            const group =
                expiredAuthority === 'group'
                    ...
                    : {
                          ...baseGroup,
                          activeSessions: baseGroup.activeSessions.map(
                              (session) =>
                                  session.sessionId ===
            ...
            await expect(
                runtimeRepository.findAllEntries(RTC_RTT_RECEIPTS_NAMESPACE),
            ).resolves.toEqual([]);
            await expect(
                runtimeRepository.findAllEntries(
                    RTC_RTT_RECOMPUTE_OUTBOX_NAMESPACE,
                ),
            ).resolves.toEqual([]);
        },
    ...
    it('reruns RTT lifecycle authority after a CAS conflict crosses group expiry', async () => {
        ...
        const baseGroup = createRttGroupSnapshot('room-retry-expiry', [
            'session-a',
            'session-b',
        ]);
        const group = {
        ...
        runtimeRepository.beforeConditionalWrite = async (
            ...
            if (
                ...
                const endpointId = decodeURIComponent(
                    key.slice('endpoint='.length),
                );
                const peerSessionId =
                    endpointId === 'session-a' ? 'session-b' : 'session-a';
        ...
        async ({ defect, surface }) => {
            ...
            await runtimeRepository.upsert(
                ...
                JSON.stringify({
                    ...
                    affectedGroupRefs:
                        defect === 'duplicate' ? [refA, refA] : [refB, refA],
                    acceptedAtEpochMs: 1,
                    outcome: 'accepted',
    ...
    it.each([
        ...
        async ({ divergent }) => {
            ...
            const measurementList = vi
                ...
                .mockRejectedValue(
                    new Error('RTT receipt replay measurement list'),
                );
            const admission = vi
    ...
    it('replays an accepted RTT after measurement and admission expiry and rejects divergent reuse', async () => {
        ...
        const group = createRttGroupSnapshot('room-replay', [
            'session-a',
            'session-b',
        ]);
        const command = {
        ...
        let readFacts: () => ReturnType<
            RtcRttRepository['readMutationFacts']
        ...
        const admissionReads = vi.spyOn(
            repository,
            'findEndpointAdmissionEntry',
        );
        const conditionalDeletes = vi.spyOn(
            runtimeRepository,
            'deleteIfRevision',
        );
        const conditionalUpdates = vi.spyOn(
            runtimeRepository,
            'upsertIfRevision',
        );
        const transactions = vi.spyOn(runtimeRepository, 'begin');
        const conditionalInserts = vi.spyOn(
            runtimeRepository,
            'insertIfAbsent',
        );
        ...
        expect(lifecycleReads).not.toHaveBeenCalled();
        expect(
            await runtimeRepository.findAllEntries(RTC_RTT_RECEIPTS_NAMESPACE),
        ).toHaveLength(1);
        expect(
    ...
    it('converges concurrent identical RTT writers through the immutable receipt winner', async () => {
        ...
        const group = createRttGroupSnapshot('room-concurrent', [
            'session-a',
            'session-b',
        ]);
        const command = {
        ...
        expect(
            results.filter(({ computed }) => computed.outcome === 'replay'),
        ).toHaveLength(1);
        expect(
            await runtimeRepository.findAllEntries(RTC_RTT_RECEIPTS_NAMESPACE),
        ).toHaveLength(1);
        ...
        try {
            ...
            for (const surface of surfaces) {
                ...
                const repository = new RtcRttRepository(runtimeRepository, {
                    ...
                    listMutationReceiptEntriesPage(input: {
                        limit: number;
    ...
    it('refreshes lifecycle facts after an RTT conflict crosses peer expiry', async () => {
        ...
        const group = createRttGroupSnapshot('room-ab', [
            'session-a',
            'session-b',
        ]);
        ...
        await expect(
            repository.findMeasurementEntry('session-a', 'session-b'),
    ...
    it('fully rereads RTT authority after conflict when a session connection boundary moves past acceptance', async () => {
        ...
        const begin = vi
            ...
            .mockImplementation(originalBegin);
        const initial = createRttGroupSnapshot('room-session-boundary', [
            'session-a',
            'session-b',
        ]);
