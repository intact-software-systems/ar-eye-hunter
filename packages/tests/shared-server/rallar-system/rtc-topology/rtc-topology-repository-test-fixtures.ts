...
export type LegacyRtcTopologyPublication = Omit<RtcTopologyPublication, 'targetGroupSnapshotVersion'>;

export function toLegacyPublication(
    publication: RtcTopologyPublication
): LegacyRtcTopologyPublication {
    const { targetGroupSnapshotVersion: _targetGroupSnapshotVersion, ...legacy } = structuredClone(publication);
    return {
        ...legacy,
        message: {
            ...legacy.message,
...
export function corruptTopologyExecutionReceipt(
    ...
    defect: 'legacy' | 'missing' | 'extra' | 'hash' | 'attempt' | 'causal' | 'storage' | 'event' | 'outbox'
): unknown {
    if (defect === 'legacy') {
        return {
    ...
    if (defect === 'event') {
        return { ...receipt, eventId: 'unexpected-event' };
    }
