...
import type { AppInboxService } from '@shared-server/rallar-system/services/AppInboxService.ts';
...
describe('createRallarMiddleware', () => {
  it('constructs queuebox runtime services around supplied repositories', () => {
    ...
    const appInboxService = {} as AppInboxService;
    const createAppInboxService = vi.fn(() => appInboxService);
    ...
    expect(runtime.appInboxService).toBe(appInboxService);
    expect(createAppInboxService).toHaveBeenCalledWith({
  ...
  it('registers an app inbox engine task that drains inbox messages', async () => {
    ...
    runtime.inboxQueueReader.onInboxMessageDo('group-state.create.v1', { onMessage });
