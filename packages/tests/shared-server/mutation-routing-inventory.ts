...
export function validateMutationRouteInventory(
  ...
  if (inventory.length !== 42) {
    issues.push(`Expected 42 source-inspected entrypoints, found ${inventory.length}`);
  }
  if (new Set(inventory.map((item) => item.type)).size !== 42) {
    issues.push('Inventory must cover all 42 source-inspected AppInbox command types');
  }
