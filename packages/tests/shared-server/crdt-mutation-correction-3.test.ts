...
describe('Task 9 correction 3 exact mutation contracts', () => {
    ...
    it('rejects non-authoritative lifecycle retention and quota set values', async () => {
        ...
        } as never)).rejects.toThrow(/retention/i);
        await expect(createCrdtMutationCommand({
            ...lifecycleInput(),
            quotaAction: {
                kind: 'set',
        ...
        } as never)).rejects.toThrow(/quota/i);
    });
    ...
    it('rejects persisted metadata with impossible lifecycle timestamps', () => {
        expect(() => toMetadata(documentRow({
            lifecycle: 'archived',
            archived_at_ts: null,
        }), toRallarCrdtDocumentKey(DOCUMENT), DOCUMENT)).toThrow(/metadata|lifecycle|corrupt/i);
    });
    ...
    it('computes compact output deterministically from identical command and read data', async () => {
        ...
        expect(computeCrdtMutation(command, read, 'server-1'))
            .toEqual(computeCrdtMutation(command, read, 'server-1'));
    });
    ...
    it('keeps audit work live through the complete 20-attempt retry horizon', () => {
        ...
        expect(Number(entry.audit.expiryTs.epochMilliseconds))
            .toBeGreaterThanOrEqual(finalRetryAtEpochMs + 60_000);
    });
