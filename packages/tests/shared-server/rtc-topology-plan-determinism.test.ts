...
// Phase 4 (M6) determinism contract for FRESH plans: identical member sets
// produce byte-identical planned graphs across servers and insertion orders,
// for every kind and both the no-RTT and RTT-mixed paths. Seeded plans stay
// deterministic given an equal previous accepted snapshot (shared durable
// state), covered by the seeded case below.
describe('rtc topology plan determinism', () => {
    ...
    it.each(sizes.map((memberCount) => ({ memberCount })))(
        ...
        ({ memberCount }) => {
            ...
            for (const seed of [7, 23, 41]) {
                ...
                expect(shuffledPlan.snapshot.activeSessionIds).toEqual(
                    reference.snapshot.activeSessionIds,
                );
            }
    ...
    it('plans byte-identical graphs for shuffled member orders with mixed RTT (N=20)', () => {
        ...
        const rttMeasurements: RttMeasurementInfo[] = members.slice(0, 8).map(
            (sessionId, index) => ({
        ...
        for (const seed of [11, 29]) {
            const shuffledPlan = planForOrder(
                shuffled(members, seed),
                shuffled(rttMeasurements, seed),
            );
            expect(JSON.stringify(shuffledPlan.snapshot.nextHopsBySessionId)).toBe(
                JSON.stringify(reference.snapshot.nextHopsBySessionId),
            );
