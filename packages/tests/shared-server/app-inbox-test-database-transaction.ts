...
async function publishAppInboxTestPendingWrites({
  ...
  for (const entry of pending.results) await repositories.results.replace(entry);
  for (const entry of pending.inbox) await repositories.inbox.enqueue(entry);
  state.outboxEntries.clear();
  for (const [key, entry] of pending.outbox) state.outboxEntries.set(key, entry);
