...
export function evaluateStaticObjectCollection(
  ...
  if (method === 'entries') return UNKNOWN_REGISTRATION_TYPES;
  if (method !== 'keys' && method !== 'values') return UNKNOWN_REGISTRATION_TYPES;
...
function evaluateObjectProjection(
  ...
  if (object?.type !== 'ObjectExpression') return UNKNOWN_REGISTRATION_TYPES;
