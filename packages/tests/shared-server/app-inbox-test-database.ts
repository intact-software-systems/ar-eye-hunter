...
export type AppInboxTestDatabaseStage =
  'resource-result-replace' | 'reservation-finish' | 'transaction-commit-return';
...
type AppInboxTestDatabaseOptions = Readonly<{
  ...
  onStage?: (stage: AppInboxTestDatabaseStage) => void | Promise<void>;
}>;
...
export function createAppInboxTestDatabase(
  ...
  database.begin = async <T>(write: (transaction: PSqlTransactionSql) => Promise<T>) => {
    const run = async (
      ...
      const transaction = (async (strings: TemplateStringsArray, ...values: unknown[]) => {
        ...
        if (
          ...
          const result = await runtime.insertIfAbsent(namespace, key, value, expireAt.getTime());
        ...
        if (query.includes('jsonb_array_elements') && query.includes('as selections')) {
          ...
          const rawSelectors =
            typeof values[0] === 'string' ? (JSON.parse(values[0]) as unknown) : values[0];
          ...
          for (const rawSelector of rawSelectors) {
            ...
            const entries =
              selector.kind === 'key'
                ...
                : await runtime.findEntriesByPrefix(selector.namespace, selector.keyPrefix ?? '');
        ...
        if (query.includes('insert into resource_inbox_results')) {
          await options.onStage?.('resource-result-replace');
        ...
        if (query.includes('update resource_inbox') && query.includes("ri_status = 'reserved'")) {
          await options.onStage?.('reservation-finish');
