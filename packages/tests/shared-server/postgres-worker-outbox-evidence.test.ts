...
describe('Postgres worker direct ResourceInbox evidence', () => {
    it('validates receipt-linked client WS and group APP lifecycle entries', () => {
        expect(() => expectWorkerOutboxLifecycleEvidence([
            entry('client-snapshot', AppTopics.clientStateSnapshot, EnqueuedType.WS_OUTBOX,
                '{"id":"client-snapshot:principal-state:snapshot"}'),
            entry('client-event', AppTopics.clientStateEvent, EnqueuedType.WS_OUTBOX,
                '{"id":"client-event:principal-state:event"}'),
        ], [{ domainStatus: 'applied', outboxIds: ['client-snapshot', 'client-event'] }], 'client', [
    ...
    it('rejects an effect whose receipt-linked entry has the wrong direct outbox type', () => {
        expect(() => expectWorkerOutboxLifecycleEvidence([
            entry('client-snapshot', AppTopics.clientStateSnapshot, EnqueuedType.APP_OUTBOX,
                '{"id":"client-snapshot:principal-state:snapshot"}'),
            entry('client-event', AppTopics.clientStateEvent, EnqueuedType.WS_OUTBOX,
                '{"id":"client-event:principal-state:event"}'),
        ], [{ domainStatus: 'applied', outboxIds: ['client-snapshot', 'client-event'] }], 'client', [
