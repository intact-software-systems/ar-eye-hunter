...
describe('RTC topology publication mutation phases', () => {
    it('keeps RTC topology mutation computation synchronous and effect-free', () => {
        ...
        for (const pattern of forbidden) {
            expect(
                source,
                `forbidden pure-module pattern ${pattern}`,
            ).not.toMatch(pattern);
        }
    });
    ...
    it('shares a strict persisted publication validator from a neutral pure contract module', () => {
        ...
        if (!existsSync(publicationContractUrl)) return;
        ...
        expect(publicationRepositorySource).toMatch(
            /rtc-topology-publication-validation/,
        );
    });
    it('computes and validates an absent topology guard deterministically from frozen input', () => {
        ...
        expect(() =>
            validateTopologyMutation({ ...input, computed: tampered }),
    ...
    it('loads only the durable publication winner and rejects a claim without its snapshot', () => {
        ...
        expect(() => computeTopologyMutation(missingSnapshot)).toThrow(
            'has no durable snapshot',
        );
        expect(() => computeTopologyMutation(missingSnapshot)).toThrow(
            'has no durable snapshot',
        );
        ...
        expect(() => computeTopologyMutation(inconsistent)).toThrow(
    ...
    it('computes duplicate, advanced, and superseded topology outcomes', () => {
        ...
        expect(() => computeTopologyMutation(corrupt)).toThrow(
            'revision conflict',
        );
        expect(() => computeTopologyMutation(corrupt)).toThrow(
            'revision conflict',
    ...
    return {
        ...
        message: {
            ...
            route: {
                ...
                resourceId: `${snapshot.overlayId}:${snapshot.sourceGroupStateCausalRevision.groupRevision}:${snapshot.sourceGroupStateCausalRevision.presenceRevision}:${snapshot.version}`,
            },
            targets: {
                mode: 'broadcast',
                scope: 'room',
