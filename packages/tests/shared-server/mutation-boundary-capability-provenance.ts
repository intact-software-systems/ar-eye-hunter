...
export function bindCapabilityNode(
  ...
  if (isCapabilityFunction(node)) return bindFunctionParameters(node, analysis);
  if (
...
function bindFunctionParameters(node: AstNode, analysis: CapabilityBindingAnalysis): boolean {
  let changed = false;
...
function bindClassProperty(node: AstNode, analysis: CapabilityBindingAnalysis): boolean {
...
function bindPattern(
  ...
  if (pattern.type === 'Identifier') {
    ...
    const sourceKey = capabilityExpressionKey(value, analysis);
    const shape = analysis.resolver.resolveType(typeAnnotation) ?? analysis.values.resolve(value);
    let changed = bindValueProvenance(targetKey, sourceKey, shape, value, analysis);
  ...
  const shape = analysis.resolver.resolveType(typeAnnotation) ?? analysis.values.resolve(value);
  return bindObjectPattern(pattern, shape, capabilityExpressionKey(value, analysis), analysis);
}
...
function bindValueProvenance(
  ...
  let changed = capability ? setCapability(analysis.receivers, targetKey, capability) : false;
  if (shape) changed = bindShape(targetKey, shape, analysis) || changed;
  const method = readCapabilityMethod(value, analysis) ?? analysis.methods.get(sourceKey);
  if (method) changed = setCapabilityMethod(analysis.methods, targetKey, method) || changed;
  return changed;
}
...
function bindObjectExpression(
  ...
  for (const property of asNodes(value.properties)) {
    const name = readCapabilityPropertyName(property.key, property.computed === true, analysis);
...
function copyProvenance(
  ...
  if (capability) changed = setCapability(analysis.receivers, targetKey, capability);
...
function bindString(
  ...
  return literal
    ? setCapability(analysis.strings, analysis.bindings.identifierKey(pattern), literal)
    : false;
}
