...
import { decodeExactSnapshotEnvelope } from '@shared-server/rallar-system/services/crdt-mutation-codec.ts';
...
describe('CRDT mutation exact nested codecs', () => {
    it('rejects extra fields in authoritative update payload batches', async () => {
        ...
        } as never)).rejects.toThrow(/payload|batch|fields|exact/i);
    });

    it('rejects extra fields in authoritative erase debug bundles', () => {
        const bundle = JSON.parse(JSON.stringify(createRallarCrdtDebugBundle({
    ...
    it('rejects extra fields in snapshot clocks and CRDT state sidecars', () => {
        ...
        expect(() => decodeExactSnapshotEnvelope({
            ...snapshot,
