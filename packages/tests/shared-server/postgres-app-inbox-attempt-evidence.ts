...
export async function readOwnedAppInboxResourceIds(
  ...
  for (const row of rows) {
    ...
    const identity = readAppInboxRequestIdentity(validation.command.data);
    if (!identity || !isSameScope(identity.scope, input.scope)) continue;
    if (!requestIds.has(identity.requestId)) continue;
  ...
  return input.requestIds.map((requestId) => {
    ...
    if (!resourceId) throw new Error(`Missing AppInbox resource for request: ${requestId}`);
    return resourceId;
  });
}

function readAppInboxRequestIdentity(value: unknown): AppInboxRequestIdentity | null {
  if (!isRecord(value) || !isRecord(value.scope) || !isRecord(value.request)) return null;
