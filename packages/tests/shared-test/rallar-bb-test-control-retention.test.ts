...
describe('rallar-bb-test control retention planning', () => {
    ...
    it('accepts the largest safe cap and returns typed limit failures for pathological inputs', () => {
        expect(
            planControlRunRetention(retentionInput({
        ...
        (giantString.runs[0] as unknown as Record<string, unknown>).giant = 'x'.repeat(CONTROL_RETENTION_PLAN_LIMITS.stringCharacters + 1);
