...
describe('fleet report runtime validation', () => {
    ...
    ] as const)('rejects %s', (_label, mutate, issueCode) => {
        const report = validReport();
        mutate(report);
...
describe('fleet report bundle runtime validation', () => {
    ...
    it.each([
        ...
        ['a missing file', validBundle('distributed-1', {
            'fleet-report.json': '{}',
            'summary.md': '',
            'agent-results.csv': '',
        }), 'missing-bundle-file'],
        ['an extra file', validBundle('distributed-1', {
            ...(validBundle().files as Record<string, unknown>),
            'unexpected.txt': 'no',
        }), 'unexpected-bundle-file'],
        ['a non-text file', validBundle('distributed-1', {
            ...(validBundle().files as Record<string, unknown>),
            'summary.md': new Uint8Array(),
        }), 'invalid-type'],
    ] as const)('rejects %s', (_label, bundle, issueCode) => {
        const result = validateControlFleetReportBundle(bundle, 'distributed-1');
