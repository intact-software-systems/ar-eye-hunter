...
import { readApiV1Matrix as readMatrix, readApiV1Recipe as readRecipe, toFlatApiV1RecipeSteps as flattenRecipeSteps } from './api-v1-recipe-test-fixture.ts';
...
describe('API-v1 medium-scale recipe', () => {
    it('defines the isolated 100-client five-group medium-scale state churn gate', () => {
        ...
        const controlGroups = parallel.groups!.filter((group) => group.name?.startsWith('group-control-lane-'));
        ...
        clientLoops.forEach((loop, laneIndex) => {
            ...
            const generationFor = (namePrefix: string) =>
                (
                    clientSessionSteps.find((step) => String(step.name).startsWith(namePrefix))?.request as { body?: { generationId?: string; }; } | undefined
                )?.body?.generationId;
            const connectedAtFor = (namePrefix: string) =>
                (
                    clientSessionSteps.find((step) => String(step.name).startsWith(namePrefix))?.request as
                        | { body?: { connectedAtEpochMs?: number; }; }
                        | undefined
        ...
        const equalDuplicateWave = afterChurn.find((step) => step.name === 'runBoundedEqualDuplicateWave');
        const changedDuplicateWave = afterChurn.find((step) => step.name === 'runBoundedChangedDuplicateWave');
        ...
        const changedPaths = flattenRecipeSteps([changedDuplicateWave!])
            ...
            .map((step) => (step.request as { path?: string; }).path);
        expect(new Set(equalPaths)).toEqual(
            new Set([
        ...
        const pollDelays = afterChurn.filter((step) => String(step.name).startsWith('delayBeforeConvergencePoll'));
        expect(
        ...
        const pollSteps = afterChurn.filter((step) => String(step.name).startsWith('pollConvergenceAttempt'));
        ...
        groupIds.forEach((groupId) => {
            ...
            expect(polling?.request).toMatchObject({
                outputs: {
                    [`final${groupId[0].toUpperCase()}${groupId.slice(1)}StateCausalRevision`]: 'body.causalRevision.groupRevision',
                    [`final${groupId[0].toUpperCase()}${groupId.slice(1)}PresenceCausalRevision`]: 'body.causalRevision.presenceRevision'
                }
            });
            ...
            expect(
                ...
                [`final${groupId[0].toUpperCase()}${groupId.slice(1)}TopologyOutboxIds`]: 'body.receipt.outboxIds'
            });
        });
