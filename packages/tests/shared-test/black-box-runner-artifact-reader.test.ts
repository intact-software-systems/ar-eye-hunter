...
describe('black-box runner artifact reader', () => {
    ...
    it('accepts legacy v0 artifact bundles and records compatibility warnings', () => {
        ...
        expect(result.warnings.some((warning) => warning.message.includes('legacy compatible v0'))).toBe(true);
    });
    ...
    it('returns actionable errors for malformed artifact bundles', () => {
        ...
        expect(badRedaction.errors.some((error) => error.message.includes('Invalid redaction placeholder'))).toBe(true);
    });
    ...
    it('validates expanded-plan replay fields', () => {
        ...
        expect(badPlan.errors.some((error) => error.path === '$.replayRecipe.execution.trafficPlan')).toBe(true);
    });
    ...
    it('validates current and legacy recipe catalog entry fixtures', () => {
        ...
        expect(legacy.warnings.some((warning) => warning.message.includes('normalized to the current command-center entry shape'))).toBe(true);
    });
});
