    ...
    await expect(
      withPlans(toLifecycleInput(), {
        ...
        waitForReady: async (input) => {
          ...
          if (input.baseUrl.endsWith(':18082')) throw tertiaryFailure;
        },
        toDiagnosticSecrets: (env) => [`secret-${env.PORT}`],
        runRecipes: async () => {
          events.push('recipes');
...
function getWithManagedApiServerPlans(): WithManagedApiServerPlans | undefined {
  return (apiV1BlackBoxRunner as Record<string, unknown>).withManagedApiServerPlans as
    WithManagedApiServerPlans | undefined;
}
