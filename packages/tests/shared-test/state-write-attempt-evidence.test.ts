...
import { deriveAppInboxAttemptObservations } from
...
function release(failure: Readonly<{
    kind: 'retryable'; code: string; name: string
}>) {
    return {
...
describe('state-write attempt evidence', () => {
    it('does not classify a generic retryable infrastructure failure as a conflict', () => {
        ...
        ], evidence, [{ commandId: 'command-1', status: 'accepted' }]);
