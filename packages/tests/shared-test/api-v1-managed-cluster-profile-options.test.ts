...
describe('managed API-v1 cluster profile options', () => {
    it.each(managedClusterProfiles)(
        ...
        (profile) => {
            ...
            expect(() => parseApiV1BlackBoxArgs([`--profile=${profile}`, '--secondary-port=18081'])).toThrow(/secondary-port.*tertiary-port/i);
            expect(() => parseApiV1BlackBoxArgs([`--profile=${profile}`, '--tertiary-port=18082'])).toThrow(/secondary-port.*tertiary-port/i);
        }
    );
    ...
    it.each(managedClusterProfiles)(
        ...
        (profile) => {
            expect(() => parseApiV1BlackBoxArgs([`--profile=${profile}`, '--backend=pglite-memory'])).toThrow(/profile.*postgres/i);
        }
    );
