...
describe('black-box runner recipe matrix', () => {
  ...
  it('offers an offline validation profile that lists recipes without live preflight gates', () => {
    const result = spawnSync('deno', ['run', '-A', path.join(runnerRoot, 'recipe-matrix.mts'), '--profile=validation', '--list'], {
      cwd: repoRoot,
      encoding: 'utf8',
    });
  ...
  it('gives AppInbox-backed API cluster preflight checks a realistic deadline', () => {
    const { entries } = readMatrix();
    const clusterEntries = entries.filter((entry) => entry.profiles.includes('api-v1-black-box-cluster'));
  ...
  it('includes gated live-provider baselines for soak, traffic, and parallel RTC patterns', () => {
    ...
    expect(
      byProfile('live-soak')
        .map((entry) => entry.id)
        .sort(),
    ).toEqual(['browser-messages-rtc-same-connection-soak-live', 'remote-messages-rtc-same-connection-soak-live']);
    expect(
      byProfile('live-traffic')
        .map((entry) => entry.id)
        .sort(),
    ).toEqual(['browser-messages-rtc-seeded-traffic-live', 'remote-messages-rtc-seeded-traffic-live']);
    expect(
      byProfile('live-parallel')
        .map((entry) => entry.id)
        .sort(),
    ).toEqual(['browser-messages-rtc-parallel-groups-live', 'remote-messages-rtc-parallel-groups-live']);

    for (const entry of [...byProfile('live-soak'), ...byProfile('live-traffic'), ...byProfile('live-parallel')]) {
  ...
  it('defines a no-browser API-v1 black-box profile', () => {
    ...
    expect(apiEntries.map((entry) => entry.id).sort()).toEqual([
      ...
      'api-v1-group-formation-managed-burst-medium',
  ...
  it('covers client-state generation fencing and public idempotency conflicts', () => {
    ...
    expect(equivalent?.request?.body?.requestId).toBe(first?.request?.body?.requestId);
    expect(Object.keys(equivalent?.request?.body ?? {})).not.toEqual(Object.keys(first?.request?.body ?? {}));
    expect(conflict?.request?.body?.requestId).toBe(first?.request?.body?.requestId);
    expect(conflict?.request?.body?.displayName).not.toBe(first?.request?.body?.displayName);
    expect(conflict?.expect?.status).toBe(409);

    for (const name of ['connectAliceClientSession', 'heartbeatAliceClientSession', 'disconnectAliceClientSession']) {
      expect(step(name)?.request?.body?.generationId).toBe('{clientGenerationId}');
    }
  });
