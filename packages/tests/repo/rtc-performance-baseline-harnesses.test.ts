...
import * as Drain from '../../../scripts/perf/rtc-baseline/rtc-data-channel-drain-bench.ts';
import { runRtcPeerConnectionDiagnostics } from '../../../scripts/perf/rtc-baseline/rtc-peer-connection-diagnostics-runtime.ts';
import * as Repository from '../../../scripts/perf/rtc-baseline/rtc-rtt-repository-filter-bench.ts';
...
import * as Close from '../../../scripts/perf/rtc-data-channel-close-retention-bench.ts';
import * as ErrorReference from '../../../scripts/perf/rtc-data-channel-error-reference-bench.ts';
import * as Replace from '../../../scripts/perf/rtc-data-channel-replace-key-bench.ts';
import * as IceQueue from '../../../scripts/perf/rtc-ice-candidate-queue-bench.ts';
import * as Diagnostics from '../../../scripts/perf/rtc-peer-connection-diagnostics-burst.ts';
import * as Listeners from '../../../scripts/perf/rtc-peer-listener-cleanup-bench.ts';
import * as RttGraph from '../../../scripts/perf/rtc-room-graph-rtt-bench.ts';
import * as Inactive from '../../../scripts/perf/rtc-topology-inactive-churn-bench.ts';
import * as Mesh from '../../../scripts/perf/rtc-topology-mesh-no-rtt-bench.ts';
import * as Star from '../../../scripts/perf/rtc-topology-star-bench.ts';
import * as Tree from '../../../scripts/perf/rtc-topology-tree-no-rtt-bench.ts';
import { FakeRuntimeStateRepository } from '../shared-server/fake-runtime-state-repository.ts';

const words = (value: string): string[] => value.trim().split(/\s+/);
const inventory = [
  [
    ...words(`contracts decoding artifact-decoding workload-catalog workload-manifest validation
artifact-validation statistics evidence-layout evidence-store failure-accounting evidence-acceptance
finalized-evidence finalized-reader envelope runtime-observation deno-adapters deno-runtime cli-options
cli-grammar cli`).map((name) => `rtc-baseline-${name}.ts`),
    'rtc-data-channel-drain-bench.ts',
    'rtc-rtt-repository-filter-bench.ts',
    'rtc-peer-connection-diagnostics-runtime.ts',
  ],
  words(`contract decoding validation artifact-validation statistics workload-catalog workload-manifest
evidence-acceptance evidence-failure evidence-store harnesses envelope finalization finalized-reader
deno-adapters deno-runtime cli-grammar cli`).map(
    (name) => `rtc-performance-baseline-${name}.test.ts`,
  ),
  [
    ...words(`peer-connection-diagnostics-burst ice-candidate-queue-bench peer-listener-cleanup-bench
...
topology-inactive-churn-bench multicast-serialization-bench`).map((name) => `rtc-${name}.ts`),
    ...words(`group-cache-fallback-bench group-manager-state-bench group-manager-peer-owners-bench
heartbeat-callback-churn-bench`).map((name) => `webrtc-${name}.ts`),
  ],
  ['rtc-data-channel-browser-soak.mjs'],
];
const baselineId = '20260807-0123456789ab-e1-local';
const denoPrefix = words('run --config=apps/api-v1/deno.json --allow-read --allow-write');
...
function accepted<T extends { readonly mode: string }>(result: RtcBaselineResult<T>) {
...
it('RTC-B03 retains the exact on-disk 24/18/16/1 inventory and exclusions', () => {
  expect(createHash('sha256').update(JSON.stringify(inventory)).digest('hex')).toBe(
    'c1f1b81f32fdb75a83648d6fbd06a109006889d8dd49232ab3a27fcdb847dbe1',
  );
  const roots = [
  ...
  expect(
    inventory
      .flatMap((files, index) => files.map((name) => join(roots[index], name)))
      .every(existsSync),
  ).toBe(true);
...
it('RTC-B02/RTC-B03 diagnostics stay create-new beneath tmp/perf/results', () => {
  ...
  const cases = [
    ...
    ['scripts/perf/rtc-topology-star-bench.ts', '--sessions=30'],
    ['scripts/perf/rtc-topology-tree-no-rtt-bench.ts', '--sessions=30', '--degree-limit=5'],
    ['scripts/perf/rtc-topology-mesh-no-rtt-bench.ts', '--sessions=30', '--mesh-param-k=2'],
    ['scripts/perf/rtc-room-graph-rtt-bench.ts', '--sessions=30'],
    ['scripts/perf/rtc-topology-inactive-churn-bench.ts', '--groups=1', '--sessions=1'],
    [
