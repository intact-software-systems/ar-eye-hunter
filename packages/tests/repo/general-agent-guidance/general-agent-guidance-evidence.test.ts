...
describe('general agent guidance evidence ledger', () => {
  ...
  it.each([
    ...
    [
      ...
      (ledger: EvidenceLedger, root: string) =>
        writeFileSync(path.join(root, ledger.runs[0].rawOutputArtifact), 'changed', 'utf8'),
    ],
    [
      'runtime preamble digest drift',
      (ledger: EvidenceLedger, root: string) =>
        writeFileSync(path.join(root, ledger.runs[0].runtimePreambleArtifact), 'changed', 'utf8'),
    ],
    [
