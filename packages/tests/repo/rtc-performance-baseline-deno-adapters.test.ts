...
import { createDenoRtcBaselineAdapters } from '../../../scripts/perf/rtc-baseline/rtc-baseline-deno-adapters.ts';
import { createRtcBaselineDenoObservation } from '../../../scripts/perf/rtc-baseline/rtc-baseline-runtime-observation.ts';
...
describe('RTC baseline Deno adapters', () => {
  ...
  it('hashes complete source and config files with literal path/kind provenance', async () => {
    ...
    expect(
      await adapters.sourceConfigHashing.read([
        ...
        { path: 'apps/api-v1/deno.json', kind: 'config' },
      ]),
    ).toEqual({
      ok: true,
      value: [
  ...
  it('implements every filesystem operation with exact paths and typed failures', async () => {
    ...
    expect(adapters.filePort.classifyError(new double.runtime.errors.AlreadyExists('exists'))).toBe(
      'already-exists',
    );
    expect(
