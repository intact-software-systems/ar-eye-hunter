...
const apiImport = (clause: string, module = 'group') =>
  `${clause} from '@shared/api/${module}-types.ts';`;
const browserFiles = (files: Files) => inDirectory('packages/shared-web/browser/rooms', files);
const scan = (sourceRecords: readonly SourceRecord[]): ScanResult =>
  scanRepositoryLayout({ repoRoot, sources: sourceRecords });
const scanFiles = (files: Files) => scan(sources(files));
const sourceFiles = (...files: string[]) =>
  sources(Object.fromEntries(files.map((file) => [file, ''])));
...
const sources = (files: Files): SourceRecord[] =>
  Object.entries(files).map(([file, raw]) => ({ file: path.resolve(repoRoot, file), raw }));
const inDirectory = (directory: string, files: Files) =>
  Object.fromEntries(Object.entries(files).map(([file, raw]) => [`${directory}/${file}`, raw]));
...
const featureFiles = (prefix: string) =>
  ['read', 'compute', 'validate', 'write'].map((action) => `${action}-${prefix}-session.ts`);
const featurePrefixCount = (files: readonly string[], directory?: string) =>
  scan(denseFixture(files, directory)).counts[rules.featurePrefixCluster];
...
const findingsFor = (result: ScanResult, ruleId: string) =>
  result.findings.filter((finding) => finding.ruleId === ruleId);
const findingKey = (finding: ScanResult['findings'][number]) =>
