...
import { readModuleReferenceEvidence, type ModuleReference, type NonliteralModuleReference } from './auth-server-module-reference-validation.ts';
...
export const authCompatibilityConsumerInventory = [
    {
        ...
        removalCondition: 'Migrate every listed caller to canonical inbox owners, then retire this service path.'
    },
    {
        ...
        governanceIdentityConsumers: [runtimeIdentityGovernanceTest],
        removalCondition: 'Move the package export and every listed caller to canonical mutation owners first.'
    },
    {
    ...
    },
    {
        ...
        governanceIdentityConsumers: [runtimeIdentityGovernanceTest],
        removalCondition: 'Move every listed issuer or secret-validation caller to canonical credentials first.'
    },
    {
        ...
        governanceIdentityConsumers: [runtimeIdentityGovernanceTest],
        removalCondition: 'Migrate every listed API, domain, fixture, and benchmark consumer to canonical persistence.'
    },
    {
        ...
        governanceIdentityConsumers: [runtimeIdentityGovernanceTest],
        removalCondition: 'Migrate every listed API and Postgres repository consumer to canonical persistence first.'
    }
...
function readConsumers(
    ...
    for (const { filePath, references } of readConsumerSourceEvidence(input)) {
        ...
        for (const reference of references) {
            if (classification === 'governance-identity' && !governanceIdentity) {
                continue;
            }
            if (classification !== 'governance-identity' && governanceIdentity) {
                continue;
            }
            if (classification !== 'all' && !reference.requiresRuntimeIdentity) {
                continue;
            }
            const resolved = resolveModuleSpecifier(filePath, reference.specifier);
            if (compatibilityPaths.has(resolved)) {
                consumers.get(resolved)?.push(filePath);
            }
        }
    }
...
function readConsumerSourceEvidence(
    ...
    if (!input && cachedRepositoryEvidence) {
        return cachedRepositoryEvidence;
    }
    const sources = input ?? {
    ...
    if (!input) {
        cachedRepositoryEvidence = evidence;
    }
    return evidence;
}
...
function requireAllAllowlistedSources(sourcePaths: readonly string[]): void {
    ...
    if (missingPath) {
        throw new SyntaxError(`${missingPath}: missing allowlisted source path`);
    }
}
...
function requireAllowedNonliteralReferences(
    ...
    for (const reference of references) {
        ...
        if (index < 0) {
            throw new SyntaxError(`${filePath}: nonliteral module reference: ${key}`);
        }
