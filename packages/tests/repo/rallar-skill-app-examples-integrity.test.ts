...
describe('Rallar skill app and example integrity', () => {
    it('provides the greenfield app workflow and audited evidence map', () => {
        ...
        const initialBoot = scaffolding.match(/## Initial Boot\n([\s\S]*?)\n## Room-Bound Vertical Slice/)?.[1] ?? '';
        const verticalSlice = scaffolding.match(/## Room-Bound Vertical Slice\n([\s\S]*?)\n## Runtime Adapter/)?.[1] ?? '';
        const runtimeAdapter = scaffolding.match(/## Runtime Adapter\n([\s\S]*?)\n## React Adapter/)?.[1] ?? '';
        ...
        expect
            .soft({
                ...
                verticalMessageCallbackValidations: verticalSlice.match(/isMessageForRoom\(roomRef, message\)/g)?.length === 2,
        ...
        expect
            .soft({
                ...
                readyHandleWait: scaffolding.includes('readyHandle.wait({'),
                ...
                messageExampleWait: messageExample.includes('sendHandle.wait({ until: AL_DELIVERY_ADMITTED_STATES'),
        ...
        expect
            .soft({
                ...
                // Matched on the statements rather than their layout: the formatter owns the
                // indentation of the fenced example this reads.
                failureCleansSubscriptions: /catch \(error\) \{\s*subscriptions\.unsubscribe\(\);\s*throw error;\s*\}/u.test(
                    verticalSlice
                )
            })
            .toEqual({
