...
import type { DenoRtcBaselineAdapters } from '../../../scripts/perf/rtc-baseline/rtc-baseline-deno-adapters.ts';
import type {
  RtcBaselineResolvedConfigurationValueDto,
  RtcBaselineResult,
} from '../../../scripts/perf/rtc-baseline/rtc-baseline-contracts.ts';
import { createRtcBaselineDenoRuntime } from '../../../scripts/perf/rtc-baseline/rtc-baseline-deno-runtime.ts';
const baselineId = '20260807-0123456789ab-e2-browser';
const browserWorker = 'scripts/perf/rtc-data-channel-browser-soak.mjs';
type FixtureOuter = {
...
const b01Outer = {
  ...
  sampleIds: [1, 2, 3, 4, 5].map(
    (innerOrdinal) =>
      `rtc-b01-peer-connection-diagnostics-burst-pairs-500-retained-001-${innerOrdinal.toString().padStart(3, '0')}`,
  ),
};
...
function runtimeAdapters(
  ...
  return {
    ...
    runtimeHost: { read: () => ({ deno: '2.4.0', ...hostFacts }) },
...
describe('RTC baseline Deno runtime composition', () => {
  ...
  it('wires observation through initialization and persists environment plus manifest', async () => {
    ...
    const adapters = runtimeAdapters(
      {
        ...
        readFile: async (path) => {
          ...
          if (!stored) throw new Error('not read');
      ...
      },
      {
        read: async () => ({
          ...
          value: [
            ...
            { path: 'playwright.config.ts', kind: 'config', sha256: 'd'.repeat(64) },
          ],
        }),
      },
    );
    ...
    const mismatch =
      '$.sourceHashes\treconciliation-mismatch\tRuntime observation field sourceHashes changed.';
    expect(
      issueText(
  ...
  it('collects persisted sample and raw records during finalization', async () => {
    ...
    const runtime = createRtcBaselineDenoRuntime(
      runtimeAdapters({
        ...
        listDirectory: async (path) => {
          if (path.endsWith('/artifacts') && useFailureOutcome) return [];
          if (path.endsWith('/artifacts'))
            return [
              { name: 'raw.json', kind: 'file' },
              { name: 'staging', kind: 'directory' },
            ];
          if (path.endsWith('/artifacts/staging')) return [{ name: 'producer.json', kind: 'file' }];
    ...
    const unsupported =
      '$.results/unknown.json\tunsupported-artifact-path\tResult artifact path is not recognized by the RTC baseline protocol.';
