...
import type { RtcBaselineJson } from '../../../scripts/perf/rtc-baseline/rtc-baseline-contracts.ts';
import {
  canonicalizeRtcBaselineRawReferences,
  classifyRtcBaselineArtifactPath,
} from '../../../scripts/perf/rtc-baseline/rtc-baseline-evidence-layout.ts';
import {
  evaluateRtcBaselineWorkloadRepeatOutcome,
  rtcBaselineRepeatWorkloadIds,
} from '../../../scripts/perf/rtc-baseline/rtc-baseline-statistics.ts';
import {
...
} from '../../../scripts/perf/rtc-baseline/rtc-baseline-workload-manifest.ts';
...
describe('RTC baseline workload manifests', () => {
  it('derives every ordered B06 outer unit and every predeclared sample identity', () => {
    ...
    const outerRows = manifest.outerAttempts.map(
      (outer) =>
        `${outer.caseId}/${outer.inputKey}/${outer.intendedPhase}/${outer.outerOrdinal}/${outer.sampleIds.join(',')}`,
    );
    ...
    expect(
      ...
      value: {
        ...
        rawResultRelativePath:
          'artifacts/staging/rtc-b05-browser-data-channel-lifecycle-iterations-25-retained-003.json',
      },
    });
      ...
      for (const unit of units) {
        ...
        expect(actual).toEqual({
          ...
          sampleIds: { '3': ['001', '002', '003'], '5': ['001', '002', '003', '004', '005'] }[
            ...
            (inner) =>
              `${sampleStem}-${unit.intendedPhase}-${String(unit.outerOrdinal).padStart(3, '0')}-${inner}`,
          ),
        });
        outerIndex += 1;
      }
