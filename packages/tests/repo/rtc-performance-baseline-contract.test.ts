...
} from '../../../scripts/perf/rtc-baseline/rtc-baseline-failure-accounting.ts';
...
describe('RTC baseline data contracts', () => {
  ...
  it('round-trips the complete persisted artifact family with singular attempt workload IDs', () => {
    ...
    expect(resolveRtcBaselineAcceptedArtifactPath(sample)).toEqual({
      ok: true,
      value:
        'results/samples/rtc-b05-browser-data-channel-lifecycle-iterations-25-retained-001-001.json',
    });
    expect(resolveRtcBaselineAcceptedArtifactPath(externalAttempt)).toEqual({
      ok: true,
      value:
        'results/external-attempts/RTC-B05-browser-data-channel-lifecycle-iterations-25-retained-001.json',
    });
