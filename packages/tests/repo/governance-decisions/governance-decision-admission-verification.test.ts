...
describe('governance decision authenticated admission', () => {
    ...
    it.each([
        ...
        for (
            const steps of [
                ...
                successfulAdmissionEvidence().workflowRuns[0].jobs[0].steps.map((step) => step.name === stepName ? { ...step, conclusion: 'failure' } : step),
    ...
    it('fails closed when two workflow runs or two classifier jobs claim admission', () => {
        ...
        expect(() => verifyGovernanceDecisionAdmission({ commitOid, decisionId, evidence: duplicateRun })).toThrow(
            'no unambiguous authenticated main-push admission'
        );
        ...
        expect(() => verifyGovernanceDecisionAdmission({ commitOid, decisionId, evidence: duplicateJob })).toThrow(
            'no unambiguous authenticated main-push admission'
        );
    });
});
