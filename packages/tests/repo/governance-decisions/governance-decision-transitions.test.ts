...
describe('governance decision transitions', () => {
    ...
    it('fails closed for stale heads, symlink plans, and duplicate receipt paths', () => {
        ...
        expect(() => computeGovernanceDecisionTransition({ request, snapshot: symlinkSnapshot })).toThrow('target plan must not be a symbolic link');
        ...
        expect(() => computeGovernanceDecisionTransition({ request, snapshot: receiptSnapshot })).toThrow('decision receipt path already exists');
    });
