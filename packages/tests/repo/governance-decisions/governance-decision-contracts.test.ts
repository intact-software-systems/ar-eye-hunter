...
describe('governance decision contracts', () => {
    ...
    it('accepts only the exact common request keys and fixed repository authority', () => {
        ...
        expect(() => decodeGovernanceDecisionRequest({ ...cancelRequest(), repository: 'other/repository' })).toThrow(
            'repository must be intact-software-systems/ar-eye-hunter'
        );
        expect(() => decodeGovernanceDecisionRequest({ ...cancelRequest(), force: false })).toThrow(
            'force must be true'
        );
        expect(() => decodeGovernanceDecisionRequest({ ...cancelRequest(), expectedHeadOid: 'A'.repeat(40) })).toThrow(
            'expectedHeadOid must be 40 lowercase hexadecimal characters'
        );
    });
    ...
    it('requires each operation exact target and payload keys', () => {
        ...
        expect(() => decodeGovernanceDecisionRequest({ ...completeRequest(), payload: { content: 'arbitrary' } })).toThrow(
            'plan.complete payload must be empty'
        );
        expect(() =>
            decodeGovernanceDecisionRequest({
                ...quarantineRequest(),
