...
import { createGovernanceDecisionFixturePlanRecord, toGovernanceDecisionFixturePlanMarkdown } from './governance-decision-fixture';
...
describe('governance decision structural commit verification', () => {
    ...
    it('rejects arbitrary declared successor bytes and non-regular changed files by replay', () => {
        ...
        const receipt = JSON.parse(receiptEntry.content);
        receipt.stateChanges.find((change: any) => change.path === successorEntry.path).after = identityForContent(arbitraryContent);
    ...
    it('rejects undeclared and declared-but-operation-forbidden mixed changes', () => {
        ...
        expect(() =>
            verifyGovernanceDecisionCommit({
                ...
                readRepositorySnapshot: (commitOid: string) => readGitRepositorySnapshot({ repoRoot: undeclared.root, commitOid })
            })
        ...
        expect(() =>
            verifyGovernanceDecisionCommit({
                ...
                readRepositorySnapshot: (commitOid: string) => readGitRepositorySnapshot({ repoRoot: declared.root, commitOid })
            })
    ...
    it('rejects a cancellation receipt that omits the target plan deletion', () => {
        ...
        expect(() =>
            verifyGovernanceDecisionCommit({
                commitOid: fixture.commitOid,
                readRepositorySnapshot: (commitOid: string) => commitOid === fixture.parentOid ? parentSnapshot : forgedCommitSnapshot
            })
        ).toThrow('commit does not equal the deterministic governance transition');
    });
    ...
    it('rejects extra receipt keys and symbolic-link receipt entries', () => {
        ...
        expect(() =>
            verifyGovernanceDecisionCommit({
                commitOid: fixture.commitOid,
                readRepositorySnapshot: (commitOid: string) => commitOid === fixture.parentOid ? parentSnapshot : withExtraKey
            })
        ...
        expect(() =>
            verifyGovernanceDecisionCommit({
                commitOid: fixture.commitOid,
                readRepositorySnapshot: (commitOid: string) => commitOid === fixture.parentOid ? parentSnapshot : symlinkReceipt
            })
...
function createAppliedDecisionFixture(options: AppliedDecisionOptions = {}) {
    ...
    const planMarkdown = options.operation === 'plan.quarantine'
