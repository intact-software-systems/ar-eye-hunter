...
import * as decoding from '../../../scripts/perf/rtc-baseline/rtc-baseline-artifact-decoding.ts';
import * as validation from '../../../scripts/perf/rtc-baseline/rtc-baseline-artifact-validation.ts';
...
const environment = JSON.parse(
  `{"schema":"rallar.rtc-baseline.environment.v1","baselineId":"20260807-0123456789ab-e3-memory-repeat-01","workloadIds":["RTC-B06"],"environmentId":"E3-memory","repeatLink":${JSON.stringify(repeatLink)},"conditionalEnvironmentDecisions":[${JSON.stringify(condition)}],"observation":${JSON.stringify(observation)}}`,
);
const manifest = JSON.parse(
  `{"schema":"rallar.rtc-baseline.manifest.v1","request":{"schema":"rallar.rtc-baseline.capture-request.v1","baselineId":"20260807-0123456789ab-e3-memory-repeat-01","workloadIds":["RTC-B06"],"environmentId":"E3-memory","retainedSampleMultiplier":2,"repeatLink":${JSON.stringify(repeatLink)},"conditionalEnvironmentDecisions":[${JSON.stringify(condition)}]},"workloadIds":["RTC-B06"],"cases":[{"workloadId":"RTC-B06","caseId":"retention-100","inputKey":"e3-memory-retention-100"}],"outerAttempts":[{"workloadId":"RTC-B06","caseId":"retention-100","inputKey":"e3-memory-retention-100","environmentId":"E3-memory","intendedPhase":"retained","outerOrdinal":1,"sampleIds":["rtc-b06-retention-100-e3-memory-retained-001-001"]}],"expectedCohorts":[${JSON.stringify(cohortIdentity)}],"repeatLink":${JSON.stringify(repeatLink)}}`,
);
const sample = JSON.parse(
  `{"schema":"rallar.rtc-baseline.sample.v1","identity":${JSON.stringify(identity)},"outcome":"failed","evidenceClass":"local-full-stack","metrics":[{"metric":"heapBytes","unit":"bytes","value":1024}],"rawEvidence":{"settledPeers":1},"rawReferences":[{"relativePath":"artifacts/raw.json","sha256":"${'b'.repeat(64)}","bytes":12}],"issues":[${JSON.stringify(recordedIssue)}],"runtimeObservation":${JSON.stringify(observation)}}`,
);
const attempt = JSON.parse(
  `{"schema":"rallar.rtc-baseline.external-attempt.v1","locator":{"workloadId":"RTC-B06","caseId":"retention-100","inputKey":"e3-memory-retention-100","intendedPhase":"retained","outerOrdinal":1,"environmentId":"E3-memory","rawResultRelativePath":"artifacts/staging/attempt.json"},"producerExitStatus":1,"producerFacts":${JSON.stringify(producerFacts)},"sampleOutcomes":[{"identity":${JSON.stringify(identity)},"outcome":"failed","issues":[${JSON.stringify(recordedIssue)}]}],"samples":[${JSON.stringify(sample)}],"issues":[${JSON.stringify(recordedIssue)}]}`,
);
const cohort = JSON.parse(
  `{"schema":"rallar.rtc-baseline.external-cohort.v1","identity":${JSON.stringify(cohortIdentity)},"outcome":"failed","rawEvidence":{"breachCount":1},"issues":[${JSON.stringify(recordedIssue)}],"samples":[${JSON.stringify(sample)}]}`,
);
const failure = JSON.parse(
  `{"schema":"rallar.rtc-baseline.finalization-failure.v1","baselineId":"20260807-0123456789ab-e3-memory-repeat-01","failureId":"finalization-001","issues":[${JSON.stringify(recordedIssue)}],"rawEvidence":{"disk":"full"}}`,
);
const summary = JSON.parse(
  `{"schema":"rallar.rtc-baseline.summary.v1","baselineId":"20260807-0123456789ab-e3-memory-repeat-01","workloadIds":["RTC-B06"],"environmentId":"E3-memory","repeatLink":${JSON.stringify(repeatLink)},"conditionalEnvironmentDecisions":[${JSON.stringify(condition)}],"sampleOutcomes":[{"identity":${JSON.stringify(identity)},"outcome":"failed","issues":[${JSON.stringify(recordedIssue)}]}],"cohortOutcomes":[{"identity":${JSON.stringify(cohortIdentity)},"outcome":"failed","issues":[${JSON.stringify(recordedIssue)}]}],"metricSummaries":[${JSON.stringify(metricSummary)}],"rawReferences":[{"relativePath":"artifacts/raw.json","sha256":"${'b'.repeat(64)}","bytes":12}]}`,
);
...
const decoders: Record<ArtifactName, (value: unknown) => DecodeResult> = {
  environment: decoding.decodeRtcBaselineEnvironment,
  manifest: decoding.decodeRtcBaselineManifest,
  sample: decoding.decodeRtcBaselineSample,
  attempt: decoding.decodeRtcBaselineExternalAttempt,
  cohort: decoding.decodeRtcBaselineExternalCohort,
  failure: decoding.decodeRtcBaselineFinalizationFailure,
  summary: decoding.decodeRtcBaselineSummary,
  runtime: decoding.decodeRtcBaselineRuntimeObservation,
};
...
function artifactPathPairs(text: string) {
  ...
  while (tokens.length > 0) pairs.push([tokens.shift() as ArtifactName, tokens.shift()!]);
  return pairs;
}
...
function valueAtPath(root: Record<string, unknown>, path: string) {
  return pathParts(path).reduce((value, key) => (value as Record<string, unknown>)[key], root);
}
...
function ownerAndField(root: Record<string, unknown>, path: string) {
  ...
  return {
    owner: valueAtPath(root, `$.${parts.slice(0, -1).join('.')}`) as Record<string, unknown>,
    field: parts.at(-1)!,
  };
}
...
describe('RTC baseline artifact decoding and validation', () => {
  ...
  it('rejects holes in every nested persisted array', () => {
    for (const [artifactName, path] of denseArrays) {
      const malformed = copyArtifact(artifacts[artifactName]);
      delete (valueAtPath(malformed, path) as unknown[])[0];
      expect(issueRows(decoders[artifactName](malformed))).toEqual([
  ...
  it('rejects unexpected fields at every persisted object boundary', () => {
    for (const [artifactName, path] of objectPaths) {
      const malformed = copyArtifact(artifacts[artifactName]);
      (valueAtPath(malformed, path) as Record<string, unknown>).unexpected = true;
  ...
  it('rejects wrong primitive, range, timestamp, path, and digest values exactly', () => {
    ...
    Object.assign(valueAtPath(malformedSample, '$.metrics[0]'), { value: '1024' });
    Object.assign(valueAtPath(malformedSample, '$.rawReferences[0]'), {
      relativePath: '../raw.json',
      sha256: 'not-a-digest',
      bytes: -1,
    });
    expect(issueText(decoding.decodeRtcBaselineSample(malformedSample)))
...
$.rawReferences[0].bytes\texpected-nonnegative-integer\tExpected a nonnegative integer.`);
    ...
    Object.assign(valueAtPath(malformedRuntime, '$.host'), { logicalCpuCount: -1 });
    Object.assign(valueAtPath(malformedRuntime, '$.timing'), { startedAtUtc: 'yesterday' });
    Object.assign(valueAtPath(malformedRuntime, '$.sourceHashes[0]'), {
      path: '/absolute.ts',
      sha256: 'not-a-digest',
    });
    Object.assign(valueAtPath(malformedRuntime, '$.configurationInputs[0]'), { value: [] });
    Object.assign(valueAtPath(malformedRuntime, '$.resolvedConfiguration[0]'), { value: null });
    Object.assign(valueAtPath(malformedRuntime, '$.allowlistedEnvironment'), { DATABASE_URL: 0 });
    expect(issueText(decoding.decodeRtcBaselineRuntimeObservation(malformedRuntime)))
...
$.samples\tcohort-member-samples-mismatch\tExternal cohort samples must exactly match its ordered member sample IDs.`,
    ...
    expect(
      issueText({
        ok: false,
        issues: validation.validateRtcBaselineExternalAttempt(invalidProducerFacts),
      }),
    ).toBe(
    ...
    expect(
      issueRows({
        ok: false,
        issues: validation.validateRtcBaselineExternalAttempt(invalidAttempt),
      }),
    ).toEqual([
      [
        '$.samples[0].identity',
    ...
    expect(
      issueRows({ ok: false, issues: validation.validateRtcBaselineExternalCohort(invalidCohort) }),
    ).toEqual([
      [
        '$.samples',
