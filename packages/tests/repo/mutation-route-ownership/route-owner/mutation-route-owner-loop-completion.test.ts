...
import { executeMutationPaths } from '../execution/mutation-execution-outcomes.ts';
import type { MutationExecutionAstNode as AstNode } from '../execution/mutation-execution-path-state.ts';
...
describe(
    'Mutation route owner loop completion contracts',
    { timeout: 30_000 },
    () => {
        ...
        it('skips a classic-for update after a matching labeled outer break', () => {
            const issues = validateInvocations(
                `outer: for (; true; registerC16(undefined, 'keys')) {
            {
                break outer;
            }
        }
        ...
        while (registerC16(undefined, 'keys'));`
            ...
            'makes nested guaranteed break equivalent to direct break',
            (direct, nested) => {
                const directIssues = validateInvocations(`${direct}
        registerC16(undefined, 'values');`);
                const nestedIssues = validateInvocations(`${nested}
        registerC16(undefined, 'values');`);
                expect(projectionState(nestedIssues)).toEqual(
                    projectionState(directIssues)
                );
        ...
        it('runs continue phases and preserves the unknown do-test exit', () => {
            const forIssues = validateInvocations(
                `for (; true; registerC16(undefined, 'keys')) {
            continue;
        }
        registerC16(undefined, 'values');`
        ...
        it('runs abrupt update and test phases only after normal or continue outcomes', () => {
            expect(executeCompletion(classicFor('BreakStatement', 'ReturnStatement')))
                .toBe('normal');
            expect(
                executeCompletion(classicFor('ContinueStatement', 'ReturnStatement'))
            ).toBe('return');
            expect(executeCompletion(doWhile('BreakStatement', 'ThrowStatement')))
                .toBe('normal');
            expect(executeCompletion(doWhile('ContinueStatement', 'ThrowStatement')))
                .toBe('throw');
        });
    }
);
...
function expectBoundaryMutation(
    name: string,
...
function validateInvocations(
    ...
    mutated = mutated.replace(
        LIVE_GROUP_COLLECTION,
        'C16_TYPE_MAP[MAP_METHOD]()'
    );
