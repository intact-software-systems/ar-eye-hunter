...
import { createMutationBoundaryLexicalValues } from '../lexical/mutation-boundary-lexical-values.ts';
import {
...
import { asCapabilityNode as asNode, walkCapabilityAst as walk } from './mutation-boundary-capability-ast.ts';
...
export function findCapabilityMutationCalls(
    ...
    if (!program) {
        throw new TypeError(`Unparsable capability source: ${filePath}`);
    }
...
function capabilityStateKey(analysis: CapabilityBindingAnalysis): string {
    const entries = <Value>(map: ReadonlyMap<string, Value>) => [...map].toSorted(([left], [right]) => left.localeCompare(right));
