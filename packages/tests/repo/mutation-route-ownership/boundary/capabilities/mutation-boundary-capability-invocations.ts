import { isCall, readPosition, walkExecution, walkReachableAst } from '../../execution/mutation-boundary-execution-walk.ts';
import { readCallFamily, resolveArgumentSlot, resolveInvocationArguments, unwrapCallableParameter } from '../callables/mutation-boundary-call-arguments.ts';
import type { InvocationArgumentSlot } from '../callables/mutation-boundary-call-arguments.ts';
import { bindCallableResolution, callableArrayResolution, collectCallableTargets } from '../callables/mutation-boundary-callable-collections.ts';
import { discoverLocalCallables, indexCallableReferences } from '../callables/mutation-boundary-callable-definitions.ts';
import { appendInvocationAlias, canonicalStorageKey, unionStoredAliases, type CallableHeapAliasWrite } from '../callables/mutation-boundary-callable-heap.ts';
import {
...
} from '../callables/mutation-boundary-callable-resolution.ts';
import { isCapturedCallableWrite, projectCallableResolution, readCallablePatternWrites } from '../callables/mutation-boundary-callable-storage.ts';
import { readInvocationCallablePosition } from '../lexical/mutation-boundary-invocation-position.ts';
import {
...
import { isCapturedFlowWrite, readFlowPatternWrites, type CapabilityFlowAccess, type ClosureExecutionWrite } from './mutation-boundary-capability-closures.ts';
...
function collectCallableAliases(program: AstNode, context: InvocationContext): void {
    walkReachableAst(program, (node, execution) => {
        ...
        for (const write of readCallablePatternWrites(target, source, context.access)) {
            if (isCapturedCallableWrite(write.owner, node, context.access)) {
                continue;
            }
...
function summarizeInvocations(
    ...
    if (readCallFamily(call, context.access) === 'bind') {
        return [];
    }
    ...
    for (const target of resolution.targets.values()) {
        if (activeFunctions.has(target.definition.functionKey)) {
            continue;
        }
        effects.push(
            ...summarizeFunction(
        ...
        if (rawParameter.type === 'RestElement') {
            bindings.set(
                ...
                callableArrayResolution(
                    arguments_.slice(index).map((slot) => resolveArgumentSlot(slot, rawParameter, call, context, resolving)),
...
function summarizeFunction(
    ...
    walkExecution(definition.node, (node, execution) => {
        ...
        for (const write of readFlowPatternWrites(pattern, value, context.access)) {
            if (!isCapturedFlowWrite(write, node, context.access)) {
                continue;
            }
            effects.push({
                kind: 'flow',
