...
import type { MutationBoundaryLexicalBindings } from '../lexical/mutation-boundary-lexical-bindings.ts';
...
import type { CapabilityTypeResolver, CapabilityTypeShape } from './mutation-boundary-capability-types.ts';
...
export function capabilityExpressionKey(
    ...
    if (node.type === 'Identifier') {
        return analysis.bindings.identifierKey(node);
    }
    if (node.type === 'ThisExpression') {
        return analysis.bindings.thisKey(node);
    }
    if (
        node.type !== 'MemberExpression' &&
        node.type !== 'OptionalMemberExpression'
