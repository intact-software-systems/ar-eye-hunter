...
function executeNode<State>(
    ...
    if (Array.isArray(value)) {
        return executeSequence(value, paths, adapter, rootFunction);
    }
    const node = value as AstNode;
    if (isFunction(node) && node !== rootFunction) {
        if (adapter.nestedFunctions === 'skip') {
            return paths;
        }
...
function executeSwitch<State>(
    ...
    return discriminants.flatMap((path) => {
        ...
        return tested.flatMap((testedPath) => {
            if (testedPath.completion.kind !== 'normal') {
                return [testedPath];
            }
            const outcomes = entries.entryIndices.flatMap((entryIndex, alternativeIndex) => {
                const entryPath = alternative
                    ...
                    : testedPath;
                const statements = cases.slice(entryIndex).flatMap((switchCase) => asNodes(switchCase.consequent));
...
function executeTry<State>(
    ...
    return caught.flatMap((path) => {
        if (path.completion.kind === 'diverge') {
            return [path];
        }
        ...
        return executeNode(
            ...
            (finalized) => finalized.completion.kind === 'normal' ? { ...finalized, completion: prior } : finalized
        );
    });
}
...
function executeCollectionLoop<State>(
    ...
    return evaluated.flatMap((path) => {
        ...
        for (const element of elements) {
            const iteration = active.map((candidate) => bindExecutionLoopPath(candidate, node.left, element ? [element] : [], !element, adapter));
