...
export function invokeWriterAfterRebind(
    ...
    let invoke: ClientStateRepository['readSnapshot'] | ClientStateRepository['updatePrincipal'] = repository.readSnapshot;
    const readers = {
        run: () => {
...
export function invokeWriterAfterMultipleRebinds(
    ...
    let invoke: ClientStateRepository['readSnapshot'] | ClientStateRepository['deletePrincipal'] = repository.readSnapshot;
    const readers = {
        nested: {
            run: () => {
                invoke = repository.readSnapshot;
