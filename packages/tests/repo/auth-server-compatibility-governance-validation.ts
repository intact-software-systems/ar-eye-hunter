...
const contracts: readonly WrapperContract[] = [
    ...
    },
    {
        ...
        groups: [
            ...
            {
                target: 'packages/shared-server/rallar-system/auth/mutation/decode-auth-mutation-command.ts',
            ...
            {
                target: 'packages/shared-server/rallar-system/auth/mutation/auth-mutation-rejected-error.ts',
                runtime: ['AuthMutationRejectedError']
            },
            {
                target: 'packages/shared-server/rallar-system/auth/mutation/read/capture-auth-mutation-facts.ts',
    ...
    },
    {
        ...
        groups: [
            ...
            {
                target: 'packages/shared-server/rallar-system/auth/persistence/auth-persistence-contracts.ts',
                runtime: [
            ...
            {
                target: 'packages/shared-server/rallar-system/auth/persistence/auth-legacy-compatibility.ts',
                runtime: [
    ...
    return (program.body as readonly AstNode[]).flatMap((statement) => {
        ...
        return ((statement.specifiers as readonly AstNode[]) ?? []).map((specifier) => {
            if (specifier.type !== 'ExportSpecifier') {
                return `unsupported:<specifier>->${target}`;
            }
...
function readLocalExports(filePath: string, source: string): readonly string[] {
    ...
    for (const statement of body) {
        const declaration = statement.type === 'ExportNamedDeclaration' ? (statement.declaration as AstNode) : statement;
        for (const [name, kind] of declarationNames(declaration)) {
...
function localExportsFromStatement(
    ...
    if (statement.declaration) {
        return declarationNames(statement.declaration as AstNode).map(toPair);
    }
