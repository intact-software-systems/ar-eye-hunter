    ...
    return {
        ...
        validate: (value, context) =>
            validateRallarCrdtServerLiveEnvelope(
                ...
                {
                    topicId:
                        context.definition?.topicId ?? toTopicId(topicScope),
                    typeId,
...
function validateEnvelopeByKind(
    ...
    switch (kind) {
        ...
        case 'catch-up-request':
            return validateRallarCrdtCatchUpRequestEnvelope(
                value,
                '$',
                options,
            );
        case 'catch-up-response':
            return validateRallarCrdtCatchUpResponseEnvelope(
                value,
                '$',
                options,
            );
...
function validateLiveDocumentScope(
    ...
    if (topicScope === 'room') {
        ...
        if (!context.roomRef) {
            issues.push({
                ...
                message:
                    'Room CRDT live messages must carry a scoped AL target groupRef.',
            });
            return issues;
        }
        ...
        if (!sameGroupRef(document.roomRef, context.roomRef)) {
            issues.push({
                ...
                message:
                    'CRDT document roomRef must match the AL message target groupRef.',
            });
        }

        if (
            context.roomId !== undefined &&
            context.roomId !== document.roomRef.groupId
        ) {
            issues.push({
                path: '$.document.roomRef.groupId',
                code: 'message-room-id-mismatch',
                message:
                    'CRDT document room groupId must match the AL message room context.',
            });
        }
    ...
    if (document.scope !== 'app' || options.allowAppDocuments !== true) {
        issues.push({
            ...
            code: 'unsupported-live-document-scope',
            message:
                'app.crdt live messages only support explicitly enabled app-scoped documents.',
        });
    }
...
function toSharedValidationOptions(
    ...
    return {
        ...
        ...(options.allowedOperationKinds
            ...
            : {}),
        maxPayloadBytes:
            options.maxUpdateBytes ?? options.validation?.maxPayloadBytes,
    };
}

function collectKnownBinaryJsonShapeIssues(
    value: unknown,
): readonly RallarCrdtValidationIssue[] {
    const issues: RallarCrdtValidationIssue[] = [];
...
function collectKnownBinaryJsonShapeIssuesAt(
    ...
    if (Array.isArray(value)) {
        value.forEach((entry, index) =>
            collectKnownBinaryJsonShapeIssuesAt(
                entry,
                `${path}[${index}]`,
                issues,
            ),
    ...
    if (
        ...
        issues.push({
            ...
            message:
                'CRDT live payloads must not include raw Buffer/Blob-like values.',
        });
        return;
    }
...
function toEnvelopeKind(
    typeId: string,
): RallarCrdtServerEnvelopeKind | undefined {
    switch (typeId) {
...
function readEnvelopeDocument(
    value: unknown,
): RallarCrdtDocumentRef | undefined {
    if (!value || typeof value !== 'object') {
        return undefined;
    }
    ...
    return document && typeof document === 'object'
        ? (document as RallarCrdtDocumentRef)
        : undefined;
}
