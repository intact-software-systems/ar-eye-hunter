...
export class PSqlQueueBox implements QueueBoxResourceEntryRepository {
    ...
    async enqueueIf(
        ...
        const observed = await this.resourceInbox.entries.findAnyByKey(resourceEntry.key);
        ...
        return await this.resourceInbox.transaction(
            async (txRepo: PSqlResourceInboxRepository) => {
                const current = await txRepo.entries.findAnyByKeyForUpdate(resourceEntry.key);
                assertSameObservedEntry(current, observed, 'Resource inbox entry changed before conditional write');
                if (observed === null) {
                    const written = await txRepo.entries.writeIfAbsentOrReplaceExpired(resourceEntry);
                    if (!hasSameResourceEntryContent(written, resourceEntry)) {
                        throw new Error('Resource inbox entry changed before conditional write');
                    }
                    return undefined;
                }
                await txRepo.entries.replace(resourceEntry);
