...
import { canReadGroupSnapshot, canUpdateGroupSnapshot, GroupPolicyDeniedError } from '../group-policy.ts';
import {
    ...
    public async readMyRealtimeStatus(
        ...
        const groupsWithCurrentSession = readableGroups.filter((snapshot) =>
            snapshot.activeSessions.some(
                (session) => session.principalId === actor.principalId && session.sessionId === actor.sessionId
            )
        );
