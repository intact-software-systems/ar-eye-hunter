...
import {
  countRecentGroupEvents,
  listRecentGroupEvents,
  readBoundedGroupSnapshots,
  type SpaStatisticsGroupStateReads,
} from './spa-statistics-group-reads.ts';
...
export type SpaStatisticsServiceOptions = Readonly<{
  ...
  groupStateService: SpaStatisticsGroupStateReads;
...
export type ReadGroupSpaStatisticsInput = ReadWorkspaceSpaStatisticsInput &
  Readonly<{
    groupId: string;
  }>;
...
export class SpaStatisticsService {
  ...
  public constructor(options: SpaStatisticsServiceOptions) {
    ...
    this.topGroupsLimit = options.topGroupsLimit ?? DEFAULT_TOP_GROUPS_LIMIT;
  ...
  public async readWorkspaceSummary(
    ...
    const readableGroups = groupScan.snapshots.filter(
      (snapshot) =>
        canReadGroupSnapshot({
          snapshot,
          actor: {
    ...
    return {
      ...
      actor: {
        ...
        groupPresenceCount: countActorGroupPresenceSessions(readableGroups, actor.principalId),
      },
      warnings: [
        warning(
      ...
      groups: {
        ...
        joinedCount: readableGroups.filter((snapshot) =>
          hasActiveMember(snapshot, actor.principalId),
        ).length,
  ...
  public async readGroupStats(
    ...
    const snapshot = await this.options.groupStateService.readCurrentSnapshot(groupRef);
    if (!snapshot) {
    ...
    const recentEvents = await listRecentGroupEvents(this.options.groupStateService, groupRef, {
      limit: this.recentEventLimit,
    });
    ...
    return {
      ...
      actor: {
        ...
        activePresenceSessionCount: countActorGroupPresenceSessions([snapshot], actor.principalId),
      },
      warnings: [
        warning(
          'bounded-recent-events',
  ...
  public async readMyRealtimeStatus(
    ...
    const groupsWithCurrentSession = readableGroups.filter((snapshot) =>
      snapshot.activeSessions.some(
        (session) =>
          session.principalId === actor.principalId && session.sessionId === actor.sessionId,
      ),
    );
    const currentSessionOpen = isSessionOpen(this.options.wsStatus?.(), actor.sessionId);
...
function toOwnerDetails(snapshot: GroupSnapshot): GroupSpaStatisticsResponse['owner'] | undefined {
...
function warning(code: SpaStatisticsWarning['code'], message: string): SpaStatisticsWarning {
  return { code, message };
}
