...
async function toIssuedSessionPublicResult(
    ...
    const expected = 'session' in request
        ? request.session
        : {
...
async function toIssuedWebSocketTicketPublicResult(
    ...
    const expectedSessionId = 'authority' in request ? request.authority.sessionId : request.ticketRecord.sessionId;
...
async function toConsumedTicketPublicResult(
    request: Extract<AuthMutationPublicResultRequest, { kind: 'consume-ws-ticket' | 'consume-agent-ticket'; }>,
    result: AuthMutationResult,
    credentialIssuer: AuthCredentialIssuer
): Promise<AuthMutationPublicResult> {
    const expectedKind = request.kind === 'consume-ws-ticket' ? 'ws-ticket-consumed' : 'agent-ticket-consumed';
...
async function toIssuedAgentTicketsPublicResult(
    ...
    const expectedTickets = 'agentIds' in request
        ? await Promise.all(
