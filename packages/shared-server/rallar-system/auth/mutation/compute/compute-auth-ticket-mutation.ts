...
type AuthTicketMutationCommand = Extract<
  AuthMutationCommand,
  { kind: 'issue-ws-ticket' | 'consume-ws-ticket' }
>;
...
function computeIssueAuthWebSocketTicket(
  ...
  return {
    ...
    outcome:
      read.ticket && equalAuthJson(read.ticket.value, command.ticketRecord) ? 'replay' : 'write',
  };
}
