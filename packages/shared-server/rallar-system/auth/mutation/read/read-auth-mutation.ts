...
} from '../../persistence/auth-session-repository.ts';
...
async function readAuthUserRegistration(
  ...
  return {
    kind: command.kind,
    byUsername:
      (await users.findByNormalizedUsernameEntry(command.user.normalizedUsername)) ?? null,
...
async function readAuthSessionIssue(
  ...
  return {
    kind: command.kind,
    userByUsername:
      (await users.findByNormalizedUsernameEntry(command.authority.normalizedUsername)) ?? null,
...
async function readAuthWebSocketTicketIssue(
  ...
  const session =
    (await sessions.findSessionBySessionIdEntry(command.ticketRecord.sessionId)) ?? null;
  return {
    kind: command.kind,
