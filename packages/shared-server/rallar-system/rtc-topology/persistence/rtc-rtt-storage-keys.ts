...
export function decodeRtcRttMeasurementStorageKey(
    storageKey: string,
    ...
    if (
        ...
        throw storageKeyCorruption(
            storageKey,
            'RTC RTT measurement key has invalid shape',
        );
    }
    let from: string;
    let to: string;
    try {
        ...
        throw storageKeyCorruption(
            storageKey,
            'RTC RTT measurement key encoding is invalid',
        );
    }
    if (
        from.length === 0 ||
        ...
        throw storageKeyCorruption(
            storageKey,
            'RTC RTT measurement key is not canonical',
        );
    }
    return [from, to];
}
...
export function decodeRtcRttEndpointAdmissionStorageKey(
    storageKey: string,
): string {
    if (!storageKey.startsWith('endpoint=')) {
        throw storageKeyCorruption(
            storageKey,
            'RTC RTT endpoint key has invalid shape',
        );
    }
    ...
    try {
        ...
        throw storageKeyCorruption(
            storageKey,
            'RTC RTT endpoint key encoding is invalid',
        );
    }
    if (
        endpointId.length === 0 ||
        toRtcRttEndpointAdmissionStorageKey(endpointId) !== storageKey
    ) {
        throw storageKeyCorruption(
            storageKey,
            'RTC RTT endpoint key is not canonical',
        );
    }
    return endpointId;
}

export function sortRtcRttSessionPair(
    left: string,
    right: string,
): readonly [string, string] {
