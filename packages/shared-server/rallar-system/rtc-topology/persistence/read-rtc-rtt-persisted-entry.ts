...
export function readLiveRtcRttMeasurementEntry(
    ...
    if (trustedSessionIdA !== undefined && trustedSessionIdB !== undefined) {
        const trusted = sortRtcRttSessionPair(
            trustedSessionIdA,
            trustedSessionIdB,
        );
        if (decoded[0] !== trusted[0] || decoded[1] !== trusted[1]) {
            throw rttCorruption(
                entry.key,
                'RTC RTT key differs from requested pair',
            );
        }
    }
    ...
    const storedPair = sortRtcRttSessionPair(
        value.sessionIdFrom,
        value.sessionIdTo,
    );
    if (storedPair[0] !== decoded[0] || storedPair[1] !== decoded[1]) {
        throw rttCorruption(
            entry.key,
            'RTC RTT value differs from physical pair',
        );
    }
...
export function readLiveRtcRttEndpointAdmissionEntry(
    ...
    if (trustedEndpointId !== undefined && endpointId !== trustedEndpointId) {
        throw rttCorruption(
            entry.key,
            'RTC RTT endpoint differs from requested slot',
        );
    }
...
export function readRtcRttReceiptEntry(
    ...
    if (trustedReceiptId !== undefined && entry.key !== trustedReceiptId) {
        throw rttCorruption(
            entry.key,
            'RTC RTT receipt differs from trusted slot',
        );
    }
    ...
    if (value.receiptId !== entry.key) {
        throw rttCorruption(
            entry.key,
            'RTC RTT receipt differs from physical key',
        );
    }
    return { entry, value };
}
