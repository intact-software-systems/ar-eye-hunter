...
import { validateRtcRttRecomputeIntent } from './validate-rtc-rtt-recompute-intent.ts';

type LegacyRtcRttRecomputeIntent = Pick<
  RtcRttRecomputeIntent,
  'outboxId' | 'receiptId' | 'groupSnapshot' | 'rtt' | 'createdAtEpochMs' | 'commandHash'
>;
...
export async function migrateLegacyRtcRttRecomputeIntents(
  ...
  if (options.oldWritersStopped !== true) {
    throw new Error('RTC RTT recompute intent migration requires old writers stopped');
  }
  const runtime = requireOptimisticRuntime(repository.runtimeRepository);
  const sources = await runtime.findAllEntries(RTC_RTT_RECOMPUTE_OUTBOX_NAMESPACE);
  for (const source of sources) {
    await runtime.begin(async (transaction) => {
      const current = await transaction.findEntry(RTC_RTT_RECOMPUTE_OUTBOX_NAMESPACE, source.key);
      if (!current) return;
      ...
      if (!receiptEntry) {
        throw corruption(current.key, 'Legacy RTC RTT recompute intent receipt is missing');
      }
...
export { migrateLegacyRtcRttRecomputeIntents as migrateLegacyRtcRttRecomputeIntentDeliveryState };

function validateCurrentIntent(entry: RuntimeStateEntry, value: JsonWireObject): void {
  try {
    validateRtcRttRecomputeIntent(value, entry.expireAtTimestamp);
  } catch (error) {
    throw corruption(
      entry.key,
      error instanceof Error ? error.message : 'Invalid RTC RTT recompute intent',
    );
  }
}
...
function readReceipt(entry: RuntimeStateEntry): RtcRttMutationReceipt {
  try {
    ...
    throw corruption(entry.key, error instanceof Error ? error.message : 'Invalid RTC RTT receipt');
  }
}

function readLegacyIntent(entry: RuntimeStateEntry): LegacyRtcRttRecomputeIntent {
  try {
    const value = parseValue(entry);
    if (!isRecord(value)) {
    ...
    throw corruption(
      entry.key,
      error instanceof Error ? error.message : 'Invalid legacy RTC RTT recompute intent',
    );
  }
}
...
function validateLegacyIntent(value: JsonWireObject): void {
  ...
  if (
    ...
    value.outboxId !== toRtcRttRecomputeOutboxId(receiptId, groupRef, value.commandHash)
  ) {
...
function parseValue(entry: RuntimeStateEntry): JsonWireValue {
  try {
    ...
    throw corruption(entry.key, error instanceof Error ? error.message : 'RTC RTT JSON is invalid');
  }
}

function assertExactKeys(value: JsonWireObject, keys: readonly string[]): void {
