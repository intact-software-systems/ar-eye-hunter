...
export type RtcRttReceiptFamilyCleanupOptions = Readonly<{
    ...
    schedule?: (
        callback: () => void,
        delayMs: number,
    ) => RtcRttReceiptFamilyCleanupTimerHandle;
...
export async function cleanupExpiredRtcRttReceipts(
    ...
    for (const receiptId of candidateIds) {
        try {
            if (
                await cleanupExpiredRtcRttReceipt(
                    repository,
                    receiptId,
                    observedAtEpochMs,
                )
            ...
            failures.push({
                ...
                error: toCleanupFailureError(
                    error instanceof Error
                        ? error
                        : new Error('RTC RTT receipt cleanup failed'),
                ),
            });
        }
    }
...
export function initRtcRttReceiptFamilyCleanup(
    ...
    const cancel =
        options.cancel ??
        ((handle: RtcRttReceiptFamilyCleanupTimerHandle) =>
...
async function cleanupExpiredRtcRttReceipt(
    ...
    return await deleteGuardedRtcRttReceipt(
        requireOptimisticRuntime(repository),
        receipt,
    );
}
...
function requireOptimisticRuntime(
    ...
    if (!isRuntimeStateOptimisticTransactionalRepositoryLike(runtime)) {
        throw new Error(
            'RTC RTT receipt cleanup requires optimistic transactions',
        );
    }
    return runtime;
}
