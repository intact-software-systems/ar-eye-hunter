...
import { computeGlobalGraphAndCacheItIfPossible, initRtcRttTopic } from './rtc-topology/topic/init-rtc-rtt-topic.ts';
...
export function initRallarSystemWsTopics(
    ...
    const rtcTopologyService = options.rtcTopologyService ?? new RallarRtcTopologyService(options.rtcTopologyOptions);
    ...
    const globalGraphRecomputeLimit = options.globalGraphRecomputeLimit ?? DEFAULT_GLOBAL_GRAPH_RECOMPUTE_LIMIT;
    ...
    const findGroupSnapshotByRef = rtcTopologyAppOutboxOptions?.findGroupSnapshotByRef ??
    ...
    if (rtcTopologyAppOutbox && rtcTopologyAppOutboxOptions) {
        ...
        rtcTopologyAppOutboxOptions.outboxQueueReader.onOutboxMessageDo(
            ...
            createRtcTopologyWorkHandler({
                ...
                onInactiveOverlay: (overlayId) => clearRtcTopologyFlushTimer(overlayId, rtcTopologyFlushTimers)
            })
        );
    }
