...
export function toRtcRttTopologyOutboxId(
    ...
    return `${receiptId}:commandHash=${encodeURIComponent(commandHash)}:group=${
        encodeURIComponent(
...
export function readRtcRttTopologyOutboxIdentity(
    ...
    const [, receiptId, versionText, encodedCommandHash, encodedGroupIdentity] = match;
    if (!receiptId || !versionText || !encodedCommandHash || !encodedGroupIdentity) {
        return null;
    }
    try {
...
function isRtcTopologyEndpointPair(value: JsonWireValue): value is readonly [string, string] {
    return (
        Array.isArray(value) &&
        value.length === 2 &&
        value.every((endpoint) => typeof endpoint === 'string' && endpoint.length > 0)
    );
}
