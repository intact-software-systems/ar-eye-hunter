...
import { toRtcRttEndpointAdmissionStorageKey, toRtcRttMeasurementStorageKey } from './rtc-rtt-storage-keys.ts';
...
export class RtcRttRepository extends RuntimeStateJsonStore {
    ...
    async readMeasurementEntry(
        ...
        const entry = await this.runtimeRepository.findEntry(RTC_RTT_LATEST_NAMESPACE, key);
        if (!entry) {
        ...
        return value ? { value, expiredEntry: undefined } : { value: undefined, expiredEntry: entry };
    }
    ...
    async listMeasurementEntries(): Promise<readonly RuntimeStateEntryValue<RttMeasurementInfo>[]> {
        const entries = await this.runtimeRepository.findAllEntries(RTC_RTT_LATEST_NAMESPACE);
        return compact(
            entries.map((entry) =>
                readLiveRtcRttMeasurementEntry({
                    entry,
    ...
    async listMeasurementEntriesPage(
        ...
        const entries = await this.listEntriesPage(RTC_RTT_LATEST_NAMESPACE, '', options);
        return compact(
            entries.map((entry) =>
                readLiveRtcRttMeasurementEntry({
                    entry,
    ...
    async listMeasurementsForSessionIds(
        ...
        return (await this.listMeasurements()).filter(
            (measurement) => sessionSet.has(measurement.sessionIdFrom) && sessionSet.has(measurement.sessionIdTo)
        );
    }
    ...
    async readEndpointAdmissionEntry(
        ...
        const value = readLiveRtcRttEndpointAdmissionEntry(entry, this.nowEpochMs(), endpointId);
        return value ? { value, expiredEntry: undefined } : { value: undefined, expiredEntry: entry };
    }

    async listEndpointAdmissionEntries(): Promise<readonly RuntimeStateEntryValue<RtcRttEndpointAdmission>[]> {
        const entries = await this.runtimeRepository.findAllEntries(
            RTC_RTT_ENDPOINT_ADMISSION_NAMESPACE
        );
        return compact(
            entries.map((entry) => readLiveRtcRttEndpointAdmissionEntry(entry, this.nowEpochMs()))
        );
    }
    ...
    async listEndpointAdmissionEntriesPage(
        ...
        const entries = await this.listEntriesPage(RTC_RTT_ENDPOINT_ADMISSION_NAMESPACE, '', options);
        return compact(
            entries.map((entry) => readLiveRtcRttEndpointAdmissionEntry(entry, this.nowEpochMs()))
        );
    }
    ...
    async commitMeasurement(
        ...
        const result = expectedRevision === null
            ? await this.putValueIfAbsent(
                RTC_RTT_LATEST_NAMESPACE,
                this.measurementKey(measurement.sessionIdFrom, measurement.sessionIdTo),
                measurement,
                purgeAfterEpochMs
            )
    ...
    async commitEndpointAdmission(
        ...
        validateRtcRttEndpointAdmission(admission, admission.endpointId, expireAtTimestamp);
        validateRtcRttEndpointAdmissionCandidateVersion(admission.version, expectedRevision);
        const result = expectedRevision === null
            ? await this.putValueIfAbsent(
                RTC_RTT_ENDPOINT_ADMISSION_NAMESPACE,
    ...
    async findMutationReceipt(receiptId: string): Promise<RtcRttMutationReceipt | undefined> {
    ...
    async probeMutationReceiptEntry(
        ...
        const entry = await this.runtimeRepository.findEntry(RTC_RTT_RECEIPTS_NAMESPACE, receiptId);
        if (!entry) {
            return undefined;
        }
        try {
    ...
    async findMutationReceiptEntry(
        ...
        return receipt && receipt.entry.expireAtTimestamp > this.nowEpochMs() ? receipt : undefined;
    }

    async listMutationReceiptEntries(): Promise<readonly RuntimeStateEntryValue<RtcRttMutationReceipt>[]> {
        const entries = await this.runtimeRepository.findAllEntries(RTC_RTT_RECEIPTS_NAMESPACE);
        return compact(entries.map((entry) => readLiveRtcRttReceiptEntry(entry, this.nowEpochMs())));
    }

    async listMutationReceiptEntriesPage(
        options: RuntimeStateEntryPageOptions
    ): Promise<readonly RuntimeStateEntryValue<RtcRttMutationReceipt>[]> {
        const entries = await this.listEntriesPage(RTC_RTT_RECEIPTS_NAMESPACE, '', options);
        return compact(entries.map((entry) => readLiveRtcRttReceiptEntry(entry, this.nowEpochMs())));
    }
    ...
    async putMeasurementIfNewer(
        ...
        // Explicit single-attempt optimistic semantics:
    ...
    async removeMeasurement(sessionIdA: string, sessionIdB: string): Promise<void> {
    ...
    readMutationFacts(): RtcRttMutationLifecycleFacts {
        ...
        return {
            requestedAtEpochMs,
            purgeAfterEpochMs: requestedAtEpochMs + (this.options.ttlMs ?? DEFAULT_RTC_RTT_TTL_MS)
        };
    }
