...
export function computeCrdtRejectedOutcome(
  ...
  const appendResult =
    command.operation === 'append' ? toCrdtAppendRejection(command, read.document, code) : null;
  const result = toCrdtMutationResult({
    command,
    status: 'rejected',
    document: read.document,
    ...
    code,
    details:
      command.operation === 'append' ? { appendResult } : toRejectedAdminResultDetails(command),
  });
  const outboxEntries =
