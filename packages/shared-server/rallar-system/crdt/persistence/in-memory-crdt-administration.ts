...
export class InMemoryCrdtAdministration<TPayload extends RallarCrdtOperationBatch, TValue> {
    ...
    private readonly recordAudit: InMemoryCrdtAdministration.Dependencies<TPayload, TValue>['recordAudit'];
    ...
    async listAfter(input: RallarCrdtListUpdatesInput): Promise<RallarCrdtUpdatePage<TPayload>> {
        ...
        const allRecords = state?.records.filter((record) => record.append.appendSequence > afterSequence) ?? [];
    ...
    async updateDocumentLifecycle(
        ...
        const metadata: RallarCrdtDocumentMetadata = {
            ...
            archivedAtEpochMs: input.lifecycle === 'archived' ? changedAtEpochMs : state.metadata.archivedAtEpochMs,
            destroyedAtEpochMs: input.lifecycle === 'destroyed' ? changedAtEpochMs : state.metadata.destroyedAtEpochMs,
