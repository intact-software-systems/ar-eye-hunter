...
type RallarCrdtAcceptedEnvelopeByKind<K extends RallarCrdtServerEnvelopeKind> = K extends RallarCrdtServerEnvelopeKind ?
    RallarCrdtAcceptedEnvelope<K> :
    never;
...
export function installRallarCrdtWsTopics(
    ...
    const shouldInstallHandlers = options.onAcceptedEnvelope || options.logRepository || options.mutationIngress;
...
function createRallarCrdtTopicDefinition(
    ...
    return {
        ...
        maxPayloadBytes,
        fanout: input.kind === 'update' || (input.kind === 'catch-up-request' && input.options.logRepository)
            ? 'none'
        ...
        authorize: input.options.authorizeDocument || input.options.policies?.length
...
async function respondToDurableCatchUpRequest(
    ...
    const snapshot = input.accepted.envelope.includeSnapshot === false
        ? undefined
