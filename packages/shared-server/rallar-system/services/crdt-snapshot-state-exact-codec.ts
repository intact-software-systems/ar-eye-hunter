    ...
    decodeDynamicSection(value, 'sequences', (entry) => {
        ...
        decodeArray(entry.entries, (item) => {
            requireExactKeys(item, [
                'elementId', 'positionId', 'value', 'insertUpdateId', 'positionUpdateId',
                'replicaId', 'lamport', 'createdAtEpochMs',
            ], 'CRDT sequence entry');
            for (const field of [
                'elementId', 'positionId', 'insertUpdateId', 'positionUpdateId', 'replicaId',
