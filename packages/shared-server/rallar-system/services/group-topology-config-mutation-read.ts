...
export async function readTopologyConfigMutation(
  ...
  const exact = await repository.readMutationExactEntries(
    command.aggregateRef,
    command.requestId,
  );
  if (exact.status === 'fallback') {
    return await readTopologyConfigMutationSequentially(
      repository,
  ...
  const groupObservation = await groupStateRepository
    .readSnapshotWithAuthorityGuard(command.aggregateRef);
...
async function readTopologyConfigMutationSequentially(
  ...
  const [
    ...
    command.requestId === null ? Promise.resolve(undefined) : repository.findMutationRecordEntry(
      command.aggregateRef,
      command.requestId,
    ),
...
export function sameTopologyInvariantGeneration(
  left: Awaited<
    ReturnType<GroupTopologyConfigRepository['findInvariantGenerationEntry']>
  >,
  right: Awaited<
    ReturnType<GroupTopologyConfigRepository['findInvariantGenerationEntry']>
  >,
