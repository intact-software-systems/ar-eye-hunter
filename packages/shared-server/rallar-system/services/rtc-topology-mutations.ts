...
export type RtcTopologyMutationComputed =
    ...
    | Readonly<{
          outcome: 'retry';
          reason:
              'publication-ahead-of-snapshot' | 'incomparable-causal-revision';
      }>
...
export function computeTopologyMutation(
    ...
    if (input.read.publicationClaim) {
        ...
        const relation = compareTopologyTuple(
            publicationSnapshot,
            storedSnapshot,
        );
...
function assertPublicationSelfConsistent(
    ...
    try {
        ...
        throw new TypeError(
            'RTC topology publication payload snapshot is invalid',
        );
    }
...
export function validateTopologyMutation(
    ...
    if (!rtcTopologySemanticEqual(recomputed, input.computed)) {
        throw new TypeError(
            'RTC topology mutation differs from canonical computation',
        );
    }
    ...
    if (
        ...
        throw new TypeError(
            'RTC topology publication differs from candidate identity',
        );
    }
    if (
        input.publication &&
        ...
        const publicationSnapshot = assertPublicationSelfConsistent(
            input.publication,
        );
        if (!rtcTopologySemanticEqual(publicationSnapshot, input.candidate)) {
            throw new TypeError(
                'RTC topology publication payload differs from candidate',
            );
        }
    }
}
