...
import { toCanonicalRtcTopologyPairIdentity } from '../rtc-topology-identifiers.ts';
...
function createRtcTopologyWorkRuntime(
  ...
  const enqueueGroupSnapshot = async (
    ...
    const envelope: RtcTopologyWorkEnvelope<RtcTopologyGroupRevisionWork> = {
      ...
      topicId: options.topicId,
      resourceId:
        deliveryId ?? toRtcTopologyGroupRevisionResourceId(overlayId, sourceGroupStateRevision),
      contextId: toRtcTopologyQueueContextId(group.group),
    ...
    return {
      effectiveSnapshotRevision: winnerEnvelope.data.sourceGroupStateRevision,
    };
  };
...
function mergeRtcTopologyRttWork(
  ...
  const selectedGroup =
    existing.requestedGroupStateRevision > incoming.requestedGroupStateRevision
      ? existing
      : incoming;
  const selectedRtt =
    existing.requestedRttVersion > incoming.requestedRttVersion ? existing : incoming;
  return {
    ...incoming,
...
function toRtcTopologyGroupRevisionResourceId(overlayId: string, stateRevision: number): string {
