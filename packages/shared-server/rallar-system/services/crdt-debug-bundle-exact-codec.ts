...
export function decodeExactDebugBundle(value: unknown): RallarCrdtDebugBundle {
    ...
    requireExactOptionalKeys(bundle, [
        'format',
        'exportedAtEpochMs',
        'reason',
        'document',
        ...
        'redaction',
        'integrity',
    ], ['metadata', 'snapshot', 'health'], 'CRDT debug bundle');
...
function decodeExactBundleIntegrity(
    ...
    requireExactOptionalKeys(integrity, [
        'bundleHash',
        'documentRefHash',
        'updateHashes',
        'updateCount',
        'sequenceGaps',
    ], ['snapshotHash', 'firstAppendSequence', 'lastAppendSequence'], 'CRDT debug bundle integrity');
...
function decodeExactHealth(value: unknown): void {
    ...
    const optional = [
        'lastServerAppendSequence', 'lastServerAckAtEpochMs', 'lastSyncError', 'snapshotAgeMs',
        'updateLogLag', 'quota', 'replayDurationMs', 'corruptLocalArtifactCount',
        'transportStrategy', 'lastLiveTransport', 'lastLiveSendStatus', 'liveSentUpdateCount',
        'liveReceivedUpdateCount', 'liveDuplicateUpdateCount', 'liveRejectedUpdateCount',
        'liveDependencyBlockedUpdateCount', 'liveRetriedUpdateCount', 'liveSyncRequestCount',
    ...
    if ('quota' in health) {
        ...
        requireExactOptionalKeys(quota, [], ['usageBytes', 'quotaBytes', 'nearingLimit'], 'health quota');
    }
}

function decodeSequenceList(value: unknown, label: string): void {
    if (
        !Array.isArray(value) || value.some((item) => !Number.isSafeInteger(item) || item < 1) ||
