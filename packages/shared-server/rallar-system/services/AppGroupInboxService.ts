...
import type { GroupPresenceSessionCleanupAppInboxPayload } from '../group-state/presence/group-presence-contracts.ts';
import { GroupPresenceService, processGroupSessionCleanup } from '../group-state/presence/group-presence-service.ts';
...
export type { GroupPresenceSessionCleanupAppInboxPayload } from '../group-state/presence/group-presence-contracts.ts';
...
export class AppGroupInboxService extends AppInboxService {
    ...
    public async enqueueGroupSessionCleanup(input: GroupPresenceSessionCleanupAppInboxPayload): Promise<number> {
    ...
    public override processEntryNoWaiting<V, R = V>(enqueue: AppInboxEnqueueInput<V>): void {
        void enqueue;
        throw new GroupMutationAuthorizationError('Authenticated group mutation authority is required.');
    }
    ...
    private async prepareAuthenticatedGroupMutation<V>(
        ...
        if (!isAuthenticatedGroupMutationInboxType(enqueue.type)) {
            throw new GroupMutationAuthorizationError('App inbox type is not an authenticated group mutation.');
        }
