...
import type { GroupStateInboxDurableResult } from '../group-state/inbox/group-state-inbox-result.ts';

import { decodeGroupStateInboxDurableResult } from '../group-state/inbox/group-state-inbox-result-codec.ts';
...
class AppGroupInboxService extends AppInboxService {
    ...
    constructor(
        ...
        this.groupStateInboxHandler = new GroupStateInboxHandler({
            ...
            persistPreparation: (context, preparation) => this.persistReservedEntryAuthority(context, preparation)
        });
    ...
    private registerTopologyStateMessageHandlers(owners: TopologyAppInboxMutationOwners): void {
        for (const type of TOPOLOGY_CONFIG_INBOX_TYPES) {
            this.onStateMessage(
                type,
                async (_payload, context) => await this.topologyAppInboxHandler.processMutation(context, owners)
            );
        }
    }
    ...
    private registerRtcRttStateMessageHandler(dependencies: RtcRttAppInboxDependencies): void {
        this.onStateMessage(
            ...
            async (_payload, context) => await this.rtcRttAppInboxHandler.processMutation(context, dependencies)
        );
    }
}
