...
export function decodeExactCausalFrontierShape(value: unknown): void {
    ...
    if ('replicaClocks' in frontier) decodeDynamicNumberRecord(frontier.replicaClocks, 'replica clocks');
}
...
export function decodeExactEncryptedEnvelopeShape(value: unknown): void {
    ...
    requireExactOptionalKeys(envelope, [
        'kind',
        'format',
        'algorithm',
        'keyId',
        ...
        'plaintextType',
        'encryptedAtEpochMs',
    ], ['visibleMetadataFields'], 'CRDT encrypted envelope');
}
