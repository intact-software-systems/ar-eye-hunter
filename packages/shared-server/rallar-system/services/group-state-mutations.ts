...
import { assertExactKeys, assertRequiredKeys, requireJsonSafe, requirePositiveSafeInteger } from '../group-state/group-state-validation-primitives.ts';
export { normalizePersistedGroupEvent, validatePersistedGroupEvent } from '../persisted-group-event.ts';
export {
...
export { computeGroupMutation, probeGroupMutationIdempotency } from '../group-state/mutation/compute-group-mutation.ts';
export { validateGroupMutation } from '../group-state/mutation/validate-group-mutation.ts';
export { validateGroupMutationRequest, validateGroupPresenceMutationRequest } from '../group-state/mutation/group-mutation-request-validation.ts';
export {
...
export { validatePersistedGroup, validatePersistedGroupMember } from '../group-state/persistence/validate-persisted-group.ts';
export {
...
export function computeGroupPresenceSummary(
  ...
  if (
    ...
    (current.causalRevision.groupRevision > groupRevision ||
      (current.causalRevision.groupRevision === groupRevision && jsonEquals(summaryContent(current), content)))
  ) {
...
export function validateGroupPresenceSummary(
  ...
  assertExactKeys(read as unknown as Record<string, unknown>, ['group', 'members', 'admissions', 'presenceSessions', 'current'], 'Group presence summary read');
  assertRequiredKeys(
  ...
  if (read.current) {
    validateRuntimeEntryValue(read.current, 'Stored current presence summary', groupStatePresenceSummaryStorageKey(ref));
    validatePresenceSummaryValue(read.current.value, ref);
  }
  const summary = computed.summary;
  validatePresenceSummaryValue(summary, ref);
  requirePositiveSafeInteger(computed.evaluatedAtEpochMs, 'Group presence summary evaluatedAtEpochMs');
  ...
  const expectedNoOp =
    current !== undefined &&
    (current.causalRevision.groupRevision > groupRevision ||
      (current.causalRevision.groupRevision === groupRevision && jsonEquals(summaryContent(current), expectedContent)));
  const shape = computed as unknown as Record<string, unknown>;
  if (computed.outcome === 'no-op') {
    assertExactKeys(shape, ['outcome', 'evaluatedAtEpochMs', 'summary'], 'Group presence summary computed result');
    ...
    assertExactKeys(shape, ['outcome', 'evaluatedAtEpochMs', 'operation', 'expectedRevision', 'summary'], 'Group presence summary computed result');
    ...
    if (
      ...
      throw new TypeError('Group presence summary write differs from canonical predecessor projection');
    }
  }
  if (read.current) {
    const comparison = compareGroupCausalRevision(summary.causalRevision, read.current.value.causalRevision);
    ...
    if (comparison === 'equal' && !jsonEquals(summaryContent(summary), summaryContent(read.current.value))) {
      throw new TypeError('Equal group presence summary tuple has different content');
    }
  }
}

function deriveGroupPresenceSummaryContent(read: GroupPresenceSummaryRead, nowEpochMs: number): ReturnType<typeof summaryContent> {
  const groupActive = read.group.value.status === 'active' && (read.group.value.expiresAtEpochMs === null || read.group.value.expiresAtEpochMs > nowEpochMs);
  ...
  const admitted = new Set(
    read.admissions.flatMap(({ value: admission }) => admission.admittedSessions.map((session) => admissionIdentity(admission.principalId, session))),
  );
  const activeSessions = (
    groupActive
      ...
      : []
  ).toSorted((left, right) => left.sessionId.localeCompare(right.sessionId) || left.generationVersion - right.generationVersion);
  const activePrincipalIds = [...new Set(activeSessions.map((session) => session.principalId))].toSorted();
  return {
    activePrincipalIds,
...
function validateGroupPresenceSummaryReadCollections(ref: GroupRef, read: GroupPresenceSummaryRead): void {
  ...
  const activeMembers = read.members.map(({ value }) => value).filter((member) => member.status === 'active');
