...
} from '../admin-operations/AdminOperationsService.ts';
import {
...
export class AppAdminInboxService extends AppInboxService {
    ...
    async pruneExpired(
        ...
        const enqueued = await this.processEntryUntilCompletionResult<AdminPruneCommand, AdminPruneEnqueueResult>({
    ...
    private compute(command: AdminPruneCommand, read: AdminPruneRead): AdminPruneComputed {
        ...
        const outboxEntries = command.dryRun
            ? []
            : command.categories.map((category) => toAdminPruneOutbox({
                kind: 'page',
                jobId: command.jobId,
                category,
...
function readCategories(value: unknown): readonly AdminPruneExpiredCategory[] {
    if (value === undefined) return ADMIN_PRUNE_EXPIRED_CATEGORIES;
