...
export class GroupStateRepository extends RuntimeStateJsonStore {
    ...
    async listGroups(scope: GroupScope): Promise<readonly Group[]> {
        return await this.listValues<Group>(GROUPS_NAMESPACE, this.scopePrefix(scope));
    }
    ...
    async findMember(
        ...
        return await this.getValue<GroupMember>(MEMBERS_NAMESPACE, this.memberKey(ref));
    }

    async listMembers(ref: GroupRef): Promise<readonly GroupMember[]> {
        return await this.listValues<GroupMember>(MEMBERS_NAMESPACE, this.memberPrefix(ref));
    }
    ...
    async findPresenceSession(
        ...
        return await this.getValue<GroupPresenceSession>(SESSIONS_NAMESPACE, this.sessionKey(ref));
    }
    ...
    async listEvents(ref: GroupRef): Promise<readonly GroupEvent[]> {
        const events = await this.listValues<GroupEvent>(EVENTS_NAMESPACE, this.eventPrefix(ref));
    ...
    async readSnapshot(ref: GroupRef): Promise<GroupSnapshot | undefined> {
        ...
        const activeSessions = this.toActiveSessions(await this.listPresenceSessions(ref));
    ...
    private toActiveSessions(
        ...
        return sessions.filter((session) => session.disconnectedAtEpochMs === undefined);
    }
    ...
    private memberKey(ref: GroupRef & Readonly<{ principalId: string }>): string {
    ...
    private sessionKey(ref: GroupRef & Readonly<{ sessionId: string }>): string {
