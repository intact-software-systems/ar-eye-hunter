import type {
    Group, GroupMember, GroupPresenceAdmission, GroupPresenceSession,
    GroupPresenceSummary, GroupRef,
...
import { admissionIdentity } from '../mutation/compute-group-presence-mutation.ts';
import {
    validateStoredGroup,
...
function validateGroupPresenceSummaryOutcome(
    ...
    const { ref, read, computed, expectedContent, groupRevision, current, expectedNoOp } =
        validation;
    ...
    if (expectedNoOp || computed.operation !== (read.current ? 'update' : 'insert') ||
...
function validateGroupPresenceSummaryCausalRevision(
    ...
    if (!validation.read.current) return;
    ...
    if (comparison === 'equal' && !jsonEquals(
