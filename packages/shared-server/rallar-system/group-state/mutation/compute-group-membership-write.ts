...
import { computeMemberPresenceAdmission } from './compute-group-presence-admission.ts';
...
function resolveOwnerPrincipalId(
  ...
  const ownerPrincipalId =
    eventType === 'ownership-transferred' ? promotedOwner?.principalId : group.ownerPrincipalId;
  if (!ownerPrincipalId) {
