...
/**
 * `connect` names the epoch it observed and the exact planned layout it
 * means to dial (product decision 32): a caller whose epoch the group has
 * advanced past, that names no current plan, or that names one the store
 * has since replaced, receives its own conflict code and re-reads the
 * current snapshot before retrying. The denial is a typed rejection value
 * through compute and becomes this error only at the handler boundary, like
 * every other rejection code.
 *
