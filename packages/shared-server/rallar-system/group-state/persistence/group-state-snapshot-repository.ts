...
export class GroupStateSnapshotRepository extends RuntimeStateJsonStore {
    ...
    async listSnapshots(scope: GroupScope): Promise<readonly GroupSnapshot[]> {
        ...
        const beforeByKey = new Map(
            read.groupsBefore.map((stored) => [stored.entry.key, stored]),
        );
        const snapshots = await Promise.all(read.groupsAfter.map(async (stored) => {
            ...
            return this.toSnapshot({
                ...
                authoritativeSessions:
                    read.sessionsByGroupId.get(stored.value.groupId) ?? [],
    ...
    private async readScopeSnapshot(scope: GroupScope): Promise<GroupStateScopeSnapshotRead> {
        ...
        const [memberEntriesRaw, summaryEntriesRaw, sessionEntriesRaw] =
            await Promise.all([
    ...
    async listSnapshotsPage(
        ...
        const candidates = await Promise.all(pageGroups.map(async ({ entry, group }) => {
