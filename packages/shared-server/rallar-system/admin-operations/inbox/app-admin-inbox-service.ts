...
type AdminPruneMutationPhase = 'read' | 'compute' | 'validate';
...
export class AppAdminInboxService {
    ...
    async pruneExpired(
        ...
        const reservation = await this.reserveCommand(identity);
        const command = decodeAdminPruneCommand(reservation.enqueue.data);
        await assertAdminPruneStoredIdentity(toAdminPruneQueueKey(identity), reservation.enqueue, command);
    ...
    private async reserveCommand(
        ...
        return await this.reservationClient.reserveMaterializedEntry(
            {
                type: AppInboxType.ADMIN_PRUNE_EXPIRED,
                topicId: key.topicId,
    ...
    private async timeAdminPrunePhase<T>(
        operation: AdminPruneMutationPhase,
