...
export type GroupGovernanceAction = 'invite' | 'remove' | 'ban' | 'unban' | 'promote' | 'demote' | 'transfer-ownership';
...
export function canSendRoomMessage(input: CanSendRoomMessageInput): GroupPolicyResult {
    ...
    const liveSession = input.snapshot.activeSessions.find(
        (session) => session.sessionId === input.senderSessionId && isLiveSession(session, input.nowEpochMs)
    );
