...
export class RtcTopologyReconnectHydrator {
  ...
  start(): void {
    ...
    if (this.#stopped) throw new Error('RTC topology reconnect hydrator is stopped');
    this.#started = true;
  ...
  async stop(): Promise<void> {
    if (!this.#stopped) {
      ...
      this.#pending.clear();
      if (this.#started) this.#socket.removeWebsocketCallbackById(CALLBACK_ID);
    }
    await this.whenIdle();
  }

  #enqueue(connection: ConnectionContext, attempt: number): void {
    if (this.#stopped || !this.#isCurrent(connection)) return;
    this.#pending.set(connection, attempt);
    if (this.#batchCancellation) return;
  ...
  #flushPending(): void {
    ...
    this.#inFlight = this.#hydrateContexts([...batch.keys()], this.#abort.signal)
      ...
      .catch((error) => {
        ...
        if (!isAbortFromSignal(failure, this.#abort.signal)) throw error;
      })
      .finally(() => {
    ...
    while (true) {
      ...
      for (const entry of page) {
        ...
        for (const connection of candidates) {
          ...
          if (outcome === 'retry') retry.add(connection);
        }
      }
      if (page.length < RTC_TOPOLOGY_HYDRATION_PAGE_SIZE) break;
  ...
  async #hydrateTopology(
    ...
    throwIfEitherAborted(signal, this.#abort.signal);
    if (!this.#isCurrent(connection)) return 'stale-generation';
    ...
    try {
      ...
      throwIfEitherAborted(signal, this.#abort.signal);
      if (!this.#isCurrent(connection)) return 'stale-generation';
      if (!sameCausalRevision(authorizationBefore, authorizationAfter)) return 'retry';
      if (
        !isAuthorized({
      ...
      if (!currentTopology?.activeSessionIds.includes(connection.id)) return 'no-topology';
      ...
      throwIfEitherAborted(signal, this.#abort.signal);
      if (this.#socket.trySendEncodedToContext(connection, encoded)) return 'sent';
...
export function materializeRtcTopologyHydrationMessage(
  ...
  const message: ALMessage = {
    ...
    route: {
      ...
      resourceId:
        `${topology.overlayId}:${revision.groupRevision}:` +
...
function isAuthorized(input: Readonly<IsAuthorizedInput>): boolean {
  ...
  if (!snapshot || snapshot.group.status !== 'active') return false;
