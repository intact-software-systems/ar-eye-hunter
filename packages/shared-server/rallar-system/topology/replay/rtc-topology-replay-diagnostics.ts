...
export type RtcTopologyReplayHydrationOutcome =
  typeof RTC_TOPOLOGY_REPLAY_HYDRATION_OUTCOMES[number];
  ...
  const record: RtcTopologyReplayDiagnosticsSink = (event) => {
    switch (event.kind) {
      ...
      case 'cursor':
        ...
        if (event.outcome === 'conflict') metrics.cursorConflictCount += 1;
        if (event.outcome === 'gap') metrics.gapCount += 1;
        return;
      case 'hydration':
...
function recordEntry(metrics: MutableMetrics, outcome: RtcTopologyReplayEntryOutcome): void {
  ...
  if (outcome === 'current-repair') metrics.directCurrentRepairCount += 1;
  if (outcome === 'no-local-recipient') metrics.noLocalRecipientCount += 1;
  if (outcome === 'send-failed') metrics.sendFailureCount += 1;
  if (outcome === 'corrupt') metrics.corruptReferenceCount += 1;
}
