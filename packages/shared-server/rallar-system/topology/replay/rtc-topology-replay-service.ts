...
export class RtcTopologyReplayService {
  ...
  start(): Promise<void> {
    ...
    if (!this.#startPromise) this.#diagnostics?.({ kind: 'wake', source: 'startup' });
  ...
  async #initialize(): Promise<void> {
    ...
    if (outcome === 'more') this.#wakePending = true;
    if (this.#wakePending) this.#ensureRun();
  }

  #ensureRun(): void {
    if (this.#runPromise || this.#stopped || !this.#initialized) return;
    this.#runPromise = this.#runRequestedTurns().finally(() => {
      this.#runPromise = undefined;
      if (this.#wakePending && !this.#stopped) this.#ensureRun();
    });
  }
  ...
  async #drainTurn(observation: MutableDrainObservation): Promise<DrainTurnOutcome> {
    ...
    if (snapshots.length === 0) return 'caught-up';
    ...
    while (candidates.length > 0 && !this.#stopped) {
      ...
      for (let index = 0; index < candidates.length; index += 1) {
        ...
        if (result.status === 'caught-up') continue;
        pageCount += 1;
        if (result.status === 'page') entryCount += result.entries.length;
        ...
        if (outcome === 'more') nextRound.push(candidates[index]!);
    ...
    if (candidates.length > 0) return 'more';
  ...
  async #handlePublisherPage(
    ...
    for (const entry of result.entries) {
      ...
      try {
        ...
        if (error instanceof RtcTopologyDeliveryLeaseLostError) throw error;
  ...
  async #hydrateAndAdvanceGap(
    ...
    if (capturedHeadSequence === expectedSequence) return 'done';
  ...
  async #advanceSuccessfulPredecessor(
    ...
    if (lastHandledSequence === expectedSequence) return true;
    const advanced = await this.#repository.compareAndSetCursor({
      consumerStreamId: this.#consumerStreamId,
      publisherStreamId,
      expectedSequence,
