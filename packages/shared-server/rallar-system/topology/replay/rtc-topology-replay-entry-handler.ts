...
export class RtcTopologyReplayEntryHandlerService implements RtcTopologyReplayEntryHandler {
  ...
  async handle(
    ...
    if (decision.status === 'gap') return decision;
    ...
    if (result.status === 'no-recipients') return { status: 'no-local-recipient' };
    if (result.status !== 'sent-live') return { status: 'send-failed' };
...
export function materializeRtcTopologyCurrentRepairMessage(
  ...
  const message: ALMessage = {
    ...
    route: {
      ...
      resourceId:
        `${currentSnapshot.overlayId}:${revision.groupRevision}:` +
