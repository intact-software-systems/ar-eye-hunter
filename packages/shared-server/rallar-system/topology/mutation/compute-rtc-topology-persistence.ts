...
} from '../publication/rtc-topology-publication-repository-contracts.ts';
...
export interface RtcTopologyPersistenceComputed {
    readonly snapshot: {
        ...
        readonly unexpectedRevisionError: RtcTopologyRepositoryInvariantCorruptionError;
    };
    readonly publication: {
        ...
        readonly expireAtIsoTimestamp: string;
        readonly collisionError: RtcTopologyPublicationCollisionError;
    } | null;
}

export function computeRtcTopologyPersistence(input: RtcTopologyPersistenceInput): RtcTopologyPersistenceComputed {
    const acceptedStorageRevision = input.expectedRevision === null ? 0 : input.expectedRevision + 1;
    ...
    return {
        snapshot: {
            ...
            unexpectedRevisionError: new RtcTopologyRepositoryInvariantCorruptionError(
                snapshot.key,
...
function computePublicationPersistence(
    ...
    if (
        ...
        input.commandHash === null || input.attemptCount === null
    ) {
        throw new TypeError('RTC topology publication persistence facts are incomplete');
    }
    const deliveryExpireAtTimestamp = publication.message.constraints?.expiresAtMs;
    if (
        !Number.isSafeInteger(input.publicationExpireAtTimestamp) ||
        typeof deliveryExpireAtTimestamp !== 'number' ||
        !Number.isSafeInteger(deliveryExpireAtTimestamp) ||
        input.publicationExpireAtTimestamp < deliveryExpireAtTimestamp
    ) {
        throw new TypeError('RTC topology publication retention must cover its delivery expiry');
    }
    ...
    return {
        ...
        collisionError: new RtcTopologyPublicationCollisionError(key)
    };
}
