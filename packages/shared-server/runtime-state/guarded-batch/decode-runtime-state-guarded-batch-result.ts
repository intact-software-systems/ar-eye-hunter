...
interface DecodeEffectResultInput {
...
export function decodeRuntimeStateGuardedBatchResult(
    ...
    const guardResult = decodeGuardResult(batch.guard, result.guard);
...
function decodeGuardResult(
    ...
    return decodeAppliedGuardResult(guard, result);
}

function decodeAppliedGuardResult(
    guard: RuntimeStateGuardedBatchGuard,
...
function decodeEffectResult(
    input: DecodeEffectResultInput
): RuntimeStateGuardedBatchEffectResult {
    ...
    if (guardStatus === 'conflict') {
        return decodeSkippedEffect(effect, result, label);
    }
    if (result.status === 'conflict') {
        return decodeConflictedEffect(effect, result, label);
    }
    if (result.status !== 'applied') {
        throw invalidResult(label + ' status is invalid after an applied guard');
    }
    return decodeAppliedEffect(effect, result, label);
}

function decodeSkippedEffect(
    effect: RuntimeStateGuardedBatchEffect,
    result: JsonWireObject,
    label: string
): RuntimeStateGuardedBatchEffectResult {
    requireExactResultKeys(result, ['status', 'effectId', 'operation', 'namespace', 'key', 'reason'], label);
...
function decodeConflictedEffect(
    effect: RuntimeStateGuardedBatchEffect,
    result: JsonWireObject,
    label: string
): RuntimeStateGuardedBatchEffectResult {
