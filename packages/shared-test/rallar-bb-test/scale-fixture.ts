...
export function createRecipeConsoleScaleFixture(
    ...
    const results = Array.from({ length: counts.results }, (_, index) => JSON.stringify(
        resultRow(index, resultPositions, resultNeedles, actionableFailure),
    )).join('\n');
    const events = Array.from({ length: counts.events }, (_, index) => JSON.stringify(
...
function needleAt(
    ...
    for (const position of ['first', 'middle', 'last'] as const) {
        if (sourcePositions[position] === index) return needles[position];
    }
    return undefined;
}
