...
export const RALLAR_COMPANION_COVERAGE_SURFACES: readonly RallarCompanionCoverageSurface[] = [
  {
    ...
    runnerBoundary:
      'Use HTTP steps for observable auth endpoints; do not add auth.* recipe commands.',
  },
  {
    surfaceId: 'browser-room-and-people-facades',
    layer: 'shared-web-facade',
    intent:
      'Direct facade room, people, state refresh, event history, replay, and subscription behavior.',
    testFiles: [
    ...
    runnerBoundary:
      'Use REST/WS observations for room membership; do not add rooms.* or people.* recipe commands.',
  },
  {
  ...
  },
  {
    ...
    layer: 'shared-web-facade',
    intent:
      'Rallar Data repository behavior, persistence, synchronization, and browser storage semantics.',
    testFiles: ['packages/tests/shared-web/rallar-data.test.ts'],
    runnerBoundary:
      'Keep data API parity in package or app tests unless it is visible as HTTP/WS/RTC traffic.',
  },
  {
    ...
    runnerBoundary:
      'Black-box recipes call public REST/WS endpoints; they do not expose server facade methods.',
  },
  {
    surfaceId: 'remote-browser-command-bridge',
    layer: 'rallar-bb-test',
    intent:
      'Visible or remote browser control through portable commands and event/result normalization.',
    testFiles: [
  ...
  },
  {
    ...
    layer: 'app-specific',
    intent:
      'Product workflows that need browser APIs, UI state, media devices, or app-specific orchestration.',
    testFiles: [
