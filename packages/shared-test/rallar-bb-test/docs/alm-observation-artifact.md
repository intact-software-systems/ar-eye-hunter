...
- `inbound` — one entry per direction (`sender`, `receiver`, `unattributed`), from the
  ...
  and its receiver's duplicate-outcome wait reads red. In `not-yet-in-sync`, the receiver's refusal
  reads `rejected` over `rtc` with a reason starting `not-yet-in-sync`; `delivered-after-refresh` reads red at
  `received-1`, because no plain-member write advances the snapshot version. A measured direction carries:
