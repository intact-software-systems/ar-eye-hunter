            ...
            return toInteractions(
                {
                    ...
                    requestTemplate: {
                        ...
                        headers: toHeaders(
                            ...
                            asRecord(input.headerTemplateFile)?.[toTechnology(interaction.technology)] ||
                                input.headerTemplateFile || input.headers
                        )
                    },
                    responseTemplate: {
...
function createScenariosFromInput(
    input: ScenarioInput,
    numOfScenarios = 1
): Array<Array<Record<string, JsonRecord[]>>> {
...
function toInteractionsWithConfig(
    input: ScenarioInput,
    scenarios: Array<Array<Record<string, JsonRecord[]>>>
): JsonRecord[] {
    return scenarios
        .flatMap((scenario) => {
            return scenario
