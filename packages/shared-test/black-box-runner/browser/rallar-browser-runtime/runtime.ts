...
import { createBlackBoxRallarDirectorController, type BlackBoxRallarDirectorLease } from './director-controller.ts';
import { createBlackBoxRallarMessagingController, type BlackBoxRallarMessagingLease } from './messaging-controller.ts';
import { createBlackBoxRallarConsoleDiagnostics, createBlackBoxRallarDiagnostics } from './diagnostics.ts';
...
function createBlackBoxRallarRuntimeInstallation(
    ...
    const wait = options.delay ?? ((ms: number) => new Promise<void>(resolve => setTimeout(resolve, Math.max(0, ms))));
    ...
    const crdtController = createBlackBoxRallarCrdtController<RallarCrdtDocument<unknown, RallarCrdtOperationBatch>>({
        generation: lifecycle.generation,
        isCurrent: lifecycle.isCurrent,
    });
    const directorController = createBlackBoxRallarDirectorController<DirectorRelayState>({
        generation: lifecycle.generation,
        isCurrent: lifecycle.isCurrent,
    });
    ...
    const eventDiagnostics = createBlackBoxRallarDiagnostics<BlackBoxRallarEvent>({
        now,
        emit: event => {
            const handler = targetWindow.__blackBoxRallarEmit;
            if (!handler) {
                return;
            }
            try {
    ...
    async function ensureCrdtLiveConnection(
        ...
        if (lifecycle.authenticationConfig()) {
            ...
            return await ensureCrdtLiveConnection(input);
        }
        if (state) {
        ...
        return await lifecycle.runExclusive('crdt-live:' + connectionKey(config), async context => {
    ...
    function directorRelaySummary(): BlackBoxRallarDirectorRelaySummary {
        return {
            handles: directorController.handles(),
            relays: directorController.entries().map(([, entry]) => ({
                handle: entry.handle,
    ...
    function requireDirectorRelay(handle: string): DirectorRelayState {
        try {
            return directorController.require(handle);
        } catch {
            throw new Error('Director relay handle is not open: ' + handle);
        }
    }

    function assertDirectorCurrent(lease: BlackBoxRallarDirectorLease): void {
        directorController.assertCurrent(lease, 'Director operation completed after the runtime closed.');
    }
    ...
    function emit(partial: Omit<BlackBoxRallarEvent, 'atEpochMs'>): void {
        eventDiagnostics.emit(partial);
    }

    function emitDiagnostic(config: BlackBoxRallarConnectionConfig, topic: string, data?: unknown): void {
        emit({
            kind: 'diagnostic',
            topic,
            connection: config.connection,
    ...
    function emitConnectPhaseStarted(config: BlackBoxRallarConnectionConfig, phase: string, data?: unknown): void {
    ...
    function emitConnectPhaseCompleted(config: BlackBoxRallarConnectionConfig, phase: string, data?: unknown): void {
    ...
    async function sessionForAuthentication(
        ...
        const session = await lifecycle.runAuthentication(config, signal =>
            acquireAuthenticationSession(config, signal),
        );
    ...
    function connectionKey(config: BlackBoxRallarConnectionConfig): string {
        return JSON.stringify(blackBoxRallarConnectionTargetOf(config));
    }
    ...
    async function send(input: BlackBoxRallarSendInput | unknown): Promise<BlackBoxRallarSendDiagnostics> {
        const runtimeState = requireState();
        const { config } = runtimeState;
        const lease = messagingController.lease();
        messagingController.assertCurrent(lease, 'Rallar send completed after the runtime closed.');
        try {
            const transport = transportOf(config);
            if (transport === 'messages.rtc') {
                return await sendMessagesRtc(input, config, lease);
            }

            return await sendRealtime(input, config, lease);
        } catch (error) {
    ...
    async function sendMessagesRtc(
        ...
        const minSnapshotVersion = toOptionalNumber(normalized.minSnapshotVersion ?? config.rallar.minSnapshotVersion);
    ...
    async function sendWs(input: unknown): Promise<BlackBoxRallarWsSendDiagnostics> {
        const runtimeState = requireState();
        const { config } = runtimeState;
        const lease = messagingController.lease();
        messagingController.assertCurrent(lease, 'Rallar send completed after the runtime closed.');
    ...
    function ensureWsMessageSubscription(
        ...
        messagingController.ensureWsSubscription(key, () => {
    ...
    async function directorAppoint(
        input: BlackBoxRallarDirectorAppointInput | unknown,
    ): Promise<BlackBoxRallarDirectorCommandDiagnostics> {
        const runtimeState = requireState();
        const config = runtimeState.config;
        const lease = directorController.lease();
        assertDirectorCurrent(lease);
        const normalized = normalizeDirectorAppointInput(input);
        const target = toDirectorTarget(normalized, config);
        const directorStatus = await rallar.director.appoint(
            target as any,
            {
    ...
    async function directorResign(
        input: BlackBoxRallarDirectorRoomInput | unknown,
    ): Promise<BlackBoxRallarDirectorCommandDiagnostics> {
        const runtimeState = requireState();
        const config = runtimeState.config;
        const lease = directorController.lease();
        ...
        const target = toDirectorTarget(normalized, config);
        const directorStatus = await rallar.director.resign(
            target as any,
            {
                scope: toDirectorScope(normalized, config),
                timeoutMs: normalized.timeoutMs,
    ...
    async function directorStatus(
        input: BlackBoxRallarDirectorStatusInput | unknown,
    ): Promise<BlackBoxRallarDirectorCommandDiagnostics> {
        const runtimeState = requireState();
        const config = runtimeState.config;
        const lease = directorController.lease();
    ...
    async function directorRelayStart(
        ...
        if (directorController.handles().includes(normalized.handle)) {
        ...
        directorController.add(normalized.handle, entry);
    ...
    async function directorIntent(
        input: BlackBoxRallarDirectorIntentInput | unknown,
    ): Promise<BlackBoxRallarDirectorCommandDiagnostics> {
        const runtimeState = requireState();
        const config = runtimeState.config;
        const lease = directorController.lease();
    ...
    async function directorSyncRequest(
    ...
    async function directorRelayStop(
        input: BlackBoxRallarDirectorHandleInput | unknown,
    ): Promise<BlackBoxRallarDirectorCommandDiagnostics> {
        const runtimeState = requireState();
        const config = runtimeState.config;
        const lease = directorController.lease();
        ...
        relay.relay.stop();
        directorController.delete(normalized.handle);
    ...
    async function closeEffect(preparation: RuntimeClosePreparation): Promise<BlackBoxRallarCloseDiagnostics> {
        ...
        try {
            ...
            for (const [handle, relay] of directorController.entries()) {
                try {
                    relay.relay.stop();
                    directorController.delete(handle);
                    if (config) {
                        emitDirectorDiagnostic(
