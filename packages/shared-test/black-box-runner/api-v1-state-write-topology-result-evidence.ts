import type { AuthoritativeTopologyReceipt } from
...
function validateStoredTopology(
    ...
    const keys = target === 'override'
        ? [
            'groupRef', 'config', 'version', 'createdAtEpochMs', 'updatedAtEpochMs',
            'updatedByPrincipalId', 'requestId', 'expiresAtEpochMs',
        ]
        : [
            'groupRef', 'config', 'version', 'createdAtEpochMs', 'updatedAtEpochMs',
            'updatedByPrincipalId', 'requestId',
        ]
    return exactKeys(stored, keys) &&
...
function matchesAuthoritative(
    ...
    return receipt.operation === authoritative.operation &&
...
function sameGroupRef(left: unknown, right: unknown): boolean {
    ...
    return Boolean(a && b && exactKeys(a, ['applicationId', 'workspaceId', 'groupId']) &&
        exactKeys(b, ['applicationId', 'workspaceId', 'groupId']) &&
        a.applicationId === b.applicationId && a.workspaceId === b.workspaceId &&
        a.groupId === b.groupId)
}
...
function sameConfig(left: unknown, right: unknown): boolean {
    ...
    return left === null && right === null || Boolean(a && b && exactKeys(a, keys) &&
        exactKeys(b, keys) && keys.every((key) => a[key] === b[key]))
}
