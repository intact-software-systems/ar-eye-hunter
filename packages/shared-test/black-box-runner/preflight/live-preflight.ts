            ...
            await recordAsyncCheck(
                ...
                async () => {
                    ...
                    assertStatus(
                        response,
                        [200],
                        'RALLAR_API_CONFIG_UNAVAILABLE',
                        '/api/config did not return HTTP 200.'
                    );
            ...
            if (!primarySession) {
                ...
                await recordAsyncCheck(
                    ...
                    async () => {
                        ...
                        const createResponse = await fetchResponse(
                            fetchImplementation,
                            urlJoin(rallarApiBaseUrl, createMutationPath),
                            {
                                method: 'POST',
                        ...
                        const joinResponse = await fetchResponse(
                            fetchImplementation,
                            urlJoin(rallarApiBaseUrl, joinMutationPath),
                            {
                                method: 'PUT',
                                headers: authHeaders(primarySession),
                                body: JSON.stringify({
                        ...
                        assertStatus(
                            joinResponse,
                            [200, 201],
                            'GROUP_JOIN_FORBIDDEN',
                            'Group join permission check failed.'
                        );
                        return {
                            groupId: group.groupId,
                ...
                const result = await recordAsyncCheck(
                    ...
                    async () => {
                        ...
                        if (!stringValue(body.ticket) || !stringValue(body.sessionId)) {
                            throw new LivePreflightCheckError(
                                'BAD_WS_TICKET_RESPONSE',
                                'WebSocket ticket response was missing ticket fields.'
                            );
                        }
                        return {
                ...
                await recordAsyncCheck(
                    ...
                    async () => {
                        ...
                        if (!Array.isArray(body.iceServers)) {
                            throw new LivePreflightCheckError(
                                'BAD_ICE_CONFIG_RESPONSE',
                                'ICE config response did not include iceServers.'
                            );
                        }
                        return {
        ...
        await recordAsyncCheck(
            ...
            async () => {
                if (!input.checkPlaywright) {
                    throw new LivePreflightCheckError(
                        'PLAYWRIGHT_CHECK_UNAVAILABLE',
                        'No Playwright checker is available.'
                    );
                }
...
function requestedLiveChecks(
    ...
    if (
        stringValue(spec.corsOrigin) || envValue(resolved.environment, 'RALLAR_PREFLIGHT_CORS_ORIGIN') ||
        spec.cors === true
    ) {
...
function toIssue(
    check: BlackBoxRunnerLivePreflightCheck,
    code: string,
    message: string
): BlackBoxRunnerLivePreflightIssue {
    return {
        severity: 'error',
        code,
        message,
...
async function fetchResponse(
    ...
    if (!fetchImplementation) {
        throw new LivePreflightCheckError(
            'FETCH_UNAVAILABLE',
            'No fetch implementation is available for live preflight.'
        );
    }
...
async function openWebSocket(
    ...
    return await new Promise<JsonRecord>((resolve, reject) => {
        ...
        const onClose = (): void => {
            if (!settled) {
                finish(() =>
                    reject(new LivePreflightCheckError('WS_UPGRADE_CLOSED', 'WebSocket closed before opening.'))
                );
            }
        };
...
function summarizeChecks(
    checks: readonly BlackBoxRunnerLivePreflightCheck[]
): BlackBoxRunnerLivePreflightReport['summary'] {
...
function uniqueCheckKinds(
    values: readonly BlackBoxRunnerLivePreflightCheckKind[]
): readonly BlackBoxRunnerLivePreflightCheckKind[] {
