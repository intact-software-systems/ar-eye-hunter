...
function includeReference(
    value: unknown
): { key?: string; path?: string; variables: JsonRecord; namePrefix?: string; nameSuffix?: string; } {
...
function expandStaticRecipeIncludes(
    ...
    const expandSteps = (steps: readonly unknown[], parentFilePath: string, stack: string[]): JsonRecord[] => {
        return steps.flatMap((step, index) => {
            ...
            const expandedFragmentSteps = expandSteps(fragmentSteps(fragment.value), fragment.filePath, [
                ...stack,
                fragment.source
            ])
    ...
    const expandNestedStepIncludes = (step: JsonRecord, parentFilePath: string, stack: string[]): JsonRecord => {
        return {
            ...
            ...(Array.isArray(step.setupSteps)
                ? { setupSteps: expandSteps(step.setupSteps, parentFilePath, stack) }
                : {}),
            ...(Array.isArray(step.cleanupSteps)
                ? { cleanupSteps: expandSteps(step.cleanupSteps, parentFilePath, stack) }
                : {}),
...
function withParallelAnnotations(
    step: JsonRecord,
    groupName: string,
    groupIndex: number,
    stepIndex: number
        ...
        const rawSteps = Array.isArray(group.steps)
            ? group.steps
                ...
                .map((nestedStep, stepIndex) =>
                    withParallelAnnotations(nestedStep as JsonRecord, groupName, groupIndex + 1, stepIndex + 1)
                )
            : [];
...
function toInlineLoopDelayStep(
    loopName: string,
    iteration: number,
    iterations: number,
    intervalMs: number
...
function toTrafficPlanExpandedConfig(
    config: ScenarioCliConfig
): { config: ScenarioCliConfig; artifact?: TrafficPlanArtifact; } {
    if (!trafficPlanMode) {
        return {
            config
        };
...
function toSoakExpandedConfig(config: ScenarioCliConfig): ScenarioCliConfig {
    ...
    const cleanupSteps = resolveSoakStepList(
        config.steps,
    ...
    const repeatedLoopSteps = Array.from({ length: iterations }).flatMap((_ignored, iterationIndex) => {
        ...
        const annotatedLoopSteps = iterationLoopSteps.map((step, loopIndex) =>
            annotateSoakStep(cloneStep(step), 'loop', iteration, loopIndex + 1)
        );
...
function baseMetrics(report: any): JsonRecord {
    ...
    return {
        ...
        waits: resultOutcomeMetrics(
            results,
            (result) => ['wait', 'expect'].includes(String(result.action || '').toLowerCase())
        ),
        latencyMs: {
    ...
    return {
        ...
        latencyMs: {
            runDuration: latencyMetric(
                runs.map((run) => numberFromPath(run.summary?.durationMs)).filter((value): value is number =>
                    value !== undefined
                )
            ),
            stepDuration: latencyMetric(
                results.map((result) => numberFromPath(result.durationMs)).filter((value): value is number =>
                    value !== undefined
                )
            ),
            connect: latencyMetric(
                results.map((result) => numberFromPath(result.actual?.connectLatencyMs)).filter((
                    value
                ): value is number => value !== undefined)
            ),
            send: latencyMetric(
                results.map((result) => numberFromPath(result.actual?.sendLatencyMs)).filter((value): value is number =>
                    value !== undefined
                )
            ),
            firstPayload: latencyMetric(
                results.map((result) => numberFromPath(result.actual?.firstPayloadLatencyMs)).filter((
                    value
                ): value is number => value !== undefined)
            )
        },
...
function withSoakReport(report: any): any {
    ...
    const setupStepCount =
        resolveSoakStepList(originalSteps, soakConfig.setupSteps || soakConfig.setup, 'setupSteps').length;
    const loopStepCount = resolveSoakLoopSteps(originalSteps, soakConfig).length;
    const cleanupStepCount =
        resolveSoakStepList(originalSteps, soakConfig.cleanupSteps || soakConfig.cleanup, 'cleanupSteps').length;
...
function numberComparison(
    ...
    actual: unknown,
    expected: unknown
): { pass: boolean; details?: JsonRecord; } {
