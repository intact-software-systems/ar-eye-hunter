...
export function readTopologyReconfigureCommand(authority: unknown) {
    ...
    const command = readExactRecord(durable.command, [
...
export function readGroupSessionCleanupCommand(data: unknown) {
    ...
    const connection = readExactRecord(cleanup.connection, [
    ...
    const authSession = readExactRecord(connection.authSession, [
...
export function readExactRecord(
    ...
    if (JSON.stringify(Object.keys(candidate).toSorted()) !==
        JSON.stringify([...keys].toSorted())) throw new TypeError(`${label} keys are invalid`);
    return candidate;
}
