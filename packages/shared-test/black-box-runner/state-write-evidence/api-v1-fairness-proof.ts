...
export async function verifyApiV1FairnessProof(
    ...
    for (const item of fixtures) {
        ...
        const proof = toProof({ recipeId: item.recipeId, fixture: item.fixture, timing, artifactDir });
        proofs.push(proof);
...
async function readEvidenceReports(artifactDir: string): Promise<
    Array<{
        recipeId: string;
        reportPath: string;
        report: any;
...
> {
    ...
    for (const summaryPath of summaries) {
        ...
        for (const run of Array.isArray(summary?.runs) ? summary.runs : []) {
            if (run?.status !== 'PASSED' || typeof run?.artifactDir !== 'string') {
                continue;
            }
            const reportPath = path.join(run.artifactDir, 'report.json');
            const report = await readJsonIfPresent(reportPath);
            if (report) {
                reports.push({ recipeId: String(run.id), reportPath, report });
            }
        }
    }
    return reports;
...
function matchesFixture(timing: Timing, fixture: Fixture): boolean {
    ...
    return (
        timing.details.resourceId === fixture.resourceId &&
...
function toProof(input: ToProofInput): ApiV1FairnessProof {
    const { recipeId, fixture, timing, artifactDir } = input;
    return {
        schemaVersion: 1,
        recipeId,
