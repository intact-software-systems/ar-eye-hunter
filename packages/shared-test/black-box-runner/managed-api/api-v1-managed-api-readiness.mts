...
export async function waitForManagedApiReady(input: WaitForManagedApiReadyInput): Promise<void> {
    ...
    const triggerTimeout = () => triggerManagedApiTimeout({ state, input, url, readLogTail, diagnosticSecrets });
...
async function observeManagedApiChildStatus({
    ...
    try {
        ...
        if (!claimManagedApiReadiness(state, 'child')) {
            return;
        }
...
function triggerManagedApiTimeout({
    ...
    if (!claimManagedApiReadiness(state, 'timeout')) {
        return;
    }
...
function settleUnexpectedManagedApiError<Value>(
    ...
    if (!claimManagedApiReadiness(state, 'error')) {
        return;
    }
...
async function pollManagedApiReadiness(input: PollManagedApiReadinessInput): Promise<void> {
    ...
    try {
        ...
        while (!state.winner) {
            ...
            if (response) {
                state.lastError = new Error(`${input.url} returned ${response.status}`);
            }
            try {
...
function checkManagedApiDeadline(input: PollManagedApiReadinessInput): void {
    if (!input.state.winner && input.now() >= input.deadline) {
        input.triggerTimeout();
    }
}
