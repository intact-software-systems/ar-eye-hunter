...
function validateApiV1ClusterOptionValues(values: ApiV1ClusterOptionValues): readonly string[] {
    const issues: string[] = [];
    const hasIncompletePorts = (values.secondaryPort === undefined) !== (values.tertiaryPort === undefined);
    const hasManagedPorts = values.secondaryPort !== undefined && values.tertiaryPort !== undefined;
    const usesManagedProfile = !values.recipesOnly && MANAGED_API_V1_CLUSTER_PROFILES.has(values.profile);
