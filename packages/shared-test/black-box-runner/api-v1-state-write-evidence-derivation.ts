...
function createDerivedEvidenceResult(input: DerivedEvidenceResultInput): Record<string, unknown> {
  ...
  return {
    ...
    completedAppInboxStatus:
      input.appInbox.length > 0 && completedCount === input.appInbox.length ? 'COMPLETED' : 'MIXED',
