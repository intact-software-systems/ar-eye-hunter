...
| P5 | **Dials are counted, not waited on.** The dial witness is the existing `rallar.browser.rtc.lifecycle` diagnostic with payload `kind: 'peer-created'`. Holding a lobby proves zero such events over a window on an agent that has never dialed; teardown after `reset` proves the per-agent count unchanged over a window, because the buffer already holds the pre-reset dials and `wait` `absent: true` would violate on them by design. The same buffer scan makes a positive `wait` after `reset` unsound (a pre-reset sample satisfies it in the first tick), so slice 1 adds an optional since-cursor to the `wait` primitive — one field plus one comparison at the `findWaitEvent(input.currentEvents(), command.match)` call site in `packages/shared-test/rallar-bb-test/wait/wait-for-event.ts` — and every post-reset wait carries it.                                                                                                                                                                                                                                                                                                                                                                    |
| P6 | **The readiness barrier is observed by the browser itself, in one command, and it observes only.** Settled Q3: `formation.readiness` polls the room-status projection until `state` is `'open'` — which `resolveRtcRoomTransportState` returns exactly when an accepted layout exists and every desired peer is ready — and, in the same tick that condition first holds, captures the `health` block and emits `rallar.browser.formation.ready`. It does **not** call `rallar.rtc.waitForRoom(room)`, whose `connect` option defaults to `true`, because a barrier that opens lanes takes part in what it measures. Nothing refreshes the room on that agent either: the in-page `rtc.connect` readiness poller and the out-of-page `LiveRtcControlClient.waitForPeerReadiness` both call `refreshRoom`, which is why neither can be the observer. So the ordering of `ready` after the change diagnostic that carried the accepted layout and after the `layoutAccepted` diagnostic is the browser's own fence, and the command is correct whether or not a layout exists when it is issued, so no scenario needs a carve-out. _Rejected:_ injecting a stale accepted layout from the test — it tests the fixture. |
...
| `reset-tears-down`         | After the manager's `reset` on the active group, every agent's `wait` for a `rallar.browser.formation.changed` diagnostic with `payloadPath: 'stage'`, `equals: 'dormant'` resolves; its `health` then reads `rtcStatus.activePeerIds`, `knownPeerIds` and `readyPeerIds` empty (the facade-level lanes, which the room block would report empty by construction), `formation.dialing: 'none'`, `accepted` and `planned` absent; each agent's `peer-created` count is unchanged over a 5 s hold; the manager's `start`, `plan`, `connect` and `formation.readiness` on all three then raise every count — the positive control. |
...
is `'open'`, subscribing through `rtc.onStatus` rather than sleeping, and never calls
`rtc.waitForRoom`, whose `connect` option defaults to `true` and would open lanes from inside the
measurement. In the same tick the condition first holds it captures `summary(roomRef)` and `now()`
...
- [ ] **Step 8: Wire it.** Widen the composition's seams: `rooms` gains `formation`
      ...
      `formation: controller.summary(roomRef)` whenever `blackBoxRallarRoomRefOf(config)` resolves a
      room (settled Q4: always present, no input flag). Two guards are part of the decision and must
