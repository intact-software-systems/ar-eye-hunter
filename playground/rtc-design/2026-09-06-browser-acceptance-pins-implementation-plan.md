...
- [ ] **Step 8: Wire it.** Widen the composition's seams: `rooms` gains `formation`
      ...
      which also increments the `unsubscribed` counter that `runtime-lifecycle.test.ts` asserts as `3`
      and which this slice moves to `4`; the `#connectEffect` catch block; the
...
| R6  | The diagnostics are torn down "where that subscription is torn down".                                            | The teardown is four sites, and one of them increments a counter `runtime-lifecycle.test.ts` asserts as `3`.                                                                        | Task 1.1 Step 8, which now names all four and the moved assertion.                                                                                                  |
