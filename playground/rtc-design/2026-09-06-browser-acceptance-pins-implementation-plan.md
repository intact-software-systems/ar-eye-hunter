...
defects in shipped behaviour; both turned out to be one defect in the harness, and all five scenarios
are green now that it is fixed. Each item below cost a run, so it is recorded whether or not the
scenario it belongs to is finished.
