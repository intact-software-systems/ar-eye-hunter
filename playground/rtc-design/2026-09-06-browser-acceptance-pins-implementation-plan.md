...
questions no reading of the code had settled. Two of the answers are candidate defects in shipped behaviour rather than problems with the spec: the pins found what they were written to find. `discovery-holds-dials` passes end to end in about
...
| L7 | **A candidate defect, not a spec gap.** After a reopen with a restored session, the returning page reports `connected: false` with `status: 'idle'` and a closed WebSocket, even though its `rtc.connect` command returned success and the restored session is intact. The session identity is verified: the pin that a reopen preserves the session id passes, and the health block carries the same session the page left with. A freshly opened agent running the identical command connects normally, so the difference is the reopen. Three hypotheses were tested and eliminated: the session identity is preserved (a pin in `reopen` asserts the restored `sessionId` equals the one the page left with, and it passes); the session is not being ended by the close (`logoutOnClose: false` and `leaveRoomOnClose: false`, as the matrix driver sets, change nothing); and the runtime is not merely uncredentialed (passing the agent's own username and password in the connect's `rallar` block changes nothing). The stored session is valid by `readSession`'s own rules, in the default `local` storage under the `auth.session` key the app reads. Note also that the harness's restore branch has never been exercised by any other test: both live scripts always supply a username and password, so `resolveLiveRtcBrowserAgentAuth` always returns `login`. | Blocks `member-progress`, `status-on-connect` and `reset-no-stale-hydration`. The evidence is a health block showing a valid session on an unconnected runtime; that belongs in a defect report against the browser runtime's restore path, not in a workaround here.                                                                                                                                    |
| L8 | **A second candidate defect.** After a `reset` on an active group, the browser's facade-level RTC peer lists do not empty. The stage reaches `dormant` and the room-scoped lists empty by construction, but `knownPeerIds` and `activePeerIds` on `health.rtcStatus` still name both peer sessions after sixty seconds of polling.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | This is exactly the claim `reset-tears-down` exists to test, and the plan reads the facade-level lists deliberately, because the room-scoped ones would be empty whether or not a lane was dropped. Either the lanes are not torn down on reset, or the scenario's statement is wrong about which surface shows it. The scenario is left asserting the claim rather than relaxed to match the behaviour. |
