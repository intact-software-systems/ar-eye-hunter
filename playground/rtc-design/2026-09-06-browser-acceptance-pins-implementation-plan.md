...
questions no reading of the code had settled. Two of the answers were first recorded as candidate
defects in shipped behaviour; both turned out to be one defect in the harness. Fixing it made three
of the five scenarios green and let the other two run for the first time, which is how L9 was found.
Each item below cost a run, so it is recorded whether or not the scenario it belongs to is finished.
...
| L7 | **A harness defect, now fixed; not a defect in the browser runtime.** A page reopened with a restored session reported `connected: false` with `status: 'idle'` and a closed WebSocket while its `rtc.connect` returned success. The runtime was never at fault: it was never sent the command. `connectPresence` built its id from the agent prefix and the scenario suffix, both preserved across a reopen, so the reopened page re-sent a `commandId` the closed page had already completed. `enqueueCommand` treats a repeated id with a matching payload fingerprint as idempotent and returns the existing envelope without queueing (`control-service.ts:268`), dispatch skips anything already complete (`:601`), and `executeResult` then matched the pre-close entry in the run's commandId-keyed result map (`live-rtc-control-client.ts:200`) and reported it as this call's outcome. Proven by a two-arm run on one reopened page: the shipped id left it `idle` with a closed socket, an otherwise identical command under an unused id connected it (`status: 'connected'`, `readyState: 'open'`). That also explains the three eliminated hypotheses — each changed the shared helper, so both calls' payloads moved together and the fingerprints stayed equal. | Fixed by minting a fresh id per issued command (`createCommandId`), and by dropping the credentials from the connect so the reopened runtime restores its session instead of logging in and minting a second one. `reset-no-stale-hydration` passes. `member-progress` and `status-on-connect` now run rather than reporting a false success, which is what surfaced L9. |
| L8 | **The same defect, not a second one.** The reset scenario's sixty-second teardown poll re-issued one `formation-health-<prefix>-<suffix>` id every iteration, so after the first sample landed every later iteration replayed it: what looked like peer lists that never empty was one frozen reading taken immediately after the stage change. The same scenario's post-reset `plan` and `connect` also reused the ids `activateGroup` had already completed, so neither was ever dispatched. No facade-level RTC defect was demonstrated. With unique ids the scenario passes unchanged — its assertions were never relaxed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `reset-tears-down` passes. The peer-list claim it makes is now genuinely observed rather than sampled once.                                                                                                                                                                                                                                                              |
| L9 | **A product race the L7 fix exposed, not a new harness defect.** With commands actually reaching the reopened page, `formation.readiness` on the RETURNING member times out at `state idle` in roughly one run in two. A control run at the L7 fix _without_ the session pin fails identically, so neither half of that fix causes it. The three scenarios that never run a readiness barrier on a reopened member are stable across the same six runs, so the barrier is what is racy, not the reopen: this is L5 arriving as a failure rather than a note — the surviving peers still hold the lane the member left with, and `settleSurvivors` asks them to look again exactly once.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `member-progress` and `status-on-connect` stay `test.fixme` rather than landing a flaky gate. Deciding between a repeated survivor refresh and a product-side re-dial is the matrix owner's call, because it changes what the pin proves.                                                                                                                                |
