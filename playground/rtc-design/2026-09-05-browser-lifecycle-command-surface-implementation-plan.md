...
| `packages/shared-web/browser/rooms/room-group-state-translation.ts`                                                                                                                                | Create request carries the policy; `RoomFormationCommand`, `toRoomFormationGroupStateRequest` (the projections moved out; see the deviations below)           | 1     |
...
| `packages/shared-web/browser/rooms/formation/room-formation-observation.ts`                                                                                                                        | `readRoomFormationStatus`, `subscribeRoomFormation`, `toRallarRoomLayout`, `toRallarRoomFormationStatus`: the status projection, read and change subscription | 1, 2  |
...
## Deviations recorded during delivery (2026-09-05)

- The status projections `toRallarRoomLayout` and `toRallarRoomFormationStatus` live in
  `packages/shared-web/browser/rooms/formation/room-formation-observation.ts` beside their only
  consumers, not in `room-group-state-translation.ts` as Task 1.4 wrote: adding them there crossed the
  translation file's cognitive-load warn tier in the changed-style gate. The translation file keeps the
  request translation only.
- `validateGroupFormationView` takes the decoded `GroupFormationView` the HTTP layer already types
  rather than `unknown`: the repository's boundary rule reserves `unknown` for `decode*` functions and
  type guards, and the validator checks the server's claims about an already decoded body.
- The subscription tests drain the overlay repositories' observer queue with
  ...
  that promise queue rather than synchronously.
- Budgets after slice 2: the browser facade measures 177.84 KiB Brotli (budget 178) and the headless
  agent 221.95 KiB (ceiling 222), each raised to the next whole KiB per the settled convention.
