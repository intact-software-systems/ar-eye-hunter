...
same day after review (decisions 1, 5, 6, 11 amended; 14–23 added); all 23 decisions are
taken and no product question is open.** This document records what
...
| 6  | **Preset evolution and landing**: `optimistic` = `auto` / `apply`, `managed` = `debounced` / `apply`, `match` = `commanded` / `hold`, `drop-in-social` = `debounced` / `apply`. `apply` for `managed` is deliberate: the manager curates who is in and when the group starts, not the wiring, so members who join after the applied layout was planned are wired in after the debounce window without a manager act; `hold` would leave them RTC-less until someone called `connect`. A stored policy may override the preset value per group. _Amended after review: the landing column is new; the evolution column is unchanged; the managed landing was confirmed on 2026-08-22._                                                                                      |
