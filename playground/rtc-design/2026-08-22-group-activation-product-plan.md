...
terminal-failure and progress proposals (decisions 1, 12, 20 and 30 amended; 35–41 added); revised
on 2026-08-24 after an implementation-readiness review (decisions 18, 24, 27, 30, 32, 33, 36 and 37
amended); and revised again after an authority/race/decision-ownership review (decisions 24, 27 and
32 amended; 42 added). All 42 decisions are taken and no product question is open.** This document
records what the Rallar product should support for application-controlled group
...
| R4  | The SPA separates "received the layout" from "connect using the layout"                                                                     | The browser holds a received layout as _planned_; `connect` binds that exact layout for dialing, while only promotion under decision 42 makes it accepted for live traffic                  |
...
| R10 | An application can return a group to a clean slate and run it again, without deleting and recreating it                                     | `reset` returns the group to `dormant`, clears the formation series and accepted identity, retires both topology slots, retains their fingerprints for traceability, and halts transport    |
...
| #  | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| -- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1  | **Held and dialing phases are distinct lifecycle stages, and dialing is a total function of the stage and the two layout roles.** Seven stages: `dormant`, `forming → planned → connecting → active`, with reconfiguration split into `reconfiguring` (new layout held) and `reconnecting` (new layout dialed). `establishing` is renamed `connecting`; `paused` is not a stage because halting is a transport fact (decision 25). The dial matrix is settled: `connecting` dials the frozen planned layout, `active` and `reconfiguring` dial the accepted layout, `reconnecting` dials their union, and every other stage dials nothing.                                                                                                                                                           |
...
| 12 | **One initiator policy governs all eight application-facing group-authority commands.** Whoever the initiator policy allows may `reset`, `start`, `plan`, `connect`, `activate`, `reconfigure`, `pause` and `resume`; `server-auto` denies principals for all eight. The field belongs to the group-authority tier (decision 26), not to establishment.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 13 | **Observed-status changes emit their own event**, `group-activation-status-changed`, carrying both axes, the coverage and evidence watermark, and `coverageBasisLayoutIdentity` — the exact accepted or first-formation planned layout from which that coverage was computed. Identity is the tuple from decision 29, never a bare version.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 18 | **Parallelism is an in-flight bound, and it is member policy.** `maxConcurrentEdgeSetups` is the maximum number of RTC setups a member may have in flight for that group; a setup ends on success, failure or timeout, and each ending wakes the next reconcile. A peer shared by multiple groups is one connection charged to every owning group's count and starts only when every owning group has a free slot, still under the member's session-wide cap. No cross-group fairness or share negotiation is promised.                                                                                                                                                                                                                                                                              |
...
| 24 | **The two layouts are the `accepted` one and the `planned` one, and an unconfirmed plan never displaces accepted traffic.** The accepted layout is what the browser heals and routes application data over. The planned layout may be held or dialed as a candidate, but it becomes accepted only through promotion as defined by decision 42. `reset` is the only operation that drops accepted without replacement. Failure discards only planned.                                                                                                                                                                                                                                                                                                                                                 |
...
| 27 | **`topology.reconfigureLanding: 'apply'` means the accepted layout follows the planned one with no lifecycle transition.** The transaction that accepts a new planned publication durably enqueues a fenced `apply-planned-layout` AppInbox command for that exact epoch and identity; its retryable transaction invokes decision 42's canonical promotion and converges the accepted row, accepted fingerprint and group identity. `hold` changes accepted only through `activate`. An `apply` group therefore never enters `reconfiguring` or `reconnecting`, preserving decision 17 structurally.                                                                                                                                                                                                 |
...
| 30 | **Observed status has two orthogonal axes: condition and remediation.** Condition is derived from coverage of the layout carrying traffic: accepted whenever accepted exists, otherwise the frozen planned candidate during initial `connecting`; stages with neither read `inactive`. Remediation says whose move it is and may read `none`, `replan-queued` or `awaiting-application`. Observation never gates. Every status fact names its exact coverage basis (decisions 13 and 33).                                                                                                                                                                                                                                                                                                            |
...
| 32 | **`connect` names the exact planned layout it means to dial, and automatic intent survives until that transition commits.** `compute` validates the expected identity against the current planned row. A manual caller receives `no-planned-layout` or `planned-layout-superseded` and decides whether to retry. For automatic progression, trigger satisfaction is durably latched until a matching `connect` commits: no publication leaves it waiting, a superseded publication reissues against the current identity, and command submission alone never consumes it. A successful transition consumes the latch and transfers race ownership to the `connecting`/`reconnecting` freeze, where topology work cannot replace the candidate until activation, failure or reset advances the stage. |
| 33 | **Status writes carry an evidence watermark scoped to `(formationEpoch, coverageBasisLayoutIdentity)`.** That fence is constant within one basis and cannot order status writes against each other, so a write must strictly advance its evidence watermark; an equal-or-older watermark is a typed drop that writes nothing and emits no event. Changing basis starts a distinct causal series.                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 36 | **`reset` clears the formation series; `start` begins a new one.** In one AppInbox transaction it sets `formationAttemptCount` to 0, clears `establishmentStartedAtEpochMs`, `lastFormationOutcome` and `acceptedLayoutIdentity`, sets `transportState` to `halted`, advances the epoch, and tombstones both accepted and planned topology slots. It retains both valid fingerprints for tracing; the planned tombstone, not fingerprint deletion, disarms change suppression so unchanged membership rebuilds normally. Membership, topology config and overrides remain.                                                                                                                                                                                                                           |
| 37 | **The formation attempt budget is per formation series, not per group lifetime.** `maxFormationAttempts` bounds one series. A successful manual or criterion `activate` atomically zeroes `formationAttemptCount`, as does `reset`; otherwise unrelated failed reconfigurations would accumulate across healthy activations. Exhaustion remains terminal for automation: `start` is denied until an explicit `reset` clears the series.                                                                                                                                                                                                                                                                                                                                                              |
...
| 42 | **Promotion is the sole atomic state change that makes a planned layout accepted.** `connect` never promotes: dialing a candidate is not acceptance. Successful `activate` and an `apply` landing invoke the same canonical promotion effect, which validates the epoch and exact planned identity and updates the accepted row, accepted fingerprint and group identity together. Rejected alternatives: promotion during `connect`, because failed reconnection must retain the live accepted layout; separate activate/apply writers, because their stored facts could diverge; and synchronous accepted-state writing by the topology publisher, because it crosses the GroupState AppInbox ownership and retry boundary.                                                                        |
...
**Promotion has one semantic owner, and it is not `connect`** (decision 42). The canonical pure
promotion effect reads the group and both topology slots, validates the expected epoch and exact
planned identity, and computes the accepted row, accepted fingerprint and
`Group.acceptedLayoutIdentity` together. A manual or criterion `activate` applies that effect in the
same AppInbox transaction as `connecting | reconnecting → active`, so the stage cannot commit without
its accepted layout. An `apply` publication atomically enqueues an internal
`apply-planned-layout` command; that command applies the same effect without a lifecycle transition
and converges through normal AppInbox retry. An identical accepted identity is an idempotent success;
absent, superseded or mismatched authority is typed and writes nothing.

`connect` binds only the expected planned identity as the dialing candidate. For automatic progression,
the durable trigger latch owns the interval before a successful transition and is not consumed by
submission or a stale denial. Once `connect` commits, the latch is consumed and topology work freezes
the candidate throughout `connecting` or `reconnecting`. `reset` is the only other transition that
changes accepted, by dropping it (decision 36). `fail-formation` targets `forming` from `connecting`
and `active` from `reconnecting` — return to whatever the accepted layout was, and there was none the
...
dial — a caller that waited for layout A must not silently dial layout B. During `connecting` or
`reconnecting`, the candidate is frozen: the join updates authority, and the unconditional follow-up
enqueue after activation or failure replans from that latest state without replacing the in-flight
candidate. Under `commanded` replanning a mid-hold join does not move any layout at all; it sets
`layoutStale` and waits for the application.
...
- **The dial gate reads the stage and both layouts.** `connecting` uses the frozen planned layout;
  `active` and `reconfiguring` use accepted; `reconnecting` uses their union; every other stage uses
  none. There is no bootstrap fallback for `phased` groups. The stage and layout identities come from
  the group snapshot the browser already holds, receives on change and hydrates on connect — no new
  transport. The same total decision binds every outbound path and inbound peer admission.
...
- Both axes are **persisted and pushed** (decision 3): a change is written by internal authority after
  ...
  not write.
- Status writes **converge** (decision 33): within one `(formationEpoch, coverageBasisLayoutIdentity)`
  a write must strictly advance its evidence watermark, so an older observation can never overwrite a
  newer one, however the writers interleave.
- Its **clocks are durable** (decision 19): dwell completion and evidence expiry produce no evidence
  ...
  formation epoch and coverage-basis layout identity; a stale entry is a drop.
- It is **truthful about its lag**: a status carries the time it was last confirmed, the coverage it
  was computed from, and `coverageBasisLayoutIdentity`.
...
Every internal command — `apply-planned-layout`, triggers, the criterion's petitions and status
writes — carries `expectedFormationEpoch` and `expectedLayout`, and `compute` rejects a mismatch as
a typed stale outcome (decision 19). Encoding the epoch in the request id deduplicates; it does not
fence.

For the connect trigger, satisfaction and candidate ownership are deliberately separate. The durable
latch owns automatic intent while the group remains `planned` or `reconfiguring`; every accepted
publication while it is present petitions the exact current identity. Only a successful `connect`
consumes it. The stage freeze then owns candidate stability until activation, failure or reset.
...
await room.formation.connect(layout); // binds exactly that layout; dialing starts under the bound
// ... the match runs ...
...
| Scenario                   | Pins                                                                                                                                                                                               |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `held-layout`              | `plan` publishes a planned layout; no RTC signaling is observed; `connect` freezes and dials it; `activate` atomically promotes it and the condition moves `inactive → initialising → active`      |
...
| `connect-trigger-handoff`  | an automatic trigger latches before publication; superseding A with B does not consume intent; `connect(B)` commits, consumes the latch and freezes B                                              |
| `dialing-layout-frozen`    | a membership/source change during `connecting` or `reconnecting` cannot replace the planned identity being dialed; the transition follow-up replans latest authority after activation or failure   |
| `apply-landing`            | accepting an `apply` publication durably enqueues promotion; retry or process restart converges accepted row, fingerprint and group identity without a lifecycle transition                        |
...
| `reset-to-dormant`         | `reset` from `active` lands in `dormant`, clears accepted identity, tombstones both topology slots, retains both fingerprints, halts transport and zeroes attempts; `start` reaches `active` again |
...
| `attempt-series-resets`    | failures followed by successful manual and criterion activation each reset the count to zero, so a later reconfiguration receives the full configured budget                                       |
...
1. **Contract closure.** The seven-stage transition table, accepted/planned ownership and layout
   identity, the transport valve and forward gate, admission × stage, receipt semantics, the
   `connect` precondition and latch-to-freeze handoff, the total condition and remediation functions,
   fingerprint staleness, the in-flight dial plan, and expected epoch/identity fences — all as pure
   functions with total matrices, landing dark.
2. **The next held-layout capabilities.** Durable accepted/planned ownership, the canonical promotion
   effect, `connect` freezing the exact layout it dials, the browser cache and complete outbound and
   inbound dial gates, hydration and readiness fencing, and the live-RTC proof. Promotion on
   `activate` is atomic with the stage transition; an `apply` publication atomically enqueues its
   route-less promotion command.
