...
`activate` is the only transition that _promotes_ a layout: it makes the planned layout accepted and
drops the edges the two do not share. `reset` is the only other transition that changes the accepted
