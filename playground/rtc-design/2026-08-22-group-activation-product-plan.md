...
amended), then revised once more after implementation-readiness review closed the dialing-layout,
promotion-ownership, reset-identity and status-basis gaps (decisions 1, 12, 13, 18, 24, 27, 30, 32,
33, 36 and 37 amended). All 41 decisions are taken and no product question is open.** This document
...
| R4  | The SPA separates "received the layout" from "connect using the layout"                                                                     | The browser holds a received layout as _planned_; `connect` binds that exact layout for dialing, while only `activate` or an `apply` landing promotes it to accepted for live traffic       |
...
| 12 | **One initiator policy governs every application-facing group-authority command.** Whoever it allows may `reset`, `start`, `plan`, `connect`, `activate`, `reconfigure`, `pause` and `resume`; `server-auto` denies principals for all eight. Policy-driven internal operations remain separately trusted and narrowly registered. The field belongs to the group-authority tier (decision 26), not to establishment.                                                                                                                                                                                                                                                                                                                                                                       |
...
| 18 | **Parallelism is an in-flight bound, and it is member policy.** `establishment.maxConcurrentEdgeSetups` is the maximum number of RTC setups a member may have in flight at once for that group; a setup ends on success, failure or timeout, and each ending wakes reconcile. A peer shared by several groups is one connection charged to every owning group's in-flight count and may start only when every owner has a free slot, while the member's session-wide cap remains the hard ceiling. No cross-group share negotiation or fairness guarantee is promised, but this deterministic all-owners admission rule is required.                                                                                                                                                        |
...
| 24 | **The two layouts are the `accepted` one and the `planned` one, and an unconfirmed plan never displaces accepted traffic.** The accepted layout is what the browser heals and routes application data over. The planned layout may be held or dialed as a candidate, but it becomes accepted only through the canonical promotion effect: atomically with a successful `activate`, or through the fenced internal `apply-planned-layout` mutation after an `apply` publication. `reset` is the only operation that drops accepted without replacement. Failure discards only planned.                                                                                                                                                                                                       |
...
| 30 | **The observed status is two axes, and each side reports only work it performs.** **Condition** — `inactive \| initialising \| active \| degraded \| failed` — is coverage of one explicit basis: accepted while accepted traffic exists, otherwise the frozen planned layout during first `connecting`. **Remediation** — `none \| replan-queued \| awaiting-application` — is what the server is doing about a gap, including an exhausted dormant series that only `reset` can clear. Browser-local repair remains separate.                                                                                                                                                                                                                                                             |
...
| 32 | **`connect` names the exact planned layout it means to dial.** `compute` validates the expected identity against the current planned row, then entering `connecting` or `reconnecting` freezes that row: topology work cannot replace the dialing candidate until activation, failure or reset advances the stage. The lifecycle transition's unconditional follow-up enqueue then replans from the latest authority, so no second durable owed-work mechanism is needed. Two typed denials remain: `no-planned-layout` and `planned-layout-superseded`. The expected identity never degrades to a silent no-op.                                                                                                                                                                            |
...
never displaced until the promotion effect commits, a failed reconnection has something to return to.
...
**Promotion has one owner, and it is not `connect`.** The canonical group-state promotion effect reads
the group and both topology slots, validates the expected epoch and planned identity, and computes the
accepted row, accepted fingerprint and `Group.acceptedLayoutIdentity` together. A manual or criterion
`activate` applies that effect in the same AppInbox transaction as `connecting | reconnecting → active`,
so the stage can never commit without its accepted layout. An `apply` publication atomically enqueues
an internal `apply-planned-layout` command; that command applies the same effect without a lifecycle
transition and converges through normal AppInbox retry. Both paths write the group event, delta, receipt
and final outbox effects in their owning transaction. An already-promoted identical identity is an
idempotent success; absent, superseded or mismatched authority is typed and writes nothing.

`connect` only freezes the expected planned identity as the dialing candidate. Topology work cannot
replace the planned row during `connecting` or `reconnecting`; source revisions remain authoritative,
and the lifecycle transition's existing unconditional follow-up enqueue replans from their latest state
after activation or failure. This is what makes the expected identity durable without adding a second
queue or group field. `reset` is the only other
transition that changes accepted, by dropping it (decision 36). `fail-formation` targets `forming` from `connecting` and
...
connect — a caller that waited for layout A must not silently dial layout B. During `connecting` or
...
- **The dial gate reads the stage and both layouts through one total matrix.** `connecting` uses the
  frozen planned layout; `active` and `reconfiguring` use accepted; `reconnecting` uses their union;
  `dormant`, `forming` and `planned` use neither and have no bootstrap fallback for `phased` groups.
  The same matrix governs every outbound path and inbound peer admission. Stage and accepted identity
  come from the hydrated group snapshot; planned identity comes from the classified publication.
...
- Both axes are **persisted and pushed** (decision 3): a change is written by internal authority after
  ...
  not write.
- Status writes **converge** (decision 33): within one `(formationEpoch, coverageBasisLayoutIdentity)` a write
...
- It is **truthful about its lag**: a status carries the time it was last confirmed, the coverage it
  was computed from, and the coverage-basis layout identity it refers to.
...
Every internal command — `apply-planned-layout`, triggers, the criterion's petitions and status writes — carries
...
await room.formation.connect(layout); // binds exactly that planned layout; dialing starts under the bound
// ... the match runs ...
...
| Kind                                        | Pins                                                                                                                                                                                                                                                                                |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pure matrices (`packages/tests/shared`)     | the stage transition and complete seven-stage planning/dial tables, the transport valve and forward gate, admission × stage, total condition/remediation functions, layout classification including `incomparable`, pacing, trigger evaluation and fingerprint staleness            |
| Shared browser tests (`WebRtcGroupManager`) | planned-only initial dialing, accepted-only live dialing, the reconnecting union, no-dial stages, bootstrap suppression, frozen dialing identity, accepted/planned cache behaviour, pacing, wake-on-completion, halted sends and repair reporting                                   |
| api-v1 black-box recipes                    | stage commands and receipts, promotion apply/conflict/retry/idempotency/rollback/convergence, preconditions and typed denials, accepted identity, attempt-series reset, snapshots/deltas/hydration/events, relay halt/resume, multi-server convergence and durable restart behavior |
...
2. **One vertical held-layout capability.** Durable accepted/planned ownership, `connect` freezing an
   exactly named dialing candidate, atomic activation promotion, durable `apply` promotion, the browser
   cache and every outbound and inbound dial gate, hydration and readiness fencing, and the live-RTC
   proof — delivered together, with new stage commands unreachable over HTTP until the whole path passes.
