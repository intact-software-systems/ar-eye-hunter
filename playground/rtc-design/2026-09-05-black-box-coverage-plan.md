...
| 15 | A presence-lease expiry removes the session from `activeSessions` and drops `onlineMemberCount`, but leaves `causalRevision.presenceRevision` and `presenceVersion` unchanged. A client polling a floored read on `minPresenceRevision`, or caching on `presenceVersion`, never observes the expiry -- the roster it holds keeps the expired member online.  | `compute-disconnect-group-presence.ts` expiry branch; pinned by `api-v1-group-presence-lease-lifecycle`  | open — the change token does not move for an expiry              |
