...
| I39 | **The status is one nullable field, not three flat ones** (2026-09-04, Slice 12b): the section's summary adds `condition`, `remediation` and `coverageBasisLayoutIdentity` to the key lists, but the product plan's own properties need seven facts, not three — both axes, the coverage the condition was banded from, the basis, the formation epoch that with the basis names the causal series, the evidence watermark decision 33 orders writes by, and the instant decision 3 calls being truthful about lag. Seven flat fields would be seven entries in each of the four hand-maintained allowlists, seven required fields on every group write for the state-write gate to measure, and nothing tying together facts that are only ever decided and written together. So `activationStatus: GroupActivationStatus \| null` follows `memberPolicy`'s nested precedent: one key per allowlist, one validated block, and I4's second key-list edit stays as cheap as I4 promised. Null is the honest initial value rather than a computed one — a group with no layout carrying traffic or being dialed has no coverage, and a status object would have to invent the rate it claims to be computed from; making the basis non-nullable _inside_ the status then leaves no invalid combination representable. _Also here:_ the layout-identity validation duplicated for `acceptedLayoutIdentity` in all three validators is extracted, because the basis needs exactly the same check.                                                                                                                                                               |
| I40 | **Only the condition is stored; remediation stays derived at read** (2026-09-04, Slice 12b): decision 3 persists and pushes "both axes", but remediation's inputs are transient in a way the condition's are not. `replanQueued` drains when the queued replan runs and `layoutStale` flips when a temporary topology override expires — neither writes a status, so a stored remediation would be stale far more often than right, and the group row would advertise "replan-queued" for a replan that finished minutes ago. The petition cannot cheaply supply them either: the planning authority carries `replanning` as a mode, not as those facts, and computing them needs the fingerprint and pending-replan reads the formation view already performs. So `GroupActivationStatus` holds the condition with the coverage, basis, epoch, watermark and instant it was confirmed at, and 12a's read-time `resolveGroupActivationRemediation` keeps the second axis where its inputs are current. The push obligation is met for the axis that has durable meaning; the advisory one is answered by the surface that can answer it truthfully. _Alternative rejected:_ carrying both transient facts on the command as observations, which was implemented first — it makes a retry write remediation computed from evidence the retry did not re-read, and persists a value with no owner keeping it fresh.                                                                                                                                                                                                                                           |
| I41 | **The clocks are a coalesced singleton row with a due `next_ts`, and I14 was right** (2026-09-04, Slice 12b): the checkpoint doubted I14's coalesced row because `rtc-topology-coalesced-group-revision-work.ts` is hard-coded to `kind: 'group-revision'`. That is the topology _wrapper_. `CoalescedAppOutboxWorkService` underneath is generic — `CoalescedAppOutboxWorkData<T>` with a pluggable `merge`, already instantiated separately by the topology outbox and the presence-summary worker — so a status clock is a new instantiation, not a change to the topology one. The mechanism is the queue's own: `resource_inbox` is unique on `(fk_ext_bank_id, ri_resource_id, ri_topic_id)` and carries `next_ts` inside `resource_inbox_runnable_ix`, and `toScheduledEntry` sets `status: NEW` when due and `RETRY` with `nextTs` when not, so the row is invisible until due with no polling. `enqueueRtt`'s `dueAtEpochMs: requestedAtEpochMs + debounceMs` is the same shape already in production. This is what makes the clock correct at three servers and more: the unique key means N nodes arming a dwell produce one row rather than N, `merge` folds a later arming into it (which is the moving due time the formation-timer shape could not express), `for update skip locked` means exactly one node runs it, and a node dying loses no clock because the row outlives the process. It also dissolves the "what wakes a quiet group" question: the clock wakes it, so decay is reported even when evidence has stopped entirely.                                                                                                     |
| I42 | **The status recipes drive coverage by its denominator, and `status-convergence` needs a staged race the black-box runner cannot express** (2026-09-04, Slice 12b): RTT evidence is injectable — `api-v1-group-formation-criterion` reports an edge over `ws.send` with an explicit `version`, which is the same monotonic term the watermark keys on — so the recipes are not blocked on a new HTTP surface. Decay is reachable too, and not by waiting out the 60 s freshness window (`evidenceFreshnessMs` is passed nowhere in production, so that wait is unshortenable): coverage is `observed / planned`, and the planned edge set comes from `nextHopsBySessionId`, so admitting one more session raises the denominator and drops the rate in one step. `status-lifecycle` is therefore deterministic end to end, dwell included. `status-convergence` is not: its subject is two writers racing, and the watermark drop is only observable when a reading's newest counted evidence fails to advance the stored one — which in a single-threaded recipe is exactly the case where the status is unchanged anyway, so the same-status no-op would satisfy the assertion whether or not the watermark rule exists. A recipe that cannot fail when the rule is removed proves nothing about it. Convergence belongs where the race is real: concurrent petitions on the cluster profile, or a unit proof over `computeUpdateGroupActivationStatus` with a stored status the command cannot advance.                                                                                                                                                  |
| I43 | **A widened layout lands on `failed`, not `degraded`, and only a clock can lift it** (2026-09-04, Slice 12b recipes): the `status-lifecycle` recipe drives decay by admitting a third session, which grows the planned edge set from one to three while one edge stays observed — coverage 0.333, which is below the `minimumViableRate` floor of 0.5 rather than merely below the success rate, so the honest band is `failed`. The first draft asserted `degraded` and was wrong; the code was right. Reporting a second edge does reach 0.667, between the floor and the success rate, but `computeGroupActivationCondition` gates `degraded` on `dwellSatisfied`, and only a clock write sets that — so lifting out of `failed` needs a further clock cycle, and pinning it on a poll window would buy a flaky recipe rather than a proof. The reachable, deterministic walk is `inactive → initialising → active → failed`, and it is worth having precisely because that last transition is the durable dwell clock reporting a decay no evidence arrival could report. `degraded` and the recovery above it wait for a recipe that can observe the clock rather than race it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**Review (2026-09-03).** A full-diff review of the rebased 11c+11d head found no correctness defect
in either landing — the attempt-count off-by-one, the timer and latch arming across `dormant`, the
superseded gate under replan/reset/second-reconfigure/tombstone interleavings, the decoder's exact-key
contract and the migration all hold — but one functional gap and a documentation set the change had
contradicted. The gap: `resolveGroupStageTrigger` still returned `null` for `reconfiguring`, so a
`presence` connect trigger fired only on its fallback timer at the new boundary while the OpenAPI text
promised "whichever is first". Both stages that hold a candidate now resolve to the connect trigger
and the topology cycle petitions at either, restoring that promise rather than weakening it. The docs:
the architecture document's "Not In V1" entry still declared this boundary unbuilt, its failure-landing
and admission prose predated 11c, and its deadline paragraph predated 11a's I32 — all four corrected,
along with the admission decision's own comment and a JSDoc left attached to the wrong function.

**Follow-up, pre-existing.** Connect-trigger latch rows are written with `NEVER_EXPIRE_AT_TIMESTAMP`
and the repository has no delete path; `reset` tombstones layouts but not latches. Parking makes
`dormant` a long-lived terminal state, so epoch-orphaned latches accumulate for failed groups. Not
introduced here, and not this slice's to fix, but the landing makes it easier to reach.
...
**Review (2026-09-03).** A full-diff review found the first head measuring the wrong layout: the
condition took its rate from `readiness`, which is scoped to the **planned** slot, while
`coverageBasisLayoutIdentity` named the **accepted** one. Product decision 30 says the condition is
coverage of the layout carrying traffic, so the pair was wrong in both directions whenever a held
candidate ran ahead of the accepted layout — routine under `commanded` replanning and throughout
`reconnecting` — and decision 33's evidence watermark is keyed on that same basis, so 12b would have
inherited the error. Coverage is now measured on the loaded snapshot the basis names, or reported as
absent when no loaded slot is that layout; the published `readiness` field keeps its planned-slot
meaning. The review also found the degenerate fixture that hid it — every route test used a plan with
no edges, which readiness reports as trivially complete — so the tests now carry a real fraction, a
held candidate with different edges, and a torn-down plan, which `computeGroupFormationReadiness`
also reads as complete and which the criterion path refuses to petition from for that reason.

Two more from the same review. The remediation axis defaulted an unreadable policy to `auto`, which
reports "the server has this in hand" on the one input where the automation does nothing at all;
`ResolveGroupActivationRemediationInput.replanning` now carries `'corrupt'` and hands a stale layout
to the application under it. And `resolveReadablePolicy` duplicated shared-server's
`toReadGroupLifecyclePolicy` character for character, in the module the route already imported from.

**Carried forward.** `requireActiveGroup` still carries the expiry rule separately from
`resolveGroupBusinessLiveness`: the predicate skips expiry when its caller passes no instant, and
giving the value form that mode would widen a shared contract for one caller's option. Both are
pinned by tests at the boundary instant, and the drift risk is recorded rather than designed away.
...
Branched from `main` @ `fced7794e`, with 13a's metrics buckets landed (#467) — the order the
section's own note requires: the bucket exists before the writer that fills it, so the writer's
write volume is counted as `stageTransition`/`activationStatus` rather than falling through to
`other`. 13b's cluster-wired recipe (#468) reads that bucket and is independent of this slice, so
the two are no longer stacked.
...
### Slice 13a start checkpoint — the formation-metrics buckets (2026-09-03)

Branched from `main` @ `4250859b5`. This is the piece the section orders **before** any automatic
...
- **`stageTransition` and `activationStatus`** on `GROUP_FORMATION_OPERATION_KINDS`, and the seven
  transition-table commands classified into the first. The admin surface types the bucket map as
  `Record<GroupFormationOperationKind, …>`, so the recorder's initialiser is compiler-forced; the
  OpenAPI models `groupFormation` as an open object, so no schema edit follows.
- **A compiler-enforced pin on the classifier.** Its sink carries an untyped operation by design — a
  diagnostic must never reject one — so the seven literals are repeated there rather than narrowed
