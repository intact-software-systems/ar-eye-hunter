...
> runtime and does not validate the corrected candidate. One fixed,
> non-rerolled corrected-runtime A-B-B-A protocol remains pending, along with
> invalidated final validation/publication gates. The
...
#### Pending corrected-runtime order-balanced comparison

The final corrected candidate requires one new fixed, non-rerolled A-B-B-A
sequence. Resolve one exact final candidate commit before position 1 and use it
unchanged for both B positions. The order remains approved base A, corrected
candidate B, the same corrected candidate B, and approved base A. Use the same
...
the historical protocol. No position may be rerun, and no containers or
benchmark processes may overlap.

The corrected run uses only these new ignored paths so historical artifacts are
...
After all four positions exist, run the unchanged focused pooling tests and the
same pooling writer with the corrected candidate commit and new paths:
...
Run the unchanged global comparator and child evaluator against only the new
pooled artifacts and manifest. Record the new manifest SHA-256 before invoking
the child evaluator and pass that exact literal. The corrected candidate keeps
the same child-policy thresholds: 1.5% inclusive adverse equivalence for shared
throughput and pooled median resource costs, 5% uncontended p95/p99 maximum
regression, 10% hot-throughput maximum regression, the same measured
conflict-depth requirements above the band, and zero tolerance for every
correctness and artifact-integrity invariant. Until that sequence passes, Task
9 performance acceptance remains pending.
...
| A structural move changes concurrency performance               | Preserve both earlier rejected comparisons, the diagnostic, the unchanged global A-B-B-A failure, and the amended child-policy result as historical-only pre-fix evidence; never rerun those fixed four positions. Run one new fixed corrected-runtime A-B-B-A sequence with distinct artifact basenames, bind its exact 18-sample pooled outputs through its new governed manifest, apply only the same 1.5% child equivalence band, retain zero-tolerance correctness, and require measured conflict-depth evidence above the band. |
...
| Task 9 structural lineage | final review fixes in progress       | Exact-base structural lineage is implemented test-first from `b8d6d8516f2c1caff46494569940c06e7ee06c43` / tree `9344df9af0b24f29341ebf8d8cebdb9d54963b69`; its three review rounds accepted Critical 0, Important 0, Minor 0 on candidate tree `ae4aab3f01cbcdb4cc933e7401f01625e64d71bf`. The non-lineage cohort starts from `0afacc09c75044e9d983cdc3f228464432cb6773` / tree `a1a4e4ee47a3f31a837d6ab7541d82d88ceee1c5` and reduces the exact `WORKTREE` comparison from 50 residual findings to zero through cohesive ownership moves and named boundary narrowing. Review round 1 reported Critical 0, Important 4, Minor 0. Candidate `093b0f929366cc9beb46c7b619e668ecf3ee2a2b` retained readonly metadata but re-review reported Critical 0, Important 3, Minor 0. Candidate `6456c595bd1eba595a8a97dc6223895bceb325f7` fixed those three but re-review reported Critical 0, Important 2, Minor 0 for stale nullish readiness state and a 404-line ratchet test. Candidate `77fa37a5bf27777b8f2861d79e445ac0220b5c9d` resolved those findings; independent review accepted Critical 0, Important 0, Minor 0 after three review-fix rounds. The final Deno gate at `5be76f2c70d1f1d1c9d162a735fc86db64fc1622` / tree `c8c0634fb35a48ea6f54f13eff4bdd915c31a0d8` then exposed the private child kill-parameter mismatch; its exact-runtime correction was independently accepted Critical 0, Important 0, Minor 0 on candidate tree `1899730916deb8ed80ca8a28fc1fd06da195a952`. The first fixed pair and controlled replacement both retained exact durable correctness but failed the locked performance requirements. Reverse-order diagnostics did not reproduce a PR-specific cause. The fixed A-B-B-A measurement retains the unchanged global failure; its later, explicitly post-result 1.5% child-policy evaluation accepted only that immutable pre-fix evidence as equivalent. The final whole-review fixes invalidate runtime acceptance; corrected-runtime performance, broader gates, final publication, PR-ready, workflow, merge, and ledger evidence remain pending. |
| Performance acceptance    | corrected-runtime protocol pending   | The governed A-B-B-A positions ran once at candidate `f92dd2b403c03dff093627e1739c46a6dd4ae084` / tree `3a26aed776c31828434620265baf2489d4d2f73f`; all source, environment, pooled-output, manifest, and unchanged-comparator hashes remain recorded in Section 10 verbatim. The unchanged global comparator retains exit 1 and its nine findings. The later human-approved 1.5% adverse-equivalence policy is explicitly post-measurement, child-only, and non-statistical. Its fail-closed evaluator accepted that immutable pre-fix result with every correctness invariant green, but it does not validate the corrected runtime. One new fixed A-B-B-A run with distinct corrected-order artifact basenames, final unchanged-tree gates, publication, PR-ready, and remote evidence remain pending.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
that the pending corrected-runtime A-B-B-A PostgreSQL 16 protocol uses four
pinned isolated environments with `--warmup=1 --runs=9 --concurrency=10` and no
reroll. It checks that distinct corrected-order artifact basenames prevent any
...
The final review fixes invalidate that runtime acceptance and leave one exact
corrected-runtime protocol pending without changing dependencies, workflows,
the global comparator, TypeScript, checker, Task 10, or API-v1 contracts.
