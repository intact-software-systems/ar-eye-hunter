...
`verified` through PRs #198 and #199. Task 4B, B04, baseline capture, and Phase
2 remain inactive.
...
`codex/shared-rtc-bench-task-4a-evidence`, after fetching exact current
`origin/main` `8dab885023ca722c717e5a40724d9db635c20fd5`, tree
`77f1e58fa8d0deb78d2103be9508ca81c93cc42c`. Compatibility review from Task
4A resulting main `c96f46f2eba10c8103b29b052c0edfbc42c05a37` found changes only in a
separate group-topology state-write plan, its tests, and its result pooler. The
Task 4A package, exact lock blobs, frozen RTC contracts, ontology Task 4
reservation, and organization sequence are unchanged: **Compatible — no plan
delta**.
...
Task 4A. That historical record activated Task 4A only; the verified closure
below activates no successor milestone.
...
The completed Task 4A reservation is exactly Section 10 of plan blob
...
**Activation state:** Task 4A is verified. PR #198 merged the exact relocation
as `03f690f3ae9d821876d50035ef7463def0985059`; PR #199 closed its generated
Deno-lock requirement and merged as
...
is verified through PRs #198 and #199. Task 4B is not activated; B04, B06, B07,
production change, optimization, raw-artifact publication, ontology
implementation, human-plan edits, and Phase 2 remain held.
...
| RTC benchmark organization   | RTC performance plan owner       | `task-4a-verified`; Task 4B inactive                        | PR #198 final head `51314f099eaf754f18dc3df11b84b1bec2b10d3e`, tree `3393d7680f20e01c34dac550414b757bca7c97c3`, passed Branch Release Gate 31637171160 and merged tree-identically as `03f690f3ae9d821876d50035ef7463def0985059`. Resulting-main run 31640463428 failed frozen Deno preparation before any recipe. PR #199 corrected only the three generated locks at head `10a161addffc6821cb6240f28e23e0773d7ee19b`, tree `d8af997cbcc9ad1476470687228eee1b16595ef6`, and passed Branch Release Gate 31642081827. Concurrent PR #197 first carried the same lock blobs to main; run 31674269978 attempt 1 failed provider parity after frozen preparation, while attempt 2 passed selection, frozen preparation, and all five supported manifests on the exact shared tree. PR #199 merged tree-identically with its parent as `c96f46f2eba10c8103b29b052c0edfbc42c05a37`, tree `85b531ff8fd5fb0e9722a79876061c40a9eaf6fd`; exact run 31674331404 succeeded through its no-delta selector. Current main retains the exact lock blobs. No benchmark execution or capture occurred. | Preserve the verified checkpoint. Start Task 4B only after explicit human activation on its separate follow-on branch/PR; keep B04, capture, and every later layer inactive.                                                                                                                                                                           |
...
1. Preserve Task 4A's exact verified PR #198/#199 source, correction, branch,
   merge, and resulting-main evidence; do not relabel either historical failed
   resulting-main run as success.
2. Keep Task 4B inactive until the human explicitly authorizes its separate
   follow-on branch and PR. B04 remains held until Task 4B's complete review,
   legacy dispositions, independent re-review, and publication gates pass.
3. Keep all benchmark capture, B06, ontology implementation, production RTC
   changes, and optimization held.
4. Preserve the ontology Task 4 shared-web browser-lane reservation and its
   build-time binding resolution. Do not edit ontology implementation or create
   benchmark-owned ontology metadata.
5. Keep B06, B07, production RTC changes, optimization, raw-artifact publication,
