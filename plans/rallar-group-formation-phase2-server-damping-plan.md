...
| 3 | Room-send authorization                                                       | `group-policy.ts:397-407` (`canSendGroupMessage` requires `isLiveSession(snapshot session, now)`)                                                                                   | kept live by the heartbeat→summary rewrite                                                                                                                                                                                                        | **breaks without #2** (idle groups would deny sends after TTL); fixed by #2                                                                                                                                                                                                |
