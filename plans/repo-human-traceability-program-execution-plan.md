...
therefore `ledger-published`. The server structure child is approved at blob
`1a74159d37f76a459009e99ca5a08f3cd620b1b4` with its authorized amendments.
Its final feature head `bec8bea4eb095de9ad3a6b47c18e6799ab811239` and tree
`c1ac6a57dad974d04264cbe1fa92313697256712` passed Branch Release Gate
`30694693554` attempt 1. PR #59 squash-merged as exact `main`
`06e0c5ab138c2ab55ac519b2244f727acd42d560`, and default workflow
`30697799787` attempt 1 passed for that exact SHA. The
[server traceability QA child](rallar-group-state-server-traceability-qa-plan.md)
published PR #61 and PR #62. PR #62 feature
`b579aa56bc656b12f3717f2b02c0e24de9244357` and tree
...
`30741608017` attempt 1 passed for that exact SHA. The
[server traceability hardening child](rallar-group-state-server-traceability-hardening-plan.md)
is complete through PR #64 and PR #65. PR #64 feature
`6ce2bcce85f3a03446c847f4b07689c1c0b1e70e`, tree
`00d475e7d40bed7060567ee391ffe1041fba443a`, Branch Release Gate
`30748239173` attempt 1, resulting `main`
`49237d8bb75d2239569aa4e3d43f8b88db799602` with the same tree, and default workflow
...
`5a6ffd385655af75b28aa22feb5a7103f87862a0` with the same tree, and default workflow
`30774354577` attempt 1 are complete. The separate five-plan server ledger was
published through PR #66: feature
`6e2ea5e4c727f431743e0ad6eab55a0fc9d9af1b`, tree
`111995e3a72eb246fd0b8028aada4fbeda65fe69`, Branch Release Gate
...
resulting `main` `df9ab8d27de17c29b927c9ed9fcce9251ba7e62a`, whose default workflow
`31095762444` attempt 1 succeeded. Evidence-ledger PR #75 published feature
`2858bf0c2a9b882a82ae4c33abf58d6e0408be8d` and frozen tree
`104478f66bcabbbcf101ea97a80d2a2060cb10ec`; Branch Release Gate run
...
attempt 1 succeeded for that exact SHA. Evidence-ledger PR #93 published feature
`aeff6435794dd70816789e4794b78e84fdfc89b0` and tree
`8bdea4402dad08dbd1892f2bd8c95671d615b8ff`, then merged as exact main
`c2cb79c020bceee7f67e6fbc364ba96ea0d6a530` with the same tree under the
human-approved plan-only build-gate exception. Hetzner run `31251480014` attempt 1 failed and is
retained only as non-gating plan-only external evidence. Auth is
`ledger-published`. Group-topology implementation is closed through PR #209,
merge `44cda16e4633a27d4315dc3a3eb41405651e39c3`, and authenticated
close-out main `8ee348e215a3e30d9b4959ce90369aea1b55b620`; its separately
authorized [evidence ledger](rallar-group-topology-evidence-ledger-plan.md) is
postponed after completing its sole evidence slice. It has no next slice and
remains short of `ledger-published` while awaiting re-authenticated publication
review and canonical closure.
...
| [Auth server structure child](rallar-auth-server-structure-plan.md)                                         | Defines authoritative auth ingress, login, credentials, sessions, mutation, persistence, compatibility, tests, and durable navigation.   | `ledger-published` through PR #93; no further auth-child work is pending.                                                  |
| [Group-topology evidence ledger](rallar-group-topology-evidence-ledger-plan.md)                             | Reconciles the closed topology implementation, immutable deviations, warnings, ratchets, compatibility, and validation evidence.         | Postponed after completing its sole evidence slice; implementation is closed and the ledger is not yet `ledger-published`. |
        ...
        -> Wave 0 governance and checker
            -> browser room/group-state boundary
                -> shared-server group-state structure
                    -> shared-server group-state traceability QA
                                    ...
                                    -> API-v1 evidence ledger
                                        -> pilot evaluation
                                            -> client-state server structure
                                                -> client-state evidence ledger
                                                    -> auth server structure
                                                        -> later auth evidence ledger
                                                            -> group-topology server structure (closed)
                                                                -> group-topology evidence ledger (postponed; evidence slice complete)
...
PRs #78, #81, and #90. Its separate evidence-only ledger published through PR
#93, so auth is `ledger-published`. Group-topology implementation is closed
through planning PR #95, amendments #125/#127/#129/#131, and implementation
PRs #103/#151/#155/#209. Its authenticated receipt preserves the deleted plan.
The separately authorized evidence ledger completed its sole evidence slice
and is now postponed with no next slice. It changes no code or performance
evidence and cannot mark the child `ledger-published` before re-authenticated
publication review and canonical closure record its external envelope.
...
Auth evidence-ledger PR #93 published feature
...
plan-only build-gate exception. Run Hetzner Supported Distributed Manifests
`31251480014` attempt 1 failed and remains non-gating external evidence for that plan-only
publication; it is not diagnosed or relabeled. Auth is `ledger-published`.

The [group-topology evidence ledger](rallar-group-topology-evidence-ledger-plan.md)
is the current bounded Wave 2 record and is postponed after completing its sole
evidence slice. The implementation plan is authentically closed at exact main
`8ee348e215a3e30d9b4959ce90369aea1b55b620` and remains deleted historical
evidence. This ledger authorizes only reciprocal planning reconciliation; it
authorizes no production change, API-v1 reorganization, RTC/RTT work,
performance command, or future publication fact.
...
Git blob 1a74159d37f76a459009e99ca5a08f3cd620b1b4 for execution, subject only
to its recorded authorized amendments. The expanded pre-merge convergence
amendment authorizes the three pending repairs and behavior-neutral Tasks 8–9
alignment in existing draft PR #59 as one path before merge. It does not approve
Task 10, a later API-v1 child, a semantic or public-contract change, or the
separate evidence-ledger publication.
...
invariants, and plan assumptions before editing. Tasks 0–6 and the prior Task 7
fixes already exist in the child progress evidence. Complete the three pending
Task 7 repairs and behavior-neutral Tasks 8–9 alignment together in existing
draft PR #59 before its single merge; this narrowly supersedes the old two-PR
sequence only. Preserve every public and persisted contract,
...
work. Run the required unchanged Postgres medium-scale gate and the fresh
mutation-path comparative gate under the child amendment. Publish reviewable
milestones and follow the non-circular completion contract. Stop for material
plan drift, an unapproved behavior, compatibility, authority, persistence, or
concurrency change, a blocker, or required human or default-branch
authorization. Do not reorganize API-v1.

Use the Mandatory Completion Handoff after substantial intervals and at the
end. After PR #59's exact resulting-main workflow succeeds, draft and obtain
exact-blob approval for
plans/rallar-group-state-server-traceability-qa-plan.md. Its two implementation
PRs now precede Task 10. Only after the later ledger is `ledger-published` may a
prompt draft plans/api-v1-group-state-route-structure-plan.md; do not implement
it.
```

## 10.1 Prompt 6A: Execute The Approved Server Traceability QA Plan

Use this prompt only after the traceability QA planning PR is merged, its exact
resulting-main workflow succeeds, and the human approves the exact QA plan Git
blob named by the final planning handoff.

The execution prompt must:

- bind that exact Git blob and resulting `main` SHA;
- create one child-specific goal and PR A branch only;
- execute PR A guidance, test ownership/renames, review-size evidence, temporary
  ratchet rules, and independent target-by-target PR #59 lineage provenance
  first; change no checker behavior; then stop for its human merge;
- require PR A's exact resulting-main workflow before creating PR B;
- execute only behavior-neutral PR B descriptor/timing, registration-lifetime,
  immutable durable/private transaction-result, narrow handler-capability,
  naming, and semantic test scope;
- preserve every PR #59 public, persisted, AppInbox, compatibility, timing,
  dependency, workflow, TypeScript, checker, and performance contract;
- require test-first changes, independent review, completion gates, governed
  mutation-path performance, and Branch Release Gate for each exact PR head;
- stop for each human merge decision;
- leave the later ledger and API-v1 child unstarted.

The final exact copy-and-paste execution prompt belongs in the post-merge
planning closure handoff, after the plan blob, planning merge SHA, and default
workflow result all exist and cannot be stale.

## 10.2 Prompt 6B: Execute The Approved Server Traceability Hardening Plan

Use this prompt only after the hardening planning PR is merged, its exact
resulting-main workflow succeeds, and the human approves the exact plan Git blob
named by the final planning handoff.

The execution prompt must:

- bind that exact Git blob and resulting `main` SHA;
- create one child-specific goal and the PR A guidance/navigation branch only;
- execute PR A guidance pressure and durable group-state navigation first,
  without production or checker behavior changes, then stop for its human merge;
- require PR A's exact resulting-main workflow before creating PR B;
- execute only the behavior-neutral discriminated descriptor relationship,
  named transaction writer port, handler-owned presence transaction decision,
  closed timed-operation inventory, semantic ratchets, and internal computed-
  result naming work approved by the child;
- preserve every public, persisted, AppInbox, compatibility, timing,
  dependency, workflow, TypeScript, checker, and performance contract;
- require test-first changes, independent review, completion gates, governed
  A-B-B-A performance, and Branch Release Gate for each exact PR head;
- stop for each human merge decision; and
- leave the later server ledger and API-v1 child unstarted.
...
Use this prompt only after the server ledger records PR #59, both traceability
QA PRs, and both traceability-hardening PRs and has independently reached
`ledger-published`.
...
| Auth server evidence ledger            | ledger-published         | PR #93 feature `aeff6435794dd70816789e4794b78e84fdfc89b0` and frozen tree `8bdea4402dad08dbd1892f2bd8c95671d615b8ff` merged as exact main `c2cb79c020bceee7f67e6fbc364ba96ea0d6a530` with the same tree under the human-approved plan-only build-gate exception. Hetzner run `31251480014` attempt 1 failed and is retained only as non-gating plan-only external evidence. Auth is `ledger-published`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Group-topology server implementation   | closed                   | Planning PR #95 plus amendments #125/#127/#129/#131 governed PRs #103/#151/#155/#209. PR #209 feature `5f415783f365fde844f7868e22ce81681d7e0ba9` and merge `44cda16e4633a27d4315dc3a3eb41405651e39c3` share tree `7b0bf26652d3183f8da013903291913b5d4f09eb`. Branch Release `31723292697` and resulting-main Hetzner `31723602513` succeeded; PR review record `31723552393` failed and is retained under the human closure disposition without a pass claim. Exact main `8ee348e215a3e30d9b4959ce90369aea1b55b620` authentically closed and deleted the implementation plan.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Group-topology evidence ledger         | postponed                | [Evidence plan](rallar-group-topology-evidence-ledger-plan.md) completed its sole evidence slice from exact main `8ee348e215a3e30d9b4959ce90369aea1b55b620` and has no next slice. The completed ledger awaits re-authenticated publication review and canonical closure; topology is not yet `ledger-published`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Other Wave 2 children                  | blocked                  | API-v1 client-state organization, RTC/RTT, CRDT, and admin remain outside this planning child and require their own approved plans.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
