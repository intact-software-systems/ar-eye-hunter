...
The implementation is `complete`. Evidence-ledger PR #75 published feature
`2858bf0c2a9b882a82ae4c33abf58d6e0408be8d`, frozen tree
`104478f66bcabbbcf101ea97a80d2a2060cb10ec`, and Branch Release Gate
`31097790516` attempt 2; it merged as exact `main`
`6b75cfc5ec61f81b465be9072b746d24ecdb5f22`, whose Run Hetzner Supported
Distributed Manifests `31100952224` attempt 1 succeeded. This child is
`ledger-published`.
...
**Successor:**
[Rallar Auth Server Structure Plan](rallar-auth-server-structure-plan.md) is the
next bounded Wave 2 child, drafted and unapproved.
...
packages/tests/repo/
  ...
  client-state-navigation-trace-evidence.ts
...
**Status:** complete and independently accepted. Accepted cohort milestones are
Task 4A
...
`f772137e6776ae5dba38880e17bce5f277b8e740`. The later Task 5 test/evidence
review fixes were accepted at `12a8c632bb7e2e16a9fbe807dfade4850eb78376` /
tree `c1edfc693c74f0c464b534abd9c6539d2b6b0da7`, with Critical 0, Important 0,
and Minor 0. These are local milestone facts, not final PR B publication
evidence.
...
**Status:** ledger-published. PR #75 recorded the planning and three
implementation envelopes, the Section 2.3 sample waiver, final code-derived
trace review, reconciled warning outcomes, and the following PR C ratchet
decisions without making a new decision or changing tests/manifests:
...
The three-plan evidence diff executed none of the source, test, manifest,
assertion, or hash changes and did not claim that any replacement had already
occurred. It published no navigation-time comparison and fabricated no human
observations. The later auth planning child begins only after the exact ledger
merge and default-workflow evidence above.
...
### 10.4 Human-authorized PR C performance remediation

The first PR C candidate was exact commit
`688227b834b0ecc1850ca00e375fd07fe446c273` / tree
`5cbd4d518a6377cc323216b7faf7529764514050`. Its non-rerolled A-B-B-A sequence
is retained as failed evidence. The pooled base, pooled candidate, and manifest
SHA-256 values were respectively
`9d89f65d9a7fac55dc0dc3f19b8046dd682fdfa195ecef44039f1b575434063a`,
`dd23cc57da979f480dae309ef733847550dffa24907b01fca442f45fcbd01b1e`,
and `74921da633a8517b22850976028612b87066024fb2fab11c0025664a566ec824`.
The unchanged global comparator and child evaluator outputs remain at SHA-256
`3c2e863cd0824b32fe67232fb1f86dfaf5ac2d8d69b024a43db82149a6fb3a9c`
and `df1eda061e34472f99299a356ff762cb816780710d43747308997531ae7666cf`.
The child evaluator rejected hot SQL statements at `+2.221031%` and hot
transaction duration at `+1.647691%`; it accepted all covered findings and
correctness invariants.

The human then authorized one evidence-led diagnosis and behavior-neutral
remediation pass without changing a contract, threshold, harness, comparator,
or evaluator. Static tracing proved that PR C adds no SQL operation. Immutable
artifact analysis showed candidate improvement for uncontended and shared
workloads and only the hot workload regression. A non-governed tight profile
isolated a `4.9-6.1%` cost in the newly introduced per-call named options object
for synchronous compute/validate timing. The behavior-neutral correction gives
compute and validate separate three-input timing operations; the same diagnostic
then measured the corrected wrapper about `0.9-1.1%` faster than its paired
...
- [x] Every predecessor public/deep path and package export remains compatible.
- [x] Canonical internal callers bypass compatibility-only wrappers.
- [x] Every request/command field, default, omission, clone, property order,
      error, and volatile invocation is exact.
- [x] AppInbox, transaction, retry, idempotency, receipt, event, outbox,
...
- [x] The later ledger independently reached `ledger-published`.
- [x] API-v1 organization and every other Wave 2 implementation remained
      unstarted through the ledger; auth planning began only afterward.
...
| Later evidence ledger      | ledger-published   | PR #75 feature `2858bf0c2a9b882a82ae4c33abf58d6e0408be8d` / tree `104478f66bcabbbcf101ea97a80d2a2060cb10ec`; Branch Release Gate `31097790516` attempt 2 success; resulting main `6b75cfc5ec61f81b465be9072b746d24ecdb5f22` / same tree; Run Hetzner Supported Distributed Manifests `31100952224` attempt 1 success.                                                                                           |
| Other Wave 2 domains       | auth planning only | The linked auth child is drafted and unapproved after this ledger closure. Auth implementation, topology, RTC, CRDT, and admin remain outside this child.                                                                                                                                                                                                                                                       |
