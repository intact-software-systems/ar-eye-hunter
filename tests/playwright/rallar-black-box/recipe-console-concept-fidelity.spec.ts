...
import { TUNE_ANALYZE_ROUTE, TUNE_SLOW_AGENT_ID } from './recipe-console-tune-run-data.ts';
...
const CONCEPT_CASES: readonly ConceptCase[] = [
    {
        ...
        ready: async (page) => {
            ...
            for (const label of featuredLabels) {
                await expect(
                    catalogRegion.locator('[data-execute-recipe]').filter({
                        hasText: label
                    })
                )
                    .toHaveCount(1);
    ...
    },
    {
        ...
        ready: async (page) => {
            ...
            expect(
                await evidencePlane.evaluate((element) => {
...
async function expectAdvancedHierarchy(
    ...
    expect(await scrollOwner.evaluate((element) => element.scrollHeight > element.clientHeight)).toBe(true);
