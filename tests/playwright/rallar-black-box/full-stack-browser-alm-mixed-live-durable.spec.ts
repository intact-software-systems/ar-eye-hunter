...
const deliveryLifecycleModuleUrl = `/@fs${
    path.join(repoRoot, 'packages/shared/alm/delivery/al-delivery-lifecycle.ts')
}`;
...
interface MixedDurableSend extends MixedDurableAdmission {
...
interface MixedScenarioArtifact {
    readonly schema: 'rallar.browser-alm-mixed-live-durable-proof.v4';
...
test.describe('browser ALM mixed live and durable progress', () => {
    ...
    test('keeps latest-value room traffic moving while durable RTC completes across reconnect', async ({
        ...
        try {
            ...
            const workload = await settleMixedWorkload(
                durableSendsPromise,
                liveSendPromise,
                reconnectPromise
            );
            if (workload.durable.status === 'fulfilled') {
                scenario.durableSends.push(...workload.durable.value);
            }
            scenario.durableDisposition = classifyMixedLiveDurableSends(scenario.durableSends);
            if (workload.live.status === 'fulfilled') {
                scenario.liveSend = workload.live.value;
            }
            if (workload.reconnect.status === 'fulfilled') {
                scenario.reconnect = workload.reconnect.value;
            }
            if (workload.firstRejection !== null) {
                stage = workload.firstRejection.stage;
                throw workload.firstRejection.reason;
            }
            stage = 'mark-receiver-durable-identities';
        ...
        const artifact: MixedScenarioArtifact = {
            schema: 'rallar.browser-alm-mixed-live-durable-proof.v4',
            executionBoundary,
            sources: {
                runtime: {
...
async function sendDurableBurst(
    ...
    return await page.evaluate(async (input) => {
        ...
        const deliveryLifecycle: typeof import('../../../packages/shared/alm/delivery/al-delivery-lifecycle.ts') =
            await import(input.deliveryLifecycleUrl);
        const { rallar } = rallarModule;
        ...
        return await Promise.all(input.logicalIdentities.map(async (logicalIdentity) => {
            const handle = await channel.sendRtc({ identity: logicalIdentity, sentAtEpochMs: Date.now() }, {
                ttlMs: input.timeoutMs
            });
            const outcome = await handle.wait({
                until: deliveryLifecycle.AL_DELIVERY_ADMITTED_STATES,
                timeoutMs: input.timeoutMs
            });
            ...
            return {
                logicalIdentity,
                msgId: handle.msgId,
                waitStatus: outcome.status,
                state: lifecycle.state,
                admittedDurable: lifecycle.evidence.admittedDurable ?? null,
                rateLimited: lifecycle.evidence.attempts.some((attempt) => attempt.unroutableReason === 'rate-limited')
            };
        }));
    }, {
        rallarUrl: rallarModuleUrl,
        deliveryLifecycleUrl: deliveryLifecycleModuleUrl,
        roomId,
