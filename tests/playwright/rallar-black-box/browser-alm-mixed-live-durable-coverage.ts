import type { RallarMessageDeliveryOutcome } from '../../../packages/shared-web/browser/messages/rallar-message-contracts.ts';
import type { ALDeliveryState } from '../../../packages/shared/alm/delivery/al-delivery-lifecycle.ts';

import type { MixedLiveDurableTerminalIdentity } from './browser-alm-mixed-live-durable-observer.ts';

export interface MixedDurableAdmission {
    readonly msgId: string;
    readonly waitStatus: RallarMessageDeliveryOutcome['status'];
    readonly state: ALDeliveryState;
    readonly admittedDurable: boolean | null;
...
export interface MixedLiveDurableCoverageInput {
    ...
    readonly durableSends: readonly MixedDurableAdmission[];
...
export function readAdmittedMixedLiveDurableIds(
    ...
    return sends
        .filter((send) => send.waitStatus === 'settled' && send.admittedDurable === true && !send.rateLimited)
        .map((send) => send.msgId);
}
