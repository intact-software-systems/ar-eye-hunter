...
test('browser-rallar provider performs live RTC connect and realtime send when configured', async ({
                                                                                                       page,
                                                                                                       request,
                                                                                                   }) => {
    ...
    await expect.poll(async () => {
        ...
        return run.results?.some(result =>
            result.commandId === connectCommandId && result.ok === true
        ) ?? false;
    }, {
        timeout: 30_000,
    }).toBe(true);
    ...
    await expect.poll(async () => {
        const run = await fetchRun(request, runId);
        return run.results?.some(result =>
            result.commandId === sendCommandId && result.ok === true
        ) ?? false;
    }, {
        timeout: 30_000,
    }).toBe(true);
