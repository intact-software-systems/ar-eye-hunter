...
async function attachMapScreenshot(
    ...
    const clip = await map.evaluate(element => {
        ...
        if (!mapFrame) throw new Error('Missing Fleet evidence map SVG frame');
...
async function attachLayoutEvidence(
    ...
    const layout = await page.evaluate(() => {
        ...
        if (!fleet) throw new Error('Missing [data-fleet-workspace]');
                ...
                return {
                    element: [
                        ...
                        element.getAttribute('data-fleet-window-focus-anchor')
                            ? `[data-fleet-window-focus-anchor="${element.getAttribute('data-fleet-window-focus-anchor')}"]`
                            : '',
