...
export const TUNE_ROUTE =
    '/?provider=simulated&v=1&experience=recipe-console&view=tune';
export const TUNE_ANALYZE_ROUTE =
...
export function createTuneDistributedRun(
    ...
    return {
        ...
        rollup: {
            ...
            ok: !right,
            summary: {
                participants: 2, requiredParticipants: 2,
                readyParticipants: 2, passedParticipants: right ? 1 : 2,
...
export function createTuneControlRun(
    ...
    const agentIds = right
        ? [TUNE_SHARED_AGENT_ID, TUNE_SLOW_AGENT_ID]
        : [TUNE_SHARED_AGENT_ID, TUNE_LEFT_AGENT_ID];
    const commandIds = agentIds.map((_, index) =>
        `${TUNE_STREAM_COMMAND_ID}-${index + 1}`
    );
    return {
        runId,
        createdAtEpochMs: TUNE_BASE_EPOCH_MS,
        ...
        agents: agentIds.map(agentId => ({
            runId, agentId, connected: true,
            lastSeenAtEpochMs: TUNE_BASE_EPOCH_MS + 4_500,
            connectionSequence: 1, reconnectCount: 0,
            receivedResultCount: 1, receivedEventCount: right ? 2 : 1,
            completedCommandIds: commandIds, resumeCompletedCommandIds: [],
        })),
        commands: commandIds.map((commandId, index) => ({
            envelope: {
                kind: 'command', protocolVersion: 1,
                runId, agentId: agentIds[index], commandId,
                command: {
                    kind: 'rtc.stream', commandId,
                    send: { type: 'tune.frame' }, count: 3, rateHz: 30,
                },
            },
...
function messageEvent(
    ...
    return {
        kind: 'event', protocolVersion: 1, runId, agentId,
