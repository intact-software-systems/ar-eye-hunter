...
export const EXECUTE_SCALE_ROUTE = '/?provider=simulated&v=1&experience=recipe-console&view=execute' +
    ...
    const runs = Array.from(
        ...
        (_, runOrdinal): ControlRunSnapshot => {
            ...
            return {
                ...
                agents: Array.from(
                    { length: agentCount },
                    (_, agentOrdinal) => executeAgent(runId, agentOrdinal, updatedAtEpochMs)
                ),
                commands: [],
                results: [],
                events: [],
