...
test.describe('full-stack Rallar Server REST workbench', () => {
    ...
    test('logs in and joins an existing group with the authenticated client id', async ({
        page,
        request,
    }) => {
        test.setTimeout(120_000);
    ...
    test('surfaces live group-join request errors from Rallar Server', async ({
        page,
        request,
    }) => {
        test.setTimeout(120_000);
        ...
        await panel.getByLabel('Endpoint').selectOption('group-member-join');
        await panel.getByLabel('Path').fill(
            `${scopePath}/groups/${encodeURIComponent(config.roomId)}/members/${
    ...
    test('runs a live REST collection for create-or-existing group and join', async ({
        page,
        request,
    }) => {
        test.setTimeout(120_000);
...
async function ensureGroupExists(
    ...
    if (readResponse.status() !== 404) {
        throw new Error(
            `Expected existing-group setup read to return 200 or 404, got HTTP ${
                readResponse.status()
...
async function createGroupFromWorkbench(
    ...
    if (![200, 201, 409].includes(createResponse.status())) {
        throw new Error(
            `Expected group setup create to return 200, 201, or 409, got HTTP ${
