...
export class MixedLiveDurableObservation {
    ...
    setBrowserSessionId(sessionId: string): void {
        ...
        this.#inboundNamespace = toBrowserALRuntimeNamespace(toBrowserSessionALInboundRuntimeStoreId(sessionId)) +
            ':inbound:admission';
    }
    ...
    private installQueueObservation(): void {
        ...
        IndexedDbQueueBox.prototype.releaseEntries = async function (releases) {
            const released = await observer.#originalReleaseEntries.call(this, releases);
    ...
    private decodeScopedDispatchEntry(entry: ResourceEntry): MixedLiveDurableDecodedEntry | undefined {
        if (
            this.#inboundNamespace === undefined || entry.typeId !== toALInboundWorkType(this.#inboundNamespace, 'rtc')
        ) {
            return undefined;
        }
...
async function observeCompletedAdmissionParent(
    ...
    const parent = computeALInboundWorkEntry({
        ...
        payload: { kind: 'admit-message', msg: message, source: { kind: 'rtc-peer', peerId: 'sender' } },
    ...
    await context.queue.releaseEntries([...reserved.values()].map((entry) => ({
        entry,
...
async function observeExpiredDispatchClaim(
    ...
    try {
        ...
        await context.queue.releaseEntries([...reserved.values()].map((entry) => ({
            entry,
            disposition: { status: EntityStatus.RETRY, delayMs: 30_000 }
        })));
    }
    return observation;
}
...
async function readBoundedMarkedRows(input: ReadBoundedMarkedRowsInput): Promise<MixedLiveDurableRows> {
    ...
    await new Promise<void>((resolve, reject) => {
        request.addEventListener('success', () => {
            ...
            if (entry.typeId === toALInboundWorkType(input.inboundNamespace, 'rtc')) {
