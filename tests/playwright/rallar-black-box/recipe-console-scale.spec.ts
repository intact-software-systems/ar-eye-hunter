...
import { verifyTuneScalePressure } from './recipe-console-tune-scale-proof.ts';
...
test(
    ...
    async ({ browser, context, page }) => {
        ...
        for (let rangeStart = 1; rangeStart <= totalEvidence; rangeStart += 64) {
            ...
            await expect(status).toHaveText(
                `Showing ${rangeStart.toLocaleString('en-US')}–${
                    rangeEnd.toLocaleString('en-US')
                } of 15,003 retained matches.`
            );
            ...
            const windowIds = await results.locator('[data-evidence-result]')
                .evaluateAll((elements) => elements.map((element) => element.getAttribute('data-evidence-id') ?? ''));
        ...
        for (
            const [needle, kind, sourceFile, expectedCommandId] of [
        ...
        await expect(
            analyzeSource(page).getByRole('link', {
...
test('keeps multibyte bidi evidence identifiers exact and isolated in RTL', async ({ context, page }) => {
    await installRecipeConsoleAnalyzeFixture(context);
...
async function verifyHistoryScale(browser: Browser): Promise<void> {
    await test.step('History keeps 5,000 run pairs searchable inside 80-row windows', async () => {
...
async function verifyExecuteScale(browser: Browser): Promise<void> {
    await test.step('Execute bounds 250 run choices and 240 live targets without writes', async () => {
        const context = await browser.newContext({
            baseURL: PRODUCTION_BASE_URL,
            colorScheme: 'light',
            deviceScaleFactor: 1,
...
async function verifyMonitorScale(browser: Browser): Promise<void> {
    await test.step('Monitor keeps every pressure collection independently bounded', async () => {
        const context = await browser.newContext({
            baseURL: PRODUCTION_BASE_URL,
            colorScheme: 'light',
            deviceScaleFactor: 1,
...
async function verifyRetentionScale(browser: Browser): Promise<void> {
    await test.step('Retention keeps aggregate consequences at 100 without confirmation', async () => {
...
async function traverseMonitorWindow(
    input: Readonly<{
        budget: number;
        itemLabel: string;
        label: string;
...
function expectPreviewOnly(
    ...
    expect(retentionRequests.filter((request) => request.kind === 'confirm' || request.kind === 'legacy')).toEqual([]);
