...
async function rowIdentities(
    ...
    return rows.evaluateAll((nodes, descriptor) =>
        nodes.map((node) => {
            ...
            return descriptor.selectors.map((selector) => node.querySelector(selector)?.textContent?.trim() ?? '').join(
                '|'
            );
        }), identity);
}
...
async function traverseOrdinalWindow(
    ...
    while (true) {
        ...
        const ordinals = await rows.evaluateAll((nodes) =>
            nodes.map((node) => Number((node as HTMLElement).dataset.monitorSourceOrdinal))
        );
...
async function traverseInspectorWindow(
    input: Readonly<{
        budget: number;
        itemLabel: string;
        expectedIdentities: readonly string[];
...
async function expectContainedLandscapeCommandBar(page: Page): Promise<void> {
    ...
    const metrics = await items.evaluateAll((nodes) =>
        nodes.map((node) => {
    ...
    for (const [index, metric] of metrics.entries()) {
        ...
        if (next) {
            expect(metric.item.right).toBeLessThanOrEqual(next.item.left + 0.5);
        }
...
test('browses every pressured Monitor collection without gaps, duplicates, or hidden authority changes', async ({ context, page }) => {
    test.slow();
...
test('windows every inspector pressure path while retaining exact selected evidence', async ({ context, page }) => {
    test.slow();
    await page.setViewportSize({ width: 1440, height: 900 });
    const fixture = await installRecipeConsoleLargeMonitorFixture(context);
    const monitor = monitorForFixture(fixture);
    ...
    const roleChoices = monitor.recipeProgress
        ...
        .map((row) => `${row.profile ?? 'Default profile'}|${row.role ?? 'All assigned roles'}`);
    await page.goto(LARGE_MONITOR_ROUTE);
    await waitForLargeMonitor(page);
...
test('retains disclosure cursors and live truth, resets diagnostic filters, and recovers focus after a threshold shrink', async ({ context, page }) => {
    test.slow();
...
test('restores a detached windowed inspector trigger to its owning range anchor', async ({ context, page }) => {
    await page.setViewportSize({ width: 900, height: 900 });
    const fixture = await installRecipeConsoleLargeMonitorFixture(context);
    await page.goto(LARGE_MONITOR_ROUTE);
    await waitForLargeMonitor(page);
...
test('restores a detached multi-window disclosure trigger to its exact range anchor', async ({ context, page }) => {
    await page.setViewportSize({ width: 900, height: 900 });
...
test('restores a disconnected unowned trigger past the hidden dock to the named work surface', async ({ context, page }) => {
    await page.setViewportSize({ width: 900, height: 900 });
    await installRecipeConsoleLargeMonitorFixture(context);
    await page.goto(LARGE_MONITOR_ROUTE);
    await waitForLargeMonitor(page);
    ...
    expect(await artifact.evaluate((element) => element.closest('[data-monitor-window-owner]') === null)).toBe(true);
...
test('restores an unmounted disclosure owner past the hidden dock to the named work surface', async ({ context, page }) => {
    await page.setViewportSize({ width: 900, height: 900 });
    await installRecipeConsoleLargeMonitorFixture(context);
    await page.goto(LARGE_MONITOR_ROUTE);
    await waitForLargeMonitor(page);
...
test('restores a disconnected portrait trigger to the visible live selection dock', async ({ context, page }) => {
    ...
    await expect(inspector).toHaveCount(0);
    await expect(
        page.locator('[data-selection-dock]')
            .getByRole('button', { name: 'Inspect' })
    ).toBeFocused();
});

test('contains exact long bidi evidence and touch window controls across desktop and mobile contracts', async ({ browser }) => {
    test.slow();
