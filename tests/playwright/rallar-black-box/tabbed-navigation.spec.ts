...
async function openRunnerSurface(page: Page, tab: RunnerSurfaceTab): Promise<void> {
  ...
  await page
    .getByLabel('Rallar workspace mode')
    .getByRole('button', { name: /Rallar black-box-runner/ })
    .click();
  await page.getByRole('tab', { name: target.visibleTab, exact: true }).click();
  await expect(page.getByRole('tab', { name: target.visibleTab, exact: true })).toHaveAttribute(
    'aria-selected',
    'true',
  );
  if (target.surfaceButton) {
    await page
      .locator('#panel-advanced')
...
test('moves legacy tab focus with roving keys and recovers it after panel navigation', async ({
  page,
}) => {
  ...
  await recipes.click();
  const openAdvanced = page
    .locator('#panel-recipes')
...
test('opens Quick Test as the default Rallar workspace screen', async ({ page }) => {
  ...
  await expect(panel.getByLabel('Quick Test route')).toContainText(
...
test('opens runner mode on Recipes and runs a local recipe from the launcher', async ({ page }) => {
  await page.goto(
    '/?provider=simulated&workspace=black-box-runner&roomId=bb-group&runId=ui-recipes-empty-run',
  );
  ...
  await expect(agentSetup).toContainText('Opened 1 agent tab with fresh one-time sessions.');
  ...
  await expect(panel.locator('.runner-launch-result')).toContainText(/finished|completed|passed/i, {
    timeout: 15_000,
  });
...
test('opens fresh same-user agent tabs through the visible UI', async ({ page }) => {
  ...
  await context.route('http://localhost:8080/api/auth/agent-session-tickets', async (route) => {
    expect(route.request().headers().authorization).toBe('Bearer operator-token-value');
  ...
  await context.route(
    ...
    async (route) => {
      ...
      const session = request.ticket ? issuedSessions.get(request.ticket) : undefined;
  ...
  await Promise.all(
    agentPages.map(async (agentPage) => {
      await agentPage.waitForURL(/mode=control/, { timeout: 10_000 });
      await agentPage.waitForLoadState('domcontentloaded');
      await expect
        .poll(
          async () =>
            await agentPage.evaluate(() => Boolean(sessionStorage.getItem('auth.session'))),
          { timeout: 10_000 },
        )
        .toBe(true);
    }),
  ...
  const popupStates = await Promise.all(
    agentPages.map(
      async (agentPage) =>
        await agentPage.evaluate(() => {
          ...
          return {
            ...
            session: sessionRaw
              ? (JSON.parse(sessionRaw) as { username?: string; sessionId?: string })
              : undefined,
  ...
  await expect(firstAgentPage.getByRole('heading', { name: 'Rallar Server Login' })).toBeVisible();
  await expect
    .poll(
      async () =>
        await firstAgentPage.evaluate(() => Boolean(sessionStorage.getItem('auth.session'))),
    )
    .toBe(false);
...
test('explains runner readiness failures before live recipe launch', async ({ page }) => {
  ...
  await expect(panel.getByRole('button', { name: 'Run in this browser' }).first()).toBeDisabled();
  await expect(
    panel.getByRole('button', { name: 'Run on connected agents' }).first(),
  ).toBeDisabled();
...
test('warns when ICE returns no TURN servers without blocking recipe actions', async ({ page }) => {
  ...
  await expect(panel.getByRole('button', { name: 'Run in this browser' }).first()).toBeEnabled();
  await expect(
    panel.getByRole('button', { name: 'Run on connected agents' }).first(),
  ).toBeEnabled();
});
...
test('keeps Quick Test group stable after create subscribe and send', async ({ page }) => {
    ...
    (window as any).__rallarDirectFacade = {
      ...
      rooms: {
        ...
        create: async (input: Record<string, unknown>) => {
          ...
          const groupId =
            typeof input.groupId === 'string' ? input.groupId : 'generated-server-group-id';
          return {
            group: {
              groupId,
  ...
  await expect(groupInput).toHaveValue('rallar');
  await expect
    .poll(async () => page.evaluate(() => (window as any).__quickCreateInputs.at(-1)))
    .toMatchObject({
      groupId: 'rallar',
      displayName: 'rallar',
    });
  ...
  await expect(page.getByLabel('Global Room')).toHaveValue('rallar');
  await expect
    .poll(async () => page.evaluate(() => (window as any).__quickJoinInputs.at(-1)))
    .toBe('rallar');

  await panel.getByRole('button', { name: 'Send WS JSON' }).click();
  await expect(page.getByLabel('Global Room')).toHaveValue('rallar');
  await expect
    .poll(async () => page.evaluate(() => (window as any).__quickSendInputs.at(-1)))
    .toMatchObject({
      roomId: 'rallar',
      groupId: 'rallar',
      contextId: 'rallar',
...
test('surfaces direct RTC, Rallar Data, and Media tabs with real-backend guardrails', async ({
  page,
}) => {
...
test('inspects control-server runs and enqueues a bulk command', async ({ page }) => {
  ...
  await expect
    .poll(() => (bulkBody as { agentIds?: string[] } | undefined)?.agentIds?.length)
    .toBe(2);
...
test('run manager does not fetch default run when the control server has no runs', async ({
  page,
}) => {
...
test('shows explicit failed feedback and expanded Rallar trace payload for REST errors', async ({
  page,
}) => {
  await page.route('http://localhost:8080/api/config', async (route) => {
    await route.fulfill({
...
test('runs a Rallar Server REST collection with assertions and extraction', async ({ page }) => {
  ...
  const serverPanel = page.locator('#panel-rallar-server');
  await serverPanel.getByLabel('Variables JSON').fill(
    JSON.stringify(
      {
  ...
  await serverPanel.getByLabel('Collection JSON').fill(
    JSON.stringify(
      {
...
test('runs auth command-center actions with redacted session output', async ({ page }) => {
  ...
  await page.goto('/?provider=simulated&apiBaseUrl=http%3A%2F%2Flocalhost%3A8080&tab=auth');
...
test('refreshes rooms and clients state with authenticated REST evidence', async ({ page }) => {
  ...
  await page.route('http://localhost:8080/api/state/apps/**', async (route) => {
    ...
    const body =
      pathname.includes('/clients/') && pathname.includes('/events')
        ? clientEvents
  ...
  await expect(page.getByRole('tab', { name: 'Groups/Clients' })).toHaveAttribute(
    'aria-selected',
    'true',
  );
  ...
  await expect(runState.locator('.metric').filter({ hasText: 'Session' })).toContainText(
    'alice-session',
  );
  ...
  await expect(panel.locator('.metric').filter({ hasText: 'Current client member' })).toContainText(
    'yes',
  );
  await expect(panel.locator('.metric').filter({ hasText: 'Other browser visible' })).toContainText(
    'no',
  );
  ...
  await expect(websocketPanel.getByLabel('WebSocket route preview')).toContainText(
    'Group bb-group',
  );
  await expect(
    websocketPanel.getByRole('button', { name: 'Send JSON to group bb-group' }),
  ).toBeEnabled();
});
...
test('surfaces browser-rallar signaling and RTC connection status', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Show details' }).click();
  await expect(
    page.locator('.run-header').locator('.metric').filter({ hasText: 'Runtime' }),
  ...
  await expect(websocketPanel.getByLabel('WebSocket route preview')).toContainText('Group awesome');
  await websocketPanel
    .getByRole('textbox', { name: 'Type ID', exact: true })
    .fill('room.black-box.ws.probe');
  await websocketPanel
    .getByRole('textbox', { name: 'Topic ID', exact: true })
    .fill('room.black-box.ws.probe');
  await websocketPanel.getByLabel('Payload JSON').fill(
    JSON.stringify(
      {
  ...
  await expect
    .poll(async () => page.evaluate(() => (window as any).__lastRallarWsSend))
    .not.toBeUndefined();
...
test('keeps WebSocket Rallar actions direct-only in simulated mode', async ({ page }) => {
  ...
  await expect(page.getByRole('tab', { name: 'WebSocket' })).toHaveAttribute(
    'aria-selected',
    'true',
  );
  ...
  await panel.getByLabel('Payload JSON').fill(
    JSON.stringify(
      {
        kind: 'playwright-ws',
...
test('wraps room-scoped WebSocket group messages and validates group configuration', async ({
  page,
}) => {
  ...
  await expect(panel.getByRole('textbox', { name: 'Context ID', exact: true })).toHaveValue(
    'bb-group',
  );
  await expect(panel.getByRole('textbox', { name: 'Type ID', exact: true })).toHaveValue(
    'room.manual.message',
  );
  await expect(panel.getByRole('textbox', { name: 'Topic ID', exact: true })).toHaveValue(
    'room.manual.message',
  );
  await expect(panel.getByRole('textbox', { name: 'Context ID', exact: true })).toHaveValue(
    'bb-group',
  ...
  await expect(receivedPanel).toContainText('No received WebSocket messages');
  await expect(
    panel.locator('.websocket-status-grid .metric').filter({ hasText: 'Group' }),
  ).toContainText('bb-group');
  await expect(
    panel.locator('.websocket-status-grid .metric').filter({ hasText: 'Selector' }),
  ).toContainText('room.manual.message / room.manual.message');
});

test('runs the RTC delivery matrix with scoped addressing and NACK diagnostics', async ({
  page,
}) => {
  await page.goto('/?provider=simulated&tab=manual-rallar');
  ...
  await panel.getByLabel('Room Ref JSON').fill(
    JSON.stringify({
      type: 'group',
  ...
  await expect(topologyPanel.locator('.metric').filter({ hasText: 'Route cmds' })).toContainText(
    /Route cmds(?:[6-9]|\d{2,})/,
  );
  await expect(topologyPanel.locator('.metric').filter({ hasText: 'RTC routes' })).toContainText(
    /RTC routes(?:[6-9]|\d{2,})/,
  );
...
test('builds and runs a command-center flow', async ({ page }) => {
  ...
  await panel.getByLabel('Variables JSON').fill(
    JSON.stringify(
      {
...
test('shows distributed recipe composite preflight before staging', async ({ page }) => {
  ...
  await page.goto(
    '/?provider=simulated&workspace=black-box-runner&tab=distributed-recipes&roomId=bb-group',
  );
  ...
  await authoring
    ...
    .selectOption('rtc-realtime-position');
  await expect(authoring.getByLabel('Prompt template preview')).toContainText(
    'Template: RTC Position Stream',
  );
  ...
  await authoring
    ...
    .fill(JSON.stringify(generatedManifest, null, 2));
  ...
  await expect(rtcRow.getByLabel('Recipe execution tree')).toContainText(
    'rtc-realtime-position-loop',
  );

  await catalog.getByLabel('Search').fill('composite');
  const compositeRow = catalog
    .locator('.distributed-recipe-row')
...
test('uses fresh world-fleet target previews after loading an older distributed run', async ({
  page,
}) => {
  const run = {
    runId: 'demo-run',
  ...
  await panel
    .locator('.distributed-run-list .distributed-run-row')
...
test('shows distributed WS and RTC runtime diagnostics in the run monitor', async ({ page }) => {
  ...
  await page.route('http://localhost:5180/**', async (route) => {
    ...
    if (
      request.method() === 'GET' &&
      url.pathname === '/distributed-runs/dist-diagnostics/artifacts'
    ) {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  await expect(runsPanel.locator('.runner-analysis-report')).toContainText(
    'Distributed command failed',
  );
  await expect(runsPanel.locator('.runner-analysis-report')).toContainText(
    'Open the composite drilldown',
  );
...
test('restores selected tab and redacted UI drafts after a fresh load', async ({ page }) => {
  ...
  await requestEditors.getByLabel('Headers JSON').fill(
    JSON.stringify({
      authorization: 'Bearer header-secret',
    }),
  );
  await requestEditors.getByLabel('Body JSON').fill(
    JSON.stringify({
  ...
  await manualPanel.getByLabel('Payload JSON').fill(
    JSON.stringify({
  ...
  await expect(
    page.locator('#panel-event-stream').getByRole('button', { name: 'message', exact: true }),
