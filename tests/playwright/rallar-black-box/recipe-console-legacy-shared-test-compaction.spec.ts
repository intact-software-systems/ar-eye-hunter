...
test(
    ...
    async ({ context, page }) => {
        ...
        for (const [index, expectedRange] of expectedRanges.entries()) {
            ...
            visitedNames.push(...await rows.locator(
                '[data-compaction-summary-name]',
            ).allTextContents());
            visitedOrdinals.push(...await rows.evaluateAll(elements =>
        ...
        await expect(page.locator('#panel-advanced #panel-shared-test'))
            .toBeVisible();
        await expect(page.locator('#panel-advanced').getByRole('button', {
            name: 'Shared Test',
            exact: true,
        })).toHaveClass(/selected/);
        expect(mutations).toEqual([]);
...
function sharedTestArtifactFiles() {
    const file = (name: string, contents?: string) => ({
        ...
        mimeType: name.endsWith('.jsonl')
            ...
            : 'application/json',
        buffer: Buffer.from(contents ?? readFileSync(
