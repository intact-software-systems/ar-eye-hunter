...
test('saves, restores, resets, and closes personal defaults from the command bar', async ({ context, page }) => {
...
test('logs an authenticated operator out through the visible account control', async ({ context, page }) => {
    await installControlMock(context);
