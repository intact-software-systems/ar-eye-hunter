...
const PRODUCTION_TUNE_URL = 'http://127.0.0.1:4176/?provider=simulated&v=1&experience=recipe-console&view=tune';

test('browses all 5,000 History pairs with bounded keyboard-safe windows', async ({ browser }) => {
    ...
    expect(
        await controls.evaluate(
            (node, scrollNode) => !(scrollNode as HTMLElement).contains(node),
...
test('keeps History touch controls and table containment intact in mobile layouts', async ({ browser }) => {
