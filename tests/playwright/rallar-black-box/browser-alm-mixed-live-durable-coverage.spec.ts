...
test('rejects contradictory durable admission and rate-limit evidence', () => {
    const input = validCoverageInput();
    input.durableSends = [
        { msgId: 'admitted', waitStatus: 'settled', state: 'queued', admittedDurable: true, rateLimited: false },
        { msgId: 'contradictory', waitStatus: 'settled', state: 'queued', admittedDurable: true, rateLimited: true }
    ];
...
test('accepts durable admission beside a clean rate-limited identity', () => {
    const input = validCoverageInput();
    const coverage = evaluateMixedLiveDurableCoverage(input);
...
function validCoverageInput(): MutableCoverageInput {
    ...
    return {
        ...
        durableSends: [
            { msgId: 'admitted', waitStatus: 'settled', state: 'queued', admittedDurable: true, rateLimited: false },
            { msgId: 'refused', waitStatus: 'settled', state: 'failed', admittedDurable: null, rateLimited: true }
        ],
