...
| `RALLAR_BLACK_BOX_CONTROL_TOKEN` | No | None | Bearer token used by headless workers for browser-agent registration and Node-side control-server read polling when read tokens are required. |
