...
| `RALLAR_GROUP_STATE_DISSEMINATION`           | No       | `dual-emit` | Per-change group-state WS emission. `snapshot-per-change` retains the bare event row plus both full-snapshot rows. `dual-emit` puts the delta envelope with its persisted audience on the event row and keeps both snapshot rows as the divergence oracle. `delta-primary` emits only the envelope row and drops both per-change snapshot rows. `RALLAR_GROUP_FORMATION_DAMPING=legacy` forces `snapshot-per-change` regardless of this value so the issue-156 pinned replay proof keeps bit-for-bit legacy emission. |
...
| `RALLAR_RTC_TOPOLOGY_GLOBAL_GRAPH_RECOMPUTE_WINDOW_MS` | No | `5000` | Sliding window for the global-graph RTT recompute rate limit. The debounce above only coalesces a burst; this caps how often a sustained RTT stream can trigger the synchronous all-pairs rebuild over every active session. A denied recompute re-arms rather than being dropped. Positive integer; only applied when the per-window count below is also set. |
| `RALLAR_RTC_TOPOLOGY_GLOBAL_GRAPH_RECOMPUTES_PER_WINDOW` | No | `2` | Maximum global-graph RTT recomputes allowed per window. Positive integer; only applied when the window above is also set. |
