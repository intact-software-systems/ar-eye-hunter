...
| `RALLAR_GROUP_STATE_DISSEMINATION`           | No       | `delta-primary` | Per-change group-state WS emission. `delta-primary` emits only the delta envelope row, with its persisted audience, and drops both per-change snapshot rows. `dual-emit` emits the same envelope row but also keeps both snapshot rows, which is the rollback setting and the divergence oracle. `snapshot-per-change` retains the pre-delta shape: a bare event row plus both snapshot rows. Independent of `RALLAR_GROUP_FORMATION_DAMPING`. |
