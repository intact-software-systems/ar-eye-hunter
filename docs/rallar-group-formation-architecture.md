...
_petition_ a transition by enqueueing a command. That command re-authorizes through AppInbox against
fresh state, so a stale petition is an idempotent replay or a typed rejection, never a wrong
transition. The six-state RTC activation status projection from the design documents
(`INACTIVE … FAILED`) is not implemented; only the readiness fraction exists.
...
`reconfiguring` is a distinct state, not `establishing` with a flag: a group that was active and is
repairing its overlay has the opposite meaning from one that has never been active under
`blocked-until-active` data policy, and the data gate reads the difference directly.
...
| `server-auto` | No principal, ever (`forbidden-role`). It appears only in `drop-in-social`, whose `immediate` formation never phases.  |
...
`failGroupFormation`, and `startGroupEstablishment`, and which must carry an `observedRate`.
...
in every state, including a group that never leaves `forming`
(`packages/tests/shared-server/group-state/group-lifecycle-safety-baseline.test.ts`). Phases gate
establishment work and the app-visible `active` signal, never the ability to be in the group.
...
then gets `status: 'absent'` and substitutes `createDefaultGroupLifecyclePolicy()`, which is the
`optimistic` preset. The equivalence is pinned once, not per tier: `api-v1-group-lifecycle-policy`
...
| `manager-count-exceeds-assigned-principals` | `count` larger than the assigned list                      |
...
(`assertGroupAdmissionWindowsOpen` runs with `invited: true`). Inviting a pending principal moves
their row to `invited`, so their accept admits instead of re-parking.
...
pure function the transitions use. Governance is not widened — an owner who is not a manager gets
`forbidden-role` (`nonManagerGrantIsForbidden`), a zero-manager group gets
...
and the admission windows are re-checked at grant time, not at parking time, and the row moves
...
**Zero-manager recovery is governance's own consent channel.** A `managed` group whose assigned
manager has not joined parks every join and can grant none of them; the owner invites the parked
...
| `elected-random-deterministic` | the electorate sorted by `rendezvousScore('epoch:<formationEpoch>', principalId, groupKey)`                                   |
...
admission grants — never group safety. `api-v1-group-manager-succession` pins the role under an
`assigned` policy: the non-manager owner is denied (`forbidden-role`), the assigned manager
...
(durable mode: `RtcRttRepository.listMeasurementsForSessionIds`; in-memory mode: the process RTT
repository). Only planned edges count, so evidence accepted for another room the pair shares is
...
next attempt under backoff: `computeFormationRetryBackoffMs(n) = min(5 000 × n, 60 000)` for attempt
`n`. When the attempts are exhausted the group rests in `forming`. That rest is not terminal: the
...
unchanged-graph gate. It self-throttles at zero evidence and fires with zero lag when evidence
changes.

**The threshold edge-trigger.** RTT-refresh work whose replan the refinement gate defers
(`RtcRttRefinementGate`: a replan only when accumulated Vivaldi movement crosses 5 ms and the
per-group 30 s floor has elapsed) does not compute a plan, so it would not petition — and under a
...
conclusion produce one transition and one replay. A petition that arrives after the group moved on
is a `lifecycle-transition-invalid` rejection the mutation compute absorbs.
...
`api-v1-group-formation-criterion` pins both legs at two principals: a `threshold` group
auto-activates from the evidence its own establishment planning pass produces (epoch 2,
`outcome: 'activated'`, rate 1); a `deadline` group with `minimumViableRate: 1` and two
presence-connected members fails below the floor at its 3 s deadline (`forming`, attempt count 1,
`outcome: 'below-floor'`, rate 0); a `deadline` group with floor 0 activates degraded at the
deadline; and a `threshold-or-deadline` group holds at `observedEdgeCount: 0` until a single RTT
report over WS on its planned edge activates it (`thresholdActivatesOnObservation`). The managed
...
stalled. Today both acceptance paths — the durable AppInbox RTT mutation composed in
`apps/api-v1/src/composition/create-api-v1-topology-services.ts` and the in-memory topic path
through the `readGroupRttReportingDegreeLimit` hook in `ws-system-topics.ts` — resolve the limit
exactly as the read-side planning filter does: the group's effective topology configuration under
the server reporting option (`readRttReportingDegreeLimit`), which means a configured
`RALLAR_RTC_RTT_REPORTING_DEGREE_LIMIT` still wins and otherwise the group's effective `degreeLimit`
is the limit. A report whose endpoints share several groups is accepted under the largest of those
...
The CRDT live topics `room.crdt` and `app.crdt` are exempt by name at the authorizer. CRDT authority
is the AppInbox append path, the topic only fans out committed updates, and collaborative documents
are lobby-phase workspace, not the competitive pre-match traffic the gate exists to hold back. Out
of scope and unchanged: RTC data-channel traffic (`realtime.room` is peer-to-peer; the server only
signals) and the reserved presence, state-sync, signaling, and RTT topics, which activation needs.

On the wire the denial is the generic NACK: the room authorizer maps every policy denial to reason
`unauthorized`, and `group-data-blocked-until-active` is typed on the HTTP surface only.
`api-v1-group-data-policy` pins it end to end: a `room.match` send before activation is NACKed
`unauthorized` and never reaches the other member, a `room.crdt` sync request flows while blocked,
`allowed` and absent-policy groups relay during `forming`, and the same message flows after the
manager activates. `api-v1-match-preset` pins the lobby NACK and the post-activation flow composed
with the rest of the preset; `api-v1-drop-in-social-preset` pins data flowing from birth.
...
Browser code reads them from the group snapshots it already receives; there is no dedicated browser
facade operation for the lifecycle.
...
Policy denials are typed `403 { type: 'api-mutation-failure', code, status }` with the
`GroupPolicyReasonCode` (`forbidden-role`, `lifecycle-manager-unavailable`,
`lifecycle-transition-invalid`, `group-admission-closed`, `group-admission-deadline-passed`,
`group-admission-capacity-reached`, `member-not-active`, …). Policy-validity rejections at creation
are the generic `400 app-inbox-malformed-command`. Over WS, every policy denial is the generic NACK
reason `unauthorized`.
...
evidence leg and the edge-trigger), and
...
| `api-v1-group-lifecycle-transitions` | the four transitions with epochs, FORMING holds planning, non-manager and illegal-transition denials |
...
'activated'`). Their deadlines sit at the 600 s clamp with sub-minute activation polls, which is
what pins the edge-trigger structurally: activation has to come from evidence, not from the
deadline evaluation.
...
- Planned mesh edges grow linearly, not quadratically: the planner's mesh is k-regular
  (`meshParamK` default `2`, with rendezvous fill), so the plan carried 37 edges at 20 sessions and
  97 at 50. The tiers still exercise planning, acceptance, and readiness bookkeeping at those
  session counts, and all-pairs _reporting_ remains the coverage guarantee.
...
- **Typed WS NACK reasons.** Every policy denial over WS is `unauthorized`; the admission and data
  codes are HTTP-only.
...
- `packages/shared-server/rallar-system/topology/replay/compute-formation-criterion-command.ts`,
  ...
  evidence leg with the damped edge-trigger, and the time leg.
