...
| `connect-names-its-layout` | `packages/tests/shared/expected-layout-fence.test.ts` and `packages/tests/shared-server/rallar-system/group-state/mutation/group-lifecycle-mutation.test.ts`; `api-v1-group-admission-approval.json` drives the stale-epoch conflict, no recipe drives the two layout denials |
