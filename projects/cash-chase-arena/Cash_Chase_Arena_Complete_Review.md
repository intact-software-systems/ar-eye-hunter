...
2. **“Star topology” is not currently a director-centered physical star.** Rallar's `star` planner connects every active session to every other session (`packages/shared-server/rallar-system/services/rallar-rtc-topology-service.ts:578-586`). Rallar Game still routes inputs specifically to the director (`packages/shared-web/game/match.ts:577-603`), so authority traffic is super-peer-shaped while connectivity is full-mesh-shaped. The documents must either accept this for 2–8 players or define a new generic Rallar director-star topology.
...
| C20 | Rallar AI integration shape is underspecified.                                                                         | Browser AI is a separate `createRallarBrowserAi` facade (`packages/shared-web/browser/rallar-ai.ts:68-98`); server AI is `createRallarServerAi` (`packages/shared-server/rallar-ai/RallarAiServer.ts:161,204`).                                                                                         | Name the concrete surface in each flow. Production content proposals run server-side; browser AI remains local creator/debug-only and lazy-loaded.                                                                       |
...
| Rooms, membership, governance, invites, event replay | **Fully provided**                            | `rallar.rooms` at `rallar.ts:1595-1687`; `createAndSwitch`, `session`, and `waitForPresence` at `rallar-rooms-facade.ts:36-126`.                                                                                                                       | Room naming/filtering, ready state payload, invite-link route.                              |
...
| Typed reliable messages and RTC→WS fallback          | **Fully provided**                            | `rallar.messages.room<T>` at `rallar-messages-facade.ts:12-53`; fallback behavior tested in `rallar-message-channel-compat.test.ts:288-379`.                                                                                                           | Define topic/type IDs and validation. Do not build a custom channel manager.                |
...
| AI proposal contracts/governance                     | **Fully provided as a platform**              | Shared schema, hashing, validation, provider policy, diagnostics, lifecycle, dedupe, replay, mock/evaluation exports at `packages/shared/rallar-ai/mod.ts:1-19`; browser facade at `rallar-ai.ts:68-98`; server facade at `RallarAiServer.ts:161-204`. | CCA schemas, prompts, domain validators, content policy, deadlines, deterministic fallback. |
...
| Director-centered physical RTC star                  | **Partially provided / semantic mismatch**    | Rallar supports configurable `star/tree/mesh`, but current `star` maps every peer to all peers at `rallar-rtc-topology-service.ts:578-586`.                                                                                                            | Accept full connectivity for small MVP or add a generic director-star topology to Rallar.   |
