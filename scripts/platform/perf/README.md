# Platform performance validation scripts

These scripts preserve the reusable shared-server harnesses created during the
...
The Deno harnesses use `apps/api-v1/deno.json` for import aliases such as
...
```sh
node scripts/platform/perf/summarize-runtime-results.mjs \
...
docker compose exec -T postgres psql -U app -d appdb \
  < scripts/platform/perf/seed-perf-db.sql
```
...
```sh
docker compose exec -T postgres psql -U app -d appdb \
  < scripts/platform/perf/explain-perf-db.sql \
...
```sh
docker compose exec -T postgres psql -U app -d appdb \
  < scripts/platform/perf/seed-perf-db-sparse-queue.sql \
