...
export function readAuthenticatedPlanGovernanceChanges(governanceInput) {
    const receiptChanges = governanceInput.changes.filter((change) => governanceReceiptPattern.test(change.path));
...
function readAuthenticatedNonPlanDecision(governanceInput, verification) {
    ...
    if (
        !governanceInput.changes.some(
            (change) => change.path === verification.receiptPath || change.oldPath === verification.receiptPath
        )
    ) {
        throw new Error(
