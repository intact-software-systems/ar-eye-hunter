...
function validateContinueDecision(issues, checkpoint, record) {
    ...
    const unrecoverableFailure = record.freshStructuralReview?.status === 'failed' ||
        failures.some(
            (failure) => ['navigation', 'ownership'].includes(failure?.kind) && failure.recoverable === false
        );
...
function validateConsolidationDecision(issues, checkpoint, record) {
    ...
    const completedCurrentConsolidation = consolidated &&
    ...
    const referencedConsolidation = Number.isInteger(consolidationDecisionIndex) &&
