...
function collectCodeContractOwnershipConflicts(input) {
  const { issues, capabilities, capability, capabilityIndex, contractPath, reportedConflicts } =
    input;
  for (const [otherIndex, otherCapability] of capabilities.entries()) {
    ...
    const conflictKey =
      [capability.owner, otherCapability.owner].sort().join('\0') + `\0${contractPath}`;
...
function selectContractConflictDeclarer(capability, otherCapability, contractPath) {
  const otherDeclaresSameContract =
    Array.isArray(otherCapability.contractPaths) &&
