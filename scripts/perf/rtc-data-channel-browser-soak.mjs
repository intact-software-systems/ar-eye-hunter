...
const out = readArg(
  'out',
  'tmp/perf/results/rtc-data-channel-browser-soak.json',
);
...
try {
  ...
  const soak = await page.evaluate(async (iterationCount) => {
    ...
    async function runIteration(index) {
      ...
      channelA.close();
      await waitFor(
        () => channelA.readyState === 'closed' && channelB?.readyState === 'closed',
      );
      pcA.close();
      pcB.close();
    ...
    return {
      ...
      closedCount: results.filter((result) =>
        result.localState === 'closed' && result.remoteState === 'closed'
      ).length,
      localErrorCount: results.filter((result) =>
        result.events.includes('local-error')
      ).length,
      remoteErrorCount: results.filter((result) =>
        result.events.includes('remote-error')
      ).length,
    };
  }, iterations);
  ...
  const output = {
    ...
    heap: {
      ...
      deltaBytes: heapBefore === undefined || heapAfter === undefined
        ? undefined
