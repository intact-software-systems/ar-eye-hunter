...
function readCanonicalMessageId(value: JsonWireValue): string | undefined {
  if (!isJsonWireObject(value)) return undefined;
  const id = value.id;
  if (!isJsonWireObject(id)) return undefined;
...
function findCommandIds(value: JsonWireValue): string[] {
  ...
  if (!isJsonWireObject(value)) return [];
  return [
...
export function computeProductionOutboxEvidence({
  ...
  return records.flatMap((record) => {
    const commandId =
      record.canonicalCommandId === undefined
        ? undefined
    ...
    const effectId =
      receipt?.identityKind === 'logical-msg-id' ? record.outboxId : record.resourceId;
