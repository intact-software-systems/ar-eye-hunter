...
export function createBenchmarkAuthSession(
  ...
  const scopeIdentity = `${encodeURIComponent(scope.applicationId)}:${encodeURIComponent(
    scope.workspaceId,
  )}`;
...
async function main(): Promise<void> {
  ...
  const reasonsText =
    options.regressionReasonsFile === undefined
      ? undefined
  ...
  return commands
    ...
    .filter((command) => {
      const expectedOperations =
        command.kind === 'profile-instance' ? ['instance', 'profile'] : ['command'];
