...
const OUT = readArg('--out') ??
    'tmp/perf/results/rtc-rtt-group-scan.json';
...
const targetSessions = targetGroup.activeSessions.map((session) =>
    session.sessionId
);
const rtts = Array.from({ length: RTTS }, (_, index) => ({
...
const maybeIndexedFinder = (groupStateSnapshotsRepository as unknown as {
...
function createGroups(
    ...
    return Array.from({ length: groupCount }, (_, groupIndex) => {
        const sessionIds = Array.from(
            ...
            (_unused, sessionIndex) =>
                `session-${groupIndex}-${sessionIndex}`,
        );
