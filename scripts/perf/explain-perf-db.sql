...
\echo 'runtime_state_store broad prefix'
EXPLAIN (ANALYZE, BUFFERS)
SELECT store_key, store_value
FROM runtime_state_store
WHERE store_namespace = 'perf-runtime-20260702'
  AND store_key LIKE 'prefix:%'
ORDER BY store_key;

\echo 'runtime_state_store prefix page'
EXPLAIN (ANALYZE, BUFFERS)
SELECT store_key, store_value
FROM runtime_state_store
WHERE store_namespace = 'perf-runtime-20260702'
...
\echo 'client_state_events legacy full list'
EXPLAIN (ANALYZE, BUFFERS)
SELECT event_json
FROM client_state_events
WHERE application_id = 'perf-app'
  AND workspace_key = 'perf-workspace'
  AND principal_id = 'perf-principal'
ORDER BY application_id, workspace_key, principal_id, snapshot_version, occurred_at_epoch_ms, event_id;

\echo 'client_state_events paged list'
EXPLAIN (ANALYZE, BUFFERS)
SELECT event_json
FROM client_state_events
WHERE application_id = 'perf-app'
...
\echo 'resource_inbox runnable new/retry'
EXPLAIN (ANALYZE, BUFFERS)
SELECT *
FROM resource_inbox
WHERE ri_type_id IN ('PERF_TYPE')
...
\echo 'resource_inbox reserved timeout'
EXPLAIN (ANALYZE, BUFFERS)
SELECT *
FROM resource_inbox
WHERE ri_type_id IN ('PERF_TYPE')
...
\echo 'app_data_store broad prefix'
EXPLAIN (ANALYZE, BUFFERS)
