...
export async function runRtcTopologyStarAcceptedSamples(input: {
  ...
  for (let index = 0; index < input.worker.sampleIds.length; index += 1) {
    ...
    const issues = validateResult(input.worker.input.sessions, result);
    if (issues.length > 0) {
...
function createExpectedSampleIds(
  ...
  const prefix =
    `rtc-b03-topology-star-sessions-${sessions}-${phase}-` + String(outerOrdinal).padStart(3, '0');
  return Array.from(
    { length: 5 },
...
function createIdentity(
  worker: RtcTopologyStarAcceptedArguments,
  index: number,
...
function validateResult(sessions: number, result: RtcTopologyStarResult) {
  ...
  const validEdges =
    edgeIdentities.length === expectedEdges &&
