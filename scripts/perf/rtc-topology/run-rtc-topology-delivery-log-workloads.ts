...
async function runDuplicatePublicationRace(
  database: BenchmarkSql,
): Promise<WorkloadResult> {
...
async function executeConcurrently(
  ...
  const results = await Promise.all(
    Array.from(
      { length: RTC_TOPOLOGY_DELIVERY_LOG_BENCHMARK_POLICY.concurrency },
      async () => {
...
async function retryDuplicateLoser(
  ...
  try {
    ...
    if (!(error instanceof Error) || !isRtcTopologyDeliveryRetryableConflict(error)) throw error;
    const retry = await input.database.begin(
      async (transaction) =>
        await input.repository.appendOrValidate(transaction, input.append),
    );
...
function appendInput(
  ...
  return {
    ...
    retainUntilEpochMs:
      Date.now() + RTC_TOPOLOGY_DELIVERY_LOG_BENCHMARK_POLICY.retentionMs,
  };
}
...
async function verifyStreams(
  ...
  const heads = await database<
    Readonly<{ stream_id: string; head_sequence: number }>[]
  >`
