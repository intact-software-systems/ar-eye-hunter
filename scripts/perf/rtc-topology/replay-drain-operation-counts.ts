...
export async function runRtcTopologyReplayDrainOperationWorkloads(): Promise<
  RtcTopologyReplayDrainOperationArtifact
> {
  const workloads = {
...
function readOutputPath(args: readonly string[]): string {
  ...
  for (const argument of args) {
    ...
    else throw new TypeError(`Unsupported RTC topology replay drain option: ${argument}`);
  }
  if (!outPath.trim()) throw new TypeError('RTC topology replay drain output must be non-empty');
  return outPath;
}
