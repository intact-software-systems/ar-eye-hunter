...
function pooledResourceMean(workload, descriptor) {
  return mean(
    workload.samples.map((sample) =>
      toNonNegativeNumber(sample[descriptor.owner]?.[descriptor.metric]),
    ),
  );
}
...
function validatePooledArtifact(artifact, expectedRole) {
  ...
  const aggregationKeys =
    aggregation && typeof aggregation === 'object' ? Object.keys(aggregation).sort() : [];
  if (
