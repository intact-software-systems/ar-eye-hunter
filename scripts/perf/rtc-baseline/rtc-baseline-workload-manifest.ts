...
import { decodeRtcBaselineManifest } from './rtc-baseline-artifact-decoding.ts';
...
export function deriveRtcBaselineCaptureManifest(
  ...
  for (const { workload, entry } of cases) {
    ...
    const retained =
      (entry.retainedOuterAttempts ?? workload.retainedOuterAttempts) *
      request.retainedSampleMultiplier;
    for (const [intendedPhase, count] of [
      ['warmup', warmups],
      ['retained', retained],
    ] as const) {
...
export function validateRtcBaselineStoredOutcomeReconciliation(input: {
  ...
  const compare = (actual: object, expected: object, problem: RtcBaselineIssueDto) => {
    if (JSON.stringify(actual) !== JSON.stringify(expected)) issues.push(problem);
  };
  ...
  if (!storedRaw.ok) issues.push(...storedRaw.issues);
  if (!summaryRaw.ok) issues.push(...summaryRaw.issues);
...
export function locateRtcBaselineExternalAttempt(
  ...
  locator: Partial<
    Pick<
      RtcBaselineAttemptLocatorDto,
      'workloadId' | 'caseId' | 'inputKey' | 'intendedPhase' | 'outerOrdinal'
    >
  >,
) {
  for (const field of [
