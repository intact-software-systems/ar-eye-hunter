...
const syntheticPrefix = ['run', '--config=apps/api-v1/deno.json', '--allow-read', '--allow-write'];
...
function syntheticCase(
  ...
  return {
    ...
    configPaths: ['apps/api-v1/deno.json'],
    configuration: fields.map(([field, flag, kind, value]) =>
      descriptor(caseKey, field, flag, kind, value),
    ),
    ...(attempts === undefined
      ? {}
...
const b03Cases = [
  ...sessionCases('topology-star', 'scripts/perf/rtc-topology-star-bench.ts'),
  ...sessionCases('topology-tree', 'scripts/perf/rtc-topology-tree-no-rtt-bench.ts', [
    ['degreeLimit', '--rtc-degree-limit', 'nonnegative-integer', 5],
  ]),
  ...sessionCases('topology-mesh', 'scripts/perf/rtc-topology-mesh-no-rtt-bench.ts', [
    ['meshParamK', '--rtc-mesh-param-k', 'nonnegative-integer', 2],
  ]),
  ...sessionCases('room-graph-rtt-sparse', 'scripts/perf/rtc-room-graph-rtt-bench.ts', [
    ['sparseDegree', '--rtc-sparse-degree', 'nonnegative-integer', 4],
  ]),
  ...sessionCases('room-graph-rtt-complete', 'scripts/perf/rtc-room-graph-rtt-bench.ts'),
  ...repositoryCases,
...
const b05Cases: Case[] = [
  {
    ...
    sourcePaths: ['scripts/perf/rtc-data-channel-browser-soak.mjs'],
