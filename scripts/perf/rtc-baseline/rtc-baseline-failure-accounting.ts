...
export { RTC_BASELINE_ACCEPTED_ARTIFACT_DIRECTORIES } from './rtc-baseline-contracts.ts';
export type {
  RtcBaselineAcceptedArtifact,
  RtcBaselineFailureArtifact,
...
export function decodeRtcBaselineFailureOutcome(
  ...
  const issues: RtcBaselineIssueDto[] = Object.keys(value)
    ...
    .map((field) =>
      rtcBaselineIssue(`$.${field}`, 'unexpected-field', `Field ${field} is not allowed.`),
    );
  const artifactKind =
    value.artifactKind === 'failure' || value.artifactKind === 'not-run'
      ? value.artifactKind
      : null;
  ...
  const failureId =
    typeof value.failureId === 'string' && value.failureId.length > 0 ? value.failureId : null;
  if (failureId === null)
  ...
  if (!decodedIssues.ok) issues.push(...decodedIssues.issues);
  ...
  if (identity.ok && failureId !== null) {
    const identityId =
      'sampleId' in identity.value ? identity.value.sampleId : identity.value.cohortId;
  ...
  if (artifactKind === 'not-run') {
    if (!('sampleId' in identity.value) || causalFailureId === null) return { ok: false, issues };
    return {
      ok: true,
...
export function resolveRtcBaselineAcceptedArtifactPath(
  ...
  if ('artifactKind' in artifact) {
    const identity =
      'sampleId' in artifact.identity ? artifact.identity.sampleId : artifact.identity.cohortId;
...
export function buildRtcBaselineFailureSequence(
  ...
  if (input.owner.kind !== 'sample' || !input.manifest) return [failure];
  ...
  const ownerIndex = identities.findIndex((identity) =>
    rtcBaselineSampleIdentityEquals(identity, ownerIdentity),
  );
  return [
    failure,
    ...identities
