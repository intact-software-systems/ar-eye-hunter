...
export function decodeRtcBaselineConditionalEnvironmentDecision(
  ...
  const decision =
    value.decision === 'required' || value.decision === 'not-required' ? value.decision : null;
  if (!decision)
    issues.push(
...
export function decodeRtcBaselineCaptureRequest(
  ...
  if (!Array.isArray(value.workloadIds)) {
    ...
    for (let index = 0; index < requestedWorkloads.length; index += 1) {
      ...
      if (workload) seen.add(workload);
    }
  }
  decodedEnvironmentId =
    environments.find((candidate) => candidate === value.environmentId) ?? null;
  ...
  if (value.repeatLink !== null) {
    ...
    else decodedRepeatLink = decoded.value;
  }
  if (!Array.isArray(value.conditionalEnvironmentDecisions)) {
    ...
    value.conditionalEnvironmentDecisions.forEach((entry, index) => {
      ...
      if (!decoded.ok) issues.push(...decoded.issues);
      else decodedDecisions.push(decoded.value);
    });
  }
  issues.push(
...
export function decodeRtcBaselineSampleIdentity(
  ...
  const decodedPhase =
    value.intendedPhase === 'warmup' || value.intendedPhase === 'retained'
      ? value.intendedPhase
      : null;
...
export function decodeRtcBaselineFailureIdentity(value: RtcBaselineJson | undefined) {
  ...
  if ('sampleId' in value) return decodeRtcBaselineSampleIdentity(value, '$.identity');
  if ('cohortId' in value) return decodeRtcBaselineCohortIdentity(value, '$.identity');
  return {
    ok: false as const,
    issues: [issue('$.identity', 'expected-object', 'Expected an identity object.')],
  };
...
export function decodeRtcBaselineIssueArray(value: RtcBaselineJson | undefined) {
  ...
  value.forEach((entry, index) => {
    ...
    if (result.ok) decoded.push(result.value);
