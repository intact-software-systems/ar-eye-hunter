...
import { configureTestCacheRepositories } from '../../../packages/tests/cache-repository-config.ts';
...
function parseArgs(): Args {
    const readValue = (name: string, fallback: string): string => {
        const prefix = `--${name}=`;
        return Deno.args.find((arg) => arg.startsWith(prefix))
    ...
    return {
        ...
        out: readValue(
            'out',
            'tmp/perf/results/rtc-topology-rtt-traffic-metrics.json',
        ),
    };
}
...
clientStateSnapshotsRepository.setClientStateSnapshots(
    sessionIds.map(createClientSnapshot),
);
