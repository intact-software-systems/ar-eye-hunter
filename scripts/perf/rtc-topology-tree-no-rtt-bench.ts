...
const OUT = readArg('--out') ??
    'tmp/perf/results/rtc-topology-tree-no-rtt.json';
const SESSIONS = Number(readArg('--sessions') ?? '300');
const RUNS = Number(readArg('--runs') ?? '5');
const DEGREE_LIMIT = Number(readArg('--degree-limit') ?? '5');
...
for (let run = 1; run <= RUNS; run++) {
    ...
    results.push({
        ...
        nextHopEdgeCount,
        sampleNextHops: result.snapshot.nextHopsBySessionId['peer-150'] ?? [],
    });
}
