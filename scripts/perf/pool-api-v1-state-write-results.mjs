  ...
  for (const role of ['approved-base', 'candidate']) {
    ...
    for (const workloadName of ['uncontended', 'shared', 'hot']) {
      const commandIds = roleSources.flatMap((source) => {
        ...
        return workload.samples.flatMap((sample) =>
          sample.commands.map((command) => command.commandId),
        );
      });
...
function countAttempts(samples, outcome) {
  return sum(
    samples.map(
      (sample) =>
        sample.attemptObservations.filter((attempt) => attempt.outcome === outcome).length,
    ),
  );
}
