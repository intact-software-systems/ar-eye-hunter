...
function validTopologyBinding(binding, topology) {
  if (!topology) {
    return [
      'acceptedVersion', 'operation', 'target', 'acceptedStorageRevision',
      'acceptedCreatedAtEpochMs', 'acceptedUpdatedAtEpochMs',
      'acceptedExpiresAtEpochMs', 'acceptedConfig', 'acceptedCausalRevision', 'outboxId',
  ...
  const expectedOutboxId = !validCausal ? null : [
    binding.receiptId, 'rtc-topology-recompute', 'group-revision',
...
function matchesTopologyResult(commandType, result, binding) {
  ...
  if (!shape || binding.operation !== shape[0] ||
    ...
    !matchesTopologyReceipt(result.receipt, binding)) return false;
  return shape[1] === null || matchesStoredTopology(result[shape[1]], binding, shape[1]);
}

const TOPOLOGY_RECEIPT_KEYS = [
  'commandId', 'requestId', 'commandHash', 'operation', 'outcome', 'attemptCount',
  'groupRef', 'target', 'acceptedVersion', 'acceptedStorageRevision',
  'acceptedCreatedAtEpochMs', 'acceptedUpdatedAtEpochMs', 'acceptedExpiresAtEpochMs',
  'acceptedConfig', 'acceptedCausalRevision', 'eventId', 'outboxId', 'outboxIds',
];
...
function matchesStoredTopology(stored, binding, target) {
  const keys = [
    'groupRef', 'config', 'version', 'createdAtEpochMs', 'updatedAtEpochMs',
    'updatedByPrincipalId', 'requestId',
...
function validAcceptedCausalRevision(value) {
  if (!hasExactKeys(value, [
    'stateRevision', 'causalRevision', 'snapshotVersion', 'metadataVersion',
  ...
  return [
    value.stateRevision, value.snapshotVersion, value.metadataVersion,
    value.rosterVersion, value.presenceVersion, value.causalRevision.groupRevision,
