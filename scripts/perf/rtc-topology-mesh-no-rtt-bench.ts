...
type BenchResult = Readonly<{
    ...
    sessionCount: number;
    topology: string;
    nextHopEdgeCount: number;
    sampleNextHops: readonly string[];
}>;

const OUT = readArg('--out') ??
    'tmp/perf/results/rtc-topology-mesh-no-rtt.json';
const SESSIONS = Number(readArg('--sessions') ?? '300');
const RUNS = Number(readArg('--runs') ?? '5');
const MESH_PARAM_K = Number(readArg('--mesh-param-k') ?? '2');
...
for (let run = 1; run <= RUNS; run++) {
    const service = new RallarRtcTopologyService({
        meshParamK: MESH_PARAM_K,
        now: () => 100,
    });
    const start = performance.now();
    const result = service.updateGroupTopology(group);
    const durationMs = performance.now() - start;
    const nextHopEdgeCount = Object.values(result.snapshot.nextHopsBySessionId)
        .reduce((total, nextHops) => total + nextHops.length, 0) / 2;
    ...
    results.push({
        ...
        nextHopEdgeCount,
        sampleNextHops: result.snapshot.nextHopsBySessionId['peer-10'] ?? [],
    });
}
