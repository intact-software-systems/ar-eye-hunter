...
const vocabularyPattern = /(?:legacy|deprecated|compatibility|compat|fallback|shim|bridge|rollback)/iu;
const predecessorNamePattern = /(?:^|[-_])(old|new|legacy|previous|v\d+)(?:[-_]|$)/iu;
const modePattern = /(?:flag|mode|variant|version)\w*\s*(?:===|==|=|:)\s*['"](?:legacy|old|previous|v\d+)['"]/iu;
...
function readChanges(mergeBase, head) {
    ...
    for (let index = 0; index < fields.length - 1;) {
        ...
        const target = kind === 'R' || kind === 'C' ? fields[index++] : kind === 'D' ? undefined : source;
...
function changedProductionFiles(changes, input) {
    ...
    for (const change of changes) {
        for (const file of [change.source, change.target].filter(Boolean)) {
            if (!isChangedProductionFile(file)) {
                continue;
            }
    ...
    return [...revisionsByPath.values()].toSorted((left, right) => left.path.localeCompare(right.path));
}
