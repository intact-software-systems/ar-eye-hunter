...
import { computePlanFactsFromTree, selectPlanChanges } from '../plan-adaptation/plan-change-facts.mjs';
...
function computeGateDeviation(request, entriesByPath, transitionInput) {
    ...
    const gateJobs = Array.isArray(evidence?.jobs)
        ...
        : [];
    const runMatches = evidence?.run?.id === request.target.workflowRunId &&
    ...
    const jobMatches = job?.run_id === request.target.workflowRunId &&
...
function computeOperation(operationInput) {
    ...
    const acceptanceStatus = operationInput.request.operation === 'plan.cancel' ? 'not-achieved' : 'admin-attested';
