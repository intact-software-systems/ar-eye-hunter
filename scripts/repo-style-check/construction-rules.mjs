...
import { createConstructionScopeModel, resolveConstructionBinding } from './construction-scope-model.mjs';
        ...
        for (const callback of node.arguments.flatMap(findConstructionCallbacks)) {
            for (const reference of collectConstructionCallbackReferences(callback)) {
                ...
                findings.push({
                    ...
                    message: `Review captured '${binding.name}' in ${constructionName}: declaration line ` +
...
function isPassThroughCallable(node) {
    ...
    const expressionBody = node.type === 'ArrowFunctionExpression' && node.body.type !== 'BlockStatement';
    const statement = node.body?.type === 'BlockStatement' && node.body.body.length === 1
        ? node.body.body[0]
        : undefined;
    ...
    return (
        ...
        call.arguments.every(
            (argument, index) => argument.type === 'Identifier' && argument.name === node.params[index].name
        )
    );
}
