...
export function createControlFleetRunReport(input: Readonly<{
    ...
    const report = {
        ...
        summary: {
            ...
            regions: uniqueValues(outcomes.map(outcome => outcome.label.region ?? UNKNOWN_REGION)).length,
            ...
            passRate: ratio(outcomes.filter(outcome => outcome.state === 'passed').length, outcomes.length),
...
export function createControlFleetAggregateReport(
    ...
    const agentIds = uniqueValues(reports.flatMap(report => report.agents.map(agent => agent.agentId)));
    ...
    return {
        ...
        staleAgentCount: uniqueValues(reports.flatMap(report =>
        ...
        timing: {
            runs: timingDistribution(reports.flatMap(report =>
...
export function filterControlFleetReports(
    ...
    return reports
        .filter(report => {
            if (filter.region && !report.regions.some(region => region.region === filter.region)) return false;
            if (filter.provider && !report.regions.some(region => region.provider === filter.provider)) return false;
            ...
            if (filter.fromEpochMs !== undefined && report.generatedAtEpochMs < filter.fromEpochMs) return false;
            if (filter.toEpochMs !== undefined && report.generatedAtEpochMs > filter.toEpochMs) return false;
            return true;
        })
        .sort((left, right) => right.generatedAtEpochMs - left.generatedAtEpochMs);
}

export function createControlFleetReportBundle(report: ControlFleetRunReport): ControlFleetReportBundle {
    return {
        fleetReportSchemaVersion: RALLAR_BLACK_BOX_FLEET_REPORT_SCHEMA_VERSION,
        ...
        files: {
            ...
            'agent-results.csv': csv([
                ['agentId', 'region', 'provider', 'state', 'durationMs', 'failedCommandCount', 'reconnectCount', 'stale'],
            ...
            'failure-signatures.csv': csv([
                ['signatureId', 'category', 'code', 'recipeId', 'transport', 'count', 'affectedAgents', 'affectedRegions', 'nextAction'],
...
function agentOutcome(input: Readonly<{
    agentId: string;
    distributedRun: ControlDistributedRunSnapshot;
    controlRun?: ControlRunSnapshot;
    ...
    const eventCount = input.controlRun?.events.filter(event => event.agentId === input.agentId).length ?? 0;
    ...
    const state = agentOutcomeState(input.distributedRun.state, failedResults.length, missing, input.failureSignatureIds.length, terminal);
    return {
...
function addFailureSignature(input: Readonly<{
...
function regionSummaries(outcomes: readonly ControlFleetAgentRunOutcome[]): readonly ControlFleetRegionSummary[] {
    const groups = new Map<string, ControlFleetAgentRunOutcome[]>();
    outcomes.forEach(outcome => {
        const key = `${outcome.label.region ?? UNKNOWN_REGION}\u0000${outcome.label.provider ?? UNKNOWN_PROVIDER}`;
    ...
    return [...groups.entries()].map(([key, rows]) => {
        ...
        return {
            ...
            timing: timingDistribution(rows.flatMap(row => row.durationMs === undefined ? [] : [row.durationMs])),
            dominantFailureSignatureId: [...failureCounts.entries()].sort((left, right) => right[1] - left[1])[0]?.[0],
        };
    }).sort((left, right) => left.region.localeCompare(right.region) || (left.provider ?? '').localeCompare(right.provider ?? ''));
}

function aggregateRegions(reports: readonly ControlFleetRunReport[]): readonly ControlFleetRegionSummary[] {
    return regionSummaries(reports.flatMap(report => report.agents.map(agent => ({
        ...agent,
        flaky: false,
    }))));
}

function aggregateFailureSignatures(reports: readonly ControlFleetRunReport[]): readonly ControlFleetFailureSignature[] {
    ...
    reports.flatMap(report => report.failureSignatures).forEach(signature => {
        ...
        existing.firstSeenAtEpochMs = minDefined(existing.firstSeenAtEpochMs, signature.firstSeenAtEpochMs);
        existing.lastSeenAtEpochMs = maxDefined(existing.lastSeenAtEpochMs, signature.lastSeenAtEpochMs);
...
function resultDurationMs(result: ControlRunSnapshot['results'][number]): number | undefined {
    ...
    if (typeof result.result?.startedAtEpochMs === 'number' && typeof result.result?.endedAtEpochMs === 'number') {
        return Math.max(0, result.result.endedAtEpochMs - result.result.startedAtEpochMs);
    }
    return undefined;
}

function linkedEvent(run: ControlDistributedRunSnapshot, commandId: string | undefined, payload: unknown): boolean {
...
function diagnosticFields(payload: unknown): Readonly<{
    severity: string;
    message: string;
    diagnosticTypeId: string;
    transport?: string;
    ...
    const topic = firstString(record.diagnosticTypeId, inner.diagnosticTypeId, data.diagnosticTypeId, record.topic, inner.topic);
    if (!severity && !topic) return undefined;
    return {
        severity: severity ?? 'info',
        message: firstString(record.message, inner.message, data.message, record.reason, inner.reason) ?? topic ?? 'diagnostic',
...
function likelyCause(
    ...
    if (category === 'targeting') return 'The resolved agent fleet did not match the distributed target policy.';
    if (category === 'readiness') return 'One or more agents did not acknowledge staging before the timeout.';
    if (category === 'barrier') return 'One or more agents did not reach synchronized barrier readiness.';
    if (category === 'diagnostic') return 'Runtime transport diagnostics correlated with the distributed run.';
    if (category === 'command') return message || code || 'A recipe command failed on at least one agent.';
...
function nextAction(
    ...
    if (category === 'targeting') return 'Check agent group/application/workspace labels and expected participant count.';
    if (category === 'readiness') return 'Inspect missing ACK agents and confirm they are logged in, connected, and not blocked on recipe load.';
    if (category === 'barrier') return 'Compare per-agent barrier readiness and reconnect/heartbeat history.';
    if (category === 'diagnostic' && transport === 'ws') return 'Inspect WebSocket subscription/topic evidence for affected regions.';
    if (category === 'diagnostic') return 'Inspect RTC lane, peer, group, and topic evidence for affected agents.';
    if (code?.includes('ASSERT')) return 'Open the failing command result and compare expected vs observed payload evidence.';
...
function fleetSummaryMarkdown(report: ControlFleetRunReport): string {
    return [
        ...
        ...report.failureSignatures.slice(0, 8).map(signature =>
            `- ${signature.title}: ${signature.count} occurrence(s), ${signature.affectedRegions.join(', ') || 'no region'} - ${signature.nextAction}`
        ),
    ].join('\n');
}
