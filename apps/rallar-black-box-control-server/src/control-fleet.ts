...
export function createControlFleetRunReport(
    ...
    const report = {
        ...
        summary: {
            ...
            failed: outcomes.filter((outcome) => outcome.state === 'failed' || outcome.state === 'timed-out')
                .length,
...
export function createControlFleetAggregateReport(
    ...
    return {
        ...
        staleAgentCount: uniqueValues(
            reports.flatMap((report) => report.agents.filter((agent) => agent.stale).map((agent) => agent.agentId))
        ).length,
...
export function filterControlFleetReports(
    ...
    return reports
        .filter((report) => {
            ...
            if (filter.recipeId && !report.recipeIds.includes(filter.recipeId)) {
                return false;
            }
            if (filter.groupId && report.group.groupId !== filter.groupId) {
                return false;
            }
            if (filter.state && report.state !== filter.state) {
                return false;
            }
...
function agentOutcomeState(
    ...
    if (runState === 'timed-out' && (missing || failureSignatures > 0)) {
        return 'timed-out';
    }
    if (failedResults > 0 || failureSignatures > 0) {
        return 'failed';
    }
    if (missing) {
        return 'missing';
    ...
    if (runState === 'passed' || runState === 'failed' || runState === 'timed-out') {
        return 'passed';
    }
    return 'unknown';
}
...
function resultDurationMs(result: ControlRunSnapshot['results'][number]): number | undefined {
    if (typeof result.result?.durationMs === 'number') {
...
function diagnosticFields(payload: unknown):
    ...
    return {
        ...
        message: firstString(record.message, inner.message, data.message, record.reason, inner.reason) ??
            topic ?? 'diagnostic',
