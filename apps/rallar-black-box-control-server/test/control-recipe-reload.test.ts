...
Deno.test('ALM reload waits for actual successful results and a later resumed connection', () => {
    ...
    const prefixResult = successfulResult(prefix);
    ...
    service.receiveClientEnvelope(successfulResult(reload));
    assertEquals(service.takeDispatchableCommands('run-1', 'agent-1'), []);
    ...
    service.receiveClientEnvelope(successfulResult(suffix));
...
Deno.test('ALM reload rejects a failed prefix without dispatching reload', () => {
    ...
    assertRight(service.enqueueCommand({ runId: 'run-1', agentId: 'agent-1', commandId: 'root', command: { kind: 'recipe.run', recipe: RELOAD_RECIPE } }));
    const [prefix] = service.takeDispatchableCommands('run-1', 'agent-1');
    const success = successfulResult(prefix);
...
function successfulResult(command: ControlCommandEnvelope): ControlResultEnvelope {
...
Deno.test('pending reload evidence survives low runtime and persistence bounds and restore', () => {
    ...
    assertRight(service.enqueueCommand({ runId: 'run-1', agentId: 'agent-1', commandId: 'root', command: { kind: 'recipe.run', recipe: RELOAD_RECIPE } }));
    const [prefix] = service.takeDispatchableCommands('run-1', 'agent-1');
    service.receiveClientEnvelope(successfulResult(prefix));
...
Deno.test('external command and result collisions cannot impersonate reload work', () => {
    ...
    assertEquals(service.receiveClientEnvelope(successfulResult(queued)).accepted, false);
    const malformed = successfulResult(prefix);
    ...
    forged.receiveClientEnvelope(successfulResult(queued));
...
Deno.test('rate limits defer derived execution and the allowlist still rejects reload', () => {
    ...
    restricted.receiveClientEnvelope(successfulResult(allowedPrefix));
...
Deno.test('the hosted combined inline recipe uses the same reload path and logical distributed link', () => {
    const service = createRallarBlackBoxControlService(toControlServiceInput({ now: () => 1_000 }));
    ...
    const manifest = {
        ...
        recipes: source.recipes.map((selection) => ({
            ...selection,
            recipe: selection.recipe && selection.role === 'sender'
                ? { ...selection.recipe, commands: [...selection.recipe.commands, ...RELOAD_RECIPE.commands] }
                : selection.recipe
        }))
    };
    ...
    for (const agentId of ['controller-01', 'controller-02']) {
        ...
        service.receiveClientEnvelope(successfulResult(stage));
    }
    ...
    assertEquals(reload.command.kind, 'agent.reload');
    const reloadResult = successfulResult(reload);
    ...
    const suffixResult = successfulResult(suffix);
    ...
    assertEquals(aggregate.results.slice(-3), [prefixValue.results.at(-1), reloadResult.result, suffixValue.results[0]]);
    assertEquals(
...
Deno.test('malformed result timing cannot become accepted reload evidence', () => {
    ...
    assertRight(service.enqueueCommand({ runId: 'run-1', agentId: 'agent-1', commandId: 'root', command: { kind: 'recipe.run', recipe: RELOAD_RECIPE } }));
    const [prefix] = service.takeDispatchableCommands('run-1', 'agent-1');
    const result = successfulResult(prefix);
...
Deno.test('a segment result must identify its actual recipe as well as its ordered children', () => {
    ...
    assertRight(service.enqueueCommand({ runId: 'run-1', agentId: 'agent-1', commandId: 'root', command: { kind: 'recipe.run', recipe: RELOAD_RECIPE } }));
    const [prefix] = service.takeDispatchableCommands('run-1', 'agent-1');
    const actual = successfulResult(prefix);
...
Deno.test('derived child identities support distinct root IDs containing lone UTF-16 surrogates', () => {
    ...
    for (const command of dispatched) {
        assertEquals(service.receiveClientEnvelope(successfulResult(command)).accepted, true);
    }
