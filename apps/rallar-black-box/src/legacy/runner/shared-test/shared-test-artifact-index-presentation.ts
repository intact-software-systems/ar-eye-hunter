...
export type SharedTestCompactionSummary = Readonly<{
    sourceOrdinal: number; name: string; transport: string; action: string;
    connection?: string; count: number; firstSequence: number; lastSequence: number;
}>;
export type SharedTestArtifactIndexPresentation = Readonly<{
    status: 'coherent' | 'inconsistent';
    truncation: Readonly<{
        totalEvents?: number; emittedEvents?: number; omittedEvents?: number; truncated?: boolean;
    }>;
    compaction: Readonly<{
        status: 'metadata-unavailable' | 'flag-invalid' | 'incoherent' | 'not-compacted' |
            'summaries-unavailable' | 'summaries-invalid' |
            'index-inconsistent' | 'compacted';
        summariesAvailable: boolean;
        summaryCount?: number; summaries: readonly SharedTestCompactionSummary[];
    }>;
}>;
export type SharedTestCompactionSummaryWindow = Readonly<{
    total: number; startIndex: number; endIndexExclusive: number;
    displayStart: number; displayEnd: number; canPrevious: boolean; canNext: boolean; rows: readonly SharedTestCompactionSummary[];
}>;
export function deriveSharedTestArtifactIndexPresentation(
    ...
    const compaction = compactionPresentation(
        artifactIndex.compaction, totalEvents, omittedEvents,
    );
    if (compaction.status === 'index-inconsistent') return inconsistentPresentation();
    return {
        status: 'coherent',
...
function compactionPresentation(
    value: unknown, totalEvents: number, omittedEvents: number,
): SharedTestArtifactIndexPresentation['compaction'] {
    const compaction = recordValue(value);
    if (!compaction) return unavailableCompaction('metadata-unavailable');
    const flag = compaction.compacted;
    const rawSummaries = compaction.repeatedSuccessSummaries;
    if (typeof flag !== 'boolean') return unavailableCompaction('flag-invalid');
    if (!flag) {
    ...
    if (!Array.isArray(rawSummaries)) return unavailableCompaction('summaries-unavailable');
    ...
    for (const [index, value] of rawSummaries.entries()) {
        ...
        if (!summary) return unavailableCompaction('summaries-invalid');
        const group = JSON.stringify([summary.name, summary.transport, summary.action,
    ...
    return {
        status: 'compacted', summariesAvailable: true,
        summaryCount: summaries.length, summaries,
    };
}
...
function unavailableCompaction(
    status: Exclude<SharedTestArtifactIndexPresentation['compaction'][
        'status'], 'compacted'>,
): SharedTestArtifactIndexPresentation['compaction'] {
    return { status, summariesAvailable: false, summaries: [] };
}
function compactionSummary(value: unknown, index: number):
    SharedTestCompactionSummary | undefined {
    ...
    const connection = summary?.connection === undefined ? undefined
    ...
    return {
        sourceOrdinal: index + 1, name,
        transport, action, connection, count,
        firstSequence, lastSequence,
    };
}
export function deriveSharedTestCompactionSummaryWindow(
    summaries: readonly SharedTestCompactionSummary[], requestedStartIndex: number,
    ...
    return {
        ...
        displayStart: total === 0 ? 0 : startIndex + 1, displayEnd: endIndexExclusive,
        canPrevious: startIndex > 0, canNext: endIndexExclusive < total,
        rows: summaries.slice(startIndex, endIndexExclusive),
    };
}
export function moveSharedTestCompactionSummaryWindow(
    window: SharedTestCompactionSummaryWindow, direction: 'previous' | 'next',
): number {
    return direction === 'previous'
        ? Math.max(0, window.startIndex - SHARED_TEST_COMPACTION_SUMMARY_WINDOW_SIZE)
        : window.canNext ? window.endIndexExclusive : window.startIndex;
}
