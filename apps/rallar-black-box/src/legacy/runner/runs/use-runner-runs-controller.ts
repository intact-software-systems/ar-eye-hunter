...
export function useRunnerRunsController({
  ...
  const initialSyntheticSeed = useMemo<SyntheticDistributedRunSeed | undefined>(() => {
    ...
    return distributedRunSeed ? createSyntheticDistributedRunSeed(distributedRunSeed) : undefined;
  }, []);
  ...
  const refreshDistributedAnalysis = async (
    ...
    try {
      ...
      const nextControlRunId =
        nextDistributedRun?.controlRunId ?? override?.controlRunId ?? controlRunId;
      ...
      const shouldLoadArtifact = Boolean(
        nextDistributedRun &&
        (options.loadArtifact || isDistributedRunTerminalState(nextDistributedRun.state)),
      );
      const nextArtifact =
        shouldLoadArtifact && nextDistributedRun
      ...
      if (!request.isCurrent() || (activeSyntheticSeedRef.current && !override)) {
        return;
      }
      ...
      setCompareRightId((current) => {
        ...
        const otherRun = list.find(
          (item) => item.distributedRunId !== nextDistributedRun?.distributedRunId,
        );
  ...
  const applySyntheticDistributedRunSeed = (seedId: DistributedRunSeedId): void => {
  ...
  const handleDistributedArtifactFiles = async (
    ...
    try {
      ...
      const { artifactFiles, analysis, snapshots, artifactBundle } =
        await readDistributedArtifactFiles(selectedFiles, generatedAtEpochMs);
      activeSyntheticSeedRef.current = undefined;
      setActiveSyntheticSeed(undefined);
      setSelectedSyntheticSeedId('');
      setImportedArtifactAnalysis(analysis);
      setImportedArtifactStatus(
        distributedArtifactImportStatus(artifactFiles, analysis.parseWarnings.length),
      );
      setDistributedRuns((current) => [
        snapshots.distributedRun,
        ...current.filter(
          (item) => item.distributedRunId !== snapshots.distributedRun.distributedRunId,
        ),
      ]);
  ...
  useEffect(() => {
    if (preferredDistributedRun || activeSyntheticSeedRef.current) {
      return;
    }
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [artifactBundle, selectedDistributedRun?.distributedRunId, selectedDistributedRun?.state]);
  ...
  const selectDistributedRun = (distributedRunId: string): void => {
    ...
    setImportedArtifactAnalysis(undefined);
    setImportedArtifactStatus(undefined);
    const selected = distributedRuns.find((item) => item.distributedRunId === distributedRunId);
