    ...
    return (
        ...
        >
            ...
            <dl className={styles.answers}>
                <div><dt>State</dt><dd>{model.source.distributedRun.state}</dd></div>
                <div><dt>Affected</dt><dd><code>{affected}</code></dd></div>
                <div><dt>Inspect next</dt><dd>{nextAction}</dd></div>
                <div><dt>Control truth</dt><dd>{connectionLabel(connection)}</dd></div>
            </dl>
            {model.report.summary.snapshotWarnings.length > 0 ? (
                <div className={styles.warnings} role="status">
                    ...
                    {model.report.summary.snapshotWarnings.map(warning => (
...
function connectionLabel(connection: MonitorConnectionTruth): string {
    ...
    if (connection === 'error') return 'Reachable with an error';
    return connection.replace('-', ' ');
}

function fallbackNextAction(state: string): string {
    if (state === 'waiting-for-ack') return 'Inspect target acknowledgements.';
