...
export function MonitorEvidenceDisclosure({
    ...
    const timeline = timelineOpen ? model.monitor.timeline.slice(
        timelineWindow.model.startIndex,
        timelineWindow.model.endIndexExclusive,
    ) : [];
    const events = eventsOpen ? model.monitor.events.slice(
        eventsWindow.model.startIndex,
        eventsWindow.model.endIndexExclusive,
    ) : [];
    const composites = compositesOpen ? model.monitor.compositeDrilldowns.slice(
                    ...
                    {events.map((item, offset) => {
                        ...
                        >
                            <button
                                ...
                                type="button"
                            ><span><strong>{item.summary}</strong><small>{item.kind} · {formatTime(item.atEpochMs)}</small></span><ExactIdentifier value={item.agentId} /></button>
                        </li>
                        );
                    })}
                </ul>
                    ...
                    {composites.map((item, offset) => {
                        ...
                        >
                            <button
                                ...
                                type="button"
                            ><ExactIdentifier value={item.commandId} /><span>{item.summary.passed} passed · {item.summary.failed} failed · {item.rows.length} rows</span></button>
                        </li>
                        );
                    })}
                </ul>
