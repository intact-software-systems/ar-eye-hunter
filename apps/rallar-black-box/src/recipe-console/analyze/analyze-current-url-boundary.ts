...
export function resolveAnalyzeOperationContext(input: Readonly<{
    baseUrl: string;
    renderedContext?: AnalyzeWorkspaceContext;
    search?: string;
}>): AnalyzeWorkspaceContext | undefined {
    if (input.search === undefined) return input.renderedContext;
    const state = parseRecipeConsoleUrl(input.search).state;
    if (!state.distributedRunId) return input.renderedContext;
    if (
        !state.controlRunId &&
