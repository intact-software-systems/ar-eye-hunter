    ...
    return (
        ...
        >
            ...
            {!current && controller.evidenceWindowError ? (
                ...
                >
                    Current query evidence is unavailable. Retry the search;
                    prior query rows are not mounted.
                </p>
            ) : null}
...
function EvidenceMetadata({
    ...
    if (exact.length === 0 && descriptive.length === 0) return null;
    return (
        <span className={styles.resultMeta}>
            ...
            {descriptive.map((value, index) => (
                <span dir="auto" key={`metadata-${index}-${value}`}>{value}</span>
            ))}
        </span>
    );
}
