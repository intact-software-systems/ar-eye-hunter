import type { AnalyzeWorkspaceOperationAuthority } from
    './analyze-workspace-state.ts';

export function analyzeOperationOwnsCurrentBoundary(input: Readonly<{
    authority: Pick<
        AnalyzeWorkspaceOperationAuthority,
        'action' | 'contextKey'
    >;
    ...
    if (input.authority.action !== 'load-control') return true;
