...
export function createAnalyzeWorkerRpcClient(
    input: Readonly<{
        getAccepted():
            | Readonly<{
    ...
    function search(
        ...
        const message: AnalyzeWorkerRequest = {
            type: 'search',
            modelGeneration: owner.modelGeneration,
            queryGeneration: nextGeneration,
            requestId: id,
            query,
            windowSize
        };
        if (!isAnalyzeWorkerRequest(message)) {
            return undefined;
        }
    ...
    function send(
        ...
        if (!owner || input.isDisposed() || id < 0) {
            return undefined;
        }
    ...
    function handleResponse(response: RpcResponse): void {
        ...
        if (response.type === 'search-complete') {
            if (pending.kind !== 'search' || response.queryGeneration !== queryGeneration) {
                return;
            }
            ...
            if (
                pending.kind !== 'window' || response.queryGeneration !== queryGeneration ||
                response.windowGeneration !== windowGeneration
            ) {
                return;
            }
            ...
            if (
                ...
                response.selectionGeneration !== selectionGeneration
            ) {
                return;
            }
            ...
            if (pending.kind !== 'tune' || response.tuneGeneration !== tuneGeneration) {
                return;
            }
