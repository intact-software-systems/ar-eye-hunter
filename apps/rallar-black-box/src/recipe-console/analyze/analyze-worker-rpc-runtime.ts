import { searchDistributedArtifactEvidenceWindow } from '@shared-test/rallar-bb-test/mod.ts';
import {
...
type RpcRequest = Extract<AnalyzeWorkerRequest, { type: 'search' | 'window' | 'select' | 'tune'; }>;
...
export function createAnalyzeWorkerRpcRuntime(
    input: Readonly<{
        ...
        input.post({
            message: {
                ...
                telemetry: analyzeWorkerTelemetry(
                    snapshot,
                    result.window,
                    analyzeWorkerDuration(input.now(), startedAt)
                )
            }
        });
    }
    ...
    async function window(
        ...
        const snapshot = input.getActive();
        if (
            !snapshot || request.queryGeneration !== queryGeneration ||
    ...
    function select(request: Extract<RpcRequest, { type: 'select'; }>): void {
        const snapshot = input.getActive();
        if (!snapshot || request.selectionGeneration <= selectionGeneration) {
            return;
        }
        selectionGeneration = request.selectionGeneration;
        const selected = request.evidenceId
            ? snapshot.catalog.entries.find((row) => analyzeEvidenceEntryMatchesProjectedId(row, request.evidenceId!))
            : undefined;
        input.post({
            message: {
    ...
    function tune(request: Extract<RpcRequest, { type: 'tune'; }>): void {
        const snapshot = input.getActive();
        if (!snapshot || request.tuneGeneration <= tuneGeneration) {
            return;
        }
