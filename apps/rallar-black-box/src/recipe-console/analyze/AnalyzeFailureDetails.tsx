import type { DistributedArtifactEvidenceFailureDetails } from '@shared-test/rallar-bb-test/mod.ts';
...
export function AnalyzeFailureDetails({
    ...
    if (density === 'row') {
        return (
            ...
            >
                ...
                {details.message ? <span className={styles.rowMessage}>{details.message}</span> : null}
            </span>
        );
    }
    ...
    return (
        ...
        >
            ...
            {details.message ? <p className={styles.message}>{details.message}</p> : null}
            {frame ? <code className={styles.frame}>{frame}</code> : null}
            {density === 'inspector' && details.stack ? <pre className={styles.stack}>{details.stack}</pre> : null}
        </section>
    );
}
