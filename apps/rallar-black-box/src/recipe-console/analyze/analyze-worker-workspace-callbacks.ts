...
import type { useAnalyzeEvidenceRequests } from './use-analyze-evidence-requests.ts';
...
export function createAnalyzeWorkerWorkspaceCallbacks(
    input: Readonly<{
        pendingRef: AnalyzeMutableRef<AnalyzePendingOperation | undefined>;
        validationErrorRef: AnalyzeMutableRef<Error | undefined>;
        pendingIdentityPatchRef: AnalyzeMutableRef<AnalyzePendingIdentityPatch | undefined>;
        ...
        setSelectedEvidence: StateSetter<AnalyzeEvidenceWindowProjection['entries'][number] | undefined>;
    ...
    return {
        ...
        onSearchComplete(response) {
            ...
            input.setTelemetry(response.telemetry);
            input.setWorkerUnavailable(undefined);
            input.setSelectedEvidence((previous) =>
                previous &&
