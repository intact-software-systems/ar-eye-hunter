        ...
        <section className={styles.setup} data-execute-agent-setup>
            <header className={styles.header}>
                ...
                <button
                    ...
                    type="button"
                >
                    {model.expanded ? 'Hide browser agent setup' : 'Add browser agents'}
                </button>
            </header>
            {model.expanded
                ? (
                    <div className={styles.body} id="execute-agent-setup-body">
                        ...
                        <dl className={styles.context}>
                            ...
                            <Fact
                                label="Group"
                                value={`${group.applicationId} / ${group.workspaceId} / ${group.groupId}`}
                            />
                        </dl>
                        {model.blocker ? <p className={styles.blocker} role="alert">{model.blocker}</p> : null}
                        <div className={styles.actions}>
                            ...
                            >
                                {model.busyAction === 'copy'
                                    ? 'Preparing links…'
                                    : `Copy ${model.count} launch ${model.count === 1 ? 'link' : 'links'}`}
                            </button>
                        </div>
                        <div
                                    ...
                                    {displacedBlockedAgentIds.map((agentId) => (
                                        <div className={styles.linkRow} key={agentId}>
                                            ...
                                            <button
                                                disabled={Boolean(model.blocker || model.busyAction)}
                                                onClick={() =>
                                                    void model.copyAgentLink(agentId)}
                                                type="button"
                                            >
                                                Copy link for {agentId}
                                            </button>
