    ...
    return (
        ...
        >
            ...
            <p
                ...
                role="status"
            >
                Run state: {state ? stateLabel(state) : 'Not created'}.
            </p>
            {unknownDistributedRunId
                ? (
                    <p className={styles.error} role="alert">
                        Distributed run <code>{requestedDistributedRunId}</code>{' '}
                        is not available in current control truth.
                    </p>
                )
                : null}
            {mutationError
            ...
            {run?.error
                ? (
                    <div className={styles.error} role="alert">
                        ...
                        {run.error.details === undefined ? null : <pre>{safeJson(run.error.details)}</pre>}
                    </div>
                )
                : null}
            {run
...
function operationErrorLabel(error: ExecuteOperationError): string {
    return error.kind.split('-').map((part) => `${part.charAt(0).toUpperCase()}${part.slice(1)}`).join(' ');
}
