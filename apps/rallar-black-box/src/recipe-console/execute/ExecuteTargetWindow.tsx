...
export function ExecuteTargetWindow({
    ...
    return <>
        ...
        >{window.model.total === 0
            ...
            } of ${window.model.total.toLocaleString('en-US')} targets.`}</span>
        {window.model.total > window.model.windowSize ? (
        ...
        >
            <table className={styles.table}>
                <thead><tr><th scope="col">Select</th><th scope="col">Agent</th><th scope="col">Identity</th><th scope="col">Group</th><th scope="col">Last evidence</th><th scope="col">Status</th></tr></thead>
                <tbody>{visible.map((row, offset) => <TargetRow
                    current={current}
                    disabled={disabled}
...
function TargetRow({ current, disabled, onToggle, row, selected, selectionLocked }: Readonly<{
    current: boolean; disabled: boolean; onToggle(agentId: string): void;
    row: DistributedRecipeTargetRow; selected: boolean; selectionLocked: boolean;
}>) {
    const selectable = current && row.targetable;
    return <tr data-execute-target data-target-status={row.status}>
        <td className={styles.selectCell}>{selectable ? <input
            aria-label={`Select ${row.agentId}`}
            checked={selected}
            disabled={disabled || selectionLocked}
            onChange={() => onToggle(row.agentId)}
            type="checkbox"
        /> : <span aria-label="Not selectable" className={styles.notSelectable}>—</span>}</td>
        <th scope="row"><ExactIdentifier value={row.agentId} /></th>
        <td><span className={styles.identity}><span>{row.principalId ?? 'Identity unavailable'}</span><small>{row.sessionId ?? 'No session'}</small></span></td>
        <td><code>{groupLabel(row)}</code></td><td>{lastEvidence(row)}</td>
        <td><div className={styles.state}><StatusMark label={statusLabel(row.status)} status={statusTone(row.status)} /><span className={styles.reason}>{row.reason}</span></div></td>
    </tr>;
}
