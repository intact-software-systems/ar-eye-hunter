...
export function executeTruthContextKey(input: Readonly<{
    baseUrl: string;
    controlRunId?: string;
}>): string {
...
export function manifestRecipeIds(
    ...
    return [...new Set(manifest.recipes.map(selection =>
        selection.recipeId ?? selection.recipe?.recipeId
...
export function executeRunConfigurationIssue(input: Readonly<{
    run?: ControlDistributedRunSnapshot;
    controlRunId?: string;
    recipeId?: string;
}>): string | undefined {
...
export function executeSafeTargetLabel(input: Readonly<{
