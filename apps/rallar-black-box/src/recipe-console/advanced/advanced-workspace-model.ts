import { ADVANCED_DIAGNOSTIC_CONTEXT_MAX_BYTES, createAdvancedLegacyHref } from './advanced-legacy-href.ts';
import { ADVANCED_SURFACE_CATALOG, type AdvancedSurface } from './advanced-surface-catalog.ts';
import type {
    AdvancedContextRow,
...
export function createAdvancedWorkspaceModel(
    ...
    const state = {
        ...
        distributedRunId: input.selection.distributedRunId ?? input.urlState.distributedRunId,
    ...
    const links = ADVANCED_SURFACE_CATALOG.map((surface) => {
        ...
        if (!href) {
            throw new Error(`Missing Advanced href for ${surface.id}.`);
        }
        return {
    ...
    return {
        ...
        contextRows: [
            ...
            ...SELECTION_FIELDS.map(([field, label]) =>
                bridgeAware(
...
function categoryFor(surface: AdvancedSurface): AdvancedWorkspaceCategory {
    if (surface.id.startsWith('runner.')) {
        return 'workflow-fallbacks';
    }
    if (surface.id.startsWith('legacy.')) {
...
function bridgeAware(
    ...
    const omissionMessage = 'Valid context omitted from one or more links because the 4,096-byte query budget is full.';
    return {
        ...row,
        status: 'omitted',
        message: row.message
