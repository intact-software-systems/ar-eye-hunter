...
export function parseControlRetentionConfirmation(
    value: unknown,
    preview: ControlRetentionPreview
): ControlRetentionConfirmation {
    assertControlRetentionResponseBudget(value);
    const root = exactRecord(value, 'confirmation', CONFIRMATION_FIELDS);
    const deletedRunIds = stringArray(
        root.deletedRunIds,
        'confirmation.deletedRunIds',
        CONTROL_RETENTION_PLAN_LIMITS.candidates
    );
...
function validateLinkedConsequences(
    ...
    for (const candidate of candidates) {
        ...
        for (const id of candidateDistributedIds) {
            if (ownerByDistributedId.has(id)) {
                fail(`duplicate distributed run ${id}`);
            }
        ...
        for (const id of candidate.fleetReportIds) {
            candidateFleetIds.push(id);
        }
    }
...
function exactRecord(
    ...
    if (names.join('\0') !== [...fields].sort().join('\0')) {
...
function exactArray(
    ...
    if (value.length > maximum) {
        fail(`${path} exceeds its ${maximum} item bound`);
    }
    ...
    if (names.length !== value.length + 1) {
        fail(`${path} must be a dense data array`);
    }
...
function assertEqualSets(left: readonly string[], right: readonly string[], path: string): void {
    ...
    if (left.length !== right.length || left.some((value) => !rightSet.has(value))) {
        fail(`${path} does not match`);
    }
}
