...
export function FleetLiveBoard({
    ...
    return (
        <section aria-labelledby="fleet-live-heading" className={styles.panel}>
            ...
            {rows.length === 0
                ? <p className={styles.empty}>No live agent evidence.</p>
                : (
                            ...
                            <tbody>
                                {rows.map((row) => (
                                    <tr key={row.agentId}>
                                        ...
                                        <td>
                                            <Status
                                                value={row.connected ? 'Connected' : row.connectionStatus}
                                                ok={row.connected}
                                            />
                                        </td>
                                        <td>
                                            <bdi dir="auto">{row.region ?? 'Unresolved'}</bdi> /{' '}
                                            <bdi dir="auto">{row.provider ?? 'Unknown'}</bdi>
                                        </td>
                                        <td>
                                            <bdi dir="auto">{row.targetReason}</bdi>
                                        </td>
