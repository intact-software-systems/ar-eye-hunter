    ...
    return (
        <section aria-label="Fleet evidence source" className={styles.root}>
            <div className={styles.truth}>
                ...
                <strong>{collection === 'absent'
                    ? 'Report collection unavailable'
                    : `${reports.length.toLocaleString('en-US')} accepted reports`}</strong>
            ...
            {selectionIssue ? (
                ...
                >{selectionIssue}
                    {requestedReportId ? <> Requested report{' '}
                        <ExactIdentifier value={requestedReportId} />.</> : null}
                    {selectionIssueValue &&
                            selectionIssueValue !== requestedReportId ? <>
                        {' '}Conflicting selection{' '}
                        <ExactIdentifier value={selectionIssueValue} />.
                    </> : null}</p>
            ) : null}
            {selectedReport ? (
                <div className={styles.selectedContext}>
                    ...
                    <div className={styles.recipeWindow}>
                        <span>{selectedReport.recipeIds.length === 1
                            ? 'Recipe' : 'Recipes'}</span>
                        {recipeWindow ? (
                            <FleetWindowControls
                        ...
                        {!recipeWindow &&
                            ...
                            <small>{selectedReport.recipeIds.length - visibleRecipes.length}{' '}
                                additional recipes omitted from this bounded detail.</small>
                        ) : null}
                    </div>
                </div>
            ) : null}
...
function reportOption(report: ControlFleetRunReport): SearchableListboxOption {
    return {
        ...
        label: `${report.state} · report generated ${
            fleetUtcTime(report.generatedAtEpochMs)
        }`,
        exactIdentifier: report.distributedRunId,
        detail: `${report.recipeIds.length.toLocaleString('en-US')} recipes · ${
            report.summary.agents.toLocaleString('en-US')} agents`,
        searchText: [
            report.distributedRunId,
            report.controlRunId,
            report.state,
