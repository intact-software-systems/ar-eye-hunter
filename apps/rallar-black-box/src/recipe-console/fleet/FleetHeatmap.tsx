import type { FleetReportHeatmap } from '@shared-test/rallar-bb-test/fleet-report-analysis.ts';
                            ...
                            <tr key={row.agent.agentId}>
                                <th scope="row">
                                    ...
                                    >
                                        ...
                                        <small>
                                            <bdi dir="auto">{row.region}</bdi> / <bdi dir="auto">{row.provider}</bdi>
                                        </small>
                                    </button>
                                </th>
                                {row.cells.map((cell, index) => (
                                    <td
                                        data-state={cell?.state ?? 'missing-evidence'}
                                        key={heatmap.runs[index]?.distributedRunId ?? index}
                                    >
...
function truth(heatmap: FleetReportHeatmap): string {
    return `${heatmap.rows.length.toLocaleString('en-US')} of ${
        heatmap.totalAgentRows.toLocaleString('en-US')
    } agents × ${heatmap.runs.length.toLocaleString('en-US')} of ${
        heatmap.totalRunColumns.toLocaleString('en-US')
    } runs`;
}
