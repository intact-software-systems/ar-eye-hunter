        ...
        <div className={styles.evidence}>
            <Layer label="Live agents" layer={model.agentMarkers}>
                {model.agentMarkers.items.map(item => (
                    ...
                    >
                        ...
                        <span>{item.agent.live?.state ?? 'Unknown'} ·{' '}
                            {item.agent.location
                ...
                {model.regionMarkers.items.map(item => {
                    ...
                    return (
                        <button
                            ...
                            type="button"
                        >
                            <strong><bdi dir="auto">{item.region.region}</bdi></strong>
                            <span>{item.region.provider
                                ? <bdi dir="auto">{item.region.provider}</bdi>
                                : 'Unknown'} ·{' '}
                                {item.region.failed} failed ·{' '}
                                <FleetLocationEvidence
            ...
            >
                {model.routePaths.items.map(item => (
                    <p key={item.id}>
                        ...
                        <ExactIdentifier value={item.route.targetAgentId} /> ·{' '}
                        {item.route.transport
                            ? <bdi dir="auto">{item.route.transport}</bdi>
                            : 'unknown transport'} ·{' '}
                        {item.route.eventCount} observations, {item.route.failedCount} failed ·{' '}
                        source <FleetLocationEvidence location={item.route.source} /> ·{' '}
                        target <FleetLocationEvidence location={item.route.target} />
                    </p>
                ))}
            </Layer>
            <Layer label="Failure locations" layer={model.failureMarkers}>
                {model.failureMarkers.items.map(item => (
                    ...
                    >
                        <ExactIdentifier value={item.agent.agentId} />
                        <span>{item.agent.historical?.failedOutcomes ?? 0} failed outcomes ·{' '}
                            {item.agent.location
                                ? <FleetLocationEvidence
                                    location={item.agent.location}
                                />
            ...
            <section className={styles.provenance} aria-labelledby="fleet-map-provenance">
                ...
                <p>{model.unresolved.agentIds.length} unresolved agents;{' '}
                    {model.unresolved.routeEndpointAgentIds.length}{' '}
                    unresolved route endpoint identities. Traverse the persistent ledger below.</p>
            </section>
        </div>
    );
}
...
function Layer<Item>({
    ...
    return (
        <section className={styles.layerEvidence} data-layer-enabled={layer.enabled}>
            <h3>{label}</h3>
            <p>{layer.renderedCount} rendered of {layer.candidateCount} candidates;{' '}
                {layer.omittedCount} omitted.</p>
            {layer.candidateCount === 0 && emptyMessage ? (
                <p data-fleet-map-no-routes>{emptyMessage}</p>
            ) : null}
            <div>{children}</div>
        </section>
    );
