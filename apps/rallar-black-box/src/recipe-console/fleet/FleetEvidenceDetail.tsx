import type { FleetReportAgentDetail } from '@shared-test/rallar-bb-test/fleet-report-analysis.ts';
import type { ControlFleetRegionSummary, ControlFleetRunReport } from '@shared-test/rallar-bb-test/fleet-report.ts';
                                    ...
                                    >
                                        {selectedAgent.runs.map((entry) => (
                                            ...
                                            >
                                                ...
                                                <span>
                                                    {entry.outcome.state} · {entry.outcome.durationMs === undefined
                                                        ? 'duration unavailable'
                                                        : `${entry.outcome.durationMs.toLocaleString('en-US')} ms`}
                                                </span>
                                            </li>
                                        ))}
                                    </ol>
                    ...
                    <div className={styles.section}>
                        ...
                        >
                            {visibleRegionRows.map((region) => (
                                ...
                                >
                                    <ExactIdentifier value={region.region} /> / {region.provider
                                        ? <ExactIdentifier value={region.provider} />
                                        : 'Unknown'} · {(region.passRate * 100).toLocaleString('en-US', {
                        ...
                        {!regionProviderWindow &&
                            ...
                            ? (
                                <p>
                                    {selectedRegionRows.length - visibleRegionRows.length}{' '}
                                    additional provider rows omitted from this bounded detail.
                                </p>
                            )
                            : null}
                    </div>
