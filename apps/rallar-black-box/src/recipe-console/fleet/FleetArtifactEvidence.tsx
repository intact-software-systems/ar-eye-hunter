        ...
        <section aria-labelledby="fleet-artifact-heading" className={styles.root}>
            <header>
                ...
                <button
                    ...
                    type="button"
                >{artifact.status === 'loading' ? 'Loading…' : 'Load artifact bundle'}</button>
            </header>
            {selectedReportId ? (
                <p className={styles.selection}>Exact report <ExactIdentifier value={selectedReportId} /></p>
            ) : <p className={styles.empty}>Select an accepted report to load its bundle.</p>}
            {artifact.message ? (
                <p className={styles.error} role="alert">{artifact.message}</p>
            ) : null}
            {artifact.model ? (
                <div className={styles.ready}>
                    <ul>{artifact.model.files.map(file => (
                        <li key={file.name}>
                            <code>{file.name}</code>
