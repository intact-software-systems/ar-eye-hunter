...
import type { ControlFleetReportValidationIssue } from
        ...
        <section aria-labelledby="fleet-quality-heading" className={styles.root}>
            <header>
                ...
                <p>{collection === 'absent'
                    ...
                    : <>{acceptedCount} of {sourceCount} reports accepted ·{' '}
                        {quarantinedCount} quarantined</>}</p>
            </header>
            {collection === 'absent' ? (
                ...
                <ol className={styles.issues}>{issues.map((issue, index) => (
                    <li key={`${issue.path}\u0000${issue.code}\u0000${index}`}>
                        <strong>{issue.code}</strong>
                        <bdi dir="ltr"><code>{issue.path}</code></bdi>
            ...
            {missingLabelAgentIds.total > 0 ? (
                <div className={styles.labels}>
                    ...
                    <div>{missingLabelAgentIds.items.map(agentId => (
