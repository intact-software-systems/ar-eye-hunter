    ...
    return (
        <div className={styles.evidence} data-current={preview.current}>
            ...
            <ul aria-label="Retention preview counts" className={styles.counts}>
                ...
                <li>{plural(preview.wouldDeleteDistributedRunIds.length,
                    'distributed run')}</li>
            ...
            {disabled ? (
                ...
                <p className={styles.empty}>
                    Detailed preview evidence is paused while confirmation is open.
                    Cancel to inspect linked consequences.
                </p>
            ) : (
...
export function RetentionCleanupResult({ confirmation, controller }: Readonly<{
    ...
    return <div className={styles.result}>
        ...
        <p>{plural(confirmation.deletedRunIds.length, 'control run')} deleted
            {' · '}{confirmation.retainedRuns} retained
            {' · '}Cap {confirmation.maxRuns}</p>
        <RetentionTotalIdDisclosure
            controller={controller}
