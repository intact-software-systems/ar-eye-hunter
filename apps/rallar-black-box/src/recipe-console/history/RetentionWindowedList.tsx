...
export const RECIPE_CONSOLE_RETENTION_WINDOW_SIZE = RECIPE_CONSOLE_RETENTION_SURFACE_ROW_BUDGET / 2;
    ...
    return (
        <>
            ...
            <span
                ...
                tabIndex={-1}
            >
                {rangeLabel(controller.model, itemLabel)}
            </span>
            {controller.model.total > controller.model.windowSize
                ? (
                    <p className={styles.truth} data-retention-window-truth={label}>
                        {outsideCount(controller.model).toLocaleString('en-US')} {itemLabel}{' '}
...
function rangeLabel(
    ...
    return model.total === 0
        ? `No ${itemLabel}.`
        : `Showing ${model.displayStart.toLocaleString('en-US')}–${model.displayEnd.toLocaleString('en-US')} of ${
            model.total.toLocaleString('en-US')
        } ${itemLabel}.`;
}
