...
export const HISTORY_FILTER_PRESET_STORAGE_KEY =
...
export type HistoryFilterStorage = Pick<
    Storage,
...
export function parseHistoryFilterPresetEnvelope(
    ...
    if (!envelope) return EMPTY_INVALID;
    ...
    if (!entries) return EMPTY_INVALID;
...
export function readHistoryFilterPresets(
    ...
    if (!storage) return { status: 'unavailable', presets: [] };
    try {
...
function exactArrayValues(value: unknown): readonly unknown[] | undefined {
    ...
    if (names.length !== value.length) return undefined;
    const nameSet = new Set(names);
    const result: unknown[] = [];
    for (let index = 0; index < value.length; index += 1) {
        const key = String(index);
        if (!nameSet.has(key) || !isEnumerableData(value, key)) return undefined;
