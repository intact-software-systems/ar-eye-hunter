...
function inspectorContent(
    ...
    if (selection.kind === 'hint') {
        const hint = source.decisions?.hints.find((row) => row.id === selection.hintId);
        if (!hint) {
            return <Missing kind="decision" />;
        }
        return (
    ...
    if (selection.kind === 'knob') {
        const knob = source.inventory?.knobs.find((row) => row.pointer === selection.pointer);
        if (!knob) {
            return <Missing kind="knob" />;
        }
        return (
            <div className={styles.section}>
                <h3>{knob.name}</h3>
    ...
    const command = source.performance?.slowestAgents.find((row) => row.agentId === selection.agentId);
    const stream = source.performance?.streamTiming?.slowestAgents.find((row) => row.agentId === selection.agentId);
    const row = selection.channel === 'stream' ? stream : command;
    if (!row) {
        return <Missing kind="agent timing" />;
    }
    return (
        <div className={styles.section}>
            <h3>{selection.agentId}</h3>
            <Facts
                values={selection.channel === 'stream'
                    ? [
                        ['Channel', 'RTC stream'],
