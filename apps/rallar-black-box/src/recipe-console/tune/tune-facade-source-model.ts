...
export function deriveTuneSourceModelFromFacade(
    input: Readonly<{
    ...
    if (!facade.candidateManifest) {
        addIssue(issues, 'reference-only', 'Candidate preview requires a bounded authoritative manifest window.');
    }
...
export function tuneFacadeIsCurrentFocus(
    ...
    if (!focusRunId || !facadeIdentityIsConsistent(facade, focusRunId)) {
        return false;
    }
    const facadeControlRunId = facade.identity.controlRunId ??
        facade.distributedRun.controlRunId;
    if (expectedControlRunIds.some((controlRunId) => controlRunId !== facadeControlRunId)) {
        return false;
    }
