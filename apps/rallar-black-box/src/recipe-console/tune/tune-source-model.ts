import type { DistributedRunAnalysis, DistributedRunPerformanceAnalysis } from '@shared-test/rallar-bb-test/distributed-artifact-analysis.ts';
import type {
...
export type TuneSourceIssueCode =
    ...
    | 'retained-context-error' | 'unsupported-artifact'
    | 'partial-control' | 'stale-control' | 'reference-only'
    | 'missing-performance' | 'unsafe-identity' | 'ambiguous-focus'
    | 'ambiguous-control' | 'invalid-manifest';
...
export function deriveTuneSourceModel(input: Readonly<{
...
function sourceIssues(input: SourceIssueInput): TuneSourceIssue[] {
    ...
    if (!input.focusRunId) addIssue(issues, 'missing-focus', 'Select a distributed run explicitly.');
        ...
        addIssue(issues, 'unsafe-identity', 'The selected run identity is quarantined from navigation and reusable output.');
    ...
    if (input.retainedRelation === 'mismatched') {
        ...
        addIssue(issues, 'retained-context-error', input.contextErrorMessage ?? 'The retained artifact context is invalid.');
...
function candidateBlockReasons(input: Readonly<{
    distributedRun?: ControlDistributedRunSnapshot;
