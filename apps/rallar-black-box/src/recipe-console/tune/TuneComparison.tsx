            ...
            {comparison.compatibilityWarnings.length > 0
                ? (
                    <ul className={styles.warnings}>
                        {comparison.compatibilityWarnings.map((warning) => <li key={warning.code}>{warning.message}
                        </li>)}
                    </ul>
                )
                : null}
            {comparison.state === 'ready' && structural
                ? (
                    <div className={styles.categories}>
                        <ComparisonCategory
                            ...
                            label="Recipe"
                            value={tuneList([
                                ...structural.recipeDelta.changedProfiles.map((value) => value.replace(' -> ', ' → ')),
                        ...
                        <ComparisonCategory
                            ...
                            label="Participant"
                            value={`Baseline only ${tuneList(structural.participantDelta.leftOnly)} · Candidate only ${
                                tuneList(structural.participantDelta.rightOnly)
                            } · Shared ${tuneList(structural.participantDelta.shared)}`}
                        />
                        <ComparisonCategory
                            category="failure"
                            label="Failure"
                            value={`${structural.failureDelta.leftCount} → ${structural.failureDelta.rightCount} · Removed ${
                                tuneList(structural.failureDelta.leftOnly)
                            } · Added ${tuneList(structural.failureDelta.rightOnly)}`}
                        />
                        <ComparisonCategory
                            category="timing"
                            label="Timing"
                            value={`Duration ${tuneSigned(structural.timingDelta.durationDeltaMs, 'ms')} · Start ${
                                tuneSigned(structural.timingDelta.startedDeltaMs, 'ms')
                            } · Complete ${tuneSigned(structural.timingDelta.completedDeltaMs, 'ms')}`}
                        />
                        <ComparisonCategory
                            category="received-message"
                            label="Received message"
                            value={`${structural.receivedMessageDelta.leftCount} → ${structural.receivedMessageDelta.rightCount} · ${
                                tuneSigned(structural.receivedMessageDelta.delta)
                            }`}
                        />
                        {comparison.performance
                            ? (
                                <ComparisonCategory
                                    ...
                                    value={`${comparison.performance.timingMetric} · ${comparison.performance.selected.statistic} ${
                                        tuneSigned(
                                            comparison.performance.selected.delta,
                                            comparison.performance.selected.unit
                                        )
                                    } · ${comparison.performance.availability}`}
                                />
                            )
                            : null}
                    </div>
                ...
                : (
                    <ul className={styles.issues}>
                        {comparison.issues.map((issue) => <li key={`${issue.field}:${issue.code}`}>{issue.message}
                        </li>)}
                    </ul>
                )}
        </section>
