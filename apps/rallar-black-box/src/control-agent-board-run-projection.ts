...
export function projectRelevantControlAgentBoardRuns(input: Readonly<{
    ...
    if (input.selected) {
        ...
        if (selectedIndex >= 0) projected.splice(selectedIndex, 1);
        if (input.selected.controlRunId === input.controlRunId) {
            if (input.selectedOrdinal === undefined) return undefined;
            projected.push({
                run: input.selected,
