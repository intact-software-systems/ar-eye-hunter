...
export async function waitForHeadlessWorkerExit(
  ...
  const shutdown = observePromise(
    Promise.resolve().then(() =>
      input.waitForShutdown(shutdownController.signal)
    ),
  );
  const completion = observePromise(
    Promise.resolve().then(() =>
      input.waitForCompletion(completionController.signal)
    ),
  );
...
export async function waitForDistributedRunTerminal(
  ...
  const timeoutError = () => new Error(
    `Timed out after ${input.timeoutMs}ms waiting for distributed run ${
      input.runId
...
function createAbortScope(input: Readonly<{
