...
function principalEntry(input: Readonly<{
    filePath: string;
    ...
    return buildWorldFleetManifestEntry({
        ...
        title: `RTC messages principal 50-agent ${long ? '60m' : '30s'} 20 Hz ${input.topologyProfile}${input.diagnostic ? ' diagnostic' : ''}`,
        description:
        ...
        distributedRunId: `${input.diagnostic ? 'world-fleet-diagnostic' : 'world-fleet'}-rtc-messages-principal-50-agent-${runIdSuffix}`,
...
function allPeerEntry(input: Readonly<{
    filePath: string;
    durationSeconds: number;
    rateHz: number;
    ...
    return buildWorldFleetManifestEntry({
        filePath: input.filePath,
        title: `RTC messages all-peer 50-agent ${label} ${input.rateHz} Hz tree${input.diagnostic ? ' diagnostic' : ''}`,
        description:
        ...
        distributedRunId: `${input.diagnostic ? 'world-fleet-diagnostic' : 'world-fleet'}-rtc-messages-all-peer-50-agent-${label}-${input.rateHz}hz-tree`,
...
function multicastMetadata(input: Readonly<{
    topologyProfile: 'tree' | 'mesh';
    senderCount: number;
    durationSeconds: number;
    rateHz: number;
