...
/** One simulated command, narrowed to the kind the runtime answers. */
type SimulatedCommand<Kind extends RallarBlackBoxTestCommand['kind']> =
    & Extract<RallarBlackBoxTestCommand, Readonly<{ kind: Kind; }>>
...
export async function runSimulatedProviderCommand(
    ...
    switch (command.kind) {
        case 'rtc.connect':
            return recordSimulatedRtcConnect(command, context);
        case 'rtc.send':
            return recordSimulatedRtcSend(command, context);
        case 'ws.open':
            return recordSimulatedWsOpen(command, context);
        case 'ws.send':
            return recordSimulatedWsSend(command, context);
        case 'ws.close':
            return recordSimulatedWsClose(command, context);
        case 'http.request':
            return recordSimulatedHttpRequest(command, context);
        default:
            return undefined;
    }
}

function recordSimulatedRtcConnect(
    command: SimulatedCommand<'rtc.connect'>,
    context: RallarBlackBoxTestCommandContext
): RallarBlackBoxTestCommandOutcome {
    const config = context.config();
...
function recordSimulatedRtcSend(
    command: SimulatedCommand<'rtc.send'>,
    context: RallarBlackBoxTestCommandContext
): RallarBlackBoxTestCommandOutcome {
    const manualMetadata = decodeManualCommandMetadata(command.metadata?.manual);
...
function recordSimulatedWsOpen(
    command: SimulatedCommand<'ws.open'>,
    context: RallarBlackBoxTestCommandContext
): RallarBlackBoxTestCommandOutcome {
    context.recordEvent({
        kind: 'diagnostic',
...
function recordSimulatedWsSend(
    command: SimulatedCommand<'ws.send'>,
    context: RallarBlackBoxTestCommandContext
): RallarBlackBoxTestCommandOutcome {
    context.recordEvent({
        kind: 'message',
...
function recordSimulatedWsClose(
    command: SimulatedCommand<'ws.close'>,
    context: RallarBlackBoxTestCommandContext
): RallarBlackBoxTestCommandOutcome {
    context.recordEvent({
        kind: 'event',
...
function recordSimulatedHttpRequest(
    command: SimulatedCommand<'http.request'>,
    context: RallarBlackBoxTestCommandContext
): RallarBlackBoxTestCommandOutcome {
