...
Status: canonical migration register; Iteration 8 guarded History retention UI code-backed through `7256379`, all legacy workflow rows visible and uncut
...
| `runner.compare` | `DistributedRunComparePanel`, `DistributedCompareList`, `compareDistributedRuns`; runs route | Select two runs and show status, recipe, participant, failure, timing, and received-message deltas. | `Tune` | `src/recipe-console/{tune,history}/**`; shared compare/tuning derivation in `packages/shared-test/rallar-bb-test/**` | Mounted only when Runs is active and at least two runs exist. | Lazy `Tune` route, unmounted when inactive. | `compareLeft`, `compareRight`, and `timingMetric` are v1 URL state; candidate changes are explicit output and never silent mutation. | Iteration 7 makes bounded explicit comparison code-backed; Iteration 8 Task 7 adds bounded History selection and selective post-cleanup reconciliation. Legacy comparison remains visible and uncut because canonical copied-URL browser acceptance and the complete workflow cut are still open. | **Capability evidence passed:** `recipe-console-tune.spec.ts` — `compares two runs across recipe participant failure timing and receive deltas`; `compares two runs and emits explicit candidate timing changes without mutation`; Task 7 focused cleanup reconciliation at `7256379`. **Full row cutover still pending:** `recipe-console-history.spec.ts` — `restores versioned view selection filters comparison and timing metric from a copied URL`, plus the Iteration 8 retention/no-cutover exit. | `/?workspace=black-box-runner&tab=runs` | `src/recipe-console/{tune,history}/**`, `packages/shared-test/rallar-bb-test/**`; `cc17169`, `382df72`, `7256379`; 2026-07-12 |
...
| `legacy.distributed-recipes` | `DistributedRecipesPanel` plus preflight/monitor/compare/authoring; `tab=distributed-recipes` (aliases `distributed`, `distributed-runs`, `dist`) → advanced/distributed | Full legacy create/list/read/resolve/stage/start/cancel/export workflow, catalog/manifest/preflight, monitor/history/compare, diagnostics, and schema/AI prompt authoring. | `Advanced` fallback; capabilities migrate to `Execute`, `Monitor`, `Analyze`, and `Tune` | `src/legacy/runner/LegacyDistributedRecipesRoute.tsx`; new feature owners under `src/recipe-console/**`; canonical APIs in `control-run-manager.ts` | Selected-only under Advanced; compatibility pane guards mount by exact legacy tab. | Lazy/unmounted inactive after polling ownership moves to a service. | Selected run, filters, comparison, and timing metric become URL state; token and artifacts stay memory-only. | Iterations 4–8 Task 7 make guided Execute, bounded Monitor, artifact Analyze, core Tune/comparison, bounded History/saved filters, and guarded retention UI code-backed. Canonical History browser acceptance, full diagnostics, schema/AI authoring, local execution, and complete workflow parity remain pending, so this operational fallback stays visible, deep-linkable, uncut, and governed by its unchanged selected-only mount policy. | **Capability evidence passed:** `recipe-console-monitor.spec.ts` — `places the failure verdict and failure list before raw event evidence`; `opens all available correlated evidence from a failure row`; `recipe-console-tune.spec.ts` — `compares two runs across recipe participant failure timing and receive deltas`; `recipe-console-advanced.spec.ts` — `tab=distributed-recipes opens the exact legacy distributed fallback`; Task 7 focused retention validation at `7256379`. **Full row cutover still pending:** Iteration 8 copied-URL/retention browser acceptance and later full diagnostics/schema/AI-authoring/local-execution proof. | `/?workspace=black-box-runner&tab=distributed-recipes` | `src/recipe-console/{execute,monitor,analyze,tune,history}/**`, `src/legacy/runner/LegacyDistributedRecipesRoute.tsx`, `control-run-manager.ts`; `42eedae`, `47c332d`, `cc17169`, `382df72`, `7256379`; 2026-07-12 |
