...
| `runner.runs` | `RunnerRunsPanel`, `RunVerdictPanel`, `CausalTrailPanel`, `RtcPerformancePanel`, `DistributedRunSummary`, participant board; `/?workspace=black-box-runner&tab=runs`, alias `runs` | Select and refresh live/historical distributed runs, retain last run handoff, import/export/copy artifacts, show local verdict/failures/report, select synthetic evidence, and compose nested monitor/analyze/compare surfaces. | `Monitor`, `Analyze`, and `Tune` | `src/recipe-console/{monitor,analyze,tune}/**` with selection/query adapters under `src/recipe-console/control/**` | Existing consolidated primary tab; mounted only while `runs` is active. It polls a selected non-terminal run and auto-loads terminal artifacts. | Monitor and Analyze UI unmount while inactive; Tune is a separate lazy entry that unmounts while inactive. Root query/retained-artifact services alone outlive views. | Selected run, control run, filters, comparison, and timing metric belong in URL state. Tokens and artifact payloads stay in memory. Poll ownership must move before unmounting. | Iterations 5–7 make bounded live monitoring, distributed artifact analysis, real timing/tuning, and explicit two-run comparison code-backed through the root query and retained artifact authority. Legacy Runs still owns server history and generic/local-run composition; saved-filter and retention replacement proof remains pending. It therefore remains visible, deep-linkable, uncut, and governed by its unchanged active-only mount policy. | **Capability evidence passed:** `recipe-console-monitor.spec.ts` — `preserves last-known evidence while a selected run refresh fails`; `recipe-console-analyze.spec.ts` — `imports a partial bundle offline and focuses the first actionable failure`; `recipe-console-tune.spec.ts` — `shows command percentiles cadence drift drops and backpressure for an RTC stream`; `recipe-console-legacy-monitor-handoff.spec.ts` — `opens the selected legacy Runs context from a two-run Monitor link`. **Full row cutover still pending:** Iteration 8 copied-URL History/retention acceptance plus remaining generic/local-run responsibility proof. | `/?workspace=black-box-runner&tab=runs` | `src/recipe-console/{monitor,analyze,tune}/**`, `src/legacy/runner/runs/legacy-run-url-selection.ts`, `control-run-manager.ts`; `42eedae`, `47c332d`, `cc17169`, `382df72`; 2026-07-12 |
| `runner.distributed-monitor` | `DistributedRunMonitorPanel`, nested composite/timeline/diagnostic/progress views inside `RunnerRunsPanel` and `DistributedRecipesPanel`; runs/distributed routes | Verdict, failure-first evidence, agent/recipe progress, ACK/barrier readiness, lifecycle timeline, structured WS/RTC diagnostics, latency, event filtering, composite loop/parallel/wait/assert drilldowns, and artifact validation. | `Monitor` | `src/recipe-console/monitor/**`; deterministic derivation remains in `packages/shared-test/rallar-bb-test/**` | Mounted only with its owning active runs/distributed tree and selected monitor. | Lazy `Monitor` route; inactive view unmounted; shared deterministic helpers only. | Selection/filter fields use v1 URL state. No duplicate monitor derivation in React. | Iteration 5 Recipe Console core Monitor replacement is code-backed: failure-first current/last-known truth, selected-failure correlations, progress/readiness, bounded secondary evidence, and current-safe Cancel/artifact operations pass. This is replacement evidence for the bounded Monitor capability, not a cutover or hide of either owning legacy workflow row. | `tests/playwright/rallar-black-box/recipe-console-monitor.spec.ts` — `places the failure verdict and failure list before raw event evidence`; `opens all available correlated evidence from a failure row`; `preserves last-known evidence while a selected run refresh fails` | `/?workspace=black-box-runner&tab=runs` or `/?workspace=black-box-runner&tab=distributed-recipes` | `src/recipe-console/monitor/**`, `packages/shared-test/rallar-bb-test/**`; `42eedae`; 2026-07-12 |
| `runner.artifact-analysis` | `DistributedRunAnalysisReportPanel`, `ImportedDistributedArtifactAnalysisPanel`, artifact import in `RunnerRunsPanel`; runs route | Live/offline artifact parsing, causal/failure report, missing-file and parse warnings, issue-ready summary/fix/performance markdown, and first actionable failure focus. | `Analyze` | `src/recipe-console/analyze/**`; validators/parsers in `packages/shared-test/rallar-bb-test/**` | Mounted only under active Runs when analysis exists. | Analyze is eager within the Recipe Console chunk, but its UI is unmounted when inactive; a bounded root hook retains imported bytes only in memory across Recipe Console view navigation. | Profile/version-aware parsing; partial bundles remain usable. Never persist large artifacts locally. Unsafe artifact identities stay out of URLs and filenames. | Iteration 6 makes the bounded distributed-run Analyze replacement code-backed for local/control import, failure-first analysis, search, quality/performance, Markdown, and export. Legacy Runs still owns history/comparison/local-run composition and the separate generic black-box-runner Shared Test importer remains visible, so no legacy row is hidden or cut over. | `tests/playwright/rallar-black-box/recipe-console-analyze.spec.ts` — `imports a partial bundle offline and focuses the first actionable failure`; `recipe-console-analyze-safety.spec.ts`; `recipe-console-analyze-visual.spec.ts`; `recipe-console-analyze-handoff.spec.ts` | `/?workspace=black-box-runner&tab=runs`; generic fallback `/?workspace=black-box-runner&tab=shared-test` | `src/recipe-console/analyze/**`, `packages/shared-test/rallar-bb-test/**`; `f96b5b4`; `abe257e`; `9b07330`; `47c332d`; 2026-07-12 |
| `runner.compare` | `DistributedRunComparePanel`, `DistributedCompareList`, `compareDistributedRuns`; runs route | Select two runs and show status, recipe, participant, failure, timing, and received-message deltas. | `Tune` | `src/recipe-console/tune/**`; shared compare/tuning derivation in `packages/shared-test/rallar-bb-test/**` | Mounted only when Runs is active and at least two runs exist. | Lazy `Tune` route, unmounted when inactive. | `compareLeft`, `compareRight`, and `timingMetric` are v1 URL state; candidate changes are explicit output and never silent mutation. | Iteration 7 makes the bounded explicit comparison replacement code-backed, including structural and selected performance deltas, compatibility warnings, restored URLs, validated candidate patch/diff, and no mutation. Legacy comparison remains visible and uncut because Iteration 8 history/saved-filter/retention proof and the complete workflow cut are still open. | **Capability evidence passed:** `recipe-console-tune.spec.ts` — `compares two runs across recipe participant failure timing and receive deltas`; `compares two runs and emits explicit candidate timing changes without mutation`. **Full row cutover still pending:** `recipe-console-history.spec.ts` — `restores versioned view selection filters comparison and timing metric from a copied URL`, plus the Iteration 8 retention/no-cutover exit. | `/?workspace=black-box-runner&tab=runs` | `src/recipe-console/tune/**`, `packages/shared-test/rallar-bb-test/**`; `cc17169`, `382df72`; 2026-07-12 |
...
| `legacy.distributed-recipes` | `DistributedRecipesPanel` plus preflight/monitor/compare/authoring; `tab=distributed-recipes` (aliases `distributed`, `distributed-runs`, `dist`) → advanced/distributed | Full legacy create/list/read/resolve/stage/start/cancel/export workflow, catalog/manifest/preflight, monitor/history/compare, diagnostics, and schema/AI prompt authoring. | `Advanced` fallback; capabilities migrate to `Execute`, `Monitor`, `Analyze`, and `Tune` | `src/legacy/runner/LegacyDistributedRecipesRoute.tsx`; new feature owners under `src/recipe-console/**`; canonical APIs in `control-run-manager.ts` | Selected-only under Advanced; compatibility pane guards mount by exact legacy tab. | Lazy/unmounted inactive after polling ownership moves to a service. | Selected run, filters, comparison, and timing metric become URL state; token and artifacts stay memory-only. | Iterations 4–7 make guided Execute, bounded Monitor, artifact Analyze, and core Tune/comparison replacements code-backed. Server history, retention, full diagnostics, schema/AI authoring, local execution, and complete workflow parity remain pending, so this operational fallback stays visible, deep-linkable, uncut, and governed by its unchanged selected-only mount policy. | **Capability evidence passed:** `recipe-console-monitor.spec.ts` — `places the failure verdict and failure list before raw event evidence`; `opens all available correlated evidence from a failure row`; `recipe-console-tune.spec.ts` — `compares two runs across recipe participant failure timing and receive deltas`; `recipe-console-advanced.spec.ts` — `tab=distributed-recipes opens the exact legacy distributed fallback`. **Full row cutover still pending:** Iteration 8 copied-URL History/retention acceptance and later full diagnostics/schema/AI-authoring/local-execution proof. | `/?workspace=black-box-runner&tab=distributed-recipes` | `src/recipe-console/{execute,monitor,analyze,tune}/**`, `src/legacy/runner/LegacyDistributedRecipesRoute.tsx`, `control-run-manager.ts`; `42eedae`, `47c332d`, `cc17169`, `382df72`; 2026-07-12 |
