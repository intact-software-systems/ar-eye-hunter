...
Status: canonical migration register; Iteration 6 artifact analysis code-backed through `f96b5b4`, `abe257e`, `9b07330`, and `47c332d`, all legacy workflow rows visible and uncut
...
| `legacy.distributed-recipes` | `DistributedRecipesPanel` plus preflight/monitor/compare/authoring; `tab=distributed-recipes` (aliases `distributed`, `distributed-runs`, `dist`) → advanced/distributed | Full legacy create/list/read/resolve/stage/start/cancel/export workflow, catalog/manifest/preflight, monitor/history/compare, diagnostics, and schema/AI prompt authoring. | `Advanced` fallback; capabilities migrate to `Execute`, `Monitor`, `Analyze`, and `Tune` | `src/legacy/runner/LegacyDistributedRecipesRoute.tsx`; new feature owners under `src/recipe-console/**`; canonical APIs in `control-run-manager.ts` | Selected-only under Advanced; compatibility pane guards mount by exact legacy tab. | Lazy/unmounted inactive after polling ownership moves to a service. | Selected run, filters, and comparison become URL state; token and artifacts stay memory-only. | Iteration 4 makes Execute and Iteration 5 makes the bounded core Monitor replacement code-backed. History, comparison, full diagnostics, and authoring parity remain pending, so this operational fallback stays visible, deep-linkable, uncut, and governed by its unchanged selected-only mount policy. | `tests/playwright/rallar-black-box/recipe-console-monitor.spec.ts` — `places the failure verdict and failure list before raw event evidence`; `opens all available correlated evidence from a failure row`; `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `tab=distributed-recipes opens the exact legacy distributed fallback` | `/?workspace=black-box-runner&tab=distributed-recipes` | `src/recipe-console/{execute,monitor}/**`, `src/legacy/runner/LegacyDistributedRecipesRoute.tsx`, `control-run-manager.ts`; `42eedae`; 2026-07-12 |
