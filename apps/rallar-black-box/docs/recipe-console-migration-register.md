...
This dated section supersedes older baseline mount/owner descriptions in the
historical surface rows below. Responsibilities and rollback URLs remain
authoritative; current lifetime and cutover state is the table here.

| Current lifetime class                                    | Stable surface IDs                                                                                                                                                                                                                                              | Actual Iteration 11 owner and policy                                                                                                                                                                                                                                                      | Cutover evidence/status                                                                                                                                                                                                 |
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Safe dynamic/unmounted                                    | `runner.recipes`, `runner.runs`, `runner.fleet`, `runner.builder`, `legacy.distributed-recipes`, `legacy.run-manager`, `legacy.shared-test-catalog`, `legacy.shared-test-import`, `direct.groups-clients`, `direct.topology`, `direct.rtc-diagnostics`          | `RunnerWorkspaceTabPanels`, `RunnerAdvancedPanel`, and `DirectConnectionTabPanels` use local `lazy`/`Suspense` boundaries and mount only the active owner. Shared Test catalog/import intentionally share one dynamic panel/chunk. Leaving the route removes its DOM owner and view work. | Dynamic entries are production-reachable and absent from the legacy static closure. All ten target chunks are exercised and unmounted; Runs polling stops. These remain operational legacy fallbacks, not retired rows. |
| Stateful exception, only inside active `LegacyExperience` | `direct.quick-test`, `direct.auth`, `direct.websocket`, `direct.rtc-realtimes`, `direct.rallar-data`, `direct.crdt`, `direct.media`, `legacy.local-workbench`, `legacy.manual-rallar`, `direct.rallar-trace`, `diagnostic.event-stream`, `direct.rallar-server` | Existing view owners remain hidden-mounted to preserve the exact documented drafts, connection/subscription/media/document, trace/event, feedback/result, execution, or busy/error state. None belongs to the Recipe Console chunk or mounts on cold Recipe Console.                      | The exception matrix is explicit and browser-tested. No exception is claimed lazy, and no secret or bulk artifact state was newly persisted.                                                                            |
| Context bridge, no runtime owner                          | all 22 Advanced catalog leaves                                                                                                                                                                                                                                  | `advanced-surface-catalog.ts`, `advanced-legacy-href.ts`, and the shared React-free classifier describe routes/links only. They import no legacy React/CSS, own no fetch/timer/credential, and cannot become a runtime registry.                                                          | Exact bounded links, all aliases, visible owner, contextual consumption, and safe return pass.                                                                                                                          |

The unreachable `RunnerCompatibilityTabPanels`,
`LegacyCompatibilityTailTabPanels`, and duplicate direct Manual pane were
removed only after exact alias/owner/draft tests proved their real Advanced or
Builder destinations. There is now exactly one DOM owner for each surface.
The actual Advanced wrapper remains small composition glue: Manual Rallar and
Local Workbench keep their stateful mounts, while Distributed Recipes, Run
Manager, and Shared Test are selected-only dynamic owners.

Ready-State #11–#13 are satisfied by the three exact tests in
`recipe-console-advanced.spec.ts`. They prove the six-item primary navigation,
all 22 canonical/contextual routes and aliases, five failure-derived tools,
safe context/return, ten lazy owners/effect shutdown, twelve exceptions, and
cold reciprocal chunk/CSS isolation. Fresh qualification passes 1,564/1,564
app tests across 148 files, shared/app type and Deno gates, a 776-module
build/chunk assertion, and 190 Recipe Console cases with one configured-live
skip, plus 30 legacy cases with
two configured-stack skips. The exact unavailable prerequisite remains `Set
...
The final independent review found that production return anchors and the
browser-proven Advanced helper had diverged. `78e2c13` routes both through one
neutral 4,096-byte builder, retains canonical `legacySurface` ahead of
lower-priority selection, and proves the exact production URL in unit and
browser coverage. Focused re-review reports no remaining finding.

No row is retired or newly hidden in the legacy rollback experience. Every old
alias and rollback URL remains operational; no public export, existing control
contract, destructive behavior, or default changed. Iteration 12 owns the
blank-URL default flip and final accessibility debt.
...
existing control-server contract changed. At that checkpoint Iterations 11–12
retained Advanced/context, safe lazy legacy ownership, default-flip, and final
accessibility risk; the qualified Iteration 11 section above supersedes that
pending state.
...
## Recipe Console browser-agent launch implementation checkpoint — 2026-07-15

Recipe Console Execute now owns inline local browser-agent setup and a
state-derived execution runway without importing `runner.recipes` React or its
legacy experience chunk. The root control provider constructs the launch
service with provider, group, control/API origins, credential policy, and
authorized token issuance. Execute supplies only the entered run ID and
generated agent IDs, retains only a sanitized cohort, and reconciles
registration through the existing root query/polling authority.

The non-legacy launch service is also consumed by `runner.recipes` during the
migration. Popup reservation/navigation, URL construction, explicit API-base
ticket issuance, identity validation, and fragment-secret rules therefore have
one implementation boundary. Legacy React state no longer retains raw launch
URLs. Existing `mode=control` URLs remain compatible, including old query-token
inputs, and all consumed launch secrets are scrubbed before lazy UI loading.

Execute presents only the current forward action. Resolve, Create, and Stage
are direct; Start and Cancel use dialogs; root polling advances registration
and ACK waits; Monitor is the successor after Start. The deterministic primary
acceptance performs exactly these eight named clicks: Composite Evidence, Open
3 browser agents, Resolve 3 targets, Create draft, Stage 3 agents, Review and
start, Start distributed run, and Monitor run. It performs no Refresh, DOM
`evaluate(...click())`, direct lifecycle call, or legacy navigation.

Configured-live qualification now passes without a skip: a logged-in operator
opens three browser-rallar agents through these visible setup controls, each
child scrubs its launch fragment and reports a distinct fresh session, the
exact cohort completes the deterministic distributed lifecycle through
Monitor, and the exported artifact contains the exact run/group/agent identity
without launch-secret fields. The scoped automated gate also passes 276 focused
unit tests, 79 control-server tests, 200/200 available Recipe Console browser
cases with its configured-live case separately enabled, 30 preserved legacy
launch/navigation cases, shared/app TypeScript, and the 818-module production
build.

The exact wider `npm run test:rallar:full-stack:postgres:distributed` command
passes the current-UI configured-live case and both simulated distributed
cases, but remains 3/4 because legacy Distributed Recipes does not render the
expected `room.black-box.rtc-realtime.position` diagnostics evidence in its
live RTC case. That repeated failure is outside this launch/lifecycle migration
but remains a release qualification failure rather than being waived.

This is still not a cutover claim. The following gates remain open until
separately recorded evidence exists:

1. The moderated five-operator usability criteria in
   `recipe-console-agent-launch-usability-report.md` pass on an exact build.
2. The exact aggregate configured-live distributed command passes, including
   its separately owned legacy live-RTC diagnostics case.

No legacy row is hidden or retired. Operational rollback remains:

```text
/?experience=legacy&workspace=black-box-runner&tab=recipes
```

A later product decision, after all three gates pass, is required to change
that availability.
...
| Stable surface groups                                                                                                                                           | Current legacy composition owner                           | Iteration 11 exit mount policy                                                                                                                                                                                  |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `runner.recipes`, `runner.runs`, `runner.fleet`, primary `runner.builder`, `runner.advanced-wrapper`                                                            | `src/legacy/shell/tabs/runner-workspace-tab-panels.tsx`    | Recipes/Runs/Fleet/Builder are active-only dynamic imports. Advanced remains hidden-mounted only to compose its explicit exceptions and selected dynamic children.                                              |
| `direct.quick-test`, `direct.auth`, `direct.websocket`, `direct.rtc-realtimes`; safe `direct.groups-clients`, `direct.topology`, `direct.rtc-diagnostics`       | `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`   | Quick/Auth/WS/RTC-Realtimes remain documented exceptions. Groups/Clients, Topology, and RTC Diagnostics are active-only dynamic imports and unmount on exit.                                                    |
| `direct.rallar-data`, `direct.crdt`, `direct.media`                                                                                                             | `src/legacy/shell/tabs/DirectResourceTabPanels.tsx`        | All three documented stateful lifetimes remain hidden-mounted only inside active legacy.                                                                                                                        |
| `legacy.local-workbench`, `legacy.manual-rallar`, `legacy.run-manager`, `legacy.distributed-recipes`, `legacy.shared-test-catalog`, `legacy.shared-test-import` | `src/legacy/runner/advanced/runner-advanced-panel.tsx`     | Workbench and Manual remain documented exceptions. Run Manager, Distributed Recipes, and the shared catalog/import panel are selected-only dynamic imports. The duplicate compatibility groups no longer exist. |
| `direct.rallar-trace`, `diagnostic.event-stream`, `direct.rallar-server`                                                                                        | `src/legacy/shell/tabs/diagnostic-evidence-tab-panels.tsx` | All remain documented stateful exceptions hidden-mounted only inside active legacy.                                                                                                                             |
...
| `runner.builder`             | `FlowBuilderPanel`; `tab=builder` (alias `flow`) and compatibility `tab=flow-builder` (alias `flows`)                                                                                                                                                                             | Compose HTTP, WS, RTC, wait, cleanup, variables, editable flow JSON, SPA recipe export, runner scenario export, schema feedback, and inline recipe execution.                                                                                                                          | `Advanced`                                                                               | `src/legacy/runner/builder/**`, composed by `src/legacy/shell/tabs/runner-workspace-tab-panels.tsx`; contextual catalog under `src/recipe-console/advanced/**`                                       | Exactly one `FlowBuilderPanel` is lazy-loaded and mounted only for the active legacy `builder` tab; `flow-builder` resolves to that same owner.                                                                                                                                                                                                                    | Iteration 11 exit policy is the current active-only dynamic owner; no hidden compatibility pane remains.                                                                                    | The draft remains view-owned only while Builder is mounted. No silent recipe mutation and no secret persistence were introduced.                                                                                                                                                                                                                             | Existing consolidation to Builder remains operational; no Recipe Console cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=black-box-runner&tab=flow-builder`                                                                     | `src/legacy/runner/builder/**`, `src/legacy/shell/tabs/runner-workspace-tab-panels.tsx`, `app-tabs.ts`; `318502d`, `84493f3`, `2502f50`; 2026-07-14                                                                                       |
| `runner.advanced-wrapper`    | `RunnerAdvancedPanel`; `tab=advanced` (alias `debug`) with `advancedSurface=<surface>` or `advanced=<surface>`                                                                                                                                                                    | Switch and deep-link among Workbench, Distributed Recipes, Run Manager, Manual Rallar, and Shared Test raw controls.                                                                                                                                                                   | `Advanced`                                                                               | `src/legacy/runner/advanced/runner-advanced-panel.tsx`, composed by `src/legacy/shell/tabs/runner-workspace-tab-panels.tsx`; Recipe Console bridge under `src/recipe-console/advanced/**`            | The wrapper is hidden-mounted only inside active `LegacyExperience`; Workbench and Manual are its two hidden-mounted stateful children, while Distributed Recipes, Run Manager, and Shared Test are selected-only dynamic imports.                                                                                                                                 | Iteration 11 exit policy is the current bounded composition; there is no surface registry or compatibility router.                                                                          | `advancedSurface`/`advanced` preserve legacy child selection; versioned Recipe Console bridge state uses canonical `legacySurface`. Exceptions remain documented per child.                                                                                                                                                                                  | Existing consolidated wrapper remains operational; no Recipe Console cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `keeps direct Rallar diagnostics out of primary navigation and opens them from Advanced`; `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                         | `/?workspace=black-box-runner&tab=advanced&advancedSurface=workbench`                                               | `src/legacy/runner/advanced/runner-advanced-panel.tsx`, `src/recipe-console/advanced/**`, `app-tabs.ts`; `318502d`, `84493f3`, `2502f50`, `78e2c13`; 2026-07-14                                                                           |
| `legacy.manual-rallar`       | `ManualRallarSection`/`ManualRallarWorkbenchPanel`, inbox/history; `tab=manual-rallar` (alias `manual`) → advanced/manual                                                                                                                                                         | Configure/join/connect/send/health/close/reset, scoped RTC and delivery matrices, NACK/negative probes, recipe/command export, received data, and command history.                                                                                                                     | `Advanced`                                                                               | `src/legacy/runner/manual/**`, composed once by `src/legacy/runner/advanced/runner-advanced-panel.tsx`                                                                                               | Exactly one Manual owner remains hidden-mounted inside the Advanced wrapper while `LegacyExperience` is active; the duplicate direct Manual pane was removed.                                                                                                                                                                                                      | Documented Iteration 11 stateful exception; lazy unmount still requires explicit redacted draft persistence and runtime-task ownership.                                                     | **Temporary stateful exception:** Manual values/payload draft and active runtime work; passwords are stripped and JSON is redacted.                                                                                                                                                                                                                          | Preserved legacy; no replacement, cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=black-box-runner&tab=manual-rallar`                                                                    | `src/legacy/runner/manual/**`, `src/legacy/runner/advanced/runner-advanced-panel.tsx`, `app-tabs.ts`; `318502d`, `2502f50`; 2026-07-14                                                                                                    |
| `legacy.local-workbench`     | `LocalWorkbenchSection`, `WorkbenchPanel`, `ControlPanel`, bootstrap/configuration/queue/report panels; `tab=local-workbench` (aliases `workbench`, `local`) → advanced/workbench                                                                                                 | Local/fake runtime recipe editing/loading/execution/cancel/reset, queue, bootstrap/config, report, and control-agent workspace used by runner-agent launch URLs.                                                                                                                       | `Advanced`                                                                               | `src/legacy/runner/workbench/**`, composed by `src/legacy/runner/advanced/runner-advanced-panel.tsx`; process-long runtime service remains `runtime-store.ts`                                        | Exactly one Workbench owner remains hidden-mounted inside the Advanced wrapper while `LegacyExperience` is active; no compatibility tree remains.                                                                                                                                                                                                                  | Documented Iteration 11 stateful exception; future lazy unmount requires external draft/execution ownership, while the runtime service may outlive the view.                                | **Temporary stateful exception:** recipe/command drafts and active local execution. Preserve `mode=control` launch bootstrap and fragment-ticket scrubbing.                                                                                                                                                                                                  | Preserved legacy; no replacement, cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=black-box-runner&tab=local-workbench`                                                                  | `src/legacy/runner/workbench/**`, `src/legacy/runner/advanced/runner-advanced-panel.tsx`, `runner-agent-launch.ts`, `runtime-store.ts`; `318502d`, `2502f50`; 2026-07-14                                                                  |
| `legacy.run-manager`         | `RunManagerPanel`; `tab=run-manager` (aliases `manager`, `control`, `orchestrator`) → advanced/run-manager                                                                                                                                                                        | Control-server run/agent selection, bounded snapshots, bulk command enqueue, reset/delete, command/result/event inspection, artifact validation/export/copy, JSONL, and failure bundle operations.                                                                                     | `Advanced`                                                                               | `src/legacy/runner/run-manager/RunManagerPanel.tsx`, lazy-loaded by `src/legacy/runner/advanced/runner-advanced-panel.tsx`; APIs live in `src/control-run-manager/`                                  | Exactly one Run Manager owner mounts only while active Advanced selects `run-manager`; it unmounts on exit and no compatibility pane remains.                                                                                                                                                                                                                      | Iteration 11 exit policy is the current selected-only dynamic owner with view-owned work stopped on exit.                                                                                   | IDs/filters may be URL-backed; the token stays memory-only and existing destructive behavior is unchanged.                                                                                                                                                                                                                                                   | Preserved, deep-linkable legacy fallback; no Recipe Console cutover or retirement.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=black-box-runner&tab=run-manager`                                                                      | `src/legacy/runner/run-manager/RunManagerPanel.tsx`, `src/legacy/runner/advanced/runner-advanced-panel.tsx`, `control-run-manager.ts`; `318502d`, `84493f3`, `2502f50`; 2026-07-14                                                        |
| `legacy.distributed-recipes` | `DistributedRecipesPanel` plus preflight/monitor/compare/authoring; `tab=distributed-recipes` (aliases `distributed`, `distributed-runs`, `dist`) → advanced/distributed                                                                                                          | Full legacy create/list/read/resolve/stage/start/cancel/export workflow, catalog/manifest/preflight, monitor/history/compare, diagnostics, and schema/AI prompt authoring.                                                                                                             | `Advanced` fallback; capabilities migrate to `Execute`, `Monitor`, `Analyze`, and `Tune` | `src/legacy/runner/distributed-recipes/**`, lazy-loaded by `src/legacy/runner/advanced/runner-advanced-panel.tsx`; new owners under `src/recipe-console/**`; APIs live in `src/control-run-manager/` | Exactly one Distributed Recipes owner mounts only while active Advanced selects `distributed`; it unmounts and stops view-owned work on exit.                                                                                                                                                                                                                      | Iteration 11 exit policy is the current selected-only dynamic owner.                                                                                                                        | Selected run, filters, comparison, and timing metric may use URL state; tokens and artifact payloads stay memory-only.                                                                                                                                                                                                                                       | Iterations 4–8 qualify guided replacement capabilities, but diagnostics, schema/AI authoring, local execution, configured-live proof, and complete row parity remain pending. This fallback stays visible, deep-linkable, operational, uncut, and unretired.                                                                                                                                                                                                                                                                                                                                         | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=black-box-runner&tab=distributed-recipes`                                                              | `src/legacy/runner/distributed-recipes/**`, `src/legacy/runner/advanced/runner-advanced-panel.tsx`, `src/recipe-console/{execute,monitor,analyze,tune,history}/**`, `control-run-manager.ts`; `318502d`, `84493f3`, `2502f50`; 2026-07-14 |
| `legacy.shared-test-catalog` | `SharedTestCatalogPanel`; `tab=shared-test` (aliases `artifacts`, `shared`, `shared-test-runner`) → advanced/shared-test                                                                                                                                                          | Browse browser-safe shared-test fixtures, capability/coverage ownership, recipe paths, and copyable runner commands.                                                                                                                                                                   | `Advanced`                                                                               | `src/legacy/runner/shared-test/SharedTestPanel.tsx`, lazy-loaded by `src/legacy/runner/advanced/runner-advanced-panel.tsx`                                                                           | Exactly one shared catalog/import panel mounts only while active Advanced selects `shared-test`; the always-mounted duplicate compatibility panel was removed.                                                                                                                                                                                                     | Iteration 11 exit policy is the current selected-only dynamic owner; catalog and import intentionally share one chunk.                                                                      | Catalog selection may use URL/personal state; no shell execution is introduced.                                                                                                                                                                                                                                                                              | Preserved, deep-linkable legacy fallback; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=black-box-runner&tab=shared-test`                                                                      | `src/legacy/runner/shared-test/**`, `src/legacy/runner/advanced/runner-advanced-panel.tsx`, `app-tabs.ts`; `318502d`, `84493f3`, `2502f50`; 2026-07-14                                                                                    |
| `legacy.shared-test-import`  | `SharedTestArtifactImportPanel`; `tab=shared-test` (aliases `artifacts`, `shared`, `shared-test-runner`)                                                                                                                                                                          | Import and validate runner artifact bundles and project imported events, RTC diagnostics/messages, failure focus, summary, and replay recipe.                                                                                                                                          | `Analyze` with `Advanced` fallback                                                       | Legacy import under `src/legacy/runner/shared-test/**`; replacement analysis under `src/recipe-console/analyze/**`                                                                                   | The import shares the single selected-only dynamic `SharedTestPanel`; no hidden compatibility instance remains. Analyze is separately unmounted while inactive.                                                                                                                                                                                                    | Iteration 11 exit policy is the current shared dynamic legacy owner; the Analyze importer remains a separate bounded replacement capability.                                                | Large files stay in memory; profile/version warnings remain visible and partial supported evidence remains usable.                                                                                                                                                                                                                                           | Legacy import remains operational and deep-linkable; no Recipe Console cutover or retirement.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`; `tests/playwright/rallar-black-box/recipe-console-analyze.spec.ts` — `imports a partial bundle offline and focuses the first actionable failure`                                                                                                                                                 | `/?workspace=black-box-runner&tab=shared-test`                                                                      | `src/legacy/runner/shared-test/**`, `src/recipe-console/analyze/**`; `318502d`, `84493f3`, `2502f50`; 2026-07-14                                                                                                                          |
| `direct.quick-test`          | `QuickRallarTestPanel`; `/?workspace=rallar&tab=quick-test`, aliases `quick`, `smoke`                                                                                                                                                                                             | Shortest real-data group create/join and room WS subscribe/send/wait/receive workflow; owns editable form values, the active subscription, received messages, current wait state, and sender/group/type/topic/context/resource evidence.                                               | `Advanced`                                                                               | `src/legacy/diagnostics/quick-test/**`, composed by `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`                                                                                         | Hidden-mounted only while `LegacyExperience` is active to retain editable values, subscription, messages, and wait state; it remains the legacy blank-URL fallback absent stored navigation.                                                                                                                                                                       | Documented Iteration 11 stateful exception; future lazy unmount requires explicit persistence/service ownership or an accepted reset contract and subscription closure.                     | **Temporary stateful exception:** editable values, active subscription, received messages, and wait state remain panel-owned. Do not unmount solely for the default flip without the named proof.                                                                                                                                                            | Existing direct diagnostic remains operational, visible, and deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=rallar&tab=quick-test`                                                                                 | `src/legacy/diagnostics/quick-test/**`, `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`, `app-tabs.ts`; `2502f50`; 2026-07-14                                                                                                    |
| `direct.auth`                | `LoginScreen`, `AuthCommandCenterPanel`; `/?workspace=rallar&tab=auth`, aliases `login`, `session`                                                                                                                                                                                | Login/register/restore/logout/session clear, API/user/password drafts, ticket creation and current ticket state, action history, negative auth checks, TTL and redacted session evidence, plus the real-provider auth gate.                                                            | Shell infrastructure and `Advanced`                                                      | `src/legacy/shell/LoginScreen.tsx` and `src/legacy/diagnostics/auth/**`; diagnostic composition in `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`                                          | The diagnostic panel is hidden-mounted only while `LegacyExperience` is active to retain drafts/ticket/history; the shared auth gate remains bootstrap infrastructure.                                                                                                                                                                                             | Documented Iteration 11 stateful exception; future lazy diagnostic unmount requires non-secret state ownership and an accepted reset of user/password/ticket state.                         | **Temporary stateful exception:** API/user/password drafts, current ticket, and action history remain panel-owned. Credentials and other secrets are never persisted.                                                                                                                                                                                        | Existing auth infrastructure/diagnostic remains operational; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `keeps direct Rallar diagnostics out of primary navigation and opens them from Advanced`; `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                         | `/?workspace=rallar&tab=auth`                                                                                       | `src/legacy/shell/LoginScreen.tsx`, `src/legacy/diagnostics/auth/**`, `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`, `runtime-store.ts`; `2502f50`, `78e2c13`; 2026-07-14                                                      |
| `direct.groups-clients`      | Thin `RoomsClientsPanel`, focused controller/view/request/contracts/derivations under `src/legacy/diagnostics/rooms-clients/**`; `/?workspace=rallar&tab=rooms-clients`, aliases `rooms`, `groups`, `clients`, `people`, `presence`                                               | Group/client/session/presence/state-event REST and direct-room actions, filters/sorts, membership assertions, cleanup, context promotion, diagnostics, and recipe snippets.                                                                                                            | `Advanced`                                                                               | `src/legacy/diagnostics/rooms-clients/**`, lazy-loaded by `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`; contextual links under `src/recipe-console/{advanced,monitor}/**`                | Mounted only for active `rooms-clients`; the panel is a dynamic import, unmounts on exit, and is absent from cold Recipe Console.                                                                                                                                                                                                                                  | Iteration 11 exit policy is the current active-only dynamic owner; view-owned refresh work ends on unmount.                                                                                 | Filters/drafts require explicit personal persistence only if later retained; bounded bridge context carries no credentials.                                                                                                                                                                                                                                  | R2 parity remains intact and the diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `keeps direct Rallar diagnostics out of primary navigation and opens them from Advanced`; `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                         | `/?workspace=rallar&tab=rooms-clients`                                                                              | `src/legacy/diagnostics/rooms-clients/**`, `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`, `src/recipe-console/{advanced,monitor}/**`; `84493f3`, `2502f50`, `78e2c13`; 2026-07-14                                              |
| `direct.websocket`           | `WebSocketCommandCenterPanel`; `/?workspace=rallar&tab=websocket`, aliases `ws`, `socket`, `websockets`                                                                                                                                                                           | Rallar app WS send/subscribe/wait/receive, raw ticket/socket diagnostics, route/payload presets, reconnect/close, visible diagnostics, and recipe/parity export.                                                                                                                       | `Advanced`                                                                               | `src/legacy/diagnostics/websocket/**`, composed by `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`; contextual links under `src/recipe-console/{advanced,monitor}/**`                       | Hidden-mounted only while `LegacyExperience` is active because it can own form and socket/subscription state.                                                                                                                                                                                                                                                      | Documented Iteration 11 stateful exception; future lazy unmount requires service ownership or explicit socket/subscription closure.                                                         | **Temporary stateful exception:** active socket/subscription and form values; tickets are never persisted.                                                                                                                                                                                                                                                   | Existing direct diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `keeps direct Rallar diagnostics out of primary navigation and opens them from Advanced`; `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                         | `/?workspace=rallar&tab=websocket`                                                                                  | `src/legacy/diagnostics/websocket/**`, `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`, `src/recipe-console/{advanced,monitor}/**`; `2502f50`, `78e2c13`; 2026-07-14                                                             |
| `direct.rtc-realtimes`       | `RtcRealtimePanel`; `/?workspace=rallar&tab=rtc-realtime`, aliases `realtime`, `rtcrealtime`                                                                                                                                                                                      | Direct `realtime` and `messages.rtc` connect/send/subscribe/wait, lane health, received messages, timing phases, and runner-recipe export.                                                                                                                                             | `Advanced`                                                                               | `src/legacy/diagnostics/rtc-realtime/**`, composed by `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`                                                                                       | Hidden-mounted only while `LegacyExperience` is active because it can own RTC subscriptions and received evidence.                                                                                                                                                                                                                                                 | Documented Iteration 11 stateful exception; future lazy unmount requires explicit connection/subscription ownership or closure.                                                             | **Temporary stateful exception:** RTC connection/subscription and received evidence; secrets remain excluded.                                                                                                                                                                                                                                                | Existing direct diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=rallar&tab=rtc-realtime`                                                                               | `src/legacy/diagnostics/rtc-realtime/**`, `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`; `2502f50`; 2026-07-14                                                                                                                 |
| `direct.topology`            | `TopologyGraphPanel`; `/?workspace=rallar&tab=topology`                                                                                                                                                                                                                           | Derive/search/filter a graphology/Sigma topology, cap visible nodes, summarize routes, and select commands from runtime events.                                                                                                                                                        | `Advanced`                                                                               | `src/legacy/diagnostics/topology/**`, lazy-loaded by `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`                                                                                        | Mounted only for active `topology`; the dynamic panel unmounts and disposes its renderer on exit and is absent from cold Recipe Console.                                                                                                                                                                                                                           | Iteration 11 exit policy is the current active-only dynamic owner.                                                                                                                          | Search/node limit may use URL/personal state; renderer resources remain view-owned and dispose on exit.                                                                                                                                                                                                                                                      | Existing direct diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=rallar&tab=topology`                                                                                   | `src/legacy/diagnostics/topology/**`, `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`; `84493f3`, `2502f50`; 2026-07-14                                                                                                          |
| `direct.rtc-diagnostics`     | `RtcDiagnosticsPanel`, `FailurePanel`, `StatsPanel`; `/?workspace=rallar&tab=rtc-diagnostics`, aliases `rtc`, `diagnostics`                                                                                                                                                       | RTC phase/peer/lane/NACK/failure evidence, latency/timeseries/scatter/histogram/waterfall/agent matrix, stats, and copyable bundles.                                                                                                                                                   | `Advanced` with contextual links from `Monitor`                                          | `src/legacy/diagnostics/rtc/**`, lazy-loaded by `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`; contextual links under `src/recipe-console/{advanced,monitor}/**`                          | Mounted only for active `rtc-diagnostics`; the dynamic panel and adjacent evidence owners unmount on exit and are absent from cold Recipe Console.                                                                                                                                                                                                                 | Iteration 11 exit policy is the current active-only dynamic owner; deterministic evidence derivation remains shared.                                                                        | Selected agent/command/transport use bounded URL context; no credential or hover-only evidence is introduced.                                                                                                                                                                                                                                                | Existing direct diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `keeps direct Rallar diagnostics out of primary navigation and opens them from Advanced`; `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                         | `/?workspace=rallar&tab=rtc-diagnostics`                                                                            | `src/legacy/diagnostics/rtc/**`, `src/legacy/shell/tabs/direct-connection-tab-panels.tsx`, `src/recipe-console/{advanced,monitor}/**`; `84493f3`, `2502f50`, `78e2c13`; 2026-07-14                                                        |
| `direct.rallar-data`         | `RallarDataPanel`; `/?workspace=rallar&tab=rallar-data`, aliases `data`, `storage`                                                                                                                                                                                                | Scoped local store open/read/write/update/CAS/delete/export/usage, change events, and cleanup.                                                                                                                                                                                         | `Advanced`                                                                               | `src/legacy/diagnostics/rallar-data/**`, composed by `src/legacy/shell/tabs/DirectResourceTabPanels.tsx`                                                                                             | Hidden-mounted only while `LegacyExperience` is active because it owns open-store and change-listener state.                                                                                                                                                                                                                                                       | Documented Iteration 11 stateful exception; future lazy unmount requires explicit listener/store closure or service ownership.                                                              | **Temporary stateful exception:** open store and change subscription. Large exports are not persisted.                                                                                                                                                                                                                                                       | Existing direct diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=rallar&tab=rallar-data`                                                                                | `src/legacy/diagnostics/rallar-data/**`, `src/legacy/shell/tabs/DirectResourceTabPanels.tsx`; `2502f50`; 2026-07-14                                                                                                                       |
| `direct.crdt`                | Thin `CrdtHealthPanel` root with nested `CrdtEditorPanel`, focused controllers, board/entities views, and contracts under `src/legacy/diagnostics/crdt/**`; `/?workspace=rallar&tab=crdt-health`, aliases `crdt`, `collaboration`                                                 | Admin document health plus collaborative editor operations, transport selection, operation groups, and document/card/entity mutation evidence.                                                                                                                                         | `Advanced`                                                                               | `src/legacy/diagnostics/crdt/**`, composed by `src/legacy/shell/tabs/DirectResourceTabPanels.tsx`                                                                                                    | Hidden-mounted only while `LegacyExperience` is active; the nested editor retains its document/subscription and the health controller retains admin state.                                                                                                                                                                                                         | Documented Iteration 11 stateful exception; future lazy unmount requires document/subscription cleanup and explicit ownership or an accepted reset for every retained state.                | **Temporary stateful exception:** active document/subscription, editor inputs/results, busy/error state, document list, selected key, and last admin result remain view-owned. Authored CRDT data remains domain-owned.                                                                                                                                      | M1 parity remains intact and the diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=rallar&tab=crdt-health`                                                                                | `src/legacy/diagnostics/crdt/**`, `src/legacy/shell/tabs/DirectResourceTabPanels.tsx`; `2502f50`; 2026-07-14                                                                                                                              |
| `direct.media`               | `MediaConsolePanel`; `/?workspace=rallar&tab=media`                                                                                                                                                                                                                               | Local media attach, audio/video controls, policy application, remote-stream subscription/events, stop/cleanup, and diagnostics.                                                                                                                                                        | `Advanced`                                                                               | `src/legacy/diagnostics/media/**`, composed by `src/legacy/shell/tabs/DirectResourceTabPanels.tsx`                                                                                                   | Hidden-mounted only while `LegacyExperience` is active because it can own media streams and remote subscriptions.                                                                                                                                                                                                                                                  | Documented Iteration 11 stateful exception; future lazy unmount requires mandatory stream/subscription cleanup.                                                                             | **Temporary stateful exception:** active media streams and remote subscription; no media payload persistence.                                                                                                                                                                                                                                                | Existing direct diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=rallar&tab=media`                                                                                      | `src/legacy/diagnostics/media/**`, `src/legacy/shell/tabs/DirectResourceTabPanels.tsx`; `2502f50`; 2026-07-14                                                                                                                             |
| `direct.rallar-server`       | Thin `RallarServerPanel`, focused controller/view/feedback/parsing/contracts under `src/legacy/diagnostics/rallar-server/**`; `/?workspace=rallar&tab=rallar-server`, alias `server`                                                                                              | Authenticated API-v1 presets/OpenAPI refresh, raw request/response lifecycle, auth injection, redacted cURL/command export, context promotion, persisted collection/variables/request drafts, assertions, extraction, and recipe export.                                               | `Advanced`                                                                               | `src/legacy/diagnostics/rallar-server/**`, composed by `src/legacy/shell/tabs/diagnostic-evidence-tab-panels.tsx`; contextual links under `src/recipe-console/{advanced,monitor}/**`                 | Hidden-mounted only while `LegacyExperience` is active to preserve drafts, invalid editable JSON, presets, feedback, results, and busy/error state.                                                                                                                                                                                                                | Documented Iteration 11 stateful exception; future lazy unmount requires explicit persistence/service ownership or an accepted reset for every retained state.                              | **Temporary stateful exception:** request/collection drafts persist redacted forms; invalid JSON, presets, feedback, results, and busy/error state remain view-owned. Credentials and response/artifact bulk remain excluded.                                                                                                                                | M1 parity remains intact and the diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `keeps direct Rallar diagnostics out of primary navigation and opens them from Advanced`; `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                         | `/?workspace=rallar&tab=rallar-server`                                                                              | `src/legacy/diagnostics/rallar-server/**`, `src/legacy/shell/tabs/diagnostic-evidence-tab-panels.tsx`, `src/recipe-console/{advanced,monitor}/**`; `2502f50`, `78e2c13`; 2026-07-14                                                       |
| `direct.rallar-trace`        | `RallarTracePanel` plus `RallarBrowserTraceBar`; `/?workspace=rallar&tab=rallar-trace`, aliases `trace`, `rallartrace`                                                                                                                                                            | Always-visible signaling/RTC/group/peer/latest-event strip and full redacted browser-runtime event/failure drill-in with source and severity filters plus an editable event limit.                                                                                                     | Shell infrastructure plus `Advanced`                                                     | `src/legacy/diagnostics/events/RallarTracePanel.tsx`, `src/legacy/shell/RallarBrowserTraceBar.tsx`, and `src/legacy/shell/tabs/diagnostic-evidence-tab-panels.tsx`                                   | The trace bar stays mounted only with active `LegacyExperience`; the full trace panel is hidden-mounted there and retains its filters/event limit. Neither mounts on cold Recipe Console.                                                                                                                                                                          | Documented Iteration 11 stateful exception for the full panel; future lazy unmount requires filter/event-limit persistence, while bounded runtime events remain service-owned.              | **Temporary stateful exception:** source/severity filters and event limit remain panel-owned; runtime events stay bounded and selected context may use URL state.                                                                                                                                                                                            | Existing infrastructure/diagnostic remains operational/deep-linkable; no cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`                                                                                                                                                                                                                                                                                                   | `/?workspace=rallar&tab=rallar-trace`                                                                               | `src/legacy/diagnostics/events/RallarTracePanel.tsx`, `src/legacy/shell/{RallarBrowserTraceBar.tsx,tabs/DiagnosticEvidenceTabPanels.tsx}`; `2502f50`; 2026-07-14                                                                          |
| `diagnostic.event-stream`    | `ExecutionFocusPanel`, `CommandHistoryPanel`, `StatsPanel`, `FailurePanel`, `EventStreamPanel`; `/?workspace=rallar&tab=event-stream` or `/?workspace=black-box-runner&tab=event-stream`, aliases `events`, `event`                                                               | Selected execution focus, command history, stats, failure list, kind/transport/text filtering, and bounded 40/100/250/500 event windows.                                                                                                                                               | `Monitor` evidence plus `Advanced` full stream                                           | Full stream under `src/legacy/diagnostics/events/**` and `src/legacy/shell/tabs/diagnostic-evidence-tab-panels.tsx`; contextual evidence under `src/recipe-console/monitor/**`                       | One full-stream owner is hidden-mounted while `LegacyExperience` is active regardless of its current workspace mode; it is absent from cold Recipe Console.                                                                                                                                                                                                        | Documented Iteration 11 stateful exception until filters/selection have explicit URL/personal-default ownership; event collection remains service-owned and bounded.                        | **Temporary stateful exception:** filter/selection persistence and runtime subscription ownership. No large raw stream persistence.                                                                                                                                                                                                                          | Existing shared diagnostic remains operational in both legacy workspace modes; no Recipe Console cutover, retirement, or new hide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/playwright/rallar-black-box/recipe-console-advanced.spec.ts` — `opens every registered legacy surface from its alias and contextual route`; `default Recipe Console does not load or poll inactive legacy routes except registered stateful exceptions`; `tests/playwright/rallar-black-box/recipe-console-monitor.spec.ts` — `opens all available correlated evidence from a failure row`                                                                                                                                                                | `/?workspace=black-box-runner&tab=event-stream` (direct fallback: `/?workspace=rallar&tab=event-stream`)            | `src/legacy/diagnostics/events/**`, `src/legacy/shell/tabs/diagnostic-evidence-tab-panels.tsx`, `src/recipe-console/monitor/**`; `42eedae`, `2502f50`; 2026-07-14                                                                         |
