...
| --- | --- | --- |
| `runner.recipes`, `runner.runs`, `runner.fleet`, primary `runner.builder`, `runner.advanced-wrapper` | `src/legacy/shell/tabs/RunnerWorkspaceTabPanels.tsx` | Recipes/Runs/Fleet/Builder are active-only dynamic imports. Advanced remains hidden-mounted only to compose its explicit exceptions and selected dynamic children. |
| `direct.quick-test`, `direct.auth`, `direct.websocket`, `direct.rtc-realtimes`; safe `direct.groups-clients`, `direct.topology`, `direct.rtc-diagnostics` | `src/legacy/shell/tabs/DirectConnectionTabPanels.tsx` | Quick/Auth/WS/RTC-Realtimes remain documented exceptions. Groups/Clients, Topology, and RTC Diagnostics are active-only dynamic imports and unmount on exit. |
...
| `legacy.local-workbench`, `legacy.manual-rallar`, `legacy.run-manager`, `legacy.distributed-recipes`, `legacy.shared-test-catalog`, `legacy.shared-test-import` | `src/legacy/runner/advanced/RunnerAdvancedPanel.tsx` | Workbench and Manual remain documented exceptions. Run Manager, Distributed Recipes, and the shared catalog/import panel are selected-only dynamic imports. The duplicate compatibility groups no longer exist. |
| `direct.rallar-trace`, `diagnostic.event-stream`, `direct.rallar-server` | `src/legacy/shell/tabs/DiagnosticEvidenceTabPanels.tsx` | All remain documented stateful exceptions hidden-mounted only inside active legacy. |
