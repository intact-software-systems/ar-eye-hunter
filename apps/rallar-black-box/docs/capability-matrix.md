...
| Fleet reporting/map | Fleet tab with live control-agent board, agent x run heatmap, region summaries, repeated failure signatures, timing distributions, report export, and deterministic SVG Fleet World Map with live agents, historical regions, failures, and observed routes | Control-server fleet reports plus live run snapshots | simulated, full-stack gated | Map v1 uses explicit coordinates or approximate lookup; no 3D globe, tile map, animated replay, or inferred route arcs yet. |
