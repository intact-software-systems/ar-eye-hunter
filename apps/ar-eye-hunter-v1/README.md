...
| Match inputs and intents                     | `match/create-arena-match-runtime.ts` registers envelope callbacks; `match/handlers/accept-arena-match-input.ts` and `accept-arena-match-intent.ts` validate the sender and apply game policy | Accepted results update snapshots and publish through the current match                                                                |
| Incoming presentation events                 | `messages/use-arena-director-message-handler.ts` and `use-arena-peer-message-handlers.ts` receive typed game messages                                                                         | Their state updates feed the arena view; `state/to-validated-player-pose.ts` owns avatar normalization                                 |
