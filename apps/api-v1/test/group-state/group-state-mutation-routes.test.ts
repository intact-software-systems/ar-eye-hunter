...
Deno.test('group aggregate routes preserve body, header, then one generated request ID', async () => {
  const enqueued: unknown[] = [];
  const snapshot = createGroupStateRouteSnapshot('room-1');
  const runtime = createGroupStateRouteTestRuntime({
    processGroupAppInbox: captureGroupStateWrite(enqueued, snapshot),
...
Deno.test('group aggregate route stops at route authentication failure before AppInbox', async () => {
  let enqueued = 0;
  const runtime = createGroupStateRouteTestRuntime({
...
Deno.test('group aggregate route serializes AppInbox failure after the awaited completion', async () => {
