...
Deno.test('auth logout accepts only the case-sensitive path request ID', async () => {
    const calls: Array<Parameters<configRoutes.ConfigRouteDependencies['appAuthInbox']['logoutSession']>[0]> = [];
    const app = createConfigRouteApp({
        appAuthInbox: ({
            logoutSession: (
                value: Parameters<configRoutes.ConfigRouteDependencies['appAuthInbox']['logoutSession']>[0]
            ) => {
                calls.push(value);
...
Deno.test(
    ...
    async () => {
        let inboxCalls = 0;
        const app = createConfigRouteApp({
            requireApiAuthSession: () => Promise.reject(authenticationRequired('Credential was rejected')),
            appAuthInbox: ({
                logoutSession: () => {
                    inboxCalls += 1;
                    return Promise.resolve(Either.ofRight({ loggedOut: true }));
...
Deno.test('logout reauthenticates before credential-proof replay disclosure', async () => {
    ...
    const app = createConfigRouteApp({
        requireApiAuthSession: () => Promise.reject(authenticationRequired('Session cannot be used any longer')),
        appAuthInbox: ({
            logoutSession: () => {
                liveLogoutCalls += 1;
                return Promise.resolve(Either.ofRight({ loggedOut: true }));
...
Deno.test('client mutation uses operation topic and target-plus-caller context', async () => {
    ...
    assert.deepEqual(enqueues, [{
        ...
        contextId: 'application=app-1:workspace=workspace-1:principal=alice:caller=alice:session=alice-session',
        senderId: 'alice',
        data: {
