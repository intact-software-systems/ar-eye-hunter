...
import { hashRtcTopologyExecutionCommand } from '@shared-server/rallar-system/repositories/RtcTopologyPublicationRepository.ts';
...
import { writeRtcTopologyPublicationOutbox } from '@shared-server/rallar-system/services/rtc-topology-ws-outbox-entry.ts';
import { computeTopologyMutation } from '@shared-server/rallar-system/services/rtc-topology-mutations.ts';
...
Deno.test('PGlite atomically publishes stale topology work without regressing latest topology', async () => {
  ...
  try {
    ...
    if (computed.outcome !== 'publish-superseded') return;

    const logicalWorkEntry = QueueBoxUtilities.toResourceEntryFromMsg(
      stalePublication.message, EnqueuedType.APP_OUTBOX,
    );
    const workEntry = {
      ...logicalWorkEntry,
    ...
    await sql.begin(async (transaction) => {
      ...
      assert.equal(await new ResourceInboxRepository(transaction).finishReserved(
        reserved.key,
        1,
...
function publication(workId: string): RtcTopologyPublication {
  ...
  const message = {
    ...
    route: {
      ...
      resourceId:
        `${snapshot.overlayId}:${sourceGroupStateCausalRevision.groupRevision}:` +
