...
import {
  createPGliteBlackBoxSnapshotPublisher,
  PGLITE_BLACK_BOX_SNAPSHOT_DIR_ENV,
} from '../../src/db/pglite-black-box-evidence-snapshot.ts';
...
Deno.test('PGlite black-box snapshot publishing atomically publishes a fresh archive', async () => {
  ...
  if (!publisher) throw new Error('Expected private snapshot publisher.');
...
for (
  const failure of [
    ...
    },
    {
      ...
      hooks: {
        beforeResponseWrite: async (response: { failure?: string }) => {
          if (!response.failure) throw new Error('normal response write failed');
        },
      },
      dumpDataDir: async () => new Blob(['snapshot-body']),
      leavesFailureResponse: true,
    },
    {
      ...
      hooks: {
        beforeResponseRename: async (response: { failure?: string }) => {
          if (!response.failure) throw new Error('normal response rename failed');
        },
      },
      dumpDataDir: async () => new Blob(['snapshot-body']),
      leavesFailureResponse: true,
    },
    {
      ...
      hooks: {
        beforeResponseWrite: async (response: { failure?: string }) => {
          if (response.failure) throw new Error('failure response write failed');
        },
      },
    ...
    },
    {
      ...
      hooks: {
        beforeResponseRename: async (response: { failure?: string }) => {
          if (response.failure) throw new Error('failure response rename failed');
        },
      },
      dumpDataDir: async () => await Promise.reject(new Error('dump failed')),
      leavesFailureResponse: false,
