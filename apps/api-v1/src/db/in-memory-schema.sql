...
CREATE TABLE IF NOT EXISTS rtc_topology_delivery_stream
(
...
CREATE TABLE IF NOT EXISTS rtc_topology_delivery_log
(
    ...
    inserted_at         timestamp(3) with time zone NOT NULL DEFAULT now(),
    CONSTRAINT rtc_topology_delivery_log_pk
        PRIMARY KEY (publisher_stream_id, sequence),
    CONSTRAINT rtc_topology_delivery_log_sequence_positive_ck CHECK (sequence >= 1),
    ...
    CONSTRAINT rtc_topology_delivery_log_publisher_fk
...
CREATE UNIQUE INDEX IF NOT EXISTS rtc_topology_delivery_log_publication_uq
...
CREATE INDEX IF NOT EXISTS rtc_topology_delivery_log_retain_until_ix
    ON rtc_topology_delivery_log (retain_until);

CREATE TABLE IF NOT EXISTS rtc_topology_replay_cursor
(
    ...
    CONSTRAINT rtc_topology_replay_cursor_consumer_fk
        FOREIGN KEY (consumer_stream_id)
        REFERENCES rtc_topology_delivery_stream (stream_id)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,
    CONSTRAINT rtc_topology_replay_cursor_publisher_fk
        FOREIGN KEY (publisher_stream_id)
        REFERENCES rtc_topology_delivery_stream (stream_id)
        ON DELETE RESTRICT
        ON UPDATE CASCADE
