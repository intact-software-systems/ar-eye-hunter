...
export function init(app: Hono, options: RallarCrdtAdminRoutesOptions): void {
    ...
    app.post('/api/crdt/admin/documents/list', async (c) =>
        withAdminError(
            c,
            async () =>
    ...
    app.post('/api/crdt/admin/documents/integrity', async (c) =>
        withAdminError(
            c,
            async () =>
    ...
    app.post('/api/crdt/admin/documents/debug-export', async (c) =>
        withAdminError(c, async () => {
    ...
    app.post('/api/crdt/admin/documents/backup-export', async (c) =>
        withAdminError(
            c,
            async () =>
    ...
    app.post('/api/crdt/admin/documents/rebuild-projection', async (c) =>
        withAdminError(c, async () => {
            const body = await readJson<{
                document?: RallarCrdtDocumentRef;
                projectionId?: string;
    ...
    app.post('/api/crdt/admin/documents/compact', async (c) =>
        withAdminError(c, async () => {
            const body = await readJson<{
                document?: RallarCrdtDocumentRef;
                reason?: string;
            ...
            const document = readDocument(body);
            const backup =
                await options.repository.exportBackupBundle(document);
            if (!backup) {
    ...
    app.post('/api/crdt/admin/documents/lifecycle', async (c) =>
        withAdminError(
            c,
            async () =>
    ...
    app.post('/api/crdt/admin/documents/erase', async (c) =>
        withAdminError(c, async () => {
            const body = await readJson<{
                document?: RallarCrdtDocumentRef;
            ...
            if (mode === 'redact-payloads') {
                const redactedBundle =
                    await options.repository.exportDebugBundle(document, {
                        reason,
                        redaction: {
                            payloadsRedacted: true,
                            reason,
...
async function requireCrdtAdminSession(
    ...
    const authorized =
        options.authorizeAdmin !== undefined
