...
export function createRallarServer(
    ...
    const crdtLogRepository =
        options.crdtLogRepository ??
            ...
            installWebSocketLifecycle: (runtime) => {
                initSharedWsLifecycle(runtime.wsQBoxServerService, {
                    disconnectClientSession: async (sessionId) => {
                        const result =
                            await getMiddleware().appClientInboxService.processAuthorisedWsClientDisconnect(
                                sessionId,
                            );
                        result.fold(
                            (error) => {
                    ...
                    disconnectGroupSessionsBySessionId: async (
                        ...
                        const result =
                            await getMiddleware().appGroupInboxService.processPresenceDisconnectsBySessionId(
                                sessionId,
                                request,
                            );
                        result.fold(
